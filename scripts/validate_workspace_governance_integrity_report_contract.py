#!/usr/bin/env python3
"""Validate workspace governance integrity report contract.

Purpose: verify that integrity reports expose a stable machine-readable shape
and that count fields match artifact records.
Governance scope: [OCE, RAG, CDCV, UWMA, PRS]
Dependencies: Python standard library, integrity report schema/example, and
...
try:
    from report_workspace_governance_integrity import build_integrity, build_integrity_report, load_witness
except ModuleNotFoundError:  # pragma: no cover - exercised when imported as a package.
    from scripts.report_workspace_governance_integrity import build_integrity, build_integrity_report, load_witness


WORKSPACE_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_SCHEMA_PATH = WORKSPACE_ROOT / "schemas" / "workspace_governance_integrity_report.schema.json"
DEFAULT_EXAMPLE_PATH = WORKSPACE_ROOT / "docs" / "workspace-governance-integrity-report-example.json"
...
class IntegrityReportContractError(ValueError):
    """Raised when the integrity report contract is invalid."""
...
def load_json_object(json_path: Path, label: str) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise IntegrityReportContractError(f"{label} must be a JSON object")
    return payload
...
def validate_schema_artifact(schema: dict[str, Any]) -> list[str]:
    ...
    errors: list[str] = []
    if schema.get("title") != "Workspace Governance Integrity Report":
        errors.append("schema title does not identify workspace governance integrity report")
    ...
    artifact_required = schema.get("$defs", {}).get("integrity_artifact", {}).get("required", [])
    if not isinstance(artifact_required, list):
        errors.append("schema integrity_artifact.required must be a list")
    else:
...
def validate_report(report: dict[str, Any]) -> list[str]:
    """Return deterministic validation errors for one integrity report."""
    ...
    if report["report_id"] != "workspace_governance_integrity":
    ...
    if not isinstance(artifacts, list):
        ...
        return errors
    for field_name in ("artifact_count", "hashed_count", "missing_count", "issue_count"):
...
def build_current_report(witness_path: Path = DEFAULT_WITNESS_PATH) -> dict[str, object]:
    """Build the current workspace integrity report without writing files."""

    witness = load_witness(witness_path)
    integrity_records = build_integrity(witness)
    return build_integrity_report(integrity_records)
...
def validate_contract(
    ...
    schema = load_json_object(schema_path, "integrity report schema")
    example = load_json_object(example_path, "integrity report example")
...
def _validate_report_counts(report: dict[str, Any], artifacts: list[Any]) -> list[str]:
    ...
    hashed_count = sum(1 for artifact in artifacts if isinstance(artifact, dict) and artifact.get("sha256") is not None)
    ...
    if report.get("hashed_count") != hashed_count:
        errors.append("hashed_count does not match artifact digests")
...
def _validate_artifact_record(artifact: Any, index: int) -> list[str]:
    ...
    if artifact["sha256"] is not None and (
        not isinstance(artifact["sha256"], str) or SHA256_PATTERN.fullmatch(artifact["sha256"]) is None
    ):
        errors.append(f"artifact {index} sha256 must be null or a lowercase SHA-256 hex digest")
    ...
    if artifact["exists"] is True and artifact["sha256"] is None:
        errors.append(f"artifact {index} existing artifact must include sha256")
    ...
    if artifact["exists"] is False and artifact["sha256"] is not None:
        errors.append(f"artifact {index} missing artifact must not include sha256")
...
def main(argv: list[str] | None = None) -> int:
    """Validate the workspace governance integrity report contract."""

    parser = argparse.ArgumentParser(description="Validate workspace governance integrity report contract.")
    parser.add_argument("--schema", type=Path, default=DEFAULT_SCHEMA_PATH, help="path to integrity report schema JSON")
    parser.add_argument("--example", type=Path, default=DEFAULT_EXAMPLE_PATH, help="path to integrity report example JSON")
    ...
    try:
        ...
        sys.stderr.write(f"[FAIL] load-integrity-contract: {exc}\n")
        sys.stderr.write("STATUS: failed\n")
        return 1

    if errors:
        for error in errors:
            sys.stderr.write(f"[FAIL] integrity-report-contract: {error}\n")
        sys.stderr.write("STATUS: failed\n")
        return 1

    sys.stdout.write("[PASS] workspace_governance_integrity_report_schema\n")
    sys.stdout.write("[PASS] workspace_governance_integrity_report_example\n")
    sys.stdout.write("[PASS] workspace_governance_integrity_report_current_output\n")
