#!/usr/bin/env python3
"""Validate operator value-binding explicit decision value-ref verification preflight."""
...
RUNTIME_GENERATED_AT = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_VALUE_EXPLICIT_DECISION_VALUE_REF_VERIFICATION_PREFLIGHT_GENERATED_AT
...
class PersonalAssistantExplicitDecisionValueRefVerificationPreflightValidation:
    """Validation result for explicit decision value-ref verification preflight."""

    valid: bool
    runtime_validated: bool
    verification_record_count: int
    verification_preflight_checked_count: int
    submitted_ref_observed_count: int
...
def validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_explicit_decision_value_ref_verification_preflight(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    receipt_schema_path: Path = DEFAULT_RECEIPT_SCHEMA,
) -> PersonalAssistantExplicitDecisionValueRefVerificationPreflightValidation:
    """Validate runtime verifier execution explicit decision value-ref verification preflight."""

    errors: list[str] = []
    schema = _load_json_object(schema_path, "explicit decision value-ref verification preflight schema", errors)
    ...
    errors.extend(_validate_explicit_decision_value_ref_verification_preflight_semantics(envelope, receipt_schema))
    _scan_secret_values(envelope, errors, path="$runtime")
    return PersonalAssistantExplicitDecisionValueRefVerificationPreflightValidation(
        valid=not errors,
        runtime_validated=not errors,
        verification_record_count=int(summary.get("verification_record_count", 0)),
        verification_preflight_checked_count=int(summary.get("verification_preflight_checked_count", 0)),
        submitted_ref_observed_count=int(summary.get("submitted_ref_observed_count", 0)),
        ...
        raw_ref_payload_count=int(summary.get("raw_ref_payload_count", 0)),
...
def _validate_explicit_decision_value_ref_verification_preflight_semantics(envelope: Mapping[str, Any], receipt_schema: Mapping[str, Any]) -> tuple[str, ...]:
    errors: list[str] = []
    expected_top = {
    ...
    records = envelope.get("verification_records")
    if not isinstance(records, list):
        errors.append("verification_records must be a list")
        return tuple(errors)
    if envelope.get("verification_record_count") != len(EXPECTED_REQUIRED_VALUE_REFS):
        errors.append("verification_record_count must equal four canonical required refs")
    record_ids: list[str] = []
    ...
    for index, record in enumerate(records):
        if not isinstance(record, dict):
            ...
            continue
        record_ids.append(str(record.get("verification_record_id", "")))
        source_record_ids.append(str(record.get("source_submitted_ref_record_id", "")))
    ...
    if tuple(names) != EXPECTED_REQUIRED_VALUE_REFS:
        errors.append("verification_records must match canonical required ref order")
    if envelope.get("verification_record_ids") != record_ids:
        errors.append("verification_record_ids must match record order")
    if envelope.get("source_submitted_ref_record_ids") != source_record_ids:
        errors.append("source_submitted_ref_record_ids must match record order")
    ...
    if metadata.get("operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_explicit_decision_value_ref_verification_preflight_is_execution") is not False:
        errors.append("receipt.metadata execution flag must be false")
    if metadata.get("verification_preflight_checked") is not True:
        errors.append("receipt.metadata verification_preflight_checked must be true")
...
def _require_record(index: int, record: Mapping[str, Any], errors: list[str]) -> None:
    ...
    for field_name, expected_value in expected.items():
        if record.get(field_name) != expected_value:
            errors.append(f"verification_records[{index}].{field_name} must be {expected_value}")
    ...
    if record.get("submitted_ref_uri") != expected_uri:
        errors.append(f"verification_records[{index}].submitted_ref_uri must match ref_name")
    if record.get("blocking_reason") != f"{ref_name}_not_verified":
        errors.append(f"verification_records[{index}].blocking_reason must match ref_name")


def _require_summary(envelope: Mapping[str, Any], records: list[Any], errors: list[str]) -> None:
    summary = _mapping(envelope.get("summary"))
    expected = {
        "verification_record_count": len(EXPECTED_REQUIRED_VALUE_REFS),
        "verification_preflight_checked_count": len(EXPECTED_REQUIRED_VALUE_REFS),
        "submitted_ref_observed_count": len(EXPECTED_REQUIRED_VALUE_REFS),
    ...
    if len(records) != len(EXPECTED_REQUIRED_VALUE_REFS):
        errors.append("summary source verification record count must stay canonical")
...
def main(argv: list[str] | None = None) -> int:
    """Run the explicit decision value-ref verification preflight validator."""
    ...
    validation = validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_explicit_decision_value_ref_verification_preflight(
    ...
    if args.json:
        ...
        print("personal assistant verifier execution explicit decision value-ref verification preflight: valid")
    else:
        print("personal assistant verifier execution explicit decision value-ref verification preflight: invalid")
