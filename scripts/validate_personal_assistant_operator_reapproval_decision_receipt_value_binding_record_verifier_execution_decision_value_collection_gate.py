#!/usr/bin/env python3
"""Validate operator value-binding verifier execution decision-value collection gate."""
...
RUNTIME_GENERATED_AT = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_COLLECTION_GATE_GENERATED_AT
...
class PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierExecutionDecisionValueCollectionGateValidation:
    """Validation result for verifier execution decision-value collection gate."""

    valid: bool
    runtime_validated: bool
    collection_gate_count: int
    operator_decision_required_count: int
    operator_decision_value_required_count: int
    actual_operator_decision_value_required_count: int
    collection_route_admission_count: int
    template_accepted_as_value_count: int
...
def validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_collection_gate(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    receipt_schema_path: Path = DEFAULT_RECEIPT_SCHEMA,
) -> PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierExecutionDecisionValueCollectionGateValidation:
    """Validate runtime verifier execution decision-value collection gate."""

    errors: list[str] = []
    schema = _load_json_object(schema_path, "decision-value collection gate schema", errors)
    ...
    errors.extend(_validate_decision_value_collection_gate_semantics(envelope, receipt_schema))
    _scan_secret_values(envelope, errors, path="$runtime")
    return PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierExecutionDecisionValueCollectionGateValidation(
        valid=not errors,
        runtime_validated=not errors,
        collection_gate_count=int(envelope.get("collection_gate_count", 0)),
        ...
        actual_operator_decision_value_required_count=int(summary.get("actual_operator_decision_value_required_count", 0)),
        collection_route_admission_count=int(summary.get("collection_route_admission_count", 0)),
        template_accepted_as_value_count=int(summary.get("template_accepted_as_value_count", 0)),
...
def _validate_decision_value_collection_gate_semantics(envelope: Mapping[str, Any], receipt_schema: Mapping[str, Any]) -> tuple[str, ...]:
    errors: list[str] = []
    expected_top = {
    ...
    records = envelope.get("collection_gates")
    if not isinstance(records, list):
        errors.append("collection_gates must be a list")
        return tuple(errors)
    if envelope.get("collection_gate_count") != len(records):
        errors.append("collection_gate_count must equal collection_gates length")
    item_ids: list[str] = []
    receipt_ids: list[str] = []
    coverage: dict[str, set[str]] = {}
    for index, record in enumerate(records):
        if not isinstance(record, dict):
            errors.append(f"collection_gates[{index}] must be an object")
            continue
        item_ids.append(str(record.get("collection_gate_item_id", "")))
        ...
        _require_source_ref(index, _mapping(record.get("decision_value_template_ref")), errors)
        _require_gate(index, _mapping(record.get("collection_gate")), errors)
        _require_false_fields(_mapping(record.get("authority_status")), FALSE_AUTHORITY_FIELDS, f"collection_gates[{index}].authority_status", errors)
        receipt = _mapping(record.get("receipt"))
        if receipt_schema:
            errors.extend(f"collection_gates[{index}].receipt {message}" for message in _validate_schema_instance(dict(receipt_schema), receipt))
        errors.extend(f"collection_gates[{index}].receipt {message}" for message in validate_personal_assistant_receipt_payload(receipt))
        metadata = _mapping(receipt.get("metadata"))
        if metadata.get("operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_collection_gate_is_execution") is not False:
            errors.append(f"collection_gates[{index}].receipt.metadata execution flag must be false")
        if metadata.get("collection_gate_created") is not True:
            errors.append(f"collection_gates[{index}].receipt.metadata collection_gate_created must be true")
        _require_false_fields(metadata, FALSE_FIELDS | FALSE_AUTHORITY_FIELDS | {"external_write_allowed"}, f"collection_gates[{index}].receipt.metadata", errors)
        receipt_ids.append(str(receipt.get("receipt_id", "")))
    if set(coverage) != EXPECTED_EVIDENCE_KINDS:
        errors.append("collection_gates must cover all governed evidence kinds")
    for evidence_kind, requirement_kinds in coverage.items():
        if requirement_kinds != EXPECTED_REQUIREMENT_KINDS:
            errors.append(f"collection_gates for {evidence_kind} must cover all verifier requirement kinds")
    if envelope.get("collection_gate_item_ids") != item_ids:
        errors.append("collection_gate_item_ids must match item order")
...
def _require_source_ref(index: int, payload: Mapping[str, Any], errors: list[str]) -> None:
    ...
    for field_name, expected_value in expected.items():
        if payload.get(field_name) != expected_value:
            errors.append(f"collection_gates[{index}].decision_value_template_ref.{field_name} must be {expected_value}")


def _require_gate(index: int, payload: Mapping[str, Any], errors: list[str]) -> None:
    for field_name in (
        "operator_decision_required",
        "operator_decision_value_required",
        "actual_operator_decision_value_required",
        ...
        if payload.get(field_name) is not True:
            errors.append(f"collection_gates[{index}].collection_gate.{field_name} must be true")
    if tuple(payload.get("accepted_decision_values", ())) != EXPECTED_DECISION_VALUES:
        errors.append(f"collection_gates[{index}].collection_gate.accepted_decision_values must match canonical values")
    if tuple(payload.get("rejected_input_kinds", ())) != EXPECTED_REJECTED_INPUT_KINDS:
        errors.append(f"collection_gates[{index}].collection_gate.rejected_input_kinds must reject generic continuations and templates")
    if tuple(payload.get("required_fields", ())) != EXPECTED_REQUIRED_FIELDS:
        errors.append(f"collection_gates[{index}].collection_gate.required_fields must match required operator refs")
    _require_false_fields(payload, FALSE_FIELDS | {"accepts_generic_continuation", "accepts_template_packet"}, f"collection_gates[{index}].collection_gate", errors)
    if payload.get("blocking_reason") != "actual_operator_decision_value_absent":
        errors.append(f"collection_gates[{index}].collection_gate blocking_reason must remain actual_operator_decision_value_absent")
...
def _require_summary(envelope: Mapping[str, Any], records: list[Any], errors: list[str]) -> None:
    ...
    expected = {
        ...
        "collection_gate_creation_count": len(records),
        "collection_route_admission_count": 0,
        "template_accepted_as_value_count": 0,
...
def main() -> int:
    ...
    result = validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_collection_gate(
    ...
    if args.json:
        ...
        print("PERSONAL ASSISTANT OPERATOR REAPPROVAL DECISION RECEIPT VALUE BINDING RECORD VERIFIER EXECUTION DECISION VALUE COLLECTION GATE VALID")
    else:
