#!/usr/bin/env python3
"""Validate the replacement decision receipt operator read model.

Purpose: prove the replacement decision operator projection is read-only and
bounded while replacement decision recording, receipt-store append, raw payload
exposure, mutation, and terminal closure remain denied.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: replacement decision read-model schema/example and replacement
decision receipt validator constants.
Invariants:
  - The read model is a projection, not decision authority.
  - Every replacement requirement appears exactly once.
...
from scripts.validate_universal_symbol_receipt_store_replacement_decision_receipt import (  # noqa: E402
    AUTHORITY_DENIAL_FIELDS,
    DEFAULT_RECEIPT_PATH as DEFAULT_REPLACEMENT_RECEIPT_PATH,
...
DEFAULT_SCHEMA_PATH = (
    REPO_ROOT / "schemas" / "universal_symbol_receipt_store_replacement_decision_read_model.schema.json"
)
DEFAULT_READ_MODEL_PATH = (
    REPO_ROOT / "examples" / "universal_symbol_receipt_store_replacement_decision_read_model.foundation.json"
)

EXPECTED_RECEIPT_REF = "examples/universal_symbol_receipt_store_replacement_decision_receipt.foundation.json"
...
READ_MODEL_CONSTRAINT_TRUE_FIELDS: tuple[str, ...] = (
    ...
    "no_raw_payload",
    "no_raw_secret",
    "no_replacement_decision_recording",
    "no_terminal_closure",
)
...
EXPECTED_OPERATOR_STATUS_SUMMARY: dict[str, str] = {
    "primary_status": "Replacement decision blocked",
    "replacement_status": "Awaiting evidence",
    "recording_status": "Blocked",
    "raw_detail_visibility": "hidden_by_default",
    "operator_action": "Collect replacement decision evidence",
}


class UniversalSymbolReceiptStoreReplacementDecisionReadModelError(ValueError):
    """Raised when the replacement decision read model violates Foundation Mode."""


def validate_universal_symbol_receipt_store_replacement_decision_read_model(
    read_model_path: Path = DEFAULT_READ_MODEL_PATH,
    schema_path: Path = DEFAULT_SCHEMA_PATH,
) -> dict[str, Any]:
    """Validate schema shape, bounded rows, and denied decision authority."""
    ...
    _validate_receipt_projection(read_model, errors)
    _validate_source_receipt_alignment(read_model, errors)
    ...
    if errors:
        raise UniversalSymbolReceiptStoreReplacementDecisionReadModelError("; ".join(errors))
    return report
...
def load_json_object(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise UniversalSymbolReceiptStoreReplacementDecisionReadModelError(f"missing file: {path}") from exc
    except json.JSONDecodeError as exc:
        raise UniversalSymbolReceiptStoreReplacementDecisionReadModelError(f"invalid json: {path}: {exc}") from exc
    if not isinstance(value, dict):
        raise UniversalSymbolReceiptStoreReplacementDecisionReadModelError(f"expected object: {path}")
    return value


def _validate_schema_boundary(schema: Mapping[str, Any], errors: list[str]) -> None:
    expected_id = "urn:mullusi:schema:universal-symbol-receipt-store-replacement-decision-read-model:1"
...
def _validate_read_model_boundary(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if read_model.get("read_model_is_not_decision_authority") is not True:
        errors.append("read model must not be decision authority")
...
def _validate_receipt_projection(read_model: Mapping[str, Any], errors: list[str]) -> None:
    projection = _mapping(read_model.get("receipt_projection"))
    if projection.get("receipt_ref") != EXPECTED_RECEIPT_REF:
        errors.append("receipt_projection.receipt_ref drift")
    if projection.get("replacement_decision") != EXPECTED_REPLACEMENT_DECISION:
        errors.append("receipt_projection.replacement_decision must remain blocked")
    if projection.get("authority_granted") is not False:
        errors.append("receipt_projection.authority_granted must remain false")
    if projection.get("replacement_requirement_count") != len(REQUIRED_REQUIREMENT_IDS):
        errors.append("receipt_projection.replacement_requirement_count drift")
    if projection.get("blocked_reason_count") != len(REQUIRED_BLOCKED_REASONS):
        errors.append("receipt_projection.blocked_reason_count drift")


def _validate_source_receipt_alignment(read_model: Mapping[str, Any], errors: list[str]) -> None:
    projection = _mapping(read_model.get("receipt_projection"))
    if projection.get("receipt_ref") != EXPECTED_RECEIPT_REF:
        return
    try:
        receipt = load_json_object(DEFAULT_REPLACEMENT_RECEIPT_PATH)
    except UniversalSymbolReceiptStoreReplacementDecisionReadModelError as exc:
        errors.append(f"source replacement receipt unavailable: {exc}")
        return

    receipt_summary = _mapping(receipt.get("contract_summary"))
    expected_counts = {
        "replacement_requirement_count": receipt_summary.get("replacement_requirement_count"),
        "blocked_reason_count": receipt_summary.get("blocked_reason_count"),
    }
    for field_name, expected_count in expected_counts.items():
        if projection.get(field_name) != expected_count:
            errors.append(f"receipt_projection.{field_name} must match source replacement receipt")
    if projection.get("replacement_decision") != receipt.get("replacement_decision"):
        errors.append("receipt_projection.replacement_decision must match source replacement receipt")
...
def _validate_requirement_rows(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    for row in rows:
        ...
        if row.get("proof_state") not in {"Unknown", "BudgetUnknown", "Fail"}:
            errors.append(f"{row.get('requirement_id')}: proof_state must block replacement")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print(f"[FAIL] universal_symbol_receipt_store_replacement_decision_read_model: {exc}")
            print("STATUS: failed")
        return 1
    ...
    if args.json:
        ...
        print("[PASS] universal_symbol_receipt_store_replacement_decision_read_model")
        print("STATUS: passed")
    return 0
