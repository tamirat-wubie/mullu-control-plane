#!/usr/bin/env python3
"""Validate the recovery witness operator read model.

Purpose: prove the recovery operator projection is read-only and bounded while
recovery binding, recovery execution, rollback execution, compensation
execution, replay-state commit, receipt-store append, raw payload exposure,
mutation, and terminal closure remain denied.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: recovery read-model schema/example and recovery witness validator
constants.
Invariants:
  - The read model is a projection, not recovery authority.
  - Every recovery requirement appears exactly once.
  - Raw detail visibility, recovery effects, and authority remain denied.
...
from scripts.validate_universal_symbol_receipt_store_recovery_witness import (  # noqa: E402
...
DEFAULT_SCHEMA_PATH = REPO_ROOT / "schemas" / "universal_symbol_receipt_store_recovery_read_model.schema.json"
DEFAULT_READ_MODEL_PATH = (
    REPO_ROOT / "examples" / "universal_symbol_receipt_store_recovery_read_model.foundation.json"
)

EXPECTED_WITNESS_REF = "examples/universal_symbol_receipt_store_recovery_witness.foundation.json"
EXPECTED_RECOVERY_DECISION = "blocked_pending_recovery_plan_snapshot_rollback_compensation_and_audit_evidence"
...
EXPECTED_OPERATOR_STATUS_SUMMARY: dict[str, str] = {
    "primary_status": "Recovery blocked",
    "rollback_status": "Awaiting rollback evidence",
    "compensation_status": "Awaiting compensation evidence",
    "raw_detail_visibility": "hidden_by_default",
    "operator_action": (
        "Collect recovery plan, rollback plan, compensation plan, snapshot, replay binding, "
        "effect boundary, incident handoff, and post-recovery audit evidence"
    ),
}


class UniversalSymbolReceiptStoreRecoveryReadModelError(ValueError):
    """Raised when the recovery read model violates Foundation Mode."""


def validate_universal_symbol_receipt_store_recovery_read_model(
    read_model_path: Path = DEFAULT_READ_MODEL_PATH,
    schema_path: Path = DEFAULT_SCHEMA_PATH,
) -> dict[str, Any]:
    """Validate schema shape, bounded rows, and denied recovery authority."""
    ...
    if errors:
        raise UniversalSymbolReceiptStoreRecoveryReadModelError("; ".join(errors))
    return report
...
def load_json_object(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise UniversalSymbolReceiptStoreRecoveryReadModelError(f"missing file: {path}") from exc
    except json.JSONDecodeError as exc:
        raise UniversalSymbolReceiptStoreRecoveryReadModelError(f"invalid json: {path}: {exc}") from exc
    if not isinstance(value, dict):
        raise UniversalSymbolReceiptStoreRecoveryReadModelError(f"expected object: {path}")
    return value


def _validate_schema_boundary(schema: Mapping[str, Any], errors: list[str]) -> None:
    expected_id = "urn:mullusi:schema:universal-symbol-receipt-store-recovery-read-model:1"
...
def _validate_read_model_boundary(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if read_model.get("read_model_is_not_recovery_authority") is not True:
        errors.append("read model must not be recovery authority")
...
def _validate_witness_projection(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if projection.get("recovery_decision") != EXPECTED_RECOVERY_DECISION:
        errors.append("witness_projection.recovery_decision must remain blocked")
    if projection.get("authority_granted") is not False:
        errors.append("witness_projection.authority_granted must remain false")
    if projection.get("recovery_requirement_count") != len(REQUIRED_REQUIREMENT_IDS):
        errors.append("witness_projection.recovery_requirement_count drift")
...
def _validate_source_witness_alignment(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if projection.get("witness_ref") != EXPECTED_WITNESS_REF:
        return
    try:
        witness = load_json_object(DEFAULT_RECOVERY_WITNESS_PATH)
    except UniversalSymbolReceiptStoreRecoveryReadModelError as exc:
        errors.append(f"source recovery witness unavailable: {exc}")
        return

    witness_summary = _mapping(witness.get("contract_summary"))
    expected_counts = {
        "recovery_requirement_count": witness_summary.get("recovery_requirement_count"),
        "blocked_reason_count": witness_summary.get("blocked_reason_count"),
    }
    for field_name, expected_count in expected_counts.items():
        if projection.get(field_name) != expected_count:
            errors.append(f"witness_projection.{field_name} must match source recovery witness")
    if projection.get("recovery_decision") != witness.get("recovery_decision"):
        errors.append("witness_projection.recovery_decision must match source recovery witness")
...
def _validate_requirement_rows(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    for row in rows:
        ...
        if row.get("proof_state") not in {"Unknown", "BudgetUnknown", "Fail"}:
            errors.append(f"{row.get('requirement_id')}: proof_state must block recovery")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print(f"[FAIL] universal_symbol_receipt_store_recovery_read_model: {exc}")
            print("STATUS: failed")
        return 1
    ...
    if args.json:
        ...
        print("[PASS] universal_symbol_receipt_store_recovery_read_model")
        print("STATUS: passed")
    return 0
