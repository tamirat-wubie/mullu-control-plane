#!/usr/bin/env python3
"""Validate the ReadOnlyWorkerRuntimeReceiptSchemaBindingActivationWitness contract.

Purpose: verify Foundation Mode witness evidence for a future live
read_only_repo_inspection runtime receipt schema-binding activation boundary
without activating schema binding, writing a schema registry, invoking a worker,
or admitting dispatch.
...
Invariants:
  ...
  - Runtime runner registration, dispatch endpoint registration, runtime receipt
    emitter registration, schema binding, schema-binding activation, schema
    registry writes, runtime dispatch, worker invocation, and runtime receipt
    emission remain unperformed.
...
from scripts.validate_read_only_worker_runtime_receipt_emitter_registration_witness import (  # noqa: E402
    DEFAULT_RECEIPT_PATH as DEFAULT_EMITTER_REGISTRATION_WITNESS_PATH,
    DEFAULT_SCHEMA_PATH as DEFAULT_EMITTER_REGISTRATION_WITNESS_SCHEMA_PATH,
    load_json_object as load_emitter_registration_json_object,
...
DEFAULT_SCHEMA_PATH = (
    WORKSPACE_ROOT / "schemas" / "read_only_worker_runtime_receipt_schema_binding_activation_witness.schema.json"
)
DEFAULT_RECEIPT_PATH = (
    WORKSPACE_ROOT / "examples" / "read_only_worker_runtime_receipt_schema_binding_activation_witness.foundation.json"
)
EXPECTED_SCHEMA_ID = "urn:mullusi:schema:read-only-worker-runtime-receipt-schema-binding-activation-witness:1"
EXPECTED_SCHEMA_TITLE = "Read-Only Worker runtime receipt schema binding activation Witness"
EXPECTED_RECEIPT_VERSION = "read_only_worker_runtime_receipt_schema_binding_activation_witness.v1"
...
EXPECTED_WITNESS_MODE = "LIVE_RUNTIME_RECEIPT_SCHEMA_BINDING_ACTIVATION_WITNESS_ONLY"
EXPECTED_RUNTIME_RECEIPT_KIND = "future_worker_runtime_receipt"
EXPECTED_TARGET_SCHEMA_BINDING_ACTIVATION_REF = (
    "candidate://runtime-receipt-schema-binding/live-activation/read-only-repo-inspection"
)
EXPECTED_SCHEMA_BINDING_ACTIVATION_PROFILE = (
    "LOCAL_READ_ONLY_REPO_INSPECTION_RUNTIME_RECEIPT_SCHEMA_BINDING_ACTIVATION"
)
EXPECTED_ADMISSION_DECISION = (
    "LIVE_RUNTIME_RECEIPT_SCHEMA_BINDING_ACTIVATION_WITNESS_BLOCKED_AWAITING_OPERATOR_APPROVAL_AND_LIVE_EVIDENCE"
)
...
REQUIRED_ACTIVATION_INPUT_REFS = (
    "evidence://operator-approval/live-runtime-receipt-schema-binding-activation",
    ...
    "evidence://runtime-receipt-schema-binding-activation-boundary/read-only-repo-inspection",
...
REQUIRED_BLOCKED_REASON_REFS = (
    "blocked://operator-approval/live-runtime-receipt-schema-binding-activation-missing",
    ...
    "blocked://runtime-receipt-schema-registry/write-not-authorized",
...
ACTIVATION_EVALUATION_TRUE_FIELDS = (
    ...
    "schema_binding_activation_boundary_required",
    "schema_registry_boundary_required",
...
class ReadOnlyWorkerRuntimeReceiptSchemaBindingActivationWitnessError(ValueError):
    """Raised when a runtime receipt schema-binding activation witness artifact cannot load."""
...
def load_json_object(json_path: Path, label: str) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ReadOnlyWorkerRuntimeReceiptSchemaBindingActivationWitnessError(f"{label} must be a JSON object")
    return payload
...
def validate_runtime_receipt_schema_binding_activation_witness_record(
    record: Any,
    schema: dict[str, Any] | None = None,
    emitter_registration_witness: dict[str, Any] | None = None,
    schema_binding_witness: dict[str, Any] | None = None,
) -> list[str]:
    """Return deterministic validation errors for one schema-binding activation witness."""

    schema_payload = schema or _load_schema(DEFAULT_SCHEMA_PATH)
    errors = _validate_schema_instance(schema_payload, record)
    if not isinstance(record, dict):
        errors.append("read-only worker runtime receipt schema-binding activation witness must be a JSON object")
        return errors

    emitter_payload = emitter_registration_witness or load_emitter_registration_json_object(
        DEFAULT_EMITTER_REGISTRATION_WITNESS_PATH,
        "ReadOnlyWorkerRuntimeReceiptEmitterRegistrationWitness",
    )
    emitter_schema = _load_schema(DEFAULT_EMITTER_REGISTRATION_WITNESS_SCHEMA_PATH)
    errors.extend(
        f"emitter registration witness: {error}"
        for error in validate_runtime_receipt_emitter_registration_witness_record(emitter_payload, emitter_schema)
    )
    ...
    _validate_top_level_refs(record, emitter_payload, schema_binding_payload, errors)
    _validate_authority_scope(record.get("authority_scope"), errors)
    _validate_activation_contract(
        record.get("runtime_receipt_schema_binding_activation_witness_contract"),
        emitter_payload,
...
def validate_runtime_receipt_schema_binding_activation_witness(
    schema_path: Path = DEFAULT_SCHEMA_PATH,
    receipt_path: Path = DEFAULT_RECEIPT_PATH,
    emitter_registration_witness_path: Path = DEFAULT_EMITTER_REGISTRATION_WITNESS_PATH,
    ...
    receipt = load_json_object(receipt_path, "ReadOnlyWorkerRuntimeReceiptSchemaBindingActivationWitness")
    emitter_registration_witness = load_emitter_registration_json_object(
        emitter_registration_witness_path,
        "ReadOnlyWorkerRuntimeReceiptEmitterRegistrationWitness",
    )
...
def build_mutated_runtime_receipt_schema_binding_activation_witness(**updates: Any) -> dict[str, Any]:
    """Build a deterministic mutated copy of the default witness receipt."""

    receipt = load_json_object(DEFAULT_RECEIPT_PATH, "ReadOnlyWorkerRuntimeReceiptSchemaBindingActivationWitness")
...
def _validate_top_level_refs(
    record: dict[str, Any],
    emitter_registration_witness: dict[str, Any],
    schema_binding_witness: dict[str, Any],
    errors: list[str],
) -> None:
    if record.get("receipt_version") != EXPECTED_RECEIPT_VERSION:
        errors.append("receipt_version must match read_only_worker_runtime_receipt_schema_binding_activation_witness.v1")
    expected_refs = {
    ...
    if record.get("selected_worker_path") != emitter_registration_witness.get("selected_worker_path"):
        errors.append("schema-binding activation witness selected_worker_path must match emitter registration witness")
    if record.get("selected_worker_path") != schema_binding_witness.get("selected_worker_path"):
        errors.append("schema-binding activation witness selected_worker_path must match schema binding witness")
...
def _validate_authority_scope(scope: Any, errors: list[str]) -> None:
    ...
    if scope.get("live_runtime_receipt_schema_binding_activation_witness_defined") is not True:
        errors.append("authority_scope.live_runtime_receipt_schema_binding_activation_witness_defined must be true")
...
def _validate_activation_contract(
    ...
    if not isinstance(contract, dict):
        errors.append("runtime_receipt_schema_binding_activation_witness_contract must be an object")
        return
    emitter_contract = emitter_registration_witness.get("runtime_receipt_emitter_registration_witness_contract", {})
    ...
    if contract.get("worker_id") != EXPECTED_WORKER_ID:
        errors.append(
            "runtime_receipt_schema_binding_activation_witness_contract.worker_id must select "
            "worker_local_read_only_repo_inspection"
        )
    if contract.get("worker_id") != emitter_contract.get("worker_id"):
        errors.append("runtime_receipt_schema_binding_activation_witness_contract.worker_id must match emitter witness")
    if contract.get("worker_id") != schema_binding_contract.get("worker_id"):
        errors.append(
            "runtime_receipt_schema_binding_activation_witness_contract.worker_id must match schema binding witness"
        )
    if contract.get("capability") != EXPECTED_WORKER_PATH:
        errors.append(
            "runtime_receipt_schema_binding_activation_witness_contract.capability must be read_only_repo_inspection"
        )
    if contract.get("operation_family") != EXPECTED_OPERATION_FAMILY:
        errors.append(
            "runtime_receipt_schema_binding_activation_witness_contract.operation_family must be local_repo_inspection"
        )
    if contract.get("witness_mode") != EXPECTED_WITNESS_MODE:
        errors.append(
            "runtime_receipt_schema_binding_activation_witness_contract.witness_mode must be "
            "LIVE_RUNTIME_RECEIPT_SCHEMA_BINDING_ACTIVATION_WITNESS_ONLY"
        )
    if contract.get("runtime_receipt_kind") != EXPECTED_RUNTIME_RECEIPT_KIND:
        errors.append(
            "runtime_receipt_schema_binding_activation_witness_contract.runtime_receipt_kind must be "
            "future_worker_runtime_receipt"
        )
    if contract.get("source_emitter_registration_witness_ref") != EXPECTED_EMITTER_REGISTRATION_WITNESS_REF:
        errors.append(
            "runtime_receipt_schema_binding_activation_witness_contract.source_emitter_registration_witness_ref "
            "is invalid"
        )
    if contract.get("source_schema_binding_witness_ref") != EXPECTED_SCHEMA_BINDING_WITNESS_REF:
        errors.append(
            "runtime_receipt_schema_binding_activation_witness_contract.source_schema_binding_witness_ref is invalid"
        )
    if contract.get("target_runtime_receipt_schema_binding_activation_ref") != EXPECTED_TARGET_SCHEMA_BINDING_ACTIVATION_REF:
        errors.append(
            "runtime_receipt_schema_binding_activation_witness_contract."
            "target_runtime_receipt_schema_binding_activation_ref is invalid"
        )
    if contract.get("schema_binding_activation_profile") != EXPECTED_SCHEMA_BINDING_ACTIVATION_PROFILE:
        errors.append(
            "runtime_receipt_schema_binding_activation_witness_contract.schema_binding_activation_profile is invalid"
        )
    ...
    if not isinstance(obligations, list) or len(obligations) < 8:
        errors.append("activation_obligations_checked must list future schema-binding activation obligations")
    else:
...
def _validate_admission_decision(decision: Any, errors: list[str]) -> None:
    ...
    if decision.get("runtime_receipt_schema_binding_activation_witness_defined") is not True:
        errors.append("admission_decision.runtime_receipt_schema_binding_activation_witness_defined must be true")
    for field_name in (
        "runtime_receipt_schema_binding_activated",
        "runtime_receipt_emitter_registered",
        "schema_registry_write_admitted",
...
def _validate_contract_summary(record: dict[str, Any], errors: list[str]) -> None:
    contract = record.get("runtime_receipt_schema_binding_activation_witness_contract")
    ...
    if not all(isinstance(value, dict) for value in (contract, evaluation, decision, refs, summary)):
        errors.append("schema-binding activation contract, evaluation, decision, refs, and summary must be objects")
        return
    expected_counts = {
...
def main(argv: list[str] | None = None) -> int:
    """Validate ReadOnlyWorkerRuntimeReceiptSchemaBindingActivationWitness artifacts from the CLI."""

    parser = argparse.ArgumentParser(
        description="Validate ReadOnlyWorkerRuntimeReceiptSchemaBindingActivationWitness contract."
    )
    parser.add_argument("--schema", type=Path, default=DEFAULT_SCHEMA_PATH)
    parser.add_argument("--receipt", type=Path, default=DEFAULT_RECEIPT_PATH)
    parser.add_argument(
        "--emitter-registration-witness",
        type=Path,
        default=DEFAULT_EMITTER_REGISTRATION_WITNESS_PATH,
    )
    ...
    errors = validate_runtime_receipt_schema_binding_activation_witness(
        args.schema,
        args.receipt,
        args.emitter_registration_witness,
    ...
    if args.json:
        print(
            json.dumps(
                {
                    "receipt_id": "read_only_worker_runtime_receipt_schema_binding_activation_witness_validation",
                    "schema_path": workspace_display_path(args.schema),
                    "receipt_path": workspace_display_path(args.receipt),
                    "emitter_registration_witness_path": workspace_display_path(args.emitter_registration_witness),
        ...
        print("[PASS] read_only_worker_runtime_receipt_schema_binding_activation_witness")
