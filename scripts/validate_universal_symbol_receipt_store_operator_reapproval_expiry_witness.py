"""Validate the Universal Symbol receipt-store operator reapproval expiry witness.

Purpose: prove UniversalSymbol receipt-store reapproval and expiry binding
remain blocked until live decision ref, issued time, expiry, reapproval window,
staleness policy, operator identity, revocation check, and audit evidence exist.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: reapproval-expiry schema/example, approval decision witness,
operator identity witness, revocation witness, docs, proof matrix, and tests.
Invariants: temporal authority, approval recording, receipt append, runtime
...
DEFAULT_SCHEMA_PATH = (
    REPO_ROOT / "schemas" / "universal_symbol_receipt_store_operator_reapproval_expiry_witness.schema.json"
)
DEFAULT_WITNESS_PATH = (
    REPO_ROOT / "examples" / "universal_symbol_receipt_store_operator_reapproval_expiry_witness.foundation.json"
)

AUTHORITY_DENIAL_FIELDS: tuple[str, ...] = (
    "receipt_store_reapproval_expiry_bound",
...
REQUIRED_REQUIREMENT_IDS: tuple[str, ...] = (
    ...
    "requirement://issued-at",
    "requirement://expires-at",
    "requirement://reapproval-window",
    "requirement://staleness-policy",
    "requirement://operator-identity-witness",
    "requirement://revocation-check",
...
TRUE_REJECTION_FIELDS: tuple[str, ...] = (
    "missing_approval_decision_blocks_reapproval",
    "missing_expiry_blocks_reapproval",
    "expired_decision_blocks_approval",
    "missing_reapproval_window_blocks_approval",
    "unknown_hard_constraint_blocks_reapproval",
...
REQUIRED_BLOCKED_REASONS: tuple[str, ...] = (
    ...
    "staleness_policy_missing",
    "operator_identity_witness_missing",
    "revocation_check_missing",
    "audit_receipt_missing",
    "receipt_store_reapproval_binding_forbidden",
...
class UniversalSymbolReceiptStoreOperatorReapprovalExpiryWitnessError(ValueError):
    """Raised when the reapproval-expiry witness violates Foundation Mode."""
...
def load_json_object(path: Path) -> dict[str, Any]:
    try:
        ...
        raise UniversalSymbolReceiptStoreOperatorReapprovalExpiryWitnessError(f"missing file: {path}") from exc
    except json.JSONDecodeError as exc:
        raise UniversalSymbolReceiptStoreOperatorReapprovalExpiryWitnessError(f"invalid json: {path}: {exc}") from exc
    if not isinstance(value, dict):
        raise UniversalSymbolReceiptStoreOperatorReapprovalExpiryWitnessError(f"expected object: {path}")
    return value


def validate_universal_symbol_receipt_store_operator_reapproval_expiry_witness(
    ...
    _validate_true_fields("temporal_constraints", witness.get("temporal_constraints"), TRUE_CONSTRAINT_FIELDS, errors)
    ...
    report = {
        ...
        "reapproval_expiry_decision": witness.get("reapproval_expiry_decision", ""),
        "authority_denial_count": len(AUTHORITY_DENIAL_FIELDS),
        "temporal_requirement_count": _list_len(witness.get("temporal_requirements")) or 0,
        "evidence_ref_count": _list_len(witness.get("evidence_refs")) or 0,
        "errors": errors,
    }
    if errors:
        raise UniversalSymbolReceiptStoreOperatorReapprovalExpiryWitnessError("; ".join(errors))
    return report


def _validate_schema_boundary(schema: Mapping[str, Any], errors: list[str]) -> None:
    if schema.get("$id") != "urn:mullusi:schema:universal-symbol-receipt-store-operator-reapproval-expiry-witness:1":
...
def _validate_witness_boundary(witness: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if witness.get("reapproval_expiry_decision") != "blocked_pending_live_reapproval_window_expiry_and_audit_evidence":
        errors.append("reapproval_expiry_decision must remain blocked")
    if witness.get("reapproval_expiry_witness_is_not_temporal_authority") is not True:
        errors.append("reapproval expiry witness must not grant temporal authority")


def _validate_requirements(witness: Mapping[str, Any], errors: list[str]) -> None:
    requirements = witness.get("temporal_requirements")
    if not isinstance(requirements, list) or not requirements:
        errors.append("temporal_requirements must be non-empty")
        return
    requirement_ids: list[str] = []
    for requirement in requirements:
        if not isinstance(requirement, dict):
            errors.append("temporal requirement entries must be objects")
            continue
        ...
        if requirement.get("proof_state") not in {"Unknown", "BudgetUnknown", "Fail"}:
            errors.append(f"{requirement.get('requirement_id')}: proof_state must block reapproval")
        if requirement.get("current_decision") != "reapproval_expiry_blocked":
            errors.append(f"{requirement.get('requirement_id')}: current_decision must be reapproval_expiry_blocked")
    ...
    _require_members("temporal_requirements", requirement_ids, REQUIRED_REQUIREMENT_IDS, errors)
...
def _validate_contract_summary(witness: Mapping[str, Any], errors: list[str]) -> None:
    ...
    observed = {
        "temporal_requirement_count": _list_len(witness.get("temporal_requirements")),
        "authority_denial_count": len(AUTHORITY_DENIAL_FIELDS),
        "temporal_constraint_count": len(TRUE_CONSTRAINT_FIELDS),
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        report = validate_universal_symbol_receipt_store_operator_reapproval_expiry_witness(args.witness, args.schema)
    except UniversalSymbolReceiptStoreOperatorReapprovalExpiryWitnessError as exc:
        if args.json:
            print(json.dumps({"valid": False, "errors": str(exc).split("; ")}, indent=2, sort_keys=True))
        else:
            print(f"[FAIL] universal_symbol_receipt_store_operator_reapproval_expiry_witness: {exc}")
            print("STATUS: failed")
        return 1
    if args.json:
        print(json.dumps(report, indent=2, sort_keys=True))
    else:
        print("[PASS] universal_symbol_receipt_store_operator_reapproval_expiry_witness")
        print("STATUS: passed")
    return 0
