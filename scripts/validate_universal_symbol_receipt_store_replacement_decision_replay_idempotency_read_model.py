#!/usr/bin/env python3
"""Validate the replacement decision replay/idempotency operator read model.

Purpose: prove the replay/idempotency operator projection is read-only and
bounded while replay binding, idempotency acceptance, receipt-store append, raw
payload exposure, mutation, and terminal closure remain denied.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: replay/idempotency read-model schema/example and replay
idempotency witness validator constants.
Invariants:
  - The read model is a projection, not replay authority.
  - Every replay requirement appears exactly once.
...
from scripts.validate_universal_symbol_receipt_store_replacement_decision_replay_idempotency_witness import (  # noqa: E402
    AUTHORITY_DENIAL_FIELDS,
    DEFAULT_WITNESS_PATH as DEFAULT_REPLAY_WITNESS_PATH,
...
DEFAULT_SCHEMA_PATH = (
    REPO_ROOT
    / "schemas"
    / "universal_symbol_receipt_store_replacement_decision_replay_idempotency_read_model.schema.json"
)
DEFAULT_READ_MODEL_PATH = (
    REPO_ROOT
    / "examples"
    / "universal_symbol_receipt_store_replacement_decision_replay_idempotency_read_model.foundation.json"
)
...
EXPECTED_OPERATOR_STATUS_SUMMARY: dict[str, str] = {
    "primary_status": "Replacement replay blocked",
    "replay_status": "Awaiting evidence",
    "idempotency_status": "Not accepted",
    "raw_detail_visibility": "hidden_by_default",
    "operator_action": "Collect replacement replay evidence",
}


class UniversalSymbolReceiptStoreReplacementDecisionReplayIdempotencyReadModelError(ValueError):
    """Raised when the replay/idempotency read model violates Foundation Mode."""


def validate_universal_symbol_receipt_store_replacement_decision_replay_idempotency_read_model(
    ...
    if errors:
        raise UniversalSymbolReceiptStoreReplacementDecisionReplayIdempotencyReadModelError(
            "; ".join(errors)
        )
    return report
...
def _validate_witness_projection(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if projection.get("replay_idempotency_decision") != EXPECTED_REPLAY_DECISION:
        errors.append("witness_projection.replay_idempotency_decision must remain blocked")
    if projection.get("authority_granted") is not False:
        errors.append("witness_projection.authority_granted must remain false")
    if projection.get("replay_requirement_count") != len(REQUIRED_REQUIREMENT_IDS):
        errors.append("witness_projection.replay_requirement_count drift")
...
def _validate_source_witness_alignment(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if projection.get("witness_ref") != EXPECTED_WITNESS_REF:
        return
    try:
        witness = load_json_object(DEFAULT_REPLAY_WITNESS_PATH)
    except UniversalSymbolReceiptStoreReplacementDecisionReplayIdempotencyReadModelError as exc:
        errors.append(f"source replay witness unavailable: {exc}")
        return

    witness_summary = _mapping(witness.get("contract_summary"))
    expected_counts = {
        "replay_requirement_count": witness_summary.get("replay_requirement_count"),
        "blocked_reason_count": witness_summary.get("blocked_reason_count"),
    }
    for field_name, expected_count in expected_counts.items():
        if projection.get(field_name) != expected_count:
            errors.append(f"witness_projection.{field_name} must match source replay witness")
    if projection.get("replay_idempotency_decision") != witness.get("replay_idempotency_decision"):
        errors.append("witness_projection.replay_idempotency_decision must match source replay witness")
...
def _validate_requirement_rows(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    for row in rows:
        ...
        if row.get("proof_state") not in {"Unknown", "BudgetUnknown", "Fail"}:
            errors.append(f"{row.get('requirement_id')}: proof_state must block replay")
...
def _validate_evidence_refs(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    _require_unique("evidence_refs", [str(ref) for ref in refs], errors)
...
def _require_members(
    ...
    _require_unique(field_name, observed_values, errors)


def _require_unique(field_name: str, observed_values: list[str], errors: list[str]) -> None:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print(
                "[FAIL] universal_symbol_receipt_store_replacement_decision_replay_idempotency_read_model: "
                f"{exc}"
            )
            print("STATUS: failed")
        return 1
    ...
    if args.json:
        ...
        print("[PASS] universal_symbol_receipt_store_replacement_decision_replay_idempotency_read_model")
        print("STATUS: passed")
    return 0
