#!/usr/bin/env python3
"""Validate Agentic Service Harness GitHub task receipt-emitter dry-run.

Purpose: prove the GitHub repository task service can record a simulated
receipt-emitter dry-run while receipt-store append and all repository effects
remain denied.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: schemas/agentic_service_harness_github_task_receipt_emitter_dry_run.schema.json,
...
Invariants:
  - The dry-run binds to the validated GitHub repo task service contract.
  - Receipt-store append is explicitly blocked.
  - Live adapter execution, repository writes, branch creation, PR creation,
...
DEFAULT_SCHEMA = (
    REPO_ROOT / "schemas" / "agentic_service_harness_github_task_receipt_emitter_dry_run.schema.json"
)
DEFAULT_EXAMPLES = (
    REPO_ROOT / "examples" / "agentic_service_harness_github_task_receipt_emitter_dry_run.foundation.json",
)
DEFAULT_OUTPUT = (
    REPO_ROOT
    / ".change_assurance"
    / "agentic_service_harness_github_task_receipt_emitter_dry_run_validation.json"
)
...
EXPECTED_EMITTER_ID = "github-task-receipt-emitter-dry-run-foundation"
EXPECTED_EMITTER_MODE = "EMITTER_DRY_RUN_ONLY"
EXPECTED_RESULT_STATE = "DRY_RUN_RECORDED"
EXPECTED_SIMULATED_RECEIPT_KIND = "future_github_task_receipt"
EXPECTED_APPEND_DECISION = "APPEND_BLOCKED_AWAITING_APPROVAL_AND_STORE_BINDING"
EXPECTED_APPEND_TARGET_REF = "receipt-store://agentic-service-harness/github-task"
...
class GitHubTaskReceiptEmitterDryRunValidation:
    """Schema and semantic validation report for GitHub task dry-run emitter."""
...
def validate_agentic_service_harness_github_task_receipt_emitter_dry_run(
    *,
...
) -> GitHubTaskReceiptEmitterDryRunValidation:
    """Validate GitHub task receipt-emitter dry-run examples."""
    errors: list[str] = []
    schema = _load_json_object(schema_path, "GitHub task receipt-emitter dry-run schema", errors)
    ...
    source_contract = _load_json_object(
        ...
        "GitHub repo task service source contract",
        errors,
    )
    ...
    for example_path in example_paths:
        example = _load_json_object(
            example_path,
            f"GitHub task receipt-emitter dry-run example {_path_label(example_path)}",
            errors,
        )
        if not example:
            continue
    ...
    return GitHubTaskReceiptEmitterDryRunValidation(
        ok=not errors,
        errors=tuple(errors),
...
def write_github_task_receipt_emitter_dry_run_validation(
    validation: GitHubTaskReceiptEmitterDryRunValidation,
    output_path: Path,
) -> Path:
    """Write one deterministic GitHub task receipt-emitter dry-run report."""
...
def _validate_source_binding(
    ...
    if example.get("source_contract_ref") != EXPECTED_SOURCE_CONTRACT_REF:
        errors.append(f"{label}: source_contract_ref must bind the GitHub repo task service fixture")
    scope = example.get("scope")
    source_scope = source_contract.get("scope")
    task_service = source_contract.get("task_service")
    ...
    if not isinstance(source_scope, Mapping):
        ...
        return
    for key in ("tenant_id", "organization_id", "project_id", "repository_connection_id", "repository_slug"):
        if scope.get(key) != source_scope.get(key):
            errors.append(f"{label}: scope.{key} must match source contract")
    if scope.get("task_service_id") != EXPECTED_TASK_SERVICE_ID:
        errors.append(f"{label}: scope.task_service_id must be {EXPECTED_TASK_SERVICE_ID}")
    if isinstance(task_service, Mapping) and task_service.get("service_id") != scope.get("task_service_id"):
        errors.append(f"{label}: scope.task_service_id must match source task_service.service_id")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Parse GitHub task receipt-emitter dry-run validation arguments."""
    parser = argparse.ArgumentParser(
        description="Validate the harness GitHub task receipt-emitter dry-run contract."
    )
...
def main(argv: list[str] | None = None) -> int:
    """CLI entry point for GitHub task receipt-emitter dry-run validation."""
    ...
    validation = validate_agentic_service_harness_github_task_receipt_emitter_dry_run(
    ...
    write_github_task_receipt_emitter_dry_run_validation(validation, Path(args.output))
    if args.json:
        print(json.dumps(validation.as_dict(), indent=2, sort_keys=True))
    elif validation.ok:
        print("AGENTIC SERVICE HARNESS GITHUB TASK RECEIPT EMITTER DRY-RUN VALID")
    else:
        print(
            "AGENTIC SERVICE HARNESS GITHUB TASK RECEIPT EMITTER DRY-RUN INVALID "
