#!/usr/bin/env python3
"""Validate operator value-binding verifier validation preflight."""
...
DEFAULT_SCHEMA = (
    REPO_ROOT
    / "schemas"
    / "personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_validation_preflight.schema.json"
)
DEFAULT_RECEIPT_SCHEMA = REPO_ROOT / "schemas" / "personal_assistant_receipt.schema.json"
RUNTIME_GENERATED_AT = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_VALIDATION_PREFLIGHT_GENERATED_AT
...
ALLOWED_POLICY_FIELD_NAMES = frozenset(
    {
        ...
        "verifier_ref_intake_projection",
        "submitted_verifier_ref_projection",
        "raw_verifier_payload_projection",
...
class PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierValidationPreflightValidation:
    """Validation result for verifier validation preflight."""

    valid: bool
    runtime_validated: bool
    verifier_validation_preflight_count: int
    submitted_verifier_ref_count: int
    shape_checked_verifier_ref_count: int
    scope_checked_verifier_ref_count: int
    validated_verifier_ref_count: int
    bound_verifier_ref_count: int
...
def validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_validation_preflight(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    receipt_schema_path: Path = DEFAULT_RECEIPT_SCHEMA,
) -> PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierValidationPreflightValidation:
    """Validate runtime verifier validation preflight."""

    errors: list[str] = []
    schema = _load_json_object(schema_path, "operator verifier validation preflight schema", errors)
    ...
    errors.extend(_validate_verifier_validation_preflight_semantics(envelope, receipt_schema))
    _scan_private_or_secret_payload(envelope, errors, path="$runtime")
    return PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierValidationPreflightValidation(
        valid=not errors,
        runtime_validated=not errors,
        verifier_validation_preflight_count=int(envelope.get("verifier_validation_preflight_count", 0)),
        submitted_verifier_ref_count=int(summary.get("submitted_verifier_ref_count", 0)),
        shape_checked_verifier_ref_count=int(summary.get("shape_checked_verifier_ref_count", 0)),
        scope_checked_verifier_ref_count=int(summary.get("scope_checked_verifier_ref_count", 0)),
        validated_verifier_ref_count=int(summary.get("validated_verifier_ref_count", 0)),
        bound_verifier_ref_count=int(summary.get("bound_verifier_ref_count", 0)),
...
def _validate_verifier_validation_preflight_semantics(
    ...
    _require_false_fields(
        effect_boundary,
        FALSE_AUTHORITY_FIELDS | {"raw_verifier_payload_present", "raw_evidence_payload_present", "raw_operator_value_present"},
        "effect_boundary",
        errors,
    )
    _require_private_payload_policy(_mapping(envelope.get("private_payload_policy")), errors)

    records = envelope.get("verifier_validation_preflights")
    if not isinstance(records, list):
        errors.append("verifier_validation_preflights must be a list")
        return tuple(errors)
    if envelope.get("verifier_validation_preflight_count") != len(records):
        errors.append("verifier_validation_preflight_count must equal verifier_validation_preflights length")
    record_ids: list[str] = []
    receipt_ids: list[str] = []
    coverage: dict[str, set[str]] = {}
    for index, record in enumerate(records):
        if not isinstance(record, dict):
            errors.append(f"verifier_validation_preflights[{index}] must be an object")
            continue
        record_ids.append(str(record.get("verifier_validation_preflight_record_id", "")))
        ...
        _require_source_ref(index, _mapping(record.get("verifier_ref_intake_ref")), errors)
        _require_verifier_validation_preflight(index, _mapping(record.get("verifier_validation_preflight")), record, errors)
        _require_false_fields(
            ...
            f"verifier_validation_preflights[{index}].authority_status",
            errors,
        )
        receipt = _mapping(record.get("receipt"))
        if receipt_schema:
            errors.extend(
                f"verifier_validation_preflights[{index}].receipt {message}"
        ...
        if receipt.get("decision") != "blocked" or receipt.get("outcome") != "AwaitingEvidence":
            errors.append(f"verifier_validation_preflights[{index}].receipt must remain blocked AwaitingEvidence")
        metadata = _mapping(receipt.get("metadata"))
        if metadata.get("operator_reapproval_decision_receipt_value_binding_record_verifier_validation_preflight_is_execution") is not False:
            errors.append(f"verifier_validation_preflights[{index}].receipt.metadata execution flag must be false")
        _require_false_fields(
            metadata,
            FALSE_AUTHORITY_FIELDS | {"raw_verifier_payload_present"},
            f"verifier_validation_preflights[{index}].receipt.metadata",
            errors,
        )
        receipt_ids.append(str(receipt.get("receipt_id", "")))
    if set(coverage) != EXPECTED_EVIDENCE_KINDS:
        errors.append("verifier_validation_preflights must cover all governed evidence kinds")
    for evidence_kind, requirement_kinds in coverage.items():
        if requirement_kinds != EXPECTED_REQUIREMENT_KINDS:
            errors.append(f"verifier_validation_preflights for {evidence_kind} must cover all verifier requirement kinds")
    if envelope.get("verifier_validation_preflight_record_ids") != record_ids:
        errors.append("verifier_validation_preflight_record_ids must match record order")
...
def _require_source_ref(index: int, payload: Mapping[str, Any], errors: list[str]) -> None:
    ...
    for field_name, expected_value in expected.items():
        if payload.get(field_name) != expected_value:
            errors.append(f"verifier_validation_preflights[{index}].verifier_ref_intake_ref.{field_name} must be {expected_value}")


def _require_verifier_validation_preflight(
    index: int,
    payload: Mapping[str, Any],
    record: Mapping[str, Any],
    errors: list[str],
) -> None:
    expected_true = {"verifier_ref_only", "verifier_ref_present", "shape_checked", "scope_checked", "preflight_checks_passed"}
    expected_false = {
    ...
    if payload.get("submitted_verifier_ref") != record.get("submitted_verifier_ref"):
        errors.append(f"verifier_validation_preflights[{index}].verifier_validation_preflight submitted ref must match record")
    if payload.get("requirement_kind") != record.get("requirement_kind"):
        errors.append(f"verifier_validation_preflights[{index}].verifier_validation_preflight requirement kind must match record")
    for field_name in expected_true:
        if payload.get(field_name) is not True:
            errors.append(f"verifier_validation_preflights[{index}].verifier_validation_preflight.{field_name} must be true")
    for field_name in expected_false:
        if payload.get(field_name) is not False:
            errors.append(f"verifier_validation_preflights[{index}].verifier_validation_preflight.{field_name} must be false")
    if payload.get("blocking_reason") != "verifier_validation_requires_governed_verifier_execution":
        errors.append(f"verifier_validation_preflights[{index}].verifier_validation_preflight blocking_reason must remain verifier execution required")
...
def _require_summary(envelope: Mapping[str, Any], records: list[Any], errors: list[str]) -> None:
    ...
    expected = {
        ...
        "shape_checked_verifier_ref_count": len(records),
        "scope_checked_verifier_ref_count": len(records),
        "raw_verifier_payload_count": 0,
...
def main() -> int:
    ...
    result = validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_validation_preflight(
    ...
    if args.json:
        ...
        print("PERSONAL ASSISTANT OPERATOR REAPPROVAL DECISION RECEIPT VALUE BINDING RECORD VERIFIER VALIDATION PREFLIGHT VALID")
    else:
