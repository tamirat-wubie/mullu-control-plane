#!/usr/bin/env python3
"""Validate Agentic Service Harness live producer operator decision record.

Purpose: prove generic continuation records no explicit operator approval or
rejection and grants no live producer authority.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: gateway.agentic_service_harness_live_producer_operator_decision_record,
schemas/agentic_service_harness_live_producer_operator_decision_record.schema.json,
examples/agentic_service_harness_live_producer_operator_decision_record.local.json,
and scripts.validate_agentic_service_harness_live_producer_operator_decision_evidence.
Invariants:
  - Decision record remains `AwaitingEvidence`.
...
DEFAULT_SCHEMA = (
    REPO_ROOT / "schemas" / "agentic_service_harness_live_producer_operator_decision_record.schema.json"
)
DEFAULT_FIXTURE = (
    REPO_ROOT / "examples" / "agentic_service_harness_live_producer_operator_decision_record.local.json"
)
...
class LiveProducerOperatorDecisionRecordValidation:
    """Validation result for operator decision record intake."""

    ok: bool
    errors: tuple[str, ...]
    schema_path: str
    fixture_path: str
    record_boundary_id: str
    record_status: str
...
def validate_live_producer_operator_decision_record(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    fixture_path: Path = DEFAULT_FIXTURE,
) -> tuple[LiveProducerOperatorDecisionRecordValidation, dict[str, Any]]:
    """Validate the checked-in decision record envelope and produced packet."""
    errors: list[str] = []
    schema = _load_json_object(schema_path, "operator decision record schema", errors)
    fixture = _load_json_object(fixture_path, "operator decision record fixture", errors)
    evidence_validation, decision_evidence = validate_live_producer_operator_decision_evidence()
    errors.extend(f"source operator decision evidence: {error}" for error in evidence_validation.errors)
    ...
    if schema and fixture:
        ...
        _validate_decision_record_semantics(fixture, errors, _path_label(fixture_path))
    if schema and produced_record:
        errors.extend(
            f"produced operator decision record: {error}"
            for error in _validate_schema_instance(schema, produced_record)
        )
        _validate_decision_record_semantics(produced_record, errors, "produced operator decision record")
    ...
    accepted = observed.get("accepted_record_shapes")
    validation = LiveProducerOperatorDecisionRecordValidation(
        ok=not errors,
        errors=tuple(errors),
        schema_path=_path_label(schema_path),
        fixture_path=_path_label(fixture_path),
        record_boundary_id=str(observed.get("record_boundary_id", "")),
        record_status=str(observed.get("record_status", "")),
        accepted_record_count=len(accepted) if isinstance(accepted, list) else 0,
        authority_denial_count=len(FALSE_AUTHORITY_FLAGS) + 1,
    )
    return validation, produced_record


def _validate_decision_record_semantics(record: Mapping[str, Any], errors: list[str], label: str) -> None:
    if record.get("record_boundary_id") != OPERATOR_DECISION_RECORD_ID:
        errors.append(f"{label}: record_boundary_id mismatch")
    ...
    _validate_accepted_records(record, errors, label)
...
def _validate_accepted_records(record: Mapping[str, Any], errors: list[str], label: str) -> None:
    accepted = record.get("accepted_record_shapes")
    if not isinstance(accepted, list):
        errors.append(f"{label}: accepted_record_shapes must be a list")
        return
    ...
    if tuple(observed_kinds) != ACCEPTED_RECORD_KINDS:
        errors.append(f"{label}: accepted record kinds must match required order")
    for entry in accepted:
        if not isinstance(entry, Mapping):
            errors.append(f"{label}: accepted record entries must be objects")
            continue
        ...
        if entry.get("authority_effect") != "records_operator_intent_only_no_live_authority":
...
def _validate_fixture_matches_produced(
    ...
    comparable_fields = (
        "record_boundary_id",
        "solver_outcome",
        "source_decision_evidence_ref",
        "record_status",
        "current_input_kind",
        "current_decision_kind",
        "generic_continuation_records_decision",
        "approval_recorded",
        "rejection_recorded",
        "authority_granted",
        "scope",
    ...
    for field_name in comparable_fields:
        if fixture.get(field_name) != produced_record.get(field_name):
            errors.append(f"fixture does not match produced operator decision record field: {field_name}")
...
def main(argv: list[str] | None = None) -> int:
    """Run the live producer operator decision record validator."""
    ...
    if args.json:
        ...
        print("AGENTIC SERVICE HARNESS LIVE PRODUCER OPERATOR DECISION RECORD VALID")
    else:
        print(
            "AGENTIC SERVICE HARNESS LIVE PRODUCER OPERATOR DECISION RECORD INVALID "
