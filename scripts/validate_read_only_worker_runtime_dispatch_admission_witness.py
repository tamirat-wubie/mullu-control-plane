#!/usr/bin/env python3
"""Validate the ReadOnlyWorkerRuntimeDispatchAdmissionWitness contract.

Purpose: verify Foundation Mode witness evidence for a future live
read_only_repo_inspection runtime dispatch admission boundary
...
Invariants:
  ...
  - Runtime runner registration, dispatch endpoint registration, runtime receipt
    emitter registration, schema binding, dispatch admission, receipt
...
DEFAULT_SCHEMA_PATH = (
    WORKSPACE_ROOT / "schemas" / "read_only_worker_runtime_dispatch_admission_witness.schema.json"
)
DEFAULT_RECEIPT_PATH = (
    WORKSPACE_ROOT / "examples" / "read_only_worker_runtime_dispatch_admission_witness.foundation.json"
)
EXPECTED_SCHEMA_ID = "urn:mullusi:schema:read-only-worker-runtime-dispatch-admission-witness:1"
EXPECTED_SCHEMA_TITLE = "Read-Only Worker runtime dispatch admission Witness"
EXPECTED_RECEIPT_VERSION = "read_only_worker_runtime_dispatch_admission_witness.v1"
...
EXPECTED_TARGET_DISPATCH_ADMISSION_REF = (
    "candidate://runtime-dispatch/admission/read-only-repo-inspection"
)
EXPECTED_DISPATCH_ADMISSION_PROFILE = (
    "LOCAL_READ_ONLY_REPO_INSPECTION_RUNTIME_DISPATCH_ADMISSION"
)
EXPECTED_ADMISSION_DECISION = (
    "LIVE_RUNTIME_DISPATCH_ADMISSION_WITNESS_BLOCKED_AWAITING_OPERATOR_APPROVAL_AND_LIVE_EVIDENCE"
)
...
REQUIRED_ACTIVATION_INPUT_REFS = (
    "evidence://operator-approval/live-runtime-dispatch-admission",
    "evidence://runtime-receipt-emission-admission-witness",
    ...
    "evidence://runtime-dispatch-admission-boundary/read-only-repo-inspection",
...
REQUIRED_BLOCKED_REASON_REFS = (
    "blocked://operator-approval/live-runtime-dispatch-admission-missing",
...
ACTIVATION_EVALUATION_TRUE_FIELDS = (
    ...
    "dispatch_admission_boundary_required",
    "receipt_store_append_boundary_required",
    "emitter_registration_required_before_schema_activation",
    "schema_binding_activation_required_before_dispatch_admission",
    "store_activation_required_before_dispatch_admission",
...
class ReadOnlyWorkerRuntimeDispatchAdmissionWitnessError(ValueError):
    """Raised when a runtime dispatch admission witness artifact cannot load."""
...
def load_json_object(json_path: Path, label: str) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ReadOnlyWorkerRuntimeDispatchAdmissionWitnessError(f"{label} must be a JSON object")
    return payload
...
def validate_runtime_dispatch_admission_witness_record(
    record: Any,
    ...
    if not isinstance(record, dict):
        errors.append("read-only worker runtime dispatch admission witness must be a JSON object")
        return errors
    ...
    _validate_activation_contract(
        record.get("runtime_dispatch_admission_witness_contract"),
        schema_binding_activation_payload,
        store_write_path_payload,
        store_activation_payload,
        schema_binding_payload,
...
def validate_runtime_dispatch_admission_witness(
    ...
    receipt = load_json_object(receipt_path, "ReadOnlyWorkerRuntimeDispatchAdmissionWitness")
...
def build_mutated_runtime_dispatch_admission_witness(**updates: Any) -> dict[str, Any]:
    """Build a deterministic mutated copy of the default witness receipt."""

    receipt = load_json_object(DEFAULT_RECEIPT_PATH, "ReadOnlyWorkerRuntimeDispatchAdmissionWitness")
...
def _validate_top_level_refs(
    ...
    if record.get("receipt_version") != EXPECTED_RECEIPT_VERSION:
        errors.append("receipt_version must match read_only_worker_runtime_dispatch_admission_witness.v1")
    expected_refs = {
    ...
    if record.get("selected_worker_path") != schema_binding_activation_witness.get("selected_worker_path"):
        errors.append(
            "dispatch admission witness selected_worker_path must match schema-binding activation witness"
        )
    if record.get("selected_worker_path") != store_write_path_witness.get("selected_worker_path"):
        errors.append("dispatch admission witness selected_worker_path must match store write-path witness")
...
def _validate_authority_scope(scope: Any, errors: list[str]) -> None:
    ...
    if scope.get("live_runtime_dispatch_admission_witness_defined") is not True:
        errors.append("authority_scope.live_runtime_dispatch_admission_witness_defined must be true")
...
def _validate_activation_contract(
    ...
    if not isinstance(contract, dict):
        errors.append("runtime_dispatch_admission_witness_contract must be an object")
        return
    ...
    if contract.get("worker_id") != activation_contract.get("worker_id"):
        errors.append(
            "runtime_dispatch_admission_witness_contract.worker_id must match schema-binding activation witness"
        )
    if contract.get("worker_id") != store_write_path_contract.get("worker_id"):
        errors.append(
            "runtime_dispatch_admission_witness_contract.worker_id must match store write-path witness"
        )
    if contract.get("worker_id") != store_activation_contract.get("worker_id"):
        errors.append(
            "runtime_dispatch_admission_witness_contract.worker_id must match store activation witness"
        )
    ...
    if contract.get("capability") != EXPECTED_WORKER_PATH:
        errors.append(
            "runtime_dispatch_admission_witness_contract.capability must be read_only_repo_inspection"
        )
    if contract.get("operation_family") != EXPECTED_OPERATION_FAMILY:
        errors.append(
            "runtime_dispatch_admission_witness_contract.operation_family must be local_repo_inspection"
        )
    ...
    if contract.get("runtime_receipt_kind") != EXPECTED_RUNTIME_RECEIPT_KIND:
        errors.append(
            "runtime_dispatch_admission_witness_contract.runtime_receipt_kind must be "
    ...
    if contract.get("source_schema_binding_activation_witness_ref") != EXPECTED_SCHEMA_BINDING_ACTIVATION_WITNESS_REF:
        errors.append(
            "runtime_dispatch_admission_witness_contract.source_schema_binding_activation_witness_ref "
            "is invalid"
        )
    if contract.get("source_store_write_path_witness_ref") != EXPECTED_STORE_WRITE_PATH_WITNESS_REF:
        errors.append(
            "runtime_dispatch_admission_witness_contract.source_store_write_path_witness_ref "
            "is invalid"
        )
    if contract.get("source_store_activation_witness_ref") != EXPECTED_STORE_ACTIVATION_WITNESS_REF:
        errors.append(
            "runtime_dispatch_admission_witness_contract.source_store_activation_witness_ref "
            "is invalid"
        )
    ...
    if contract.get("dispatch_admission_profile") != EXPECTED_DISPATCH_ADMISSION_PROFILE:
        errors.append(
            "runtime_dispatch_admission_witness_contract.dispatch_admission_profile is invalid"
        )
    ...
    if not isinstance(obligations, list) or len(obligations) < 8:
        errors.append("activation_obligations_checked must list future dispatch admission obligations")
    else:
...
def _validate_admission_decision(decision: Any, errors: list[str]) -> None:
    ...
    if decision.get("runtime_dispatch_admission_witness_defined") is not True:
        errors.append("admission_decision.runtime_dispatch_admission_witness_defined must be true")
    for field_name in (
...
def _validate_contract_summary(record: dict[str, Any], errors: list[str]) -> None:
    contract = record.get("runtime_dispatch_admission_witness_contract")
    ...
    if not all(isinstance(value, dict) for value in (contract, evaluation, decision, refs, summary)):
        errors.append("dispatch admission contract, evaluation, decision, refs, and summary must be objects")
        return
    expected_counts = {
...
def main(argv: list[str] | None = None) -> int:
    """Validate ReadOnlyWorkerRuntimeDispatchAdmissionWitness artifacts from the CLI."""

    parser = argparse.ArgumentParser(
        description="Validate ReadOnlyWorkerRuntimeDispatchAdmissionWitness contract."
    )
    ...
    if args.json:
        print(
            json.dumps(
                {
                    "receipt_id": "read_only_worker_runtime_dispatch_admission_witness_validation",
        ...
        print("[PASS] read_only_worker_runtime_dispatch_admission_witness")
