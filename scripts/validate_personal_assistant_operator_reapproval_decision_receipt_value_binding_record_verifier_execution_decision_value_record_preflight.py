#!/usr/bin/env python3
"""Validate operator value-binding verifier execution decision-value record preflight."""
...
from mcoi_runtime.personal_assistant import (  # noqa: E402
    ...
    build_default_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight,
)
...
DEFAULT_SCHEMA = (
    REPO_ROOT
    / "schemas"
    / "personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight.schema.json"
)
DEFAULT_RECEIPT_SCHEMA = REPO_ROOT / "schemas" / "personal_assistant_receipt.schema.json"
RUNTIME_GENERATED_AT = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_PREFLIGHT_GENERATED_AT
...
class PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierExecutionDecisionValueRecordPreflightValidation:
    """Validation result for verifier execution decision-value record preflight."""

    valid: bool
    runtime_validated: bool
    record_preflight_count: int
    record_contract_ready_count: int
    record_preflight_admission_count: int
...
def validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    receipt_schema_path: Path = DEFAULT_RECEIPT_SCHEMA,
) -> PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierExecutionDecisionValueRecordPreflightValidation:
    """Validate runtime verifier execution decision-value record preflight."""

    errors: list[str] = []
    schema = _load_json_object(schema_path, "decision-value record preflight schema", errors)
    receipt_schema = _load_json_object(receipt_schema_path, "receipt schema", errors)
    envelope = build_default_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight(
    ...
    errors.extend(_validate_decision_value_record_preflight_semantics(envelope, receipt_schema))
    _scan_secret_values(envelope, errors, path="$runtime")
    return PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierExecutionDecisionValueRecordPreflightValidation(
        valid=not errors,
        runtime_validated=not errors,
        record_preflight_count=int(envelope.get("record_preflight_count", 0)),
        record_contract_ready_count=int(summary.get("record_contract_ready_count", 0)),
        record_preflight_admission_count=int(summary.get("record_preflight_admission_count", 0)),
...
def _validate_decision_value_record_preflight_semantics(
    ...
    for field_name in (
        "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight_allowed",
        "decision_value_record_path_ref_binding_allowed",
        "operator_decision_value_record_preflight_projection_allowed",
    ...
    records = envelope.get("record_preflights")
    if not isinstance(records, list):
        errors.append("record_preflights must be a list")
        return tuple(errors)
    if envelope.get("record_preflight_count") != len(records):
        errors.append("record_preflight_count must equal record_preflights length")
    item_ids: list[str] = []
    receipt_ids: list[str] = []
    coverage: dict[str, set[str]] = {}
    for index, record in enumerate(records):
        if not isinstance(record, dict):
            errors.append(f"record_preflights[{index}] must be an object")
            continue
        item_ids.append(str(record.get("record_preflight_item_id", "")))
        ...
        _require_source_ref(index, _mapping(record.get("record_path_ref")), errors)
        _require_record_preflight(index, _mapping(record.get("record_preflight")), errors)
        _require_false_fields(
            _mapping(record.get("authority_status")),
            FALSE_AUTHORITY_FIELDS,
            f"record_preflights[{index}].authority_status",
            errors,
        )
        receipt = _mapping(record.get("receipt"))
        if receipt_schema:
            errors.extend(f"record_preflights[{index}].receipt {message}" for message in _validate_schema_instance(dict(receipt_schema), receipt))
        errors.extend(f"record_preflights[{index}].receipt {message}" for message in validate_personal_assistant_receipt_payload(receipt))
        metadata = _mapping(receipt.get("metadata"))
        if metadata.get("operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight_is_execution") is not False:
            errors.append(f"record_preflights[{index}].receipt.metadata execution flag must be false")
        if metadata.get("record_preflight_created") is not True:
            errors.append(f"record_preflights[{index}].receipt.metadata record_preflight_created must be true")
        _require_false_fields(metadata, FALSE_FIELDS | {"external_write_allowed"}, f"record_preflights[{index}].receipt.metadata", errors)
        receipt_ids.append(str(receipt.get("receipt_id", "")))
    if set(coverage) != EXPECTED_EVIDENCE_KINDS:
        errors.append("record_preflights must cover all governed evidence kinds")
    for evidence_kind, requirement_kinds in coverage.items():
        if requirement_kinds != EXPECTED_REQUIREMENT_KINDS:
            errors.append(f"record_preflights for {evidence_kind} must cover all verifier requirement kinds")
    if envelope.get("record_preflight_item_ids") != item_ids:
        errors.append("record_preflight_item_ids must match item order")
...
def _require_source_ref(index: int, payload: Mapping[str, Any], errors: list[str]) -> None:
    ...
    for field_name, expected_value in expected.items():
        if payload.get(field_name) != expected_value:
            errors.append(f"record_preflights[{index}].record_path_ref.{field_name} must be {expected_value}")


def _require_record_preflight(index: int, payload: Mapping[str, Any], errors: list[str]) -> None:
    for field_name in (
        "record_contract_ready",
        "record_preflight_created",
        "operator_decision_required",
        "operator_decision_value_required",
        "actual_operator_decision_value_required",
        ...
        if payload.get(field_name) is not True:
            errors.append(f"record_preflights[{index}].record_preflight.{field_name} must be true")
    if tuple(payload.get("accepted_record_kinds", ())) != EXPECTED_RECORD_KINDS:
        errors.append(f"record_preflights[{index}].record_preflight.accepted_record_kinds must match canonical values")
    if tuple(payload.get("rejected_input_kinds", ())) != EXPECTED_REJECTED_INPUT_KINDS:
        errors.append(f"record_preflights[{index}].record_preflight.rejected_input_kinds must reject generic continuations and templates")
    if tuple(payload.get("required_fields", ())) != EXPECTED_REQUIRED_FIELDS:
        errors.append(f"record_preflights[{index}].record_preflight.required_fields must match required operator refs")
    _require_false_fields(
        payload,
        FALSE_FIELDS | {"accepts_generic_continuation", "accepts_template_packet"},
        f"record_preflights[{index}].record_preflight",
        errors,
    )
    if payload.get("blocking_reason") != "actual_operator_decision_value_absent_record_path_unadmitted":
        errors.append(f"record_preflights[{index}].record_preflight blocking_reason must remain governed")
...
def _require_summary(envelope: Mapping[str, Any], records: list[Any], errors: list[str]) -> None:
    ...
    expected = {
        ...
        "record_preflight_creation_count": len(records),
        "record_preflight_admission_count": 0,
...
def main() -> int:
    ...
    result = validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight(
    ...
    if args.json:
        ...
        print("PERSONAL ASSISTANT OPERATOR REAPPROVAL DECISION RECEIPT VALUE BINDING RECORD VERIFIER EXECUTION DECISION VALUE RECORD PREFLIGHT VALID")
    else:
