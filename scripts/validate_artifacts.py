...
from mcoi_runtime.core.document import (  # noqa: E402
    extract_json_fields,
    ingest_document,
    verify_extraction,
)
from mcoi_runtime.core.template_validator import (  # noqa: E402
...
TENANT_STATUSES = frozenset({"active", "suspended", "provisioning", "decommissioning", "archived"})
WORKSPACE_STATUSES = frozenset({"active", "suspended", "provisioning", "archived"})
ENVIRONMENT_KINDS = frozenset({"development", "staging", "production", "sandbox", "dr"})
ISOLATION_LEVELS = frozenset({"strict", "standard", "shared", "custom"})
SCOPE_BOUNDARY_KINDS = frozenset(
    {"memory", "connector", "budget", "campaign", "program", "control", "report", "graph"}
)
PROMOTION_STATUSES = frozenset(
    {"requested", "approved", "in_progress", "completed", "failed", "rolled_back"}
)
RECORD_KINDS = frozenset(
    {"operational", "compliance", "audit", "evidence", "communication", "financial", "legal"}
)
RETENTION_STATUSES = frozenset({"active", "expired", "disposed", "held", "pending_review"})
HOLD_STATUSES = frozenset({"active", "released", "expired"})
...
CAUSAL_STATUSES = frozenset({"active", "hypothesized", "confirmed", "refuted"})
CAUSAL_EDGE_KINDS = frozenset({"direct", "indirect", "mediating", "confounding", "moderating"})
INTERVENTION_DISPOSITIONS = frozenset({"proposed", "applied", "rejected", "rolled_back"})
COUNTERFACTUAL_STATUSES = frozenset({"pending", "evaluated", "confirmed", "rejected"})
CAUSAL_ATTRIBUTION_STRENGTHS = frozenset({"strong", "moderate", "weak", "none"})
ALGORITHM_KINDS = frozenset({"graph_search", "matching", "routing", "scheduling", "packing", "constraint_sat"})
SOLVE_STATUSES = frozenset({"pending", "running", "solved", "infeasible", "timeout", "failed"})
CONSTRAINT_KINDS = frozenset({"equality", "inequality", "range", "exclusion", "dependency", "temporal"})
ASSIGNMENT_STRATEGIES = frozenset({"greedy", "balanced", "min_cost", "round_robin"})
...
def _validate_tenant_record_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if errors:
        return errors
    for field_name in ("tenant_id", "name", "status", "isolation_level", "owner"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["status"] not in TENANT_STATUSES:
        errors.append(
            f"{_relative_path(path)}: field 'status' must be one of {', '.join(sorted(TENANT_STATUSES))}"
        )
    if payload["isolation_level"] not in ISOLATION_LEVELS:
        errors.append(
            f"{_relative_path(path)}: field 'isolation_level' must be one of {', '.join(sorted(ISOLATION_LEVELS))}"
        )
    workspace_ids = payload["workspace_ids"]
    if not isinstance(workspace_ids, list):
        errors.append(f"{_relative_path(path)}: field 'workspace_ids' must be an array")
    else:
        for index, workspace_id in enumerate(workspace_ids):
            errors.extend(_require_non_empty_text(workspace_id, field_name=f"workspace_ids[{index}]", path=path))
        if len(set(workspace_ids)) != len(workspace_ids):
            errors.append(f"{_relative_path(path)}: workspace_ids must not contain duplicates")
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    errors.extend(_validate_iso8601_text(payload["updated_at"], field_name="updated_at", path=path))
    if not errors and _parse_iso8601_text(payload["updated_at"]) < _parse_iso8601_text(payload["created_at"]):
        errors.append(f"{_relative_path(path)}: updated_at must not precede created_at")
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors


def _validate_workspace_record_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if errors:
        return errors
    for field_name in ("workspace_id", "tenant_id", "name", "status", "isolation_level"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["status"] not in WORKSPACE_STATUSES:
        errors.append(
            f"{_relative_path(path)}: field 'status' must be one of {', '.join(sorted(WORKSPACE_STATUSES))}"
        )
    if payload["isolation_level"] not in ISOLATION_LEVELS:
        errors.append(
            f"{_relative_path(path)}: field 'isolation_level' must be one of {', '.join(sorted(ISOLATION_LEVELS))}"
        )
    for field_name in ("environment_ids", "resource_bindings"):
        values = payload[field_name]
        if not isinstance(values, list):
            errors.append(f"{_relative_path(path)}: field '{field_name}' must be an array")
            continue
        for index, value in enumerate(values):
            errors.extend(_require_non_empty_text(value, field_name=f"{field_name}[{index}]", path=path))
        if len(set(values)) != len(values):
...
def _validate_environment_record_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if errors:
        return errors
    for field_name in ("environment_id", "workspace_id", "kind", "name"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["kind"] not in ENVIRONMENT_KINDS:
        errors.append(
            f"{_relative_path(path)}: field 'kind' must be one of {', '.join(sorted(ENVIRONMENT_KINDS))}"
        )
    connector_ids = payload["connector_ids"]
    if not isinstance(connector_ids, list):
        errors.append(f"{_relative_path(path)}: field 'connector_ids' must be an array")
    else:
        for index, connector_id in enumerate(connector_ids):
            errors.extend(_require_non_empty_text(connector_id, field_name=f"connector_ids[{index}]", path=path))
        if len(set(connector_ids)) != len(connector_ids):
            errors.append(f"{_relative_path(path)}: connector_ids must not contain duplicates")
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    errors.extend(_validate_iso8601_text(payload["updated_at"], field_name="updated_at", path=path))
    if not errors and _parse_iso8601_text(payload["updated_at"]) < _parse_iso8601_text(payload["created_at"]):
        errors.append(f"{_relative_path(path)}: updated_at must not precede created_at")
...
def _validate_boundary_policy_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if errors:
        return errors
    for field_name in ("policy_id", "tenant_id", "boundary_kind", "isolation_level", "description"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["boundary_kind"] not in SCOPE_BOUNDARY_KINDS:
        errors.append(
            f"{_relative_path(path)}: field 'boundary_kind' must be one of {', '.join(sorted(SCOPE_BOUNDARY_KINDS))}"
        )
    if payload["isolation_level"] not in ISOLATION_LEVELS:
        errors.append(
            f"{_relative_path(path)}: field 'isolation_level' must be one of {', '.join(sorted(ISOLATION_LEVELS))}"
        )
    if not isinstance(payload["enforced"], bool):
        errors.append(f"{_relative_path(path)}: field 'enforced' must be boolean")
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors
...
def _validate_workspace_binding_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if errors:
        return errors
    for field_name in ("binding_id", "workspace_id", "resource_ref_id", "resource_type", "environment_id"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["resource_type"] not in SCOPE_BOUNDARY_KINDS:
        errors.append(
            f"{_relative_path(path)}: field 'resource_type' must be one of {', '.join(sorted(SCOPE_BOUNDARY_KINDS))}"
        )
    errors.extend(_validate_iso8601_text(payload["bound_at"], field_name="bound_at", path=path))
    return errors


def _validate_environment_promotion_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if errors:
        return errors
    for field_name in ("promotion_id", "source_environment_id", "target_environment_id", "status", "promoted_by"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["status"] not in PROMOTION_STATUSES:
        errors.append(
            f"{_relative_path(path)}: field 'status' must be one of {', '.join(sorted(PROMOTION_STATUSES))}"
        )
    if not isinstance(payload["compliance_check_passed"], bool):
        errors.append(f"{_relative_path(path)}: field 'compliance_check_passed' must be boolean")
    errors.extend(_validate_iso8601_text(payload["requested_at"], field_name="requested_at", path=path))
    if payload["completed_at"]:
        errors.extend(_validate_iso8601_text(payload["completed_at"], field_name="completed_at", path=path))
    if not errors:
        if payload["source_environment_id"] == payload["target_environment_id"]:
            errors.append(f"{_relative_path(path)}: source_environment_id must not equal target_environment_id")
        terminal_statuses = {"completed", "failed", "rolled_back"}
        nonterminal_statuses = {"requested", "approved", "in_progress"}
        if payload["status"] in terminal_statuses and not payload["completed_at"]:
            errors.append(f"{_relative_path(path)}: terminal promotions must carry completed_at")
        if payload["status"] in nonterminal_statuses and payload["completed_at"]:
            errors.append(f"{_relative_path(path)}: non-terminal promotions must keep completed_at empty")
        if payload["status"] == "completed" and not payload["compliance_check_passed"]:
            errors.append(f"{_relative_path(path)}: completed promotions must keep compliance_check_passed true")
        if payload["completed_at"] and _parse_iso8601_text(payload["completed_at"]) < _parse_iso8601_text(payload["requested_at"]):
            errors.append(f"{_relative_path(path)}: completed_at must not precede requested_at")
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors


def _validate_isolation_violation_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if not isinstance(payload["escalated"], bool):
        errors.append(f"{_relative_path(path)}: field 'escalated' must be boolean")
    errors.extend(_validate_iso8601_text(payload["detected_at"], field_name="detected_at", path=path))
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors
...
def _validate_tenant_health_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if errors:
        return errors
    errors.extend(_require_non_empty_text(payload["tenant_id"], field_name="tenant_id", path=path))
    for field_name in (
        "total_workspaces",
        "active_workspaces",
        "total_environments",
        ...
        errors.extend(_require_non_negative_int(payload[field_name], field_name=field_name, path=path))
    errors.extend(
        _require_number_in_range(payload["compliance_pct"], field_name="compliance_pct", path=path, minimum=0.0, maximum=1.0)
    )
    if not errors:
        if payload["active_workspaces"] > payload["total_workspaces"]:
            errors.append(f"{_relative_path(path)}: active_workspaces must not exceed total_workspaces")
        if payload["total_environments"] < payload["active_workspaces"]:
            errors.append(f"{_relative_path(path)}: total_environments must be at least active_workspaces")
    errors.extend(_validate_iso8601_text(payload["assessed_at"], field_name="assessed_at", path=path))
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors
...
def _validate_tenant_decision_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if errors:
        return errors
    for field_name in ("decision_id", "tenant_id", "title", "description", "decided_by"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    errors.extend(
        _require_number_in_range(payload["confidence"], field_name="confidence", path=path, minimum=0.0, maximum=1.0)
    )
...
def _validate_tenant_closure_report_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if not errors:
        if payload["total_environments"] < payload["total_workspaces"]:
            errors.append(f"{_relative_path(path)}: total_environments must be at least total_workspaces")
        if payload["total_promotions"] > payload["total_environments"]:
            errors.append(f"{_relative_path(path)}: total_promotions must not exceed total_environments")
    errors.extend(_validate_iso8601_text(payload["closed_at"], field_name="closed_at", path=path))
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors
...
def _validate_record_descriptor_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if payload["kind"] not in RECORD_KINDS:
        errors.append(
            f"{_relative_path(path)}: field 'kind' must be one of {', '.join(sorted(RECORD_KINDS))}"
        )
    if payload["authority"] not in RECORD_AUTHORITIES:
        errors.append(
            f"{_relative_path(path)}: field 'authority' must be one of {', '.join(sorted(RECORD_AUTHORITIES))}"
        )
    if payload["evidence_grade"] not in EVIDENCE_GRADES:
        errors.append(
            f"{_relative_path(path)}: field 'evidence_grade' must be one of {', '.join(sorted(EVIDENCE_GRADES))}"
        )
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
...
def _validate_retention_schedule_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if errors:
        return errors
    for field_name in ("schedule_id", "record_id", "tenant_id", "status", "disposal_disposition", "scope_ref_id"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    errors.extend(_require_non_negative_int(payload["retention_days"], field_name="retention_days", path=path))
    if payload["status"] not in RETENTION_STATUSES:
        errors.append(
            f"{_relative_path(path)}: field 'status' must be one of {', '.join(sorted(RETENTION_STATUSES))}"
        )
    if payload["disposal_disposition"] not in DISPOSAL_DISPOSITIONS:
        errors.append(
            f"{_relative_path(path)}: field 'disposal_disposition' must be one of {', '.join(sorted(DISPOSAL_DISPOSITIONS))}"
        )
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    errors.extend(_validate_iso8601_text(payload["expires_at"], field_name="expires_at", path=path))
    if not errors and _parse_iso8601_text(payload["expires_at"]) < _parse_iso8601_text(payload["created_at"]):
...
def _validate_legal_hold_record_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if errors:
        return errors
    for field_name in ("hold_id", "record_id", "tenant_id", "reason", "authority", "status"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["authority"] not in RECORD_AUTHORITIES:
        errors.append(
            f"{_relative_path(path)}: field 'authority' must be one of {', '.join(sorted(RECORD_AUTHORITIES))}"
        )
    if payload["status"] not in HOLD_STATUSES:
        errors.append(
            f"{_relative_path(path)}: field 'status' must be one of {', '.join(sorted(HOLD_STATUSES))}"
        )
    errors.extend(_validate_iso8601_text(payload["placed_at"], field_name="placed_at", path=path))
    if payload["released_at"]:
        errors.extend(_validate_iso8601_text(payload["released_at"], field_name="released_at", path=path))
    if not errors:
        if payload["status"] == "active" and payload["released_at"]:
            errors.append(f"{_relative_path(path)}: active legal holds must keep released_at empty")
        if payload["status"] in {"released", "expired"} and not payload["released_at"]:
            errors.append(f"{_relative_path(path)}: released or expired legal holds must carry released_at")
        if payload["released_at"] and _parse_iso8601_text(payload["released_at"]) < _parse_iso8601_text(payload["placed_at"]):
            errors.append(f"{_relative_path(path)}: released_at must not precede placed_at")
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors


def _validate_disposition_review_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
        expected_fields=("review_id", "record_id", "reviewer_id", "decision", "reason", "reviewed_at", "metadata"),
        kind="runtime fixture",
    )
    if errors:
        return errors
    for field_name in ("review_id", "record_id", "reviewer_id", "decision", "reason"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["decision"] not in DISPOSAL_DISPOSITIONS:
        errors.append(
            f"{_relative_path(path)}: field 'decision' must be one of {', '.join(sorted(DISPOSAL_DISPOSITIONS))}"
        )
    errors.extend(_validate_iso8601_text(payload["reviewed_at"], field_name="reviewed_at", path=path))
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
...
def _validate_record_link_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
        expected_fields=("link_id", "record_id", "target_type", "target_id", "relationship", "created_at"),
        kind="runtime fixture",
    )
    if errors:
        return errors
    for field_name in ("link_id", "record_id", "target_type", "target_id", "relationship"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if not errors and payload["target_type"] == "record" and payload["record_id"] == payload["target_id"]:
        errors.append(f"{_relative_path(path)}: record links must not self-reference the same record_id")
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    return errors


def _validate_record_snapshot_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    for field_name in (
        ...
        errors.extend(_require_non_negative_int(payload[field_name], field_name=field_name, path=path))
    if not errors:
        if payload["active_holds"] > payload["total_holds"]:
            errors.append(f"{_relative_path(path)}: active_holds must not exceed total_holds")
        if payload["total_holds"] > payload["total_records"]:
            errors.append(f"{_relative_path(path)}: total_holds must not exceed total_records")
        if payload["total_disposals"] > payload["total_records"]:
            errors.append(f"{_relative_path(path)}: total_disposals must not exceed total_records")
    errors.extend(_validate_iso8601_text(payload["captured_at"], field_name="captured_at", path=path))
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors
...
def _validate_record_violation_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
        expected_fields=("violation_id", "record_id", "tenant_id", "operation", "reason", "detected_at", "metadata"),
        kind="runtime fixture",
    )
    if errors:
        return errors
    for field_name in ("violation_id", "record_id", "tenant_id", "operation", "reason"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    errors.extend(_validate_iso8601_text(payload["detected_at"], field_name="detected_at", path=path))
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
...
def _validate_preservation_decision_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
        expected_fields=("decision_id", "record_id", "preserve", "reason", "authority", "decided_at"),
        kind="runtime fixture",
    )
    if errors:
        return errors
    for field_name in ("decision_id", "record_id", "reason", "authority"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["authority"] not in RECORD_AUTHORITIES:
        errors.append(
            f"{_relative_path(path)}: field 'authority' must be one of {', '.join(sorted(RECORD_AUTHORITIES))}"
        )
    if not isinstance(payload["preserve"], bool):
        errors.append(f"{_relative_path(path)}: field 'preserve' must be boolean")
    errors.extend(_validate_iso8601_text(payload["decided_at"], field_name="decided_at", path=path))
    return errors


def _validate_disposal_decision_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if errors:
        return errors
    for field_name in ("decision_id", "record_id", "tenant_id", "disposition", "reason", "authority"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["disposition"] not in DISPOSAL_DISPOSITIONS:
        errors.append(
            f"{_relative_path(path)}: field 'disposition' must be one of {', '.join(sorted(DISPOSAL_DISPOSITIONS))}"
        )
    if payload["authority"] not in RECORD_AUTHORITIES:
        errors.append(
            f"{_relative_path(path)}: field 'authority' must be one of {', '.join(sorted(RECORD_AUTHORITIES))}"
...
def _validate_records_closure_report_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    for field_name in ("report_id", "tenant_id"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    for field_name in ("total_records", "total_preserved", "total_disposed", "total_held", "total_violations"):
        errors.extend(_require_non_negative_int(payload[field_name], field_name=field_name, path=path))
    if not errors and payload["total_preserved"] + payload["total_disposed"] + payload["total_held"] > payload["total_records"]:
        errors.append(
            f"{_relative_path(path)}: total_preserved plus total_disposed plus total_held must not exceed total_records"
        )
    errors.extend(_validate_iso8601_text(payload["closed_at"], field_name="closed_at", path=path))
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
...
def _validate_causal_node_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
        expected_fields=("node_id", "tenant_id", "display_name", "status", "created_at", "metadata"),
        kind="runtime fixture",
    )
    if errors:
        return errors
    for field_name in ("node_id", "tenant_id", "display_name"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["status"] not in CAUSAL_STATUSES:
        errors.append(
            f"{_relative_path(path)}: field 'status' must be one of {', '.join(sorted(CAUSAL_STATUSES))}"
        )
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
...
def _validate_causal_edge_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if errors:
        return errors
    for field_name in ("edge_id", "tenant_id", "cause_ref", "effect_ref"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["kind"] not in CAUSAL_EDGE_KINDS:
        errors.append(
            f"{_relative_path(path)}: field 'kind' must be one of {', '.join(sorted(CAUSAL_EDGE_KINDS))}"
        )
    if payload["strength"] not in CAUSAL_ATTRIBUTION_STRENGTHS:
        errors.append(
            f"{_relative_path(path)}: field 'strength' must be one of {', '.join(sorted(CAUSAL_ATTRIBUTION_STRENGTHS))}"
        )
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    if not errors and payload["cause_ref"] == payload["effect_ref"]:
        errors.append(f"{_relative_path(path)}: cause_ref and effect_ref must be different")
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors


def _validate_intervention_record_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if errors:
        return errors
    for field_name in ("intervention_id", "tenant_id", "target_node_ref", "expected_effect"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["disposition"] not in INTERVENTION_DISPOSITIONS:
        errors.append(
            f"{_relative_path(path)}: field 'disposition' must be one of {', '.join(sorted(INTERVENTION_DISPOSITIONS))}"
        )
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
...
def _validate_counterfactual_scenario_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
        expected_fields=("scenario_id", "tenant_id", "intervention_ref", "premise", "status", "created_at", "metadata"),
        kind="runtime fixture",
    )
    if errors:
        return errors
    for field_name in ("scenario_id", "tenant_id", "intervention_ref", "premise"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["status"] not in COUNTERFACTUAL_STATUSES:
        errors.append(
            f"{_relative_path(path)}: field 'status' must be one of {', '.join(sorted(COUNTERFACTUAL_STATUSES))}"
        )
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
...
def _validate_causal_attribution_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if errors:
        return errors
    for field_name in ("attribution_id", "tenant_id", "outcome_ref", "cause_ref"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["strength"] not in CAUSAL_ATTRIBUTION_STRENGTHS:
        errors.append(
            f"{_relative_path(path)}: field 'strength' must be one of {', '.join(sorted(CAUSAL_ATTRIBUTION_STRENGTHS))}"
    ...
    errors.extend(_require_non_negative_int(payload["evidence_count"], field_name="evidence_count", path=path))
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    if not errors and payload["outcome_ref"] == payload["cause_ref"]:
        errors.append(f"{_relative_path(path)}: outcome_ref and cause_ref must be different")
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors


def _validate_propagation_record_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
        expected_fields=("propagation_id", "tenant_id", "source_ref", "target_ref", "hop_count", "created_at", "metadata"),
        kind="runtime fixture",
    )
    if errors:
        return errors
    for field_name in ("propagation_id", "tenant_id", "source_ref", "target_ref"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    errors.extend(_require_non_negative_int(payload["hop_count"], field_name="hop_count", path=path))
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    if not errors:
        if payload["source_ref"] == payload["target_ref"]:
            errors.append(f"{_relative_path(path)}: source_ref and target_ref must be different")
        if payload["hop_count"] < 1:
            errors.append(f"{_relative_path(path)}: hop_count must be at least 1")
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors


def _validate_causal_decision_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
        expected_fields=("decision_id", "tenant_id", "attribution_ref", "disposition", "reason", "decided_at", "metadata"),
        kind="runtime fixture",
    )
    if errors:
        return errors
    for field_name in ("decision_id", "tenant_id", "attribution_ref", "disposition", "reason"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    errors.extend(_validate_iso8601_text(payload["decided_at"], field_name="decided_at", path=path))
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
...
def _validate_causal_assessment_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    for field_name in ("assessment_id", "tenant_id"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    for field_name in ("total_nodes", "total_edges", "total_interventions"):
        errors.extend(_require_non_negative_int(payload[field_name], field_name=field_name, path=path))
    errors.extend(
        _require_number_in_range(
            payload["attribution_coverage"],
            field_name="attribution_coverage",
            path=path,
            minimum=0.0,
            maximum=1.0,
        )
    ...
    errors.extend(_validate_iso8601_text(payload["assessed_at"], field_name="assessed_at", path=path))
    if not errors and payload["total_nodes"] == 0 and payload["total_edges"] > 0:
        errors.append(f"{_relative_path(path)}: total_edges must be 0 when total_nodes is 0")
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors


def _validate_causal_snapshot_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
...
def _validate_causal_closure_report_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
...
def _validate_constraint_definition_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if errors:
        return errors
    for field_name in ("constraint_id", "tenant_id", "expression"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["kind"] not in CONSTRAINT_KINDS:
        errors.append(
            f"{_relative_path(path)}: field 'kind' must be one of {', '.join(sorted(CONSTRAINT_KINDS))}"
        )
    if not isinstance(payload["variable_refs"], str):
        errors.append(f"{_relative_path(path)}: field 'variable_refs' must be a string")
    errors.extend(_require_non_negative_int(payload["priority"], field_name="priority", path=path))
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
...
def _validate_solver_problem_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if errors:
        return errors
    for field_name in ("problem_id", "tenant_id"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["algorithm"] not in ALGORITHM_KINDS:
        errors.append(
            f"{_relative_path(path)}: field 'algorithm' must be one of {', '.join(sorted(ALGORITHM_KINDS))}"
        )
    if payload["status"] not in SOLVE_STATUSES:
        errors.append(
            f"{_relative_path(path)}: field 'status' must be one of {', '.join(sorted(SOLVE_STATUSES))}"
        )
    errors.extend(_require_non_negative_int(payload["constraint_count"], field_name="constraint_count", path=path))
    errors.extend(_require_non_negative_int(payload["variable_count"], field_name="variable_count", path=path))
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors
...
def _validate_solver_solution_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if errors:
        return errors
    for field_name in ("solution_id", "tenant_id", "problem_ref"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["status"] not in SOLVE_STATUSES:
        errors.append(
            f"{_relative_path(path)}: field 'status' must be one of {', '.join(sorted(SOLVE_STATUSES))}"
        )
    if isinstance(payload["objective_value"], bool) or not isinstance(payload["objective_value"], (int, float)):
        errors.append(f"{_relative_path(path)}: field 'objective_value' must be a numeric value")
    elif not math.isfinite(payload["objective_value"]):
        errors.append(f"{_relative_path(path)}: field 'objective_value' must be finite")
    errors.extend(_require_non_negative_int(payload["iterations"], field_name="iterations", path=path))
    errors.extend(_require_non_negative_float(payload["duration_ms"], field_name="duration_ms", path=path))
    errors.extend(_validate_iso8601_text(payload["solved_at"], field_name="solved_at", path=path))
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors


def _validate_graph_node_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
        expected_fields=("node_id", "tenant_id", "label", "weight", "created_at", "metadata"),
        kind="runtime fixture",
    )
    if errors:
        return errors
    for field_name in ("node_id", "tenant_id", "label"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    errors.extend(_require_non_negative_float(payload["weight"], field_name="weight", path=path))
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors
...
def _validate_graph_edge_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
        expected_fields=("edge_id", "tenant_id", "from_node", "to_node", "weight", "created_at", "metadata"),
        kind="runtime fixture",
    )
    if errors:
        return errors
    for field_name in ("edge_id", "tenant_id", "from_node", "to_node"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    errors.extend(_require_non_negative_float(payload["weight"], field_name="weight", path=path))
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    if not errors and payload["from_node"] == payload["to_node"]:
        errors.append(f"{_relative_path(path)}: from_node and to_node must be different")
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors


def _validate_schedule_slot_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
        expected_fields=("slot_id", "tenant_id", "resource_ref", "start_at", "end_at", "priority", "created_at", "metadata"),
        kind="runtime fixture",
    )
    if errors:
        return errors
    for field_name in ("slot_id", "tenant_id", "resource_ref"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    errors.extend(_validate_iso8601_text(payload["start_at"], field_name="start_at", path=path))
    errors.extend(_validate_iso8601_text(payload["end_at"], field_name="end_at", path=path))
    errors.extend(_require_non_negative_int(payload["priority"], field_name="priority", path=path))
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    if not errors and _parse_iso8601_text(payload["end_at"]) <= _parse_iso8601_text(payload["start_at"]):
        errors.append(f"{_relative_path(path)}: end_at must be after start_at")
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors


def _validate_constraint_assignment_record_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    if errors:
        return errors
    for field_name in ("assignment_id", "tenant_id", "resource_ref", "task_ref"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["strategy"] not in ASSIGNMENT_STRATEGIES:
        errors.append(
            f"{_relative_path(path)}: field 'strategy' must be one of {', '.join(sorted(ASSIGNMENT_STRATEGIES))}"
        )
    errors.extend(_require_non_negative_float(payload["cost"], field_name="cost", path=path))
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    if not errors and payload["resource_ref"] == payload["task_ref"]:
        errors.append(f"{_relative_path(path)}: resource_ref and task_ref must be different")
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors


def _validate_dependency_chain_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
        expected_fields=("chain_id", "tenant_id", "source_ref", "target_ref", "lag", "created_at", "metadata"),
        kind="runtime fixture",
    )
    if errors:
        return errors
    for field_name in ("chain_id", "tenant_id", "source_ref", "target_ref"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    errors.extend(_require_non_negative_int(payload["lag"], field_name="lag", path=path))
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
...
def _validate_constraint_snapshot_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    errors.extend(_validate_iso8601_text(payload["captured_at"], field_name="captured_at", path=path))
    if not errors:
        if payload["total_nodes"] == 0 and payload["total_edges"] > 0:
            errors.append(f"{_relative_path(path)}: total_edges must be 0 when total_nodes is 0")
        if payload["total_solutions"] > payload["total_problems"]:
            errors.append(f"{_relative_path(path)}: total_solutions must not exceed total_problems")
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors


def _validate_constraint_closure_report_fixture(path: Path) -> list[str]:
    payload = _load_json_object(path, kind="MCOI runtime fixture")
    errors = _validate_exact_object_fields(
        payload,
        path=path,
    ...
    for field_name in (
        ...
        errors.extend(_require_non_negative_int(payload[field_name], field_name=field_name, path=path))
    errors.extend(_validate_iso8601_text(payload["created_at"], field_name="created_at", path=path))
    if not errors and payload["total_solutions"] > payload["total_problems"]:
        errors.append(f"{_relative_path(path)}: total_solutions must not exceed total_problems")
    if not isinstance(payload["metadata"], dict):
        errors.append(f"{_relative_path(path)}: field 'metadata' must be an object")
    return errors
...
def _validate_knowledge_claim_fixture(path: Path) -> list[str]:
    ...
    if errors:
        return errors
    for field_name in ("claim_id", "tenant_id", "content", "source_ref"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["status"] not in KNOWLEDGE_STATUSES:
        errors.append(
            f"{_relative_path(path)}: field 'status' must be one of {', '.join(sorted(KNOWLEDGE_STATUSES))}"
        )
...
def _validate_evidence_source_fixture(path: Path) -> list[str]:
    ...
    if errors:
        return errors
    for field_name in ("source_id", "tenant_id", "display_name"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["origin"] not in EVIDENCE_ORIGINS:
        errors.append(
            f"{_relative_path(path)}: field 'origin' must be one of {', '.join(sorted(EVIDENCE_ORIGINS))}"
        )
    errors.extend(
        _require_number_in_range(
...
def _validate_trust_assessment_fixture(path: Path) -> list[str]:
    ...
    if errors:
        return errors
    for field_name in ("assessment_id", "tenant_id", "claim_ref", "source_ref", "basis"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    if payload["trust_level"] not in TRUST_LEVELS:
        errors.append(
            f"{_relative_path(path)}: field 'trust_level' must be one of {', '.join(sorted(TRUST_LEVELS))}"
...
def _validate_source_reliability_record_fixture(path: Path) -> list[str]:
    ...
    for field_name in ("previous_score", "updated_score"):
        errors.extend(
            _require_number_in_range(payload[field_name], field_name=field_name, path=path, minimum=0.0, maximum=1.0)
        )
    errors.extend(_validate_iso8601_text(payload["updated_at"], field_name="updated_at", path=path))
...
def _validate_claim_conflict_fixture(path: Path) -> list[str]:
    ...
    if errors:
        return errors
    for field_name in ("conflict_id", "tenant_id", "claim_a_ref", "claim_b_ref"):
        errors.extend(_require_non_empty_text(payload[field_name], field_name=field_name, path=path))
    ...
    if not isinstance(payload["resolution_basis"], str):
        errors.append(f"{_relative_path(path)}: field 'resolution_basis' must be a string")
    errors.extend(_validate_iso8601_text(payload["detected_at"], field_name="detected_at", path=path))
    if not errors:
        if payload["claim_a_ref"] == payload["claim_b_ref"]:
            errors.append(f"{_relative_path(path)}: claim_a_ref and claim_b_ref must be different")
        if payload["disposition"] in {"first_wins", "second_wins", "merged"} and not payload["resolution_basis"].strip():
...
MCOI_RUNTIME_FIXTURE_VALIDATORS: dict[str, MCOIRuntimeFixtureValidator] = {
    ...
    "causal_assessment.json": _validate_causal_assessment_fixture,
    "causal_attribution.json": _validate_causal_attribution_fixture,
    "causal_closure_report.json": _validate_causal_closure_report_fixture,
    "causal_decision.json": _validate_causal_decision_fixture,
    "causal_edge.json": _validate_causal_edge_fixture,
    "causal_node.json": _validate_causal_node_fixture,
    "causal_snapshot.json": _validate_causal_snapshot_fixture,
    ...
    "constraint_closure_report.json": _validate_constraint_closure_report_fixture,
    "constraint_definition.json": _validate_constraint_definition_fixture,
    "constraint_snapshot.json": _validate_constraint_snapshot_fixture,
    "counterfactual_scenario.json": _validate_counterfactual_scenario_fixture,
    ...
    "dependency_chain.json": _validate_dependency_chain_fixture,
    ...
    "graph_edge.json": _validate_graph_edge_fixture,
    "graph_node.json": _validate_graph_node_fixture,
    ...
    "isolation_violation.json": _validate_isolation_violation_fixture,
    ...
    "record_descriptor.json": _validate_record_descriptor_fixture,
    "record_link.json": _validate_record_link_fixture,
    "record_snapshot.json": _validate_record_snapshot_fixture,
    "record_violation.json": _validate_record_violation_fixture,
    "records_closure_report.json": _validate_records_closure_report_fixture,
    "refund_record.json": _validate_refund_record_fixture,
    "intervention_record.json": _validate_intervention_record_fixture,
    "propagation_record.json": _validate_propagation_record_fixture,
    ...
    "schedule_slot.json": _validate_schedule_slot_fixture,
    "solver_problem.json": _validate_solver_problem_fixture,
    "solver_solution.json": _validate_solver_solution_fixture,
    ...
    "tenant_closure_report.json": _validate_tenant_closure_report_fixture,
    "tenant_decision.json": _validate_tenant_decision_fixture,
    "tenant_health.json": _validate_tenant_health_fixture,
    "tenant_record.json": _validate_tenant_record_fixture,
    ...
    "workspace_binding.json": _validate_workspace_binding_fixture,
    "workspace_record.json": _validate_workspace_record_fixture,
    "writeoff_record.json": _validate_writeoff_record_fixture,
    "aging_snapshot.json": _validate_aging_snapshot_fixture,
    "boundary_policy.json": _validate_boundary_policy_fixture,
    "connector_cost_profile.json": _validate_connector_cost_profile_fixture,
    "cost_estimate.json": _validate_cost_estimate_fixture,
    "assignment_record.json": _validate_constraint_assignment_record_fixture,
}
