"""Validate the Foundation Mode read-only document worker path selection.

Purpose: keep the document worker bounded to local, text-like, read-only
inspection.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA]
Dependencies: schemas/read_only_document_worker_path.schema.json and
examples/read_only_document_worker_path.foundation.json.
Invariants: document inspection is local, read-only, no-spend, no-secret,
no-network, format-allowlisted, and path-boundary witnessed.
"""
...
SCHEMA_PATH = REPO_ROOT / "schemas" / "read_only_document_worker_path.schema.json"
EXAMPLE_PATH = REPO_ROOT / "examples" / "read_only_document_worker_path.foundation.json"
SELECTED_CAPABILITY_ID = "document.inspect_read_only"
...
REQUIRED_RECEIPTS = frozenset(
    {
        ...
        "input_hash_receipt",
        "document_boundary_witness",
        "format_allowlist_witness",
...
def validate_read_only_document_worker_path(
    payload: dict[str, Any],
    schema_path: Path = SCHEMA_PATH,
) -> list[str]:
    """Return validation errors for the read-only document worker path."""
    schema = _load_schema(schema_path)
    errors = _validate_schema_instance(schema, payload)

    if payload.get("selected_capability_id") != SELECTED_CAPABILITY_ID:
        errors.append("selected_capability_id must remain document.inspect_read_only")

    for flag_name in REQUIRED_FALSE_FLAGS:
        if payload.get(flag_name) is not False:
            errors.append(f"{flag_name} must be false")

    parser_scope = payload.get("parser_scope", {})
    if not isinstance(parser_scope, dict):
        errors.append("parser_scope must be an object")
    else:
        if parser_scope.get("rich_document_parsing_allowed") is not False:
            errors.append("parser_scope.rich_document_parsing_allowed must be false")
        supported_extensions = parser_scope.get("supported_extensions", [])
        if not isinstance(supported_extensions, list) or ".md" not in supported_extensions:
            errors.append("parser_scope.supported_extensions must include .md")
    ...
    if not isinstance(blocked_inputs, dict):
        ...
        for blocked_name in (
            "absolute_paths_outside_document_root",
            "raw_secret_values",
            "write_requests",
            "network_targets",
            "tenant_external_resources",
            "rich_document_binary_parsing",
        ):
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Path to the read-only document worker selection JSON.",
    )
    ...
    try:
        ...
        print(f"read-only document worker path validation failed: {exc}")
        return 1
    ...
    print(
        "read-only document worker path ok: "
