#!/usr/bin/env python3
"""Validate Agentic Service Harness live producer secret handoff packet.

Purpose: define the secret handoff witness packet while keeping credential
values, authority scope, storage boundary, redaction proof, rotation policy,
audit log, and adapter evidence linkage AwaitingEvidence.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: schemas/agentic_service_harness_live_producer_secret_handoff_packet.schema.json,
examples/agentic_service_harness_live_producer_secret_handoff_packet.awaiting_evidence.json,
scripts.validate_agentic_service_harness_live_producer_secret_handoff_preflight,
and scripts.validate_schemas.
Invariants:
  - Packet remains AwaitingEvidence and non-terminal.
  - Required component refs remain future refs.
  - Credential possession, reading, printing, serialization, storage, mutation routes,
    and live execution fail closed.
"""
...
DEFAULT_SCHEMA = REPO_ROOT / "schemas" / "agentic_service_harness_live_producer_secret_handoff_packet.schema.json"
DEFAULT_FIXTURE = (
    REPO_ROOT / "examples" / "agentic_service_harness_live_producer_secret_handoff_packet.awaiting_evidence.json"
)
PACKET_ID = "agentic-service-harness-live-producer-secret-handoff-packet"
...
FORBIDDEN_LIVE_CLAIM = re.compile(
    ...
    r"adapter_credentials_present|adapter_credentials_serialized|external_adapter_integrated|"
...
class LiveProducerSecretHandoffPacketValidation:
    """Validation result for the live producer secret handoff packet."""
...
def validate_live_producer_secret_handoff_packet(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    fixture_path: Path = DEFAULT_FIXTURE,
) -> tuple[LiveProducerSecretHandoffPacketValidation, dict[str, Any]]:
    """Validate the checked-in live producer secret handoff packet."""
    errors: list[str] = []
    schema = _load_json_object(schema_path, "secret handoff packet schema", errors)
    fixture = _load_json_object(fixture_path, "secret handoff packet fixture", errors)
    ...
    if getattr(source_validation, "secret_handoff_status", "") != "AwaitingEvidence":
        errors.append("source secret handoff preflight must remain AwaitingEvidence")
    ...
    validation = LiveProducerSecretHandoffPacketValidation(
        ok=not errors,
        errors=tuple(errors),
...
def _validate_packet_semantics(packet: Mapping[str, Any], errors: list[str], label: str) -> None:
    for field_name, expected_value in (
        ...
        ("packet_status", "blocked_awaiting_secret_handoff_components"),
    ...
    if packet.get("source_preflight_ref") != "examples/agentic_service_harness_live_producer_secret_handoff_preflight.local.json":
...
def _validate_validator_ref(packet: Mapping[str, Any], errors: list[str], label: str) -> None:
    ...
    if validator.get("command") != "python scripts/validate_agentic_service_harness_live_producer_secret_handoff_packet.py":
...
def main(argv: list[str] | None = None) -> int:
    """Run the live producer secret handoff packet validator."""
    args = build_arg_parser().parse_args(argv)
    validation, fixture = validate_live_producer_secret_handoff_packet(
    ...
    if args.json:
        ...
        print("AGENTIC SERVICE HARNESS LIVE PRODUCER SECRET HANDOFF PACKET VALID")
    else:
        print(
            "AGENTIC SERVICE HARNESS LIVE PRODUCER SECRET HANDOFF PACKET INVALID "
