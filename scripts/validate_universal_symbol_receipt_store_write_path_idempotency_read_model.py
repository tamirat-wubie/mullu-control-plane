#!/usr/bin/env python3
"""Validate the write-path idempotency witness operator read model.

Purpose: prove the write-path idempotency operator projection is read-only and
bounded while idempotency binding, duplicate append, receipt-store append, raw
payload exposure, mutation, and terminal closure remain denied.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: write-path idempotency read-model schema/example and write-path
idempotency witness validator constants.
Invariants:
  - The read model is a projection, not idempotency authority.
  - Every idempotency requirement appears exactly once.
  - Raw detail visibility, duplicate append, and authority remain denied.
...
from scripts.validate_universal_symbol_receipt_store_write_path_idempotency_witness import (  # noqa: E402
...
DEFAULT_SCHEMA_PATH = (
    REPO_ROOT / "schemas" / "universal_symbol_receipt_store_write_path_idempotency_read_model.schema.json"
)
DEFAULT_READ_MODEL_PATH = (
    REPO_ROOT / "examples" / "universal_symbol_receipt_store_write_path_idempotency_read_model.foundation.json"
)

EXPECTED_WITNESS_REF = "examples/universal_symbol_receipt_store_write_path_idempotency_witness.foundation.json"
EXPECTED_IDEMPOTENCY_DECISION = "blocked_pending_key_derivation_digest_replay_and_collision_evidence"
...
READ_MODEL_CONSTRAINT_TRUE_FIELDS: tuple[str, ...] = (
    ...
    "no_raw_payload",
    "no_raw_secret",
    "no_write_path_idempotency_binding",
    "no_duplicate_append",
    "no_terminal_closure",
)
...
EXPECTED_OPERATOR_STATUS_SUMMARY: dict[str, str] = {
    "primary_status": "Write-path idempotency blocked",
    "key_status": "Awaiting deterministic evidence",
    "collision_status": "Awaiting replay evidence",
    "raw_detail_visibility": "hidden_by_default",
    "operator_action": "Collect key derivation, canonical input, digest, replay collision, and audit evidence",
}


class UniversalSymbolReceiptStoreWritePathIdempotencyReadModelError(ValueError):
    """Raised when the write-path idempotency read model violates Foundation Mode."""


def validate_universal_symbol_receipt_store_write_path_idempotency_read_model(
    read_model_path: Path = DEFAULT_READ_MODEL_PATH,
    schema_path: Path = DEFAULT_SCHEMA_PATH,
) -> dict[str, Any]:
    """Validate schema shape, bounded rows, and denied idempotency authority."""
    ...
    if errors:
        raise UniversalSymbolReceiptStoreWritePathIdempotencyReadModelError("; ".join(errors))
    return report
...
def load_json_object(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise UniversalSymbolReceiptStoreWritePathIdempotencyReadModelError(f"missing file: {path}") from exc
    except json.JSONDecodeError as exc:
        raise UniversalSymbolReceiptStoreWritePathIdempotencyReadModelError(f"invalid json: {path}: {exc}") from exc
    if not isinstance(value, dict):
        raise UniversalSymbolReceiptStoreWritePathIdempotencyReadModelError(f"expected object: {path}")
    return value


def _validate_schema_boundary(schema: Mapping[str, Any], errors: list[str]) -> None:
    expected_id = "urn:mullusi:schema:universal-symbol-receipt-store-write-path-idempotency-read-model:1"
...
def _validate_read_model_boundary(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if read_model.get("read_model_is_not_idempotency_authority") is not True:
        errors.append("read model must not be idempotency authority")
...
def _validate_witness_projection(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if projection.get("idempotency_decision") != EXPECTED_IDEMPOTENCY_DECISION:
        errors.append("witness_projection.idempotency_decision must remain blocked")
    if projection.get("authority_granted") is not False:
        errors.append("witness_projection.authority_granted must remain false")
    if projection.get("idempotency_requirement_count") != len(REQUIRED_REQUIREMENT_IDS):
        errors.append("witness_projection.idempotency_requirement_count drift")
...
def _validate_source_witness_alignment(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if projection.get("witness_ref") != EXPECTED_WITNESS_REF:
        return
    try:
        witness = load_json_object(DEFAULT_WRITE_PATH_IDEMPOTENCY_WITNESS_PATH)
    except UniversalSymbolReceiptStoreWritePathIdempotencyReadModelError as exc:
        errors.append(f"source write-path idempotency witness unavailable: {exc}")
        return

    witness_summary = _mapping(witness.get("contract_summary"))
    expected_counts = {
        "idempotency_requirement_count": witness_summary.get("idempotency_requirement_count"),
        "blocked_reason_count": witness_summary.get("blocked_reason_count"),
    }
    for field_name, expected_count in expected_counts.items():
        if projection.get(field_name) != expected_count:
            errors.append(f"witness_projection.{field_name} must match source write-path idempotency witness")
    if projection.get("idempotency_decision") != witness.get("idempotency_decision"):
        errors.append("witness_projection.idempotency_decision must match source write-path idempotency witness")
...
def _validate_requirement_rows(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    for row in rows:
        ...
        if row.get("proof_state") not in {"Unknown", "BudgetUnknown", "Fail"}:
            errors.append(f"{row.get('requirement_id')}: proof_state must block idempotency")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print(f"[FAIL] universal_symbol_receipt_store_write_path_idempotency_read_model: {exc}")
            print("STATUS: failed")
        return 1
    ...
    if args.json:
        ...
        print("[PASS] universal_symbol_receipt_store_write_path_idempotency_read_model")
        print("STATUS: passed")
    return 0
