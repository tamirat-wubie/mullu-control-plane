...
def _validate_github_pr_terminal_closure_ready(
    ...
    certificate_read_model_row = re.search(
        r"^\| GitHub PR terminal closure certificate read model command-preview minting binding PR \| READY \| .+command-preview certificate minting evidence.+source minting id/ref/time.+source decision value request ref/status.+approve_terminal_certificate.+command preview ref.+redacted command preview.+command-preview-bound flag.+preserving actual-diff certificate minting evidence.+read-only, projection-only, reference-only.+new terminal-closure authority\. \|$",
        map_text,
        re.MULTILINE,
    )
...
def _validate_next_pr_sequence(map_text: str, errors: list[str]) -> None:
    sequence_markers = (
        "harness(pr): bind PR creation dry-run receipt to command-preview certificate read-model evidence",
    )
    positions: list[int] = []
