...
REQUIRED_GITHUB_PR_TERMINAL_CLOSURE_TERMS = (
    ...
    "command-preview-bound operator approval gate evidence",
...
def _validate_github_pr_terminal_closure_ready(
    ...
    approval_gate_row = re.search(
        r"^\| GitHub PR terminal closure operator approval gate command-preview candidate binding PR \| READY \| .+command-preview terminal closure certificate witness.+command-preview operator response binding.+redacted command preview.+Operator approval remains required and uncollected.+terminal closure remain blocked\. \|$",
        map_text,
        re.MULTILINE,
    )
