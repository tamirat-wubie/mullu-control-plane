...
REQUIRED_GITHUB_PR_CI_GATE_TERMS = (
    ...
    "command-preview repository-effect rollback evidence",
...
REQUIRED_GITHUB_PR_TERMINAL_CLOSURE_TERMS = (
    ...
    "GitHub PR terminal closure certificate minting command-preview decision value record binding PR",
    "agentic_service_harness_github_pr_terminal_closure_certificate_minting",
    "command-preview-bound certificate minting evidence",
...
def _validate_github_pr_ci_gate_ready(
    ...
    closure_row = re.search(
        r"^\| GitHub PR CI gate before ready-for-review command-preview rollback binding PR \| READY \| .+command-preview repository-effect rollback evidence.+source rollback witness id/ref.+source UAO witness id/ref.+source branch-write binding id/ref.+command-preview repository-effect rollback witness.+actual-diff repository-effect rollback witness.+terminal authority is granted\. \|$",
        map_text,
        re.MULTILINE,
    )
    if closure_row is None:
...
def _validate_github_pr_terminal_closure_ready(
    ...
    certificate_minting_row = re.search(
        r"^\| GitHub PR terminal closure certificate minting command-preview decision value record binding PR \| READY \| .+command-preview decision value record evidence.+preserves actual-diff decision value record evidence.+source decision value record id/ref.+source request id/ref/status.+approve_terminal_certificate.+command-preview decision value request evidence.+redacted command preview.+actual non-empty diff receipt ref.+GitHub PR proof thread.+destructive-operation authority\. \|$",
        map_text,
        re.MULTILINE,
    )
    ...
    certificate_read_model_row = re.search(
        r"^\| GitHub PR terminal closure certificate read model command-preview minting binding PR \| READY \| .+command-preview certificate minting evidence.+preserving actual-diff certificate minting evidence.+source minting id/ref/time.+approve_terminal_certificate.+redacted command preview.+command-preview-bound status.+read-only, projection-only, reference-only.+new terminal-closure authority\. \|$",
        map_text,
        re.MULTILINE,
    )
    ...
    pr_creation_read_model_binding_row = re.search(
        r"^\| GitHub PR creation admission command-preview terminal certificate read-model binding PR \| READY \| .+agentic_service_harness_github_pr_admission_preflight.+command-preview terminal certificate read-model evidence.+source minting ref.+source certificate id.+redacted command preview.+command-preview-bound status.+projection-only status.+pull-request creation.+new terminal-closure authority\. \|$",
        map_text,
        re.MULTILINE,
    )
    if pr_creation_read_model_binding_row is None:
        errors.append(
            "missing ready row: GitHub PR creation admission command-preview terminal certificate read-model binding PR"
        )

    pr_creation_dry_run_receipt_row = re.search(
        r"^\| GitHub PR creation dry-run receipt command-preview read-model binding PR \| READY \| .+agentic_service_harness_github_pr_creation_dry_run_receipt.+command-preview certificate read-model evidence.+source read-model id/ref.+command preview ref.+redacted command preview.+approve_terminal_certificate.+projection-only status.+pull-request creation denial.+dry-run-only PR creation.+runtime PR execution.+terminal closure\. \|$",
        map_text,
        re.MULTILINE,
    )
    ...
    pr_operator_response_command_preview_row = re.search(
        r"^\| GitHub PR operator response command-preview binding PR \| READY \| .+agentic_service_harness_github_pr_operator_response_command_preview_binding.+operator response witness.+command-preview approval request evidence.+source approval request binding ref.+source command-preview execution-admission binding.+no-secret-value evidence.+operator response.+command execution.+branch writes.+pull-request creation.+repository writes.+connector calls.+mutation routes.+receipt-store append.+secret material.+terminal closure remain blocked\. \|$",
        map_text,
        re.MULTILINE,
    )
...
def _validate_next_pr_sequence(map_text: str, errors: list[str]) -> None:
    sequence_markers = (
        "harness(live-producer): add review submission receipt append authority evidence packet",
    )
    positions: list[int] = []
