...
REQUIRED_ACTUAL_DIFF_RECEIPT_ADMISSION_TERMS = (
    "Actual diff collection receipt admission preflight PR",
    "agentic_service_harness_actual_diff_collection_receipt_admission_preflight",
    "filesystem-write admission preflight",
    "actual diff collection receipt",
    "non-empty diff receipt",
    "receipt-store append",
    "raw diff bodies",
    "raw file content",
...
def validate_readiness_map(map_path: Path = DEFAULT_MAP) -> ReadinessMapValidation:
    ...
    _require_all(
        map_text,
        REQUIRED_ACTUAL_DIFF_RECEIPT_ADMISSION_TERMS,
        "actual_diff_receipt_admission_term",
        errors,
    )
    _require_all(
        map_text,
    ...
    _validate_actual_diff_receipt_admission_ready(map_text, errors)
...
def _validate_actual_diff_receipt_admission_ready(
    map_text: str,
    errors: list[str],
) -> None:
    closure_row = re.search(
        r"^\| Actual diff collection receipt admission preflight PR \| READY \| .+actual diff collection receipt.+terminal closure remain blocked\. \|$",
        map_text,
        re.MULTILINE,
    )
    if closure_row is None:
        errors.append("missing ready row: Actual diff collection receipt admission preflight PR")
...
def _validate_next_pr_sequence(map_text: str, errors: list[str]) -> None:
    sequence_markers = (
        "harness(workspace): bind concrete filesystem write receipt and redacted diff bundle",
    )
    positions: list[int] = []
