...
Invariants:
  ...
  - Task creation admission remains closed as an evidence-bound blocked
    preflight.
...
def _validate_task_creation_admission_ready(map_text: str, errors: list[str]) -> None:
    ready_row = re.search(
        r"^\| Task creation admission preflight PR \| READY \| .+task route creation.+terminal closure remain blocked\. \|$",
        map_text,
        re.MULTILINE,
    )
    if ready_row is None:
