...
REQUIRED_GITHUB_PR_TERMINAL_CLOSURE_TERMS = (
    ...
    "command-preview candidate evidence",
    "command-preview terminal closure certificate witness",
    "command-preview operator response binding",
    "redacted command preview",
...
def _validate_github_pr_terminal_closure_ready(
    ...
    approval_gate_row = re.search(
        r"^\| GitHub PR terminal closure operator approval gate command-preview candidate binding PR \| READY \| .+command-preview candidate evidence.+actual-diff terminal closure certificate witness.+Operator approval remains required and uncollected.+terminal closure remain blocked\. \|$",
        map_text,
        re.MULTILINE,
    )
...
def _validate_next_pr_sequence(map_text: str, errors: list[str]) -> None:
    sequence_markers = (
        "harness(pr): bind terminal closure decision contract to command-preview approval gate",
    )
    positions: list[int] = []
