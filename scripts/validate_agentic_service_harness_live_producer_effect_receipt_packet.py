#!/usr/bin/env python3
"""Validate Agentic Service Harness live producer effect receipt packet.

Purpose: define the first concrete live producer witness packet while keeping
the actual effect receipt AwaitingEvidence until admitted action, effect hash,
reconciliation, rollback link, and redaction proof exist.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: schemas/agentic_service_harness_live_producer_effect_receipt_packet.schema.json,
examples/agentic_service_harness_live_producer_effect_receipt_packet.awaiting_evidence.json,
scripts.validate_agentic_service_harness_live_producer_effect_receipt_preflight,
and scripts.validate_schemas.
Invariants:
  - Packet remains AwaitingEvidence and non-terminal.
  - Required component refs remain future refs.
  - Live execution, runtime writes, mutation routes, and credential-shaped values fail closed.
"""
...
DEFAULT_SCHEMA = REPO_ROOT / "schemas" / "agentic_service_harness_live_producer_effect_receipt_packet.schema.json"
DEFAULT_FIXTURE = (
    REPO_ROOT / "examples" / "agentic_service_harness_live_producer_effect_receipt_packet.awaiting_evidence.json"
)
PACKET_ID = "agentic-service-harness-live-producer-effect-receipt-packet"
...
class LiveProducerEffectReceiptPacketValidation:
    """Validation result for the live producer effect receipt packet."""
...
def validate_live_producer_effect_receipt_packet(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    fixture_path: Path = DEFAULT_FIXTURE,
) -> tuple[LiveProducerEffectReceiptPacketValidation, dict[str, Any]]:
    """Validate the checked-in live producer effect receipt packet."""
    errors: list[str] = []
    schema = _load_json_object(schema_path, "effect receipt packet schema", errors)
    fixture = _load_json_object(fixture_path, "effect receipt packet fixture", errors)
    ...
    if getattr(source_validation, "effect_receipt_status", "") != "AwaitingEvidence":
        errors.append("source effect receipt preflight must remain AwaitingEvidence")
    ...
    validation = LiveProducerEffectReceiptPacketValidation(
        ok=not errors,
        errors=tuple(errors),
...
def _validate_packet_semantics(packet: Mapping[str, Any], errors: list[str], label: str) -> None:
    for field_name, expected_value in (
        ...
        ("packet_status", "blocked_awaiting_effect_receipt_components"),
    ...
    if packet.get("source_preflight_ref") != "examples/agentic_service_harness_live_producer_effect_receipt_preflight.local.json":
...
def _validate_validator_ref(packet: Mapping[str, Any], errors: list[str], label: str) -> None:
    ...
    if validator.get("command") != "python scripts/validate_agentic_service_harness_live_producer_effect_receipt_packet.py":
...
def main(argv: list[str] | None = None) -> int:
    """Run the live producer effect receipt packet validator."""
    args = build_arg_parser().parse_args(argv)
    validation, fixture = validate_live_producer_effect_receipt_packet(
    ...
    if args.json:
        ...
        print("AGENTIC SERVICE HARNESS LIVE PRODUCER EFFECT RECEIPT PACKET VALID")
    else:
        print(
            "AGENTIC SERVICE HARNESS LIVE PRODUCER EFFECT RECEIPT PACKET INVALID "
