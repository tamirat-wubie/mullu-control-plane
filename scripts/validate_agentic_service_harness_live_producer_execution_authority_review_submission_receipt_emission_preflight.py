#!/usr/bin/env python3
"""Validate live producer review submission receipt emission preflight.

Purpose: define the no-effect preflight for future receipt emission after the
review submission receipt contract without admitting receipt emission.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: receipt emission preflight schema, fixture, source receipt
contract validator, and scripts.validate_schemas.
Invariants: no review submission, receipt emission, receipt append, live
execution, connector call, runtime write, secret access, provider call,
...
from scripts.validate_agentic_service_harness_live_producer_execution_authority_review_submission_receipt_contract import (  # noqa: E402
    CONTRACT_STATUS as SOURCE_RECEIPT_CONTRACT_STATUS,
...
DEFAULT_SCHEMA = (
    REPO_ROOT
    / "schemas"
    / "agentic_service_harness_live_producer_execution_authority_review_submission_receipt_emission_preflight.schema.json"
)
DEFAULT_FIXTURE = (
    REPO_ROOT
    / "examples"
    / "agentic_service_harness_live_producer_execution_authority_review_submission_receipt_emission_preflight.awaiting_evidence.json"
)
PREFLIGHT_ID = "agentic-service-harness-live-producer-execution-authority-review-submission-receipt-emission-preflight"
PREFLIGHT_STATUS = "blocked_awaiting_review_submission_receipt_emission_evidence"
SOURCE_CONTRACT_ID = "agentic-service-harness-live-producer-execution-authority-review-submission-receipt-contract"
SOURCE_CONTRACT_REF = (
    "examples/agentic_service_harness_live_producer_execution_authority_review_submission_receipt_contract.awaiting_evidence.json"
)
...
FORBIDDEN_LIVE_CLAIM = re.compile(
    ...
    r"review_submission_receipt_append_admitted|digest_collection_complete|"
...
class LiveProducerExecutionAuthorityReviewSubmissionReceiptEmissionPreflightValidation:
    """Validation result for the review submission receipt emission preflight."""
    ...
    emission_requirement_count: int
    missing_emission_requirement_count: int
...
def validate_live_producer_execution_authority_review_submission_receipt_emission_preflight(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    fixture_path: Path = DEFAULT_FIXTURE,
) -> tuple[LiveProducerExecutionAuthorityReviewSubmissionReceiptEmissionPreflightValidation, dict[str, Any]]:
    """Validate the checked-in review submission receipt emission preflight."""
    errors: list[str] = []
    schema = _load_json_object(schema_path, "review submission receipt emission preflight schema", errors)
    fixture = _load_json_object(fixture_path, "review submission receipt emission preflight fixture", errors)
    source_validation, _source = validate_live_producer_execution_authority_review_submission_receipt_contract()
    errors.extend(f"source review submission receipt contract: {error}" for error in source_validation.errors)
    if source_validation.contract_status != SOURCE_RECEIPT_CONTRACT_STATUS:
        errors.append("source review submission receipt contract must remain blocked AwaitingEvidence")
    ...
    requirements = fixture.get("receipt_emission_requirements")
    missing_requirements = fixture.get("missing_receipt_emission_requirements")
    validation = LiveProducerExecutionAuthorityReviewSubmissionReceiptEmissionPreflightValidation(
        ok=not errors,
        errors=tuple(errors),
        ...
        emission_requirement_count=len(requirements) if isinstance(requirements, list) else 0,
        missing_emission_requirement_count=len(missing_requirements) if isinstance(missing_requirements, list) else 0,
    )
...
def _validate_preflight_semantics(preflight: Mapping[str, Any], errors: list[str], label: str) -> None:
    for field_name, expected in (
        ...
        ("source_review_submission_receipt_contract_status", SOURCE_RECEIPT_CONTRACT_STATUS),
        ("planning_only", True),
        ("read_only", True),
        ("receipt_emission_preflight_ready", True),
    ...
    _validate_requirement_collection(preflight.get("receipt_emission_requirements"), True, errors, label)
    _validate_requirement_collection(preflight.get("missing_receipt_emission_requirements"), False, errors, label)
    _validate_emission_policy(_mapping(preflight.get("receipt_emission_policy")), errors, label)
...
def _validate_source_binding(preflight: Mapping[str, Any], errors: list[str], label: str) -> None:
    binding = _mapping(preflight.get("source_review_submission_receipt_contract_binding"))
    expected = {
        "source_contract_id": SOURCE_CONTRACT_ID,
        "source_contract_ref": SOURCE_CONTRACT_REF,
        "source_validator_command": SOURCE_CONTRACT_VALIDATOR,
        "source_status": SOURCE_RECEIPT_CONTRACT_STATUS,
        "source_blocks_receipt_emission": True,
    ...
    for field_name, expected_value in expected.items():
        if binding.get(field_name) != expected_value:
            errors.append(f"{label}: source_review_submission_receipt_contract_binding.{field_name} must be {expected_value!r}")


def _validate_requirement_collection(value: Any, require_future_refs: bool, errors: list[str], label: str) -> None:
    if not isinstance(value, list):
        errors.append(f"{label}: receipt emission requirement collection must be a list")
        return
    observed_ids = tuple(item.get("input_id") for item in value if isinstance(item, Mapping))
    if observed_ids != REVIEW_INPUT_IDS:
        errors.append(f"{label}: receipt emission requirement order mismatch")
        missing_ids = [input_id for input_id in REVIEW_INPUT_IDS if input_id not in observed_ids]
        if missing_ids:
            errors.append(f"{label}: missing receipt emission inputs: {', '.join(missing_ids)}")
    for item in value:
        if not isinstance(item, Mapping):
            errors.append(f"{label}: receipt emission requirement entries must be objects")
            continue
        ...
        if item.get("receipt_emission_admissible", False) is not False:
            errors.append(f"{label}: {input_id} must not be admissible for receipt emission")
        if require_future_refs:
            for ref_field in ("required_receipt_ref", "required_receipt_digest_ref", "required_receipt_store_path_ref"):
...
def _validate_emission_policy(policy: Mapping[str, Any], errors: list[str], label: str) -> None:
    expected = {
        "policy_id": "agentic-service-harness-live-producer-review-submission-receipt-emission-preflight-policy",
        "all_receipt_refs_required": True,
        "all_receipt_digest_refs_required": True,
        "all_receipt_store_path_refs_required": True,
        "uao_receipt_emission_required": True,
    ...
    for field_name, expected_value in expected.items():
        if policy.get(field_name) != expected_value:
            errors.append(f"{label}: receipt_emission_policy.{field_name} must be {expected_value!r}")
...
def main(argv: list[str] | None = None) -> int:
    """Run the live producer authority review submission receipt emission preflight validator."""
    ...
    validation, _fixture = validate_live_producer_execution_authority_review_submission_receipt_emission_preflight(
    ...
    if args.json:
        ...
        print("AGENTIC SERVICE HARNESS LIVE PRODUCER EXECUTION AUTHORITY REVIEW SUBMISSION RECEIPT EMISSION PREFLIGHT VALID")
    else:
