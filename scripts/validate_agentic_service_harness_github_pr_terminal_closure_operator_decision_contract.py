#!/usr/bin/env python3
"""Validate Agentic Service Harness GitHub PR terminal closure decision contract.

Purpose: prove the terminal closure operator decision must be an explicit typed
value before certificate minting or terminal closure can proceed.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: schemas/agentic_service_harness_github_pr_terminal_closure_operator_decision_contract.schema.json,
examples/agentic_service_harness_github_pr_terminal_closure_operator_decision_contract.foundation.json,
scripts.validate_agentic_service_harness_github_pr_terminal_closure_operator_approval_gate, and scripts.validate_schemas.
Invariants:
  - The contract binds to the validated terminal closure operator approval gate.
  - Generic continuation is not an operator approval decision.
  - The contract grants no authority, certificate minting, or terminal closure.
"""
...
from scripts.validate_agentic_service_harness_github_pr_terminal_closure_operator_approval_gate import (  # noqa: E402
    DEFAULT_EXAMPLES as DEFAULT_SOURCE_APPROVAL_GATE_EXAMPLES,
    DEFAULT_SCHEMA as DEFAULT_SOURCE_APPROVAL_GATE_SCHEMA,
    validate_agentic_service_harness_github_pr_terminal_closure_operator_approval_gate,
)
...
DEFAULT_SCHEMA = (
    REPO_ROOT
    / "schemas"
    / "agentic_service_harness_github_pr_terminal_closure_operator_decision_contract.schema.json"
)
DEFAULT_EXAMPLES = (
    REPO_ROOT
    / "examples"
    / "agentic_service_harness_github_pr_terminal_closure_operator_decision_contract.foundation.json",
)
DEFAULT_OUTPUT = (
    REPO_ROOT
    / ".change_assurance"
    / "agentic_service_harness_github_pr_terminal_closure_operator_decision_contract_validation.json"
)
EXPECTED_SOURCE_APPROVAL_GATE_REF = (
    "examples/agentic_service_harness_github_pr_terminal_closure_operator_approval_gate.foundation.json"
)
EXPECTED_BINDING_ID = "agentic_service_harness_github_pr_terminal_closure_operator_decision_contract"
EXPECTED_SOURCE_APPROVAL_GATE_BINDING_ID = "agentic_service_harness_github_pr_terminal_closure_operator_approval_gate"
EXPECTED_CONTRACT_ID = "terminal-closure-operator-decision-contract.github-pr-chain"
EXPECTED_OPERATOR_DECISION_REF = "approval://github-pr-terminal-closure-certificate/operator-decision"
EXPECTED_CONTRACT_STATUS = "awaiting_explicit_operator_decision_value"
EXPECTED_ALLOWED_DECISION_VALUES = ("approve_terminal_certificate", "deny_terminal_certificate")
...
REQUIRED_FALSE_FLAGS = (
    ...
    "repository_written_by_contract",
    "connector_called_by_contract",
    "mutation_route_admitted_by_contract",
    "receipt_store_appended_by_contract",
    "secret_values_serialized_by_contract",
)
REQUIRED_TRUE_FLAGS = (
    "operator_decision_contract_ready",
    "read_only",
...
COMMAND_PREVIEW_CERTIFICATE_CANDIDATE_EVIDENCE_KEYS = (
    "source_terminal_closure_certificate_witness_ref",
    "source_binding_id",
    "source_requested_evidence_ref",
    "source_terminal_closure_certificate_request_id",
    "source_command_preview_effect_reconciliation_evidence_ref",
    "source_effect_reconciliation_witness_ref",
    "source_effect_reconciliation_binding_id",
    "source_ci_gate_before_ready_for_review_witness_ref",
    "source_ci_gate_binding_id",
    "source_repository_effect_rollback_plan_witness_id",
    "source_repository_effect_rollback_plan_witness_ref",
    "source_uao_admission_witness_id",
    "source_uao_admission_witness_ref",
    "source_branch_write_binding_id",
    "source_branch_write_binding_ref",
    "source_command_preview_ref",
    "source_redacted_command_preview",
    "source_argument_vector_template",
    "source_placeholder_refs",
    "source_actual_non_empty_diff_receipt_ref",
    "source_changed_file_refs",
    "source_diff_refs",
    "source_redacted_diff_bundle_ref",
    "source_redacted_output_ref",
    "source_terminal_closure_certificate_collected",
    "source_terminal_closure_authorized",
    "source_effect_reconciliation_collected",
    "source_authority_granted",
...
class GitHubPrTerminalClosureOperatorDecisionContractValidation:
    """Schema and semantic validation report for the terminal closure decision contract."""
...
def validate_agentic_service_harness_github_pr_terminal_closure_operator_decision_contract(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    example_paths: Sequence[Path] = DEFAULT_EXAMPLES,
    source_approval_gate_schema_path: Path = DEFAULT_SOURCE_APPROVAL_GATE_SCHEMA,
    source_approval_gate_example_paths: Sequence[Path] = DEFAULT_SOURCE_APPROVAL_GATE_EXAMPLES,
) -> GitHubPrTerminalClosureOperatorDecisionContractValidation:
    """Validate GitHub PR terminal closure operator decision contract examples."""
    errors: list[str] = []
    schema = _load_json_object(schema_path, "GitHub PR terminal closure operator decision contract schema", errors)
    source_validation = validate_agentic_service_harness_github_pr_terminal_closure_operator_approval_gate(
        schema_path=source_approval_gate_schema_path,
        example_paths=source_approval_gate_example_paths,
    )
    if not source_validation.ok:
        errors.extend(f"source PR terminal closure operator approval gate: {error}" for error in source_validation.errors)
    source_approval_gate = _load_json_object(
        source_approval_gate_example_paths[0],
        "GitHub PR terminal closure operator approval gate source",
        errors,
    )
    ...
    for example_path in example_paths:
        example = _load_json_object(
            example_path,
            f"GitHub PR terminal closure operator decision contract {_path_label(example_path)}",
            errors,
        )
        if not example:
            continue
        ...
        _validate_terminal_closure_operator_decision_contract_semantics(
            example,
            source_approval_gate,
            errors,
            _path_label(example_path),
        )
    return GitHubPrTerminalClosureOperatorDecisionContractValidation(
        ok=not errors,
        errors=tuple(errors),
        schema_path=_path_label(schema_path),
        example_paths=tuple(_path_label(path) for path in example_paths),
        example_count=len(examples),
        source_approval_gate_ref=EXPECTED_SOURCE_APPROVAL_GATE_REF,
    )


def write_github_pr_terminal_closure_operator_decision_contract_validation(
    validation: GitHubPrTerminalClosureOperatorDecisionContractValidation,
    output_path: Path,
) -> Path:
    """Write one deterministic terminal closure operator decision contract validation report."""
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(validation.as_dict(), indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return output_path


def _validate_terminal_closure_operator_decision_contract_semantics(
    payload: Mapping[str, Any],
    source_approval_gate: Mapping[str, Any],
    errors: list[str],
    label: str,
) -> None:
    _require_equal(payload, ("binding_id",), EXPECTED_BINDING_ID, errors, label)
    _require_equal(payload, ("source_approval_gate_ref",), EXPECTED_SOURCE_APPROVAL_GATE_REF, errors, label)
    _require_equal(payload, ("solver_outcome",), "AwaitingEvidence", errors, label)
    _require_equal(payload, ("witness_kind",), "terminal_closure_operator_decision_contract", errors, label)
    _require_equal(payload, ("requested_operator_decision_ref",), EXPECTED_OPERATOR_DECISION_REF, errors, label)
    _require_equal(payload, ("contract_status",), EXPECTED_CONTRACT_STATUS, errors, label)
    _require_equal(payload, ("decision_contract", "contract_id"), EXPECTED_CONTRACT_ID, errors, label)
    _require_equal(
        payload,
        ("decision_contract", "source_approval_gate_binding_id"),
        EXPECTED_SOURCE_APPROVAL_GATE_BINDING_ID,
        errors,
        label,
    )
    _require_equal(
        payload,
        ("decision_contract", "source_approval_gate_ref"),
        EXPECTED_SOURCE_APPROVAL_GATE_REF,
        errors,
        label,
    )
    _require_equal(
        payload,
        ("decision_contract", "operator_decision_ref"),
    ...
    _require_equal(
        payload,
        ("decision_contract", "allowed_decision_values"),
        list(EXPECTED_ALLOWED_DECISION_VALUES),
        errors,
        label,
    )
    _require_equal(payload, ("decision_contract", "decision_value_source"), "operator_explicit_input_only", errors, label)
    _require_equal(payload, ("decision_contract", "required_contract_result"), EXPECTED_CONTRACT_STATUS, errors, label)
    _require_equal(payload, ("effect_boundary", "network_policy"), "none", errors, label)
    if source_approval_gate:
        _require_equal(
            payload,
            ("scope", "repository_slug"),
            _get_nested(source_approval_gate, ("scope", "repository_slug")),
            errors,
            label,
        )
        _require_equal(
            ...
            _get_nested(source_approval_gate, ("scope", "repository_connection_id")),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("decision_contract", "source_approval_gate_binding_id"),
            _get_nested(source_approval_gate, ("binding_id",)),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("terminal_closure",),
            _get_nested(source_approval_gate, ("terminal_closure",)),
            errors,
            label,
        )
        ...
        for key in COMMAND_PREVIEW_CERTIFICATE_CANDIDATE_EVIDENCE_KEYS:
            _require_equal(
                payload,
                ("decision_contract", "command_preview_certificate_candidate_evidence", key),
                _get_nested(source_approval_gate, ("approval_gate", "command_preview_certificate_candidate_evidence", key)),
                errors,
                label,
            )
    ...
    if not isinstance(observed_witnesses, list):
        ...
        errors.append(f"{label}: remaining_witnesses must require explicit decision value before certificate")
...
def build_mutated_terminal_closure_operator_decision_contract(**updates: Any) -> dict[str, Any]:
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--source-approval-gate-schema", type=Path, default=DEFAULT_SOURCE_APPROVAL_GATE_SCHEMA)
    parser.add_argument("--source-approval-gate-example", type=Path, action="append", dest="source_approval_gate_examples")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    validation = validate_agentic_service_harness_github_pr_terminal_closure_operator_decision_contract(
        schema_path=args.schema,
        example_paths=tuple(args.examples) if args.examples else DEFAULT_EXAMPLES,
        source_approval_gate_schema_path=args.source_approval_gate_schema,
        source_approval_gate_example_paths=(
            tuple(args.source_approval_gate_examples)
            if args.source_approval_gate_examples
            else DEFAULT_SOURCE_APPROVAL_GATE_EXAMPLES
        ),
    )
    write_github_pr_terminal_closure_operator_decision_contract_validation(validation, args.output)
    if args.json:
        print(json.dumps(validation.as_dict(), indent=2, sort_keys=True))
    elif validation.ok:
        print("AGENTIC SERVICE HARNESS GITHUB PR TERMINAL CLOSURE OPERATOR DECISION CONTRACT VALID")
    else:
