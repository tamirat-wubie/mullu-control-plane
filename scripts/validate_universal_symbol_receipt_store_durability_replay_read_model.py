#!/usr/bin/env python3
"""Validate the durability replay witness operator read model.

Purpose: prove the durability replay operator projection is read-only and
bounded while durability replay binding, replay execution, receipt-store append,
raw payload exposure, mutation, and terminal closure remain denied.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: durability replay read-model schema/example and durability replay
witness validator constants.
Invariants:
  - The read model is a projection, not replay authority.
  - Every durability replay requirement appears exactly once.
  - Raw detail visibility, replay execution, and authority remain denied.
...
from scripts.validate_universal_symbol_receipt_store_durability_replay_witness import (  # noqa: E402
...
DEFAULT_SCHEMA_PATH = (
    REPO_ROOT / "schemas" / "universal_symbol_receipt_store_durability_replay_read_model.schema.json"
)
DEFAULT_READ_MODEL_PATH = (
    REPO_ROOT / "examples" / "universal_symbol_receipt_store_durability_replay_read_model.foundation.json"
)

EXPECTED_WITNESS_REF = "examples/universal_symbol_receipt_store_durability_replay_witness.foundation.json"
EXPECTED_DURABILITY_REPLAY_DECISION = "blocked_pending_ordered_replay_digest_crash_window_and_audit_evidence"
...
EXPECTED_OPERATOR_STATUS_SUMMARY: dict[str, str] = {
    "primary_status": "Durability replay blocked",
    "ordered_replay_status": "Awaiting ordered replay evidence",
    "crash_window_status": "Awaiting crash-window evidence",
    "raw_detail_visibility": "hidden_by_default",
    "operator_action": (
        "Collect ordered replay, append sequence, digest chain, idempotency reuse, "
        "crash-window, durability receipt, rollback handoff, and audit evidence"
    ),
}


class UniversalSymbolReceiptStoreDurabilityReplayReadModelError(ValueError):
    """Raised when the durability replay read model violates Foundation Mode."""


def validate_universal_symbol_receipt_store_durability_replay_read_model(
    read_model_path: Path = DEFAULT_READ_MODEL_PATH,
    schema_path: Path = DEFAULT_SCHEMA_PATH,
) -> dict[str, Any]:
    """Validate schema shape, bounded rows, and denied replay authority."""
    ...
    if errors:
        raise UniversalSymbolReceiptStoreDurabilityReplayReadModelError("; ".join(errors))
    return report
...
def load_json_object(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise UniversalSymbolReceiptStoreDurabilityReplayReadModelError(f"missing file: {path}") from exc
    except json.JSONDecodeError as exc:
        raise UniversalSymbolReceiptStoreDurabilityReplayReadModelError(f"invalid json: {path}: {exc}") from exc
    if not isinstance(value, dict):
        raise UniversalSymbolReceiptStoreDurabilityReplayReadModelError(f"expected object: {path}")
    return value


def _validate_schema_boundary(schema: Mapping[str, Any], errors: list[str]) -> None:
    expected_id = "urn:mullusi:schema:universal-symbol-receipt-store-durability-replay-read-model:1"
...
def _validate_read_model_boundary(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if read_model.get("read_model_is_not_replay_authority") is not True:
        errors.append("read model must not be replay authority")
...
def _validate_witness_projection(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if projection.get("durability_replay_decision") != EXPECTED_DURABILITY_REPLAY_DECISION:
        errors.append("witness_projection.durability_replay_decision must remain blocked")
    if projection.get("authority_granted") is not False:
        errors.append("witness_projection.authority_granted must remain false")
    if projection.get("durability_replay_requirement_count") != len(REQUIRED_REQUIREMENT_IDS):
        errors.append("witness_projection.durability_replay_requirement_count drift")
...
def _validate_source_witness_alignment(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if projection.get("witness_ref") != EXPECTED_WITNESS_REF:
        return
    try:
        witness = load_json_object(DEFAULT_DURABILITY_REPLAY_WITNESS_PATH)
    except UniversalSymbolReceiptStoreDurabilityReplayReadModelError as exc:
        errors.append(f"source durability replay witness unavailable: {exc}")
        return

    witness_summary = _mapping(witness.get("contract_summary"))
    expected_counts = {
        "durability_replay_requirement_count": witness_summary.get("durability_replay_requirement_count"),
        "blocked_reason_count": witness_summary.get("blocked_reason_count"),
    }
    for field_name, expected_count in expected_counts.items():
        if projection.get(field_name) != expected_count:
            errors.append(f"witness_projection.{field_name} must match source durability replay witness")
    if projection.get("durability_replay_decision") != witness.get("durability_replay_decision"):
        errors.append("witness_projection.durability_replay_decision must match source durability replay witness")
...
def _validate_requirement_rows(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    for row in rows:
        ...
        if row.get("proof_state") not in {"Unknown", "BudgetUnknown", "Fail"}:
            errors.append(f"{row.get('requirement_id')}: proof_state must block durability replay")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print(f"[FAIL] universal_symbol_receipt_store_durability_replay_read_model: {exc}")
            print("STATUS: failed")
        return 1
    ...
    if args.json:
        ...
        print("[PASS] universal_symbol_receipt_store_durability_replay_read_model")
        print("STATUS: passed")
    return 0
