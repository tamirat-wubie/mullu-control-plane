#!/usr/bin/env python3
"""Validate finance email/calendar operator input requests.

Purpose: prove a finance email/calendar operator input request is schema-valid
...
and emitted finance email/calendar operator input request JSON.
Invariants:
  - Handoff allowance is consistent with readiness and missing inputs.
...
DEFAULT_OUTPUT = (
    REPO_ROOT
    / ".change_assurance"
    / "finance_approval_email_calendar_operator_input_request_validation.json"
)
REQUEST_ID_PATTERN = re.compile(
    r"^finance-email-calendar-operator-input-request-[0-9a-f]{16}$"
)
INPUT_ID_PATTERN = re.compile(r"^finance-email-calendar-input-[0-9a-f]{12}$")


@dataclass(frozen=True, slots=True)
class FinanceEmailCalendarOperatorInputRequestValidation:
    """Validation result for one finance email/calendar operator request."""
...
def validate_finance_email_calendar_operator_input_request(
    *,
    request_path: Path = DEFAULT_REQUEST,
    schema_path: Path = DEFAULT_SCHEMA,
    require_blocked: bool = False,
) -> FinanceEmailCalendarOperatorInputRequestValidation:
    """Validate one finance email/calendar operator input request."""
    errors: list[str] = []
    try:
        schema = _load_schema(schema_path)
    except OSError:
        schema = {}
        errors.append("finance email/calendar operator input request schema file missing")
    request = _load_json_object(request_path, "finance email/calendar operator input request", errors)
    if schema and request:
        errors.extend(_validate_schema_instance(schema, request))
        _validate_semantics(request, errors)
        if require_blocked and request.get("handoff_allowed") is not False:
            errors.append("require blocked: finance email/calendar handoff allowed")
    handoff_allowed = bool(request.get("handoff_allowed") is True) if request else False
    return FinanceEmailCalendarOperatorInputRequestValidation(
        valid=not errors,
        handoff_allowed=handoff_allowed,
        request_path=_path_label(request_path),
        schema_path=_path_label(schema_path),
        errors=tuple(errors),
        next_action=_next_action(request) if request else "emit finance email/calendar operator input request",
    )


def write_finance_email_calendar_operator_input_request_validation(
    validation: FinanceEmailCalendarOperatorInputRequestValidation,
    output_path: Path,
) -> Path:
...
def _validate_semantics(request: dict[str, Any], errors: list[str]) -> None:
    if not REQUEST_ID_PATTERN.fullmatch(str(request.get("request_id", ""))):
        errors.append("request_id must match finance-email-calendar-operator-input-request pattern")
    ...
    expected_handoff_allowed = ready and required_inputs_empty and blocked_actions_empty
    if request.get("handoff_allowed") is not expected_handoff_allowed:
        errors.append("handoff_allowed must equal ready with no required inputs or blocked actions")
    ...
    if isinstance(required_inputs, list):
        ...
        if any(not INPUT_ID_PATTERN.fullmatch(input_id) for input_id in input_ids):
            errors.append("required input ids must match finance-email-calendar-input pattern")
    ...
    if (
        ...
        or "finance_approval_email_calendar_binding_receipt" not in source_artifacts
    ):
        errors.append("source_artifacts must include finance_approval_email_calendar_binding_receipt")


def _next_action(request: dict[str, Any]) -> str:
    if request.get("handoff_allowed") is True:
    ...
    return "inspect finance email/calendar operator input request"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Validate finance email/calendar operator input request."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print("finance email/calendar operator input request valid")
    else:
        print(f"finance email/calendar operator input request invalid errors={list(validation.errors)}")
