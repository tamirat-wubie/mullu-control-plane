#!/usr/bin/env python3
"""Validate the ReadOnlyWorkerEffectReconciliationWitness contract.
...
ReadOnlyWorkerPhiGovDispatchAuthorizationWitness validation.
Invariants:
...
from scripts.validate_read_only_worker_phi_gov_dispatch_authorization_witness import (  # noqa: E402
    DEFAULT_RECEIPT_PATH as DEFAULT_PHI_GOV_DISPATCH_AUTHORIZATION_WITNESS_PATH,
    DEFAULT_SCHEMA_PATH as DEFAULT_PHI_GOV_DISPATCH_AUTHORIZATION_WITNESS_SCHEMA_PATH,
    load_json_object as load_phi_gov_dispatch_authorization_json_object,
    validate_phi_gov_dispatch_authorization_witness_record,
)
...
DEFAULT_SCHEMA_PATH = (
    WORKSPACE_ROOT
    / "schemas"
    / "read_only_worker_effect_reconciliation_witness.schema.json"
)
DEFAULT_RECEIPT_PATH = (
    WORKSPACE_ROOT
    / "examples"
    / "read_only_worker_effect_reconciliation_witness.foundation.json"
)
EXPECTED_SCHEMA_ID = "urn:mullusi:schema:read-only-worker-effect-reconciliation-witness:1"
EXPECTED_SCHEMA_TITLE = "Read-Only Worker Effect Reconciliation Witness"
EXPECTED_RECEIPT_VERSION = "read_only_worker_effect_reconciliation_witness.v1"
EXPECTED_PHI_GOV_DISPATCH_AUTHORIZATION_WITNESS_REF = (
...
EXPECTED_WITNESS_MODE = "EFFECT_RECONCILIATION_WITNESS_ONLY"
EXPECTED_TARGET_EFFECT_RECONCILIATION_REF = (
    "candidate://effect-reconciliation/read-only-repo-inspection"
)
EXPECTED_PHI_GOV_AUTHORIZATION_PROFILE = (
    "LOCAL_READ_ONLY_REPO_INSPECTION_EFFECT_RECONCILIATION"
)
EXPECTED_ADMISSION_DECISION = (
    "EFFECT_RECONCILIATION_WITNESS_BLOCKED_AWAITING_AUTHORIZATION_EVIDENCE"
)
...
TRUE_ACTIVATION_FIELDS = (
    "upstream_phi_gov_dispatch_authorization_witness_validated",
...
DENIED_AUTHORITY_FIELDS = (
    ...
    "effect_reconciliation_performed",
...
class ReadOnlyWorkerEffectReconciliationWitnessError(ValueError):
...
def load_json_object(json_path: Path, label: str) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ReadOnlyWorkerEffectReconciliationWitnessError(
...
def validate_schema_artifact(schema: dict[str, Any]) -> list[str]:
    ...
    if isinstance(required_fields, list) and isinstance(properties, dict):
        for field_name in (
            "receipt_id",
            "receipt_version",
            "phi_gov_dispatch_authorization_witness_ref",
...
def _validate_default_upstream_witnesses() -> tuple[str, ...]:
    ...
    phi_gov_witness_payload = load_phi_gov_dispatch_authorization_json_object(
        DEFAULT_PHI_GOV_DISPATCH_AUTHORIZATION_WITNESS_PATH,
        "ReadOnlyWorkerPhiGovDispatchAuthorizationWitness",
    )
    phi_gov_witness_schema = _load_schema(
        DEFAULT_PHI_GOV_DISPATCH_AUTHORIZATION_WITNESS_SCHEMA_PATH
    )
    upstream_errors.extend(
        f"phi_gov_dispatch_authorization_witness: {error}"
        for error in validate_phi_gov_dispatch_authorization_witness_record(
            phi_gov_witness_payload,
            phi_gov_witness_schema,
        )
    )
...
def validate_effect_reconciliation_witness_record(
    record: Any,
    schema: dict[str, Any] | None = None,
    phi_gov_dispatch_authorization_witness: dict[str, Any] | None = None,
) -> list[str]:
    ...
    if not isinstance(record, dict):
        errors.append("ReadOnlyWorkerEffectReconciliationWitness must be object")
        return errors

    errors.extend(_validate_schema_instance(schema_payload, record))

    if phi_gov_dispatch_authorization_witness is None:
        ...
        errors.extend(
            f"phi_gov_dispatch_authorization_witness: {error}"
            for error in validate_phi_gov_dispatch_authorization_witness_record(
                phi_gov_dispatch_authorization_witness,
                phi_gov_witness_schema,
            )
        )
    ...
    if record.get("receipt_version") != EXPECTED_RECEIPT_VERSION:
        errors.append(
            "receipt_version must match read_only_worker_effect_reconciliation_witness.v1"
        )
    if record.get("phi_gov_dispatch_authorization_witness_ref") != (
        EXPECTED_PHI_GOV_DISPATCH_AUTHORIZATION_WITNESS_REF
    ):
        errors.append("phi_gov_dispatch_authorization_witness_ref is invalid")
    ...
    if not isinstance(authority, dict):
        ...
        if authority.get("effect_reconciliation_witness_defined") is not True:
            errors.append(
                "authority_scope.effect_reconciliation_witness_defined must be true"
            )
    ...
    contract = record.get("effect_reconciliation_contract")
    if not isinstance(contract, dict):
        errors.append("effect_reconciliation_contract must be an object")
    else:
        if contract.get("worker_id") != EXPECTED_WORKER_ID:
            errors.append("effect_reconciliation_contract.worker_id is invalid")
        if contract.get("capability") != EXPECTED_WORKER_PATH:
            errors.append("effect_reconciliation_contract.capability is invalid")
        if contract.get("operation_family") != EXPECTED_OPERATION_FAMILY:
            errors.append("effect_reconciliation_contract.operation_family is invalid")
        ...
        if contract.get("source_phi_gov_dispatch_authorization_witness_ref") != (
            EXPECTED_PHI_GOV_DISPATCH_AUTHORIZATION_WITNESS_REF
        ):
            errors.append(
                "effect_reconciliation_contract."
        ...
        if contract.get("target_effect_reconciliation_ref") != (
            EXPECTED_TARGET_EFFECT_RECONCILIATION_REF
        ):
            errors.append(
                "effect_reconciliation_contract."
                "target_effect_reconciliation_ref is invalid"
            )
        ...
        _require_refs(
            ...
            REQUIRED_SOURCE_RECEIPT_REFS,
            "effect_reconciliation_contract.required_source_receipt_refs",
            errors,
        )
        _require_refs(
            contract.get("required_authorization_input_refs"),
            REQUIRED_AUTHORIZATION_INPUT_REFS,
            "effect_reconciliation_contract.required_authorization_input_refs",
            errors,
        )
        _require_refs(
            contract.get("validation_refs"),
            REQUIRED_VALIDATION_REFS,
            "effect_reconciliation_contract.validation_refs",
            errors,
        )
    ...
    if not isinstance(decision, dict):
        ...
        if decision.get("effect_reconciliation_witness_defined") is not True:
            errors.append(
                "admission_decision.effect_reconciliation_witness_defined must be true"
            )
        _require_refs(
...
def validate_effect_reconciliation_witness(
    ...
    receipt = load_json_object(receipt_path, "ReadOnlyWorkerEffectReconciliationWitness")
    return validate_effect_reconciliation_witness_record(receipt, schema)
...
def main(argv: list[str] | None = None) -> int:
    """Validate ReadOnlyWorkerEffectReconciliationWitness artifacts."""

    parser = argparse.ArgumentParser(
        description="Validate ReadOnlyWorkerEffectReconciliationWitness contract."
    )
    ...
    errors = validate_effect_reconciliation_witness(args.schema, args.receipt)
    if args.json:
        payload = {
            ...
            "errors": errors,
            "receipt": {
                "receipt_id": "read_only_worker_effect_reconciliation_witness_validation",
        ...
        print("[FAIL] read_only_worker_effect_reconciliation_witness", file=sys.stderr)
        for error in errors:
            print(f"  - {error}", file=sys.stderr)
    else:
        print("[PASS] read_only_worker_effect_reconciliation_witness")
