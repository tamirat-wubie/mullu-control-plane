#!/usr/bin/env python3
"""Validate operator value-binding explicit decision value-ref verification result request."""
...
RUNTIME_GENERATED_AT = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_VALUE_EXPLICIT_DECISION_VALUE_REF_VERIFICATION_RESULT_REQUEST_GENERATED_AT
...
FALSE_FIELDS = frozenset(
    {
        ...
        "explicit_decision_value_ref_verification_preflight_satisfied",
...
class PersonalAssistantExplicitDecisionValueRefVerificationResultRequestValidation:
    """Validation result for explicit decision value-ref verification result request."""

    valid: bool
    runtime_validated: bool
    result_request_record_count: int
    verification_preflight_checked_count: int
...
def validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_explicit_decision_value_ref_verification_result_request(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    receipt_schema_path: Path = DEFAULT_RECEIPT_SCHEMA,
) -> PersonalAssistantExplicitDecisionValueRefVerificationResultRequestValidation:
    """Validate runtime verifier execution explicit decision value-ref result request."""

    errors: list[str] = []
    schema = _load_json_object(schema_path, "explicit decision value-ref verification result request schema", errors)
    ...
    errors.extend(_validate_explicit_decision_value_ref_verification_result_request_semantics(envelope, receipt_schema))
    _scan_secret_values(envelope, errors, path="$runtime")
    return PersonalAssistantExplicitDecisionValueRefVerificationResultRequestValidation(
        valid=not errors,
        runtime_validated=not errors,
        result_request_record_count=int(summary.get("result_request_record_count", 0)),
        verification_preflight_checked_count=int(summary.get("verification_preflight_checked_count", 0)),
...
def _validate_explicit_decision_value_ref_verification_result_request_semantics(envelope: Mapping[str, Any], receipt_schema: Mapping[str, Any]) -> tuple[str, ...]:
    errors: list[str] = []
    expected_top = {
    ...
    records = envelope.get("result_request_records")
    if not isinstance(records, list):
        errors.append("result_request_records must be a list")
        return tuple(errors)
    if envelope.get("result_request_record_count") != len(EXPECTED_REQUIRED_VALUE_REFS):
        errors.append("result_request_record_count must equal four canonical required refs")
    record_ids: list[str] = []
    ...
    for index, record in enumerate(records):
        if not isinstance(record, dict):
            errors.append(f"result_request_records[{index}] must be an object")
            continue
        record_ids.append(str(record.get("result_request_record_id", "")))
        source_record_ids.append(str(record.get("source_verification_record_id", "")))
        submitted_ref_uris.append(str(record.get("submitted_ref_uri", "")))
        names.append(str(record.get("ref_name", "")))
        _require_record(index, record, errors)
    if tuple(names) != EXPECTED_REQUIRED_VALUE_REFS:
        errors.append("result_request_records must match canonical required ref order")
    if envelope.get("result_request_record_ids") != record_ids:
        errors.append("result_request_record_ids must match record order")
    if envelope.get("source_verification_record_ids") != source_record_ids:
        errors.append("source_verification_record_ids must match record order")
    if envelope.get("submitted_ref_uris") != submitted_ref_uris:
        errors.append("submitted_ref_uris must match record order")
    ...
    if metadata.get("operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_explicit_decision_value_ref_verification_result_request_is_execution") is not False:
        errors.append("receipt.metadata execution flag must be false")
    if metadata.get("verification_result_requested") is not True:
        errors.append("receipt.metadata verification_result_requested must be true")
    ...
    if metadata.get("explicit_decision_value_refs_verified") is not False:
        errors.append("receipt.metadata explicit_decision_value_refs_verified must be false")
...
def _require_record(index: int, record: Mapping[str, Any], errors: list[str]) -> None:
    expected = {
    ...
    for field_name, expected_value in expected.items():
        if record.get(field_name) != expected_value:
            errors.append(f"result_request_records[{index}].{field_name} must be {expected_value}")
    ...
    if record.get("submitted_ref_uri") != expected_uri:
        errors.append(f"result_request_records[{index}].submitted_ref_uri must match ref_name")
    if record.get("blocking_reason") != f"{ref_name}_verification_result_not_present":
        errors.append(f"result_request_records[{index}].blocking_reason must match ref_name")


def _require_summary(envelope: Mapping[str, Any], records: list[Any], errors: list[str]) -> None:
    summary = _mapping(envelope.get("summary"))
    expected = {
        "result_request_record_count": len(EXPECTED_REQUIRED_VALUE_REFS),
        "verification_preflight_checked_count": len(EXPECTED_REQUIRED_VALUE_REFS),
    ...
    if len(records) != len(EXPECTED_REQUIRED_VALUE_REFS):
        errors.append("summary source result request record count must stay canonical")
...
def main(argv: list[str] | None = None) -> int:
    """Run the explicit decision value-ref verification result request validator."""
    ...
    validation = validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_explicit_decision_value_ref_verification_result_request(
    ...
    if args.json:
        ...
        print("personal assistant verifier execution explicit decision value-ref verification result request: valid")
    else:
        print("personal assistant verifier execution explicit decision value-ref verification result request: invalid")
