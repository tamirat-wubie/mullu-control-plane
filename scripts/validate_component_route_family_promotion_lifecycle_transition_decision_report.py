#!/usr/bin/env python3
"""Validate Component Harness promotion lifecycle-transition decision reports.

Purpose: prove denied route-binding decisions can produce a denial-only
lifecycle-transition decision without lifecycle state change, promotion, or
authority grant.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: lifecycle-transition decision schema/example, runtime builder,
and promotion route-binding decision validation.
Invariants:
  - A lifecycle-transition decision report is not a lifecycle transition
    receipt.
  - Denied lifecycle transition cannot change component state.
  - No lifecycle-transition decision can grant authority, execute, call
    connectors, or claim terminal closure.
"""
...
from mcoi_runtime.app.component_route_family_promotion_lifecycle_transition_decision_report import (  # noqa: E402
...
from scripts.validate_component_route_family_promotion_route_binding_decision_report import (  # noqa: E402
...
DEFAULT_SCHEMA = (
    REPO_ROOT
    / "schemas"
    / "component_route_family_promotion_lifecycle_transition_decision_report.schema.json"
)
DEFAULT_EXAMPLE = (
    REPO_ROOT
    / "examples"
...
DEFAULT_OUTPUT = (
    REPO_ROOT
    / ".change_assurance"
    / "component_route_family_promotion_lifecycle_transition_decision_report_validation.json"
)
...
class ComponentRouteFamilyPromotionLifecycleTransitionDecisionReportValidation:
    """Schema and semantic validation report for lifecycle-transition decisions."""
    ...
    decision: str
    lifecycle_transition_decision_count: int
    lifecycle_transition_denial_count: int
    lifecycle_transition_authorization_count: int
    lifecycle_state_change_count: int
...
def validate_component_route_family_promotion_lifecycle_transition_decision_report(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    example_path: Path = DEFAULT_EXAMPLE,
) -> ComponentRouteFamilyPromotionLifecycleTransitionDecisionReportValidation:
    """Validate lifecycle-transition decision schema, example, and runtime projection."""

    errors: list[str] = []
    schema = _load_json_object(schema_path, "component route-family promotion lifecycle transition schema", errors)
    example = _load_json_object(example_path, "component route-family promotion lifecycle transition example", errors)

    route_binding_validation = validate_component_route_family_promotion_route_binding_decision_report()
    if not route_binding_validation.ok:
        errors.extend(
            f"component route-family promotion route-binding decision validation failed: {error}"
            for error in route_binding_validation.errors
        )

    runtime_report = build_component_route_family_promotion_lifecycle_transition_decision_report()
    if schema and example:
        ...
        _validate_lifecycle_transition_decision_semantics(example, errors, _path_label(example_path))
    _validate_lifecycle_transition_decision_semantics(
        runtime_report,
        errors,
        "runtime component route-family promotion lifecycle transition decision",
    )

    summary = runtime_report.get("summary", {})
    return ComponentRouteFamilyPromotionLifecycleTransitionDecisionReportValidation(
        ok=not errors,
        errors=tuple(errors),
        ...
        lifecycle_transition_decision_count=(
            int(summary.get("lifecycle_transition_decision_count", 0)) if isinstance(summary, dict) else 0
        ),
        lifecycle_transition_denial_count=(
            int(summary.get("lifecycle_transition_denial_count", 0)) if isinstance(summary, dict) else 0
        ),
        lifecycle_transition_authorization_count=(
            int(summary.get("lifecycle_transition_authorization_count", 0)) if isinstance(summary, dict) else 0
        ),
        lifecycle_state_change_count=(
            int(summary.get("lifecycle_state_change_count", 0)) if isinstance(summary, dict) else 0
        ),
    )


def write_component_route_family_promotion_lifecycle_transition_decision_report_validation(
    validation: ComponentRouteFamilyPromotionLifecycleTransitionDecisionReportValidation,
    output_path: Path,
) -> Path:
    """Write a deterministic lifecycle-transition decision validation receipt."""
...
def _validate_lifecycle_transition_decision_semantics(
    report: dict[str, Any],
    errors: list[str],
    label: str,
) -> None:
    ...
    if report.get("lifecycle_transition_decision_state") != "denied_pending_route_binding_witness":
        errors.append(
            f"{label}: lifecycle_transition_decision_state must be denied_pending_route_binding_witness"
        )
    if report.get("promotion_decision") != "blocked_lifecycle_transition_not_authorized":
        errors.append(f"{label}: promotion_decision must be blocked_lifecycle_transition_not_authorized")
    ...
    if report.get("current_lifecycle_state") != CURRENT_LIFECYCLE_STATE:
        errors.append(f"{label}: current_lifecycle_state must be {CURRENT_LIFECYCLE_STATE}")
    if report.get("requested_lifecycle_state") != REQUESTED_LIFECYCLE_STATE:
        errors.append(f"{label}: requested_lifecycle_state must be {REQUESTED_LIFECYCLE_STATE}")
    if report.get("resulting_lifecycle_state") != RESULTING_LIFECYCLE_STATE:
        errors.append(f"{label}: resulting_lifecycle_state must remain {RESULTING_LIFECYCLE_STATE}")
    for field_name in (
        "record_evidence_satisfied",
        "lifecycle_transition_decision_is_not_lifecycle_receipt",
        "lifecycle_transition_decision_is_not_state_change",
        "lifecycle_transition_decision_is_not_promotion_approval",
        "lifecycle_transition_decision_is_not_authority_grant",
    ...
    for field_name in (
        ...
        if _string_list(report.get(field_name)):
            errors.append(f"{label}: {field_name} must remain empty until separate lifecycle evidence exists")

    decisions = report.get("lifecycle_transition_decisions")
    summary = report.get("summary")
    if not isinstance(decisions, list) or len(decisions) != 1:
        errors.append(f"{label}: lifecycle_transition_decisions must contain exactly one decision")
        return
    ...
    if not isinstance(decision, dict):
        errors.append(f"{label}: lifecycle_transition_decisions entries must be objects")
        return
    _validate_lifecycle_transition_decision(decision, errors, label)

    decision_id = str(decision.get("lifecycle_transition_decision_id", ""))
    source_route_binding_id = str(decision.get("source_route_binding_decision_id", ""))
    ...
    if _string_list(report.get("lifecycle_transition_decision_refs")) != [decision_id]:
        errors.append(f"{label}: lifecycle_transition_decision_refs must match lifecycle_transition_decision_id")
    if _string_list(report.get("source_route_binding_decision_refs")) != [source_route_binding_id]:
    ...
    missing_witnesses = set(_string_list(report.get("missing_lifecycle_transition_witnesses")))
    ...
    if required_followups != REQUIRED_FOLLOWUP_DECISIONS:
        errors.append(f"{label}: required_followup_decisions must match lifecycle follow-up set")
    if missing_witnesses != MISSING_LIFECYCLE_TRANSITION_WITNESSES:
        errors.append(f"{label}: missing_lifecycle_transition_witnesses must match lifecycle witness set")

    expected_counts = {
        "lifecycle_transition_decision_count": 1,
        "lifecycle_transition_denial_count": 1 if decision.get("decision_state") == "denied" else 0,
        "lifecycle_transition_authorization_count": (
            1 if decision.get("lifecycle_transition_authorized") is True else 0
        ),
        "lifecycle_state_change_count": 1 if decision.get("lifecycle_state_changed") is True else 0,
    ...
    for field_name, expected_count in expected_counts.items():
        if summary.get(field_name) != expected_count:
            errors.append(f"{label}: summary.{field_name} must match lifecycle transition decision")
...
def _validate_lifecycle_transition_decision(decision: dict[str, Any], errors: list[str], label: str) -> None:
    if decision.get("gate_id") != "lifecycle_transition_gate":
        errors.append(f"{label}: lifecycle transition decision gate_id must be lifecycle_transition_gate")
    if decision.get("record_kind") != "lifecycle_transition":
        errors.append(f"{label}: lifecycle transition decision record_kind must be lifecycle_transition")
    for field_name, expected_value in (
        ...
        if decision.get(field_name) != expected_value:
            errors.append(f"{label}: lifecycle transition decision {field_name} must be {expected_value}")
    for field_name in (
        "record_evidence_satisfied",
        ...
        "decision_is_not_lifecycle_receipt",
        "decision_is_not_state_change",
        "decision_is_not_promotion_approval",
        "decision_is_not_authority_grant",
        "foundation_fixture_decision_is_not_live_operator_evidence",
    ):
        if decision.get(field_name) is not True:
            errors.append(f"{label}: lifecycle transition decision {field_name} must be true")
    for field_name in (
        "action_requirement_satisfied",
        "lifecycle_transition_authorized",
        "lifecycle_state_changed",
        ...
        if decision.get(field_name) is not False:
            errors.append(f"{label}: lifecycle transition decision {field_name} must be false")
    ...
    for field_name in (
        ...
        if _string_list(decision.get(field_name)):
            errors.append(f"{label}: lifecycle transition decision {field_name} must remain empty")
    if set(_string_list(decision.get("missing_lifecycle_transition_witnesses"))) != MISSING_LIFECYCLE_TRANSITION_WITNESSES:
        errors.append(f"{label}: missing_lifecycle_transition_witnesses must match lifecycle witness set")
    if not decision.get("decision_reason"):
        errors.append(f"{label}: lifecycle transition decision must carry decision_reason")
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    """Parse lifecycle-transition decision report validation arguments."""

    parser = argparse.ArgumentParser(description="Validate Component Harness lifecycle-transition decision report.")
...
def main(argv: Sequence[str] | None = None) -> int:
    """CLI entry point for lifecycle-transition decision report validation."""
    ...
    if args.json:
        ...
        print("COMPONENT ROUTE FAMILY PROMOTION LIFECYCLE TRANSITION DECISION REPORT VALID")
    else:
