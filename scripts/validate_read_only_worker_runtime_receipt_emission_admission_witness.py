#!/usr/bin/env python3
"""Validate the ReadOnlyWorkerRuntimeReceiptEmissionAdmissionWitness contract.

Purpose: verify Foundation Mode witness evidence for a future live
read_only_repo_inspection runtime receipt emission admission boundary
...
Invariants:
  ...
  - Runtime runner registration, dispatch endpoint registration, runtime receipt
    emitter registration, schema binding, receipt emission admission, receipt
...
from scripts.validate_read_only_worker_runtime_receipt_store_activation_witness import (  # noqa: E402
    DEFAULT_RECEIPT_PATH as DEFAULT_STORE_ACTIVATION_WITNESS_PATH,
    DEFAULT_SCHEMA_PATH as DEFAULT_STORE_ACTIVATION_WITNESS_SCHEMA_PATH,
    load_json_object as load_store_activation_json_object,
...
DEFAULT_SCHEMA_PATH = (
    WORKSPACE_ROOT / "schemas" / "read_only_worker_runtime_receipt_emission_admission_witness.schema.json"
)
DEFAULT_RECEIPT_PATH = (
    WORKSPACE_ROOT / "examples" / "read_only_worker_runtime_receipt_emission_admission_witness.foundation.json"
)
EXPECTED_SCHEMA_ID = "urn:mullusi:schema:read-only-worker-runtime-receipt-emission-admission-witness:1"
EXPECTED_SCHEMA_TITLE = "Read-Only Worker runtime receipt emission admission Witness"
EXPECTED_RECEIPT_VERSION = "read_only_worker_runtime_receipt_emission_admission_witness.v1"
...
EXPECTED_WITNESS_MODE = "LIVE_RUNTIME_RECEIPT_EMISSION_ADMISSION_WITNESS_ONLY"
EXPECTED_RUNTIME_RECEIPT_KIND = "future_worker_runtime_receipt"
EXPECTED_TARGET_RECEIPT_EMISSION_ADMISSION_REF = (
    "candidate://runtime-receipt-emission/admission/read-only-repo-inspection"
)
EXPECTED_RECEIPT_EMISSION_ADMISSION_PROFILE = (
    "LOCAL_READ_ONLY_REPO_INSPECTION_RUNTIME_RECEIPT_EMISSION_ADMISSION"
)
EXPECTED_ADMISSION_DECISION = (
    "LIVE_RUNTIME_RECEIPT_EMISSION_ADMISSION_WITNESS_BLOCKED_AWAITING_OPERATOR_APPROVAL_AND_LIVE_EVIDENCE"
)
...
REQUIRED_ACTIVATION_INPUT_REFS = (
    ...
    "evidence://runtime-receipt-store-activation-witness",
...
REQUIRED_BLOCKED_REASON_REFS = (
    ...
    "blocked://runtime-receipt-store/activation-witness-missing",
...
ACTIVATION_EVALUATION_DENIED_FIELDS = (
    "runtime_receipt_emission_admission_performed",
...
class ReadOnlyWorkerRuntimeReceiptEmissionAdmissionWitnessError(ValueError):
    """Raised when a runtime receipt emission admission witness artifact cannot load."""
...
def load_json_object(json_path: Path, label: str) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ReadOnlyWorkerRuntimeReceiptEmissionAdmissionWitnessError(f"{label} must be a JSON object")
    return payload
...
def validate_schema_artifact(schema: dict[str, Any]) -> list[str]:
    ...
    if isinstance(required_fields, list) and isinstance(properties, dict):
        for field_name in (
            ...
            "receipt_version",
            "authority_scope",
            "runtime_receipt_emission_admission_witness_contract",
...
def validate_runtime_receipt_emission_admission_witness_record(
    record: Any,
    ...
    """Return deterministic validation errors for one receipt emission admission witness."""

    schema_payload = schema or _load_schema(DEFAULT_SCHEMA_PATH)
    errors = _validate_schema_instance(schema_payload, record)
    if not isinstance(record, dict):
        errors.append("read-only worker runtime receipt emission admission witness must be a JSON object")
        return errors
    ...
    store_activation_payload = store_activation_witness or load_store_activation_json_object(
        DEFAULT_STORE_ACTIVATION_WITNESS_PATH,
        "ReadOnlyWorkerRuntimeReceiptStoreActivationWitness",
    )
    store_activation_schema = _load_schema(DEFAULT_STORE_ACTIVATION_WITNESS_SCHEMA_PATH)
    errors.extend(
        f"store activation witness: {error}"
        for error in validate_runtime_receipt_store_activation_witness_record(
            store_activation_payload,
            store_activation_schema,
        )
    )
    ...
    _validate_activation_contract(
        record.get("runtime_receipt_emission_admission_witness_contract"),
        schema_binding_activation_payload,
        store_write_path_payload,
        store_activation_payload,
        schema_binding_payload,
...
def validate_runtime_receipt_emission_admission_witness(
    ...
    receipt = load_json_object(receipt_path, "ReadOnlyWorkerRuntimeReceiptEmissionAdmissionWitness")
    ...
    store_activation_witness = load_store_activation_json_object(
        store_activation_witness_path,
        "ReadOnlyWorkerRuntimeReceiptStoreActivationWitness",
    )
...
def build_mutated_runtime_receipt_emission_admission_witness(**updates: Any) -> dict[str, Any]:
    """Build a deterministic mutated copy of the default witness receipt."""

    receipt = load_json_object(DEFAULT_RECEIPT_PATH, "ReadOnlyWorkerRuntimeReceiptEmissionAdmissionWitness")
...
def _validate_top_level_refs(
    ...
    if record.get("receipt_version") != EXPECTED_RECEIPT_VERSION:
        errors.append("receipt_version must match read_only_worker_runtime_receipt_emission_admission_witness.v1")
    expected_refs = {
        ...
        "store_activation_witness_ref": EXPECTED_STORE_ACTIVATION_WITNESS_REF,
    ...
    if record.get("selected_worker_path") != schema_binding_activation_witness.get("selected_worker_path"):
        errors.append(
            "receipt emission admission witness selected_worker_path must match schema-binding activation witness"
        )
    if record.get("selected_worker_path") != store_write_path_witness.get("selected_worker_path"):
        errors.append("receipt emission admission witness selected_worker_path must match store write-path witness")
    if record.get("selected_worker_path") != store_activation_witness.get("selected_worker_path"):
        errors.append("receipt emission admission witness selected_worker_path must match store activation witness")
    if record.get("selected_worker_path") != schema_binding_witness.get("selected_worker_path"):
        errors.append("receipt emission admission witness selected_worker_path must match schema binding witness")
...
def _validate_authority_scope(scope: Any, errors: list[str]) -> None:
    ...
    if scope.get("live_runtime_receipt_emission_admission_witness_defined") is not True:
        errors.append("authority_scope.live_runtime_receipt_emission_admission_witness_defined must be true")
...
def _validate_activation_contract(
    ...
    if not isinstance(contract, dict):
        errors.append("runtime_receipt_emission_admission_witness_contract must be an object")
        return
    ...
    if contract.get("worker_id") != EXPECTED_WORKER_ID:
        errors.append(
            "runtime_receipt_emission_admission_witness_contract.worker_id must select "
            "worker_local_read_only_repo_inspection"
        )
    if contract.get("worker_id") != activation_contract.get("worker_id"):
        errors.append(
            "runtime_receipt_emission_admission_witness_contract.worker_id must match schema-binding activation witness"
        )
    if contract.get("worker_id") != store_write_path_contract.get("worker_id"):
        errors.append(
            "runtime_receipt_emission_admission_witness_contract.worker_id must match store write-path witness"
        )
    if contract.get("worker_id") != store_activation_contract.get("worker_id"):
        errors.append(
            "runtime_receipt_emission_admission_witness_contract.worker_id must match store activation witness"
        )
    if contract.get("worker_id") != schema_binding_contract.get("worker_id"):
        errors.append(
            "runtime_receipt_emission_admission_witness_contract.worker_id must match schema binding witness"
        )
    if contract.get("capability") != EXPECTED_WORKER_PATH:
        errors.append(
            "runtime_receipt_emission_admission_witness_contract.capability must be read_only_repo_inspection"
        )
    if contract.get("operation_family") != EXPECTED_OPERATION_FAMILY:
        errors.append(
            "runtime_receipt_emission_admission_witness_contract.operation_family must be local_repo_inspection"
        )
    if contract.get("witness_mode") != EXPECTED_WITNESS_MODE:
        errors.append(
            "runtime_receipt_emission_admission_witness_contract.witness_mode must be "
            "LIVE_RUNTIME_RECEIPT_EMISSION_ADMISSION_WITNESS_ONLY"
        )
    if contract.get("runtime_receipt_kind") != EXPECTED_RUNTIME_RECEIPT_KIND:
        errors.append(
            "runtime_receipt_emission_admission_witness_contract.runtime_receipt_kind must be "
    ...
    if contract.get("source_schema_binding_activation_witness_ref") != EXPECTED_SCHEMA_BINDING_ACTIVATION_WITNESS_REF:
        errors.append(
            "runtime_receipt_emission_admission_witness_contract.source_schema_binding_activation_witness_ref "
            "is invalid"
        )
    if contract.get("source_store_write_path_witness_ref") != EXPECTED_STORE_WRITE_PATH_WITNESS_REF:
        errors.append(
            "runtime_receipt_emission_admission_witness_contract.source_store_write_path_witness_ref "
            "is invalid"
        )
    if contract.get("source_store_activation_witness_ref") != EXPECTED_STORE_ACTIVATION_WITNESS_REF:
        errors.append(
            "runtime_receipt_emission_admission_witness_contract.source_store_activation_witness_ref "
            "is invalid"
        )
    if contract.get("source_schema_binding_witness_ref") != EXPECTED_SCHEMA_BINDING_WITNESS_REF:
        errors.append(
            "runtime_receipt_emission_admission_witness_contract.source_schema_binding_witness_ref is invalid"
        )
    if contract.get("target_runtime_receipt_emission_admission_ref") != EXPECTED_TARGET_RECEIPT_EMISSION_ADMISSION_REF:
        errors.append(
            "runtime_receipt_emission_admission_witness_contract."
            "target_runtime_receipt_emission_admission_ref is invalid"
        )
    if contract.get("receipt_emission_admission_profile") != EXPECTED_RECEIPT_EMISSION_ADMISSION_PROFILE:
        errors.append(
            "runtime_receipt_emission_admission_witness_contract.receipt_emission_admission_profile is invalid"
        )
    ...
    if not isinstance(obligations, list) or len(obligations) < 8:
        errors.append("activation_obligations_checked must list future receipt emission admission obligations")
    else:
...
def _validate_admission_decision(decision: Any, errors: list[str]) -> None:
    ...
    if decision.get("runtime_receipt_emission_admission_witness_defined") is not True:
        errors.append("admission_decision.runtime_receipt_emission_admission_witness_defined must be true")
    for field_name in (
...
def _validate_contract_summary(record: dict[str, Any], errors: list[str]) -> None:
    contract = record.get("runtime_receipt_emission_admission_witness_contract")
    ...
    if not all(isinstance(value, dict) for value in (contract, evaluation, decision, refs, summary)):
        errors.append("receipt emission admission contract, evaluation, decision, refs, and summary must be objects")
        return
    expected_counts = {
...
def main(argv: list[str] | None = None) -> int:
    """Validate ReadOnlyWorkerRuntimeReceiptEmissionAdmissionWitness artifacts from the CLI."""

    parser = argparse.ArgumentParser(
        description="Validate ReadOnlyWorkerRuntimeReceiptEmissionAdmissionWitness contract."
    )
    ...
    errors = validate_runtime_receipt_emission_admission_witness(
        args.schema,
        args.receipt,
    ...
    if args.json:
        print(
            json.dumps(
                {
                    "receipt_id": "read_only_worker_runtime_receipt_emission_admission_witness_validation",
        ...
        print("[PASS] read_only_worker_runtime_receipt_emission_admission_witness")
