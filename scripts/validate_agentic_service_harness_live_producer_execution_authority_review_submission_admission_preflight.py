#!/usr/bin/env python3
"""Validate live producer execution authority review submission admission preflight.

Purpose: define the no-effect admission preflight for future review submission of
redacted live authority digest evidence.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: review submission admission preflight schema, fixture, source review intake
digest packet validator, and scripts.validate_schemas.
...
DEFAULT_SCHEMA = (
    REPO_ROOT
    / "schemas"
    / "agentic_service_harness_live_producer_execution_authority_review_submission_admission_preflight.schema.json"
)
DEFAULT_FIXTURE = (
    REPO_ROOT
    / "examples"
    / "agentic_service_harness_live_producer_execution_authority_review_submission_admission_preflight.awaiting_evidence.json"
)
PREFLIGHT_ID = "agentic-service-harness-live-producer-execution-authority-review-submission-admission-preflight"
PREFLIGHT_STATUS = "blocked_awaiting_review_submission_admission_evidence"
...
class LiveProducerExecutionAuthorityReviewSubmissionAdmissionValidation:
    """Validation result for the review submission admission gate."""
...
def validate_live_producer_execution_authority_review_submission_admission_preflight(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    fixture_path: Path = DEFAULT_FIXTURE,
) -> tuple[LiveProducerExecutionAuthorityReviewSubmissionAdmissionValidation, dict[str, Any]]:
    """Validate the checked-in review submission admission gate."""
    errors: list[str] = []
    schema = _load_json_object(schema_path, "review submission admission preflight schema", errors)
    fixture = _load_json_object(fixture_path, "review submission admission preflight fixture", errors)
    packet_validation, _packet = validate_live_producer_execution_authority_review_intake_digest_packet()
    errors.extend(f"source review intake digest packet: {error}" for error in packet_validation.errors)
    if packet_validation.packet_status != PACKET_STATUS:
        errors.append("source review intake digest packet must remain blocked AwaitingEvidence")
    ...
    requirements = fixture.get("submission_admission_requirements")
    missing_requirements = fixture.get("missing_submission_admission_requirements")
    validation = LiveProducerExecutionAuthorityReviewSubmissionAdmissionValidation(
        ok=not errors,
        errors=tuple(errors),
...
def _validate_admission_semantics(admission: Mapping[str, Any], errors: list[str], label: str) -> None:
    for field_name, expected in (
        ...
        ("source_review_intake_digest_packet_status", PACKET_STATUS),
        ("planning_only", True),
        ("read_only", True),
        ("submission_admission_preflight_ready", True),
    ...
    _validate_requirement_collection(admission.get("submission_admission_requirements"), True, errors, label)
    _validate_requirement_collection(admission.get("missing_submission_admission_requirements"), False, errors, label)
...
def _validate_source_binding(admission: Mapping[str, Any], errors: list[str], label: str) -> None:
    binding = _mapping(admission.get("source_review_intake_digest_packet_binding"))
    expected = {
        "source_packet_id": "agentic-service-harness-live-producer-execution-authority-review-intake-digest-packet",
        "source_packet_ref": (
            "examples/agentic_service_harness_live_producer_execution_authority_review_intake_digest_packet.awaiting_evidence.json"
        ),
    ...
    for field_name, expected_value in expected.items():
        if binding.get(field_name) != expected_value:
            errors.append(f"{label}: source_review_intake_digest_packet_binding.{field_name} must be {expected_value!r}")
...
def _validate_submission_policy(policy: Mapping[str, Any], errors: list[str], label: str) -> None:
    expected = {
        "policy_id": "agentic-service-harness-live-producer-review-submission-admission-policy",
...
def main(argv: list[str] | None = None) -> int:
    """Run the live producer authority review submission admission validator."""
    ...
    validation, _fixture = validate_live_producer_execution_authority_review_submission_admission_preflight(
    ...
    if args.json:
        ...
        print("AGENTIC SERVICE HARNESS LIVE PRODUCER EXECUTION AUTHORITY REVIEW SUBMISSION ADMISSION PREFLIGHT VALID")
    else:
