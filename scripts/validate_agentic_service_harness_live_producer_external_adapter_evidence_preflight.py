#!/usr/bin/env python3
"""Validate Agentic Service Harness live producer external adapter evidence preflight.

Purpose: make the external adapter evidence requirement explicit without
integrating an adapter, serializing credentials, opening network egress, or
granting live producer authority.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: schemas/agentic_service_harness_live_producer_external_adapter_evidence_preflight.schema.json,
examples/agentic_service_harness_live_producer_external_adapter_evidence_preflight.local.json,
and scripts.validate_agentic_service_harness_live_producer_effect_receipt_preflight.
Invariants:
  - External adapter evidence remains AwaitingEvidence.
  - Adapter integration, credential serialization, runtime writes, and egress remain denied.
...
DEFAULT_SCHEMA = (
    REPO_ROOT
    / "schemas"
    / "agentic_service_harness_live_producer_external_adapter_evidence_preflight.schema.json"
)
DEFAULT_FIXTURE = (
    REPO_ROOT
    / "examples"
    / "agentic_service_harness_live_producer_external_adapter_evidence_preflight.local.json"
)
PREFLIGHT_ID = "agentic-service-harness-live-producer-external-adapter-evidence-preflight"
...
FORBIDDEN_LIVE_CLAIM = re.compile(
    ...
    r"live_producer_implemented|live_effect_observed|effect_receipt_collected)=true\b",
    re.IGNORECASE,
)


@dataclass(frozen=True, slots=True)
class LiveProducerExternalAdapterEvidencePreflightValidation:
    """Validation result for the live producer external adapter evidence preflight."""
    ...
    external_adapter_evidence_status: str
...
def validate_live_producer_external_adapter_evidence_preflight(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    fixture_path: Path = DEFAULT_FIXTURE,
) -> LiveProducerExternalAdapterEvidencePreflightValidation:
    """Validate the checked-in live producer external adapter evidence preflight."""
    errors: list[str] = []
    schema = _load_json_object(schema_path, "external adapter evidence preflight schema", errors)
    fixture = _load_json_object(fixture_path, "external adapter evidence preflight fixture", errors)
    source_validation = validate_live_producer_effect_receipt_preflight()
    errors.extend(f"source effect receipt preflight: {error}" for error in source_validation.errors)
    ...
    if schema and fixture:
        ...
        _validate_preflight_semantics(fixture, source_validation, errors, _path_label(fixture_path))

    missing_evidence = fixture.get("missing_evidence")
    remaining_witnesses = fixture.get("remaining_witnesses")
    return LiveProducerExternalAdapterEvidencePreflightValidation(
        ok=not errors,
        errors=tuple(errors),
        ...
        external_adapter_evidence_status=str(fixture.get("external_adapter_evidence_status", "")),
...
def _validate_preflight_semantics(
    ...
    if source_validation.ok is not True:
        errors.append(f"{label}: source effect receipt preflight must validate")
    if source_validation.effect_receipt_status != "AwaitingEvidence":
        errors.append(f"{label}: source effect receipt must remain AwaitingEvidence")
    for field_name, expected_value in (
        ("solver_outcome", "AwaitingEvidence"),
        ("source_effect_receipt_status", "AwaitingEvidence"),
        ("operator_approval_status", "Satisfied"),
        ("target_witness_kind", "external_adapter_evidence"),
        ("external_adapter_evidence_status", "AwaitingEvidence"),
        ("preflight_decision", "blocked_until_external_adapter_evidence_exists"),
        ("planning_only", True),
        ("read_only", True),
    ...
    _validate_required_external_adapter_evidence(preflight, errors, label)
    ...
    _validate_credential_surface(preflight, errors, label)
...
def _validate_required_external_adapter_evidence(
    preflight: Mapping[str, Any],
    errors: list[str],
    label: str,
) -> None:
    evidence = _mapping(preflight.get("required_external_adapter_evidence"))
    if not evidence:
        errors.append(f"{label}: required_external_adapter_evidence must be an object")
        return
    for field_name in (
        "evidence_ref",
        "required_provider_identity_ref",
        "required_adapter_descriptor_ref",
        "required_capability_scope_ref",
        "required_egress_policy_ref",
        "required_redaction_ref",
        "required_effect_receipt_ref",
    ):
        if not evidence.get(field_name):
            errors.append(f"{label}: required_external_adapter_evidence.{field_name} required")
    if evidence.get("status") != "AwaitingEvidence":
        errors.append(f"{label}: required external adapter evidence status must be AwaitingEvidence")
    if evidence.get("blocks_live_producer") is not True:
        errors.append(f"{label}: required external adapter evidence must block live producer")
    if evidence.get("adapter_credentials_present") is not False:
        errors.append(f"{label}: required external adapter evidence adapter_credentials_present must be false")
    if evidence.get("adapter_credentials_serialized") is not False:
        errors.append(f"{label}: required external adapter evidence adapter_credentials_serialized must be false")
    if evidence.get("authority_granted") is not False:
        errors.append(f"{label}: required external adapter evidence authority_granted must be false")
...
def main(argv: list[str] | None = None) -> int:
    """Run the live producer external adapter evidence preflight validator."""
    ...
    if args.json:
        ...
        print("AGENTIC SERVICE HARNESS LIVE PRODUCER EXTERNAL ADAPTER EVIDENCE PREFLIGHT VALID")
    else:
        print(
            "AGENTIC SERVICE HARNESS LIVE PRODUCER EXTERNAL ADAPTER EVIDENCE PREFLIGHT INVALID "
