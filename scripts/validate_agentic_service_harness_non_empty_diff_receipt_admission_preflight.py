#!/usr/bin/env python3
"""Validate Agentic Service Harness non-empty diff receipt admission preflight.

Purpose: prove future non-empty diff receipts remain blocked until branch/workspace
authority, cleanup, redaction, UAO admission, and receipt-store evidence exist.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: schemas/agentic_service_harness_non_empty_diff_receipt_admission_preflight.schema.json,
...
Invariants:
  - The preflight binds to the zero-diff actual diff collection receipt.
  - Non-empty changed-file refs and diff refs remain empty while authority is absent.
...
DEFAULT_SCHEMA = (
    REPO_ROOT
    / "schemas"
    / "agentic_service_harness_non_empty_diff_receipt_admission_preflight.schema.json"
)
DEFAULT_EXAMPLES = (
    REPO_ROOT
    / "examples"
    / "agentic_service_harness_non_empty_diff_receipt_admission_preflight.foundation.json",
)
DEFAULT_OUTPUT = (
    REPO_ROOT
    / ".change_assurance"
    / "agentic_service_harness_non_empty_diff_receipt_admission_preflight_validation.json"
)
EXPECTED_REPORT_ID = "agentic_service_harness_non_empty_diff_receipt_admission_preflight"
...
EXPECTED_UAO_REF = "evidence://uao-non-empty-diff-receipt-admission"
...
REQUIRED_BEFORE_REFS = (
    ...
    "evidence://diff-path-allowlist-reviewed",
...
REQUIRED_BLOCKERS = (
    ...
    "blocked://uao-non-empty-diff-receipt/not-verified",
...
REQUIRED_RECEIPT_REFS = {
    ...
    "uao_non_empty_diff_receipt_admission_ref": EXPECTED_UAO_REF,
}
REQUIRED_FALSE_FLAGS = (
    "non_empty_diff_receipt_admitted",
    ...
    "uao_non_empty_diff_receipt_admission_verified",
...
class NonEmptyDiffReceiptAdmissionPreflightValidation:
    """Schema and semantic validation report for non-empty diff receipt admission."""
...
def validate_agentic_service_harness_non_empty_diff_receipt_admission_preflight(
    *,
...
) -> NonEmptyDiffReceiptAdmissionPreflightValidation:
    """Validate non-empty diff receipt admission preflight examples."""
    errors: list[str] = []
    schema = _load_json_object(schema_path, "non-empty diff receipt admission schema", errors)
    ...
    for example_path in example_paths:
        example = _load_json_object(
            example_path,
            f"non-empty diff receipt admission example {_path_label(example_path)}",
            errors,
        )
        if not example:
            continue
        ...
        _validate_preflight_semantics(example, zero_diff_receipt, errors, _path_label(example_path))
    return NonEmptyDiffReceiptAdmissionPreflightValidation(
        ok=not errors,
        errors=tuple(errors),
...
def write_non_empty_diff_receipt_admission_preflight_validation(
    validation: NonEmptyDiffReceiptAdmissionPreflightValidation,
    output_path: Path,
) -> Path:
    """Write one deterministic non-empty diff receipt admission validation report."""
...
def build_mutated_preflight(**updates: Any) -> dict[str, Any]:
    ...
    payload = _load_json_object(DEFAULT_EXAMPLES[0], "default non-empty diff receipt admission example", [])
...
def _validate_preflight_semantics(
    ...
    _validate_scope(preflight, zero_diff_receipt, errors, label)
    _validate_source_receipts(preflight, zero_diff_receipt, errors, label)
    _validate_admission_gates(preflight, errors, label)
    _validate_non_empty_diff_contract(preflight, errors, label)
...
def _validate_scope(
    ...
    zero_scope = _mapping(zero_diff_receipt.get("scope"))
    for field in ("project_id", "run_id", "sandbox_id", "repository_connection_id", "repository_slug", "mode"):
    ...
    _check_value(scope, ("foundation_phase",), "foundation_non_empty_diff_receipt_admission_preflight", errors, label)
...
def _validate_source_receipts(
    ...
    _check_value(source_receipts, ("zero_diff_receipt_ref",), EXPECTED_ZERO_DIFF_RECEIPT_REF, errors, label)
    ...
    if source_receipts.get("zero_diff_changed_file_count") != 0:
        errors.append(f"{label}: source_receipts.zero_diff_changed_file_count must be 0")
    if source_receipts.get("zero_diff_refs") != []:
        errors.append(f"{label}: source_receipts.zero_diff_refs must be empty")
...
def _validate_admission_gates(preflight: Mapping[str, Any], errors: list[str], label: str) -> None:
    ...
    _require_all_refs(
        gates.get("required_before_non_empty_diff_receipt_refs"),
        REQUIRED_BEFORE_REFS,
        "admission_gates.required_before_non_empty_diff_receipt_refs",
        errors,
        label,
    )
    _require_all_refs(
...
def _validate_non_empty_diff_contract(preflight: Mapping[str, Any], errors: list[str], label: str) -> None:
    contract = _mapping(preflight.get("non_empty_diff_receipt_contract"))
    if contract.get("candidate_changed_file_count") != 0:
        errors.append(f"{label}: non_empty_diff_receipt_contract.candidate_changed_file_count must be 0")
    if contract.get("changed_file_refs") != []:
        errors.append(f"{label}: non_empty_diff_receipt_contract.changed_file_refs must be empty")
    if contract.get("diff_refs") != []:
        errors.append(f"{label}: non_empty_diff_receipt_contract.diff_refs must be empty")
    if contract.get("redacted_diff_bundle_ref") != "diff-bundle://not-admitted":
        errors.append(f"{label}: non_empty_diff_receipt_contract.redacted_diff_bundle_ref must remain not-admitted")
    if contract.get("receipt_append_ref") != "blocked://receipt-store-write-path/not-verified":
        errors.append(f"{label}: non_empty_diff_receipt_contract.receipt_append_ref must remain blocked")
...
def _validate_next_action(preflight: Mapping[str, Any], errors: list[str], label: str) -> None:
    ...
    if not isinstance(next_action, str):
        ...
        return
    required_phrases = ("GitHub PR admission preflight", "non-empty diff receipt", "terminal closure blocked")
...
def main(argv: list[str] | None = None) -> int:
    """Run non-empty diff receipt admission preflight validation."""
    args = build_arg_parser().parse_args(argv)
    examples = tuple(args.examples) if args.examples else DEFAULT_EXAMPLES
    validation = validate_agentic_service_harness_non_empty_diff_receipt_admission_preflight(
        schema_path=args.schema,
        example_paths=examples,
    )
    write_non_empty_diff_receipt_admission_preflight_validation(validation, Path(args.output))
    if args.json:
        print(json.dumps(validation.as_dict(), indent=2, sort_keys=True))
    elif validation.ok:
        print("AGENTIC SERVICE HARNESS NON-EMPTY DIFF RECEIPT ADMISSION PREFLIGHT VALID")
    else:
        print(
            "AGENTIC SERVICE HARNESS NON-EMPTY DIFF RECEIPT ADMISSION PREFLIGHT INVALID "
