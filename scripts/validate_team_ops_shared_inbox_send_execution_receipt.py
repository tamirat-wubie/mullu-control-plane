#!/usr/bin/env python3
"""Validate TeamOps shared inbox send execution receipts.

Purpose: reject malformed, raw-content-bearing, locally effect-bearing,
unapproved, unprepared, or unevidenced TeamOps send-execution receipts.
Governance scope: TeamOps external-send evidence admission, redacted evidence,
and local provider-effect rejection.
...
class TeamOpsSharedInboxSendExecutionReceiptValidation:
    """Validation result for one TeamOps send-execution receipt."""
...
def validate_team_ops_shared_inbox_send_execution_receipt(
    *,
    receipt_path: Path = DEFAULT_OUTPUT,
    schema_path: Path = DEFAULT_SCHEMA,
    require_ready: bool = False,
) -> TeamOpsSharedInboxSendExecutionReceiptValidation:
    """Validate one TeamOps shared inbox send-execution receipt."""
    ...
    try:
        ...
        schema = {}
        errors.append("TeamOps send execution receipt schema file missing")
    receipt = _load_json_object(receipt_path, "TeamOps send execution receipt", errors)
    if schema and receipt:
        ...
        if require_ready and not _receipt_ready(receipt):
            errors.append("TeamOps send execution receipt ready must be true")
    ready = not errors and _receipt_ready(receipt)
    return TeamOpsSharedInboxSendExecutionReceiptValidation(
        valid=not errors,
        ready=ready,
        ...
        send_execution_ready=receipt.get("send_execution_ready") is True,
        external_message_sent=receipt.get("external_message_sent") is True,
...
def write_team_ops_shared_inbox_send_execution_receipt_validation(
    validation: TeamOpsSharedInboxSendExecutionReceiptValidation,
    output_path: Path,
) -> Path:
    """Write one TeamOps send-execution validation receipt."""
...
def _validate_semantics(receipt: dict[str, Any], errors: list[str]) -> None:
    ...
    if not RECEIPT_ID_PATTERN.fullmatch(str(receipt.get("receipt_id", ""))):
        errors.append("receipt_id must match TeamOps send execution pattern")
    ...
    if receipt.get("requires_sent_message_observation_receipt") is not True:
        errors.append("requires_sent_message_observation_receipt must be true")
...
def _validate_ready_receipt(receipt: dict[str, Any], errors: list[str]) -> None:
    if receipt.get("send_preparation_receipt_valid") is not True:
    ...
    for field_name in ("approval_request_ref", "approval_decision_ref", "approver_ref"):
    ...
    for field_name in ("send_preparation_ref", "prepared_message_ref", "thread_ref"):
        if not str(receipt.get(field_name, "")).strip():
            errors.append(f"passed receipt requires {field_name}")
    for field_name in ("recipient_hash", "prepared_message_hash", "dispatch_receipt_hash", "provider_message_hash"):
    ...
    if receipt.get("send_execution_state") != "sent":
        errors.append("passed receipt requires send_execution_state=sent")
    if receipt.get("send_execution_ready") is not True:
        errors.append("passed receipt requires send_execution_ready=true")
    for field_name in ("send_execution_ref", "dispatch_receipt_ref", "provider_message_ref"):
        if not str(receipt.get(field_name, "")).strip():
            errors.append(f"passed receipt requires {field_name}")
    if receipt.get("send_execution_observed") is not True:
        errors.append("passed receipt requires send_execution_observed=true")
    if receipt.get("external_message_sent") is not True:
        errors.append("passed receipt requires external_message_sent=true")
...
def _validate_blocked_receipt(receipt: dict[str, Any], errors: list[str]) -> None:
    ...
    if receipt.get("send_execution_ready") is not False:
        errors.append("blocked receipt must not be send-execution ready")
    if receipt.get("external_message_sent") is not False:
        errors.append("blocked receipt must not claim external_message_sent")
...
def _validate_failed_receipt(receipt: dict[str, Any], errors: list[str]) -> None:
    ...
    if receipt.get("send_execution_ready") is not False:
        errors.append("failed receipt must not be send-execution ready")
    if receipt.get("external_message_sent") is not False:
        errors.append("failed receipt must not claim external_message_sent")
    if not isinstance(receipt.get("blocked_until"), list) or not receipt.get("blocked_until"):
...
def _receipt_ready(receipt: dict[str, Any]) -> bool:
    return (
        ...
        and receipt.get("send_execution_state") == "sent"
        and receipt.get("send_execution_ready") is True
        ...
        and SHA256_HEX_PATTERN.fullmatch(str(receipt.get("dispatch_receipt_hash", ""))) is not None
        and SHA256_HEX_PATTERN.fullmatch(str(receipt.get("provider_message_hash", ""))) is not None
        and receipt.get("send_execution_observed") is True
        and receipt.get("external_message_sent") is True
        ...
        and receipt.get("requires_sent_message_observation_receipt") is True
...
def _next_action(receipt: dict[str, Any]) -> str:
    if _receipt_ready(receipt):
        return "verify TeamOps sent-message observation and close workflow with replay evidence"
    ...
    return "regenerate TeamOps shared inbox send execution receipt"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Parse TeamOps send-execution validation arguments."""

    parser = argparse.ArgumentParser(description="Validate TeamOps shared inbox send execution receipt.")
...
def main(argv: list[str] | None = None) -> int:
    """CLI entry point for TeamOps send-execution validation."""
    ...
    write_team_ops_shared_inbox_send_execution_receipt_validation(validation, Path(args.output))
    if args.json:
        print(json.dumps(validation.as_dict(), indent=2, sort_keys=True))
    elif validation.valid:
        print(f"TeamOps shared inbox send execution receipt valid ready={validation.ready}")
    else:
        print(f"TeamOps shared inbox send execution receipt invalid errors={list(validation.errors)}")
