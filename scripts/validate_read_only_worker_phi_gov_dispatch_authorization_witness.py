#!/usr/bin/env python3
"""Validate the ReadOnlyWorkerPhiGovDispatchAuthorizationWitness contract.

Purpose: verify Foundation Mode witness evidence for future Phi_gov dispatch
authorization before read_only_repo_inspection runtime dispatch without
...
ReadOnlyWorkerUaoDispatchAuthorizationWitness validation, and
...
Invariants:
  ...
  - Future dispatch still requires Phi_gov model freeze, episode snapshot,
    governance authority, UAO dispatch authorization evidence, no-bypass proof,
    constraint satisfaction, LifeMeaningJudgment, active runtime lease admission,
    trusted runtime clock, WorkerFailureReceipt obligations, and effect
    reconciliation.
...
from scripts.validate_read_only_worker_uao_dispatch_authorization_witness import (  # noqa: E402
    DEFAULT_RECEIPT_PATH as DEFAULT_UAO_DISPATCH_AUTHORIZATION_WITNESS_PATH,
    DEFAULT_SCHEMA_PATH as DEFAULT_UAO_DISPATCH_AUTHORIZATION_WITNESS_SCHEMA_PATH,
    load_json_object as load_uao_dispatch_authorization_json_object,
...
DEFAULT_SCHEMA_PATH = (
    WORKSPACE_ROOT
    / "schemas"
    / "read_only_worker_phi_gov_dispatch_authorization_witness.schema.json"
)
DEFAULT_RECEIPT_PATH = (
    WORKSPACE_ROOT
    / "examples"
    / "read_only_worker_phi_gov_dispatch_authorization_witness.foundation.json"
)
EXPECTED_SCHEMA_ID = "urn:mullusi:schema:read-only-worker-phi-gov-dispatch-authorization-witness:1"
EXPECTED_SCHEMA_TITLE = "Read-Only Worker Phi_gov Dispatch Authorization Witness"
EXPECTED_RECEIPT_VERSION = "read_only_worker_phi_gov_dispatch_authorization_witness.v1"
EXPECTED_UAO_DISPATCH_AUTHORIZATION_WITNESS_REF = (
...
EXPECTED_WITNESS_MODE = "PHI_GOV_DISPATCH_AUTHORIZATION_WITNESS_ONLY"
EXPECTED_TARGET_PHI_GOV_DISPATCH_AUTHORIZATION_REF = (
    "candidate://phi-gov/dispatch-authorization/read-only-repo-inspection"
)
EXPECTED_PHI_GOV_AUTHORIZATION_PROFILE = (
    "LOCAL_READ_ONLY_REPO_INSPECTION_PHI_GOV_DISPATCH_AUTHORIZATION"
)
EXPECTED_ADMISSION_DECISION = (
    "PHI_GOV_DISPATCH_AUTHORIZATION_WITNESS_BLOCKED_AWAITING_AUTHORIZATION_EVIDENCE"
)
...
REQUIRED_AUTHORIZATION_INPUT_REFS = (
    ...
    "evidence://phi-gov/episode-state-snapshot",
    "evidence://phi-gov/governance-authority",
    "evidence://phi-gov/uao-dispatch-authorization",
    "evidence://phi-gov/no-bypass/dispatch",
    "evidence://phi-gov/constraint-satisfaction",
    "evidence://phi-gov/life-meaning-judgment",
    "evidence://phi-gov/active-runtime-lease-admission",
    "evidence://phi-gov/runtime-dispatch-admission-boundary",
    "evidence://phi-gov/worker-failure-receipt-on-error",
    "evidence://phi-gov/effect-reconciliation-required",
    "evidence://runtime-clock/trusted-now",
)
REQUIRED_VALIDATION_REFS = (
    "scripts/validate_read_only_worker_phi_gov_dispatch_authorization_witness.py",
    ...
    "tests/test_validate_read_only_worker_phi_gov_dispatch_authorization_witness.py",
)
...
REQUIRED_BLOCKED_REASON_REFS = (
    "blocked://phi-gov/model-freeze-missing",
    "blocked://phi-gov/episode-state-snapshot-missing",
    "blocked://phi-gov/governance-authority-missing",
    "blocked://phi-gov/uao-dispatch-authorization-missing",
    "blocked://phi-gov/no-bypass-not-proven",
    "blocked://phi-gov/constraint-satisfaction-missing",
    "blocked://phi-gov/life-meaning-judgment-missing",
    "blocked://phi-gov/active-lease-admission-missing",
    "blocked://phi-gov/runtime-dispatch-admission-boundary-missing",
    "blocked://runtime-clock/trusted-now-missing",
    "blocked://effect-reconciliation/not-proven",
)
REQUIRED_RECEIPT_REFS = {
    "read_only_worker_phi_gov_dispatch_authorization_witness_schema": (
...
TRUE_ACTIVATION_FIELDS = (
    "upstream_uao_dispatch_authorization_witness_validated",
    "upstream_active_runtime_lease_admission_witness_validated",
    "phi_gov_model_freeze_required",
    "episode_state_snapshot_required",
    "governance_authority_required",
    "uao_dispatch_authorization_required",
    "no_bypass_rule_required",
    "constraint_satisfaction_required",
...
class ReadOnlyWorkerPhiGovDispatchAuthorizationWitnessError(ValueError):
...
def load_json_object(json_path: Path, label: str) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ReadOnlyWorkerPhiGovDispatchAuthorizationWitnessError(
...
def validate_schema_artifact(schema: dict[str, Any]) -> list[str]:
    ...
    if isinstance(required_fields, list) and isinstance(properties, dict):
        for field_name in (
            "receipt_id",
            "receipt_version",
            "uao_dispatch_authorization_witness_ref",
            "active_runtime_lease_admission_witness_ref",
            "authority_scope",
            "phi_gov_dispatch_authorization_contract",
...
def validate_phi_gov_dispatch_authorization_witness_record(
    record: Any,
    schema: dict[str, Any] | None = None,
    uao_dispatch_authorization_witness: dict[str, Any] | None = None,
    ...
    if not isinstance(record, dict):
        errors.append("ReadOnlyWorkerPhiGovDispatchAuthorizationWitness must be object")
        return errors

    errors.extend(_validate_schema_instance(schema_payload, record))

    uao_witness_payload = uao_dispatch_authorization_witness
    if uao_witness_payload is None:
        uao_witness_payload = load_uao_dispatch_authorization_json_object(
            DEFAULT_UAO_DISPATCH_AUTHORIZATION_WITNESS_PATH,
            "ReadOnlyWorkerUaoDispatchAuthorizationWitness",
        )
    uao_witness_schema = _load_schema(DEFAULT_UAO_DISPATCH_AUTHORIZATION_WITNESS_SCHEMA_PATH)
    errors.extend(
        f"uao_dispatch_authorization_witness: {error}"
        for error in validate_uao_dispatch_authorization_witness_record(
            uao_witness_payload,
            uao_witness_schema,
        )
    )
    ...
    if record.get("receipt_version") != EXPECTED_RECEIPT_VERSION:
        errors.append(
            "receipt_version must match read_only_worker_phi_gov_dispatch_authorization_witness.v1"
        )
    if record.get("uao_dispatch_authorization_witness_ref") != (
        EXPECTED_UAO_DISPATCH_AUTHORIZATION_WITNESS_REF
    ):
        errors.append("uao_dispatch_authorization_witness_ref is invalid")
    ...
    if not isinstance(authority, dict):
        ...
        if authority.get("phi_gov_dispatch_authorization_witness_defined") is not True:
            errors.append(
                "authority_scope.phi_gov_dispatch_authorization_witness_defined must be true"
            )
    ...
    contract = record.get("phi_gov_dispatch_authorization_contract")
    if not isinstance(contract, dict):
        errors.append("phi_gov_dispatch_authorization_contract must be an object")
    else:
        if contract.get("worker_id") != EXPECTED_WORKER_ID:
            errors.append("phi_gov_dispatch_authorization_contract.worker_id is invalid")
        if contract.get("capability") != EXPECTED_WORKER_PATH:
            errors.append("phi_gov_dispatch_authorization_contract.capability is invalid")
        if contract.get("operation_family") != EXPECTED_OPERATION_FAMILY:
            errors.append("phi_gov_dispatch_authorization_contract.operation_family is invalid")
        if contract.get("witness_mode") != EXPECTED_WITNESS_MODE:
            errors.append(
                "phi_gov_dispatch_authorization_contract.witness_mode must be "
                "PHI_GOV_DISPATCH_AUTHORIZATION_WITNESS_ONLY"
            )
        if contract.get("source_uao_dispatch_authorization_witness_ref") != (
            EXPECTED_UAO_DISPATCH_AUTHORIZATION_WITNESS_REF
        ):
            errors.append(
                "phi_gov_dispatch_authorization_contract."
        ...
        if contract.get("target_phi_gov_dispatch_authorization_ref") != (
            EXPECTED_TARGET_PHI_GOV_DISPATCH_AUTHORIZATION_REF
        ):
            errors.append(
                "phi_gov_dispatch_authorization_contract."
                "target_phi_gov_dispatch_authorization_ref is invalid"
            )
        if contract.get("phi_gov_authorization_profile") != EXPECTED_PHI_GOV_AUTHORIZATION_PROFILE:
            errors.append(
                "phi_gov_dispatch_authorization_contract.phi_gov_authorization_profile is invalid"
            )
        _require_refs(
            contract.get("required_source_receipt_refs"),
            REQUIRED_SOURCE_RECEIPT_REFS,
            "phi_gov_dispatch_authorization_contract.required_source_receipt_refs",
            errors,
        )
        _require_refs(
            contract.get("required_authorization_input_refs"),
            REQUIRED_AUTHORIZATION_INPUT_REFS,
            "phi_gov_dispatch_authorization_contract.required_authorization_input_refs",
            errors,
        )
        _require_refs(
            contract.get("validation_refs"),
            REQUIRED_VALIDATION_REFS,
            "phi_gov_dispatch_authorization_contract.validation_refs",
            errors,
        )
    ...
    if not isinstance(decision, dict):
        ...
        if decision.get("phi_gov_dispatch_authorization_witness_defined") is not True:
            errors.append(
                "admission_decision.phi_gov_dispatch_authorization_witness_defined must be true"
            )
        _require_refs(
...
def validate_phi_gov_dispatch_authorization_witness(
    ...
    receipt = load_json_object(receipt_path, "ReadOnlyWorkerPhiGovDispatchAuthorizationWitness")
    return validate_phi_gov_dispatch_authorization_witness_record(receipt, schema)


def build_mutated_phi_gov_dispatch_authorization_witness(**updates: Any) -> dict[str, Any]:
    """Return a deep-copied fixture with double-underscore path updates."""

    receipt = load_json_object(
        DEFAULT_RECEIPT_PATH,
        "ReadOnlyWorkerPhiGovDispatchAuthorizationWitness",
    )
...
def main(argv: list[str] | None = None) -> int:
    """Validate ReadOnlyWorkerPhiGovDispatchAuthorizationWitness artifacts."""

    parser = argparse.ArgumentParser(
        description="Validate ReadOnlyWorkerPhiGovDispatchAuthorizationWitness contract."
    )
    ...
    errors = validate_phi_gov_dispatch_authorization_witness(args.schema, args.receipt)
    if args.json:
        payload = {
            ...
            "errors": errors,
            "receipt": {
                "receipt_id": "read_only_worker_phi_gov_dispatch_authorization_witness_validation",
        ...
        print("[FAIL] read_only_worker_phi_gov_dispatch_authorization_witness", file=sys.stderr)
        for error in errors:
            print(f"  - {error}", file=sys.stderr)
    else:
        print("[PASS] read_only_worker_phi_gov_dispatch_authorization_witness")
