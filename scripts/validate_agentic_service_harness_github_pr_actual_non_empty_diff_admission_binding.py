#!/usr/bin/env python3
"""Validate Agentic Service Harness GitHub PR actual non-empty diff binding.

Purpose: prove GitHub PR admission consumes the actual non-empty diff receipt
binding before any PR creation path is considered.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: schemas/agentic_service_harness_github_pr_actual_non_empty_diff_admission_binding.schema.json,
...
scripts.validate_agentic_service_harness_actual_non_empty_diff_receipt_binding,
and scripts.validate_schemas.
Invariants:
  - The binding consumes the GitHub PR admission preflight.
  - The binding consumes the actual non-empty diff receipt binding.
  - Redacted changed-file and diff refs match the source actual diff binding.
  - PR creation, branch writes, repository writes, connector calls, mutation
    routes, raw content, receipt append, secrets, and terminal closure fail
    closed.
"""
...
from scripts.validate_agentic_service_harness_actual_non_empty_diff_receipt_binding import (  # noqa: E402
    DEFAULT_EXAMPLES as DEFAULT_ACTUAL_DIFF_BINDING_EXAMPLES,
    DEFAULT_SCHEMA as DEFAULT_ACTUAL_DIFF_BINDING_SCHEMA,
    validate_agentic_service_harness_actual_non_empty_diff_receipt_binding,
)
from scripts.validate_agentic_service_harness_github_pr_admission_preflight import (  # noqa: E402
    DEFAULT_EXAMPLES as DEFAULT_PR_ADMISSION_PREFLIGHT_EXAMPLES,
    DEFAULT_SCHEMA as DEFAULT_PR_ADMISSION_PREFLIGHT_SCHEMA,
...
DEFAULT_OUTPUT = (
    REPO_ROOT
    / ".change_assurance"
    / "agentic_service_harness_github_pr_actual_non_empty_diff_admission_binding_validation.json"
)
EXPECTED_BINDING_ID = "agentic_service_harness_github_pr_actual_non_empty_diff_admission_binding"
EXPECTED_SOURCE_PR_ADMISSION_PREFLIGHT_REF = (
    "examples/agentic_service_harness_github_pr_admission_preflight.foundation.json"
)
EXPECTED_SOURCE_ACTUAL_DIFF_BINDING_REF = (
    "examples/agentic_service_harness_actual_non_empty_diff_receipt_binding.foundation.json"
)
EXPECTED_SOURCE_PREFLIGHT_REPORT_ID = "agentic_service_harness_github_pr_admission_preflight"
EXPECTED_SOURCE_ACTUAL_DIFF_BINDING_ID = "agentic_service_harness_actual_non_empty_diff_receipt_binding"
...
REQUIRED_RECEIPT_REFS = {
    ...
    "github_pr_actual_non_empty_diff_admission_binding_example": (
        "examples/agentic_service_harness_github_pr_actual_non_empty_diff_admission_binding.foundation.json"
    ),
    ...
    "actual_non_empty_diff_receipt_binding_example": EXPECTED_SOURCE_ACTUAL_DIFF_BINDING_REF,
...
class GitHubPrActualNonEmptyDiffAdmissionBindingValidation:
    """Schema and semantic validation report for PR actual diff admission binding."""
    ...
    source_actual_non_empty_diff_receipt_binding_ref: str
...
def validate_agentic_service_harness_github_pr_actual_non_empty_diff_admission_binding(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    example_paths: Sequence[Path] = DEFAULT_EXAMPLES,
    source_pr_admission_schema_path: Path = DEFAULT_PR_ADMISSION_PREFLIGHT_SCHEMA,
    source_pr_admission_example_paths: Sequence[Path] = DEFAULT_PR_ADMISSION_PREFLIGHT_EXAMPLES,
    source_actual_diff_binding_schema_path: Path = DEFAULT_ACTUAL_DIFF_BINDING_SCHEMA,
    source_actual_diff_binding_example_paths: Sequence[Path] = DEFAULT_ACTUAL_DIFF_BINDING_EXAMPLES,
) -> GitHubPrActualNonEmptyDiffAdmissionBindingValidation:
    """Validate GitHub PR actual non-empty diff admission binding examples."""
    errors: list[str] = []
    schema = _load_json_object(schema_path, "GitHub PR actual non-empty diff admission binding schema", errors)
    pr_preflight_validation = validate_agentic_service_harness_github_pr_admission_preflight(
        schema_path=source_pr_admission_schema_path,
        example_paths=source_pr_admission_example_paths,
    )
    if not pr_preflight_validation.ok:
        errors.extend(f"source GitHub PR admission preflight: {error}" for error in pr_preflight_validation.errors)
    actual_diff_validation = validate_agentic_service_harness_actual_non_empty_diff_receipt_binding(
        schema_path=source_actual_diff_binding_schema_path,
        example_paths=source_actual_diff_binding_example_paths,
    )
    if not actual_diff_validation.ok:
        errors.extend(f"source actual non-empty diff binding: {error}" for error in actual_diff_validation.errors)
    source_preflight = _load_json_object(
        source_pr_admission_example_paths[0],
        "GitHub PR admission preflight source",
        errors,
    )
    source_actual_diff_binding = _load_json_object(
        source_actual_diff_binding_example_paths[0],
        "actual non-empty diff receipt binding source",
        errors,
    )
    ...
    for example_path in example_paths:
        example = _load_json_object(
            example_path,
            f"GitHub PR actual non-empty diff admission binding {_path_label(example_path)}",
            errors,
        )
        if not example:
            continue
        ...
        _validate_pr_actual_non_empty_diff_binding_semantics(
            example,
            source_preflight,
    ...
    return GitHubPrActualNonEmptyDiffAdmissionBindingValidation(
        ok=not errors,
        errors=tuple(errors),
        ...
        source_actual_non_empty_diff_receipt_binding_ref=EXPECTED_SOURCE_ACTUAL_DIFF_BINDING_REF,
    )


def write_github_pr_actual_non_empty_diff_admission_binding_validation(
    validation: GitHubPrActualNonEmptyDiffAdmissionBindingValidation,
    output_path: Path,
) -> Path:
    """Write one deterministic PR actual non-empty diff binding validation report."""
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(validation.as_dict(), indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return output_path


def _validate_pr_actual_non_empty_diff_binding_semantics(
    ...
    _require_equal(payload, ("source_pr_admission_preflight_ref",), EXPECTED_SOURCE_PR_ADMISSION_PREFLIGHT_REF, errors, label)
    _require_equal(
        payload,
        ("source_actual_non_empty_diff_receipt_binding_ref",),
        EXPECTED_SOURCE_ACTUAL_DIFF_BINDING_REF,
        errors,
        label,
    )
    ...
    _require_equal(
        payload,
        ("witness_kind",),
        "github_pr_actual_non_empty_diff_admission_binding",
        errors,
        label,
    )
    _require_equal(
        payload,
        ("binding_status",),
        "actual_non_empty_diff_bound_to_pr_admission_without_pr_creation",
        errors,
        label,
    )
    _require_equal(
        payload,
        ("pr_admission_diff_binding", "source_preflight_report_id"),
        EXPECTED_SOURCE_PREFLIGHT_REPORT_ID,
        errors,
        label,
    )
    _require_equal(
        payload,
        ("pr_admission_diff_binding", "source_actual_non_empty_diff_receipt_binding_id"),
        EXPECTED_SOURCE_ACTUAL_DIFF_BINDING_ID,
        errors,
        label,
    )
    _require_equal(
    ...
    _require_equal(
        payload,
        ("pr_admission_diff_binding", "redacted_diff_bundle_ref"),
        EXPECTED_REDACTED_DIFF_BUNDLE_REF,
        errors,
        label,
    )
    _require_equal(
        payload,
        ("pr_admission_diff_binding", "redacted_output_ref"),
        EXPECTED_REDACTED_OUTPUT_REF,
        errors,
        label,
    )
    ...
    if source_preflight:
        ...
        _require_equal(
            source_preflight,
            ("simulated_pr_admission", "pull_request_opened"),
            False,
            errors,
            "GitHub PR admission preflight source",
        )
    ...
    if source_actual_diff_binding:
        _require_equal(
            ...
            EXPECTED_SOURCE_ACTUAL_DIFF_BINDING_ID,
            errors,
            "actual non-empty diff binding source",
        )
        _require_equal(
            source_actual_diff_binding,
            ("actual_non_empty_diff_binding", "actual_non_empty_diff_receipt_ref"),
            EXPECTED_ACTUAL_NON_EMPTY_DIFF_RECEIPT_REF,
            errors,
            "actual non-empty diff binding source",
        )
        ...
        binding = _mapping(source_actual_diff_binding.get("actual_non_empty_diff_binding"))
        _require_equal(
            payload,
            ("pr_admission_diff_binding", "changed_file_count"),
            binding.get("changed_file_count"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("pr_admission_diff_binding", "changed_file_refs"),
            binding.get("changed_file_refs"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("pr_admission_diff_binding", "diff_refs"),
            binding.get("diff_refs"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("pr_admission_diff_binding", "redacted_diff_bundle_ref"),
            binding.get("redacted_diff_bundle_ref"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("pr_admission_diff_binding", "redacted_output_ref"),
            binding.get("redacted_output_ref"),
            errors,
            label,
        )
        changed_file_refs = binding.get("changed_file_refs")
        diff_refs = binding.get("diff_refs")
        if not isinstance(changed_file_refs, list) or not changed_file_refs:
            errors.append("actual non-empty diff binding source: changed_file_refs must be non-empty")
        if not isinstance(diff_refs, list) or not diff_refs:
            errors.append("actual non-empty diff binding source: diff_refs must be non-empty")
    ...
    for path, required_refs in (
        (("pr_admission_diff_binding", "required_before_pr_refs"), REQUIRED_BEFORE_PR_REFS),
        (("pr_admission_diff_binding", "blocked_reason_refs"), REQUIRED_BLOCKERS),
    ):
    ...
    if isinstance(next_action, str):
        for phrase in (
            ...
            "actual non-empty diff admission binding",
            "PR creation",
...
def build_mutated_pr_actual_non_empty_diff_binding(**updates: Any) -> dict[str, Any]:
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--source-pr-admission-schema", type=Path, default=DEFAULT_PR_ADMISSION_PREFLIGHT_SCHEMA)
    parser.add_argument("--source-pr-admission-example", type=Path, action="append", dest="source_pr_admission_examples")
    parser.add_argument("--source-actual-diff-binding-schema", type=Path, default=DEFAULT_ACTUAL_DIFF_BINDING_SCHEMA)
    parser.add_argument(
        "--source-actual-diff-binding-example",
        type=Path,
        action="append",
        dest="source_actual_diff_binding_examples",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    validation = validate_agentic_service_harness_github_pr_actual_non_empty_diff_admission_binding(
        schema_path=args.schema,
        example_paths=tuple(args.examples) if args.examples else DEFAULT_EXAMPLES,
        source_pr_admission_schema_path=args.source_pr_admission_schema,
        source_pr_admission_example_paths=(
            tuple(args.source_pr_admission_examples)
            if args.source_pr_admission_examples
            else DEFAULT_PR_ADMISSION_PREFLIGHT_EXAMPLES
        ),
        source_actual_diff_binding_schema_path=args.source_actual_diff_binding_schema,
        source_actual_diff_binding_example_paths=(
            tuple(args.source_actual_diff_binding_examples)
            if args.source_actual_diff_binding_examples
            else DEFAULT_ACTUAL_DIFF_BINDING_EXAMPLES
        ),
    )
    write_github_pr_actual_non_empty_diff_admission_binding_validation(validation, args.output)
    if args.json:
        print(json.dumps(validation.as_dict(), indent=2, sort_keys=True))
    elif validation.ok:
        print("AGENTIC SERVICE HARNESS GITHUB PR ACTUAL NON-EMPTY DIFF ADMISSION BINDING VALID")
    else:
