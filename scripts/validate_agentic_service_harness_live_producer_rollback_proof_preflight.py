#!/usr/bin/env python3
"""Validate Agentic Service Harness live producer rollback proof preflight.

Purpose: make the rollback proof requirement explicit without executing
rollback, verifying recovery, or granting live producer authority.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: schemas/agentic_service_harness_live_producer_rollback_proof_preflight.schema.json,
examples/agentic_service_harness_live_producer_rollback_proof_preflight.local.json,
and scripts.validate_agentic_service_harness_live_producer_secret_handoff_preflight.
Invariants:
  - Rollback proof remains AwaitingEvidence.
  - Rollback execution, recovery verification, credential access, and live authority remain denied.
  - Mutation routes, credential-like values, live-effect claims, and terminal closure fail closed.
"""
...
DEFAULT_SCHEMA = (
    REPO_ROOT
    / "schemas"
    / "agentic_service_harness_live_producer_rollback_proof_preflight.schema.json"
)
DEFAULT_FIXTURE = (
    REPO_ROOT
    / "examples"
    / "agentic_service_harness_live_producer_rollback_proof_preflight.local.json"
)
PREFLIGHT_ID = "agentic-service-harness-live-producer-rollback-proof-preflight"
...
ALLOWED_SECRET_KEYS = {
    ...
    "source_secret_handoff_preflight_ref",
    "source_secret_handoff_status",
}
...
FORBIDDEN_LIVE_CLAIM = re.compile(
    r"\b(?:rollback_executed|recovery_verified|secret_values_present|secret_values_read|"
    r"secret_values_serialized|secret_values_printed|external_adapter_integrated|"
    r"adapter_credentials_present|adapter_credentials_serialized|live_producer_implemented|"
    r"live_effect_observed|effect_receipt_collected|terminal_closure)=true\b",
    re.IGNORECASE,
)


@dataclass(frozen=True, slots=True)
class LiveProducerRollbackProofPreflightValidation:
    """Validation result for the live producer rollback proof preflight."""
...
def validate_live_producer_rollback_proof_preflight(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    fixture_path: Path = DEFAULT_FIXTURE,
) -> LiveProducerRollbackProofPreflightValidation:
    """Validate the checked-in live producer rollback proof preflight."""
    errors: list[str] = []
    schema = _load_json_object(schema_path, "rollback proof preflight schema", errors)
    fixture = _load_json_object(fixture_path, "rollback proof preflight fixture", errors)
    source_validation = validate_live_producer_secret_handoff_preflight()
    errors.extend(f"source secret handoff preflight: {error}" for error in source_validation.errors)
    ...
    return LiveProducerRollbackProofPreflightValidation(
        ok=not errors,
        errors=tuple(errors),
        ...
        rollback_proof_status=str(fixture.get("rollback_proof_status", "")),
...
def _validate_preflight_semantics(
    ...
    if source_validation.ok is not True:
        errors.append(f"{label}: source secret handoff preflight must validate")
    if source_validation.secret_handoff_status != "AwaitingEvidence":
        errors.append(f"{label}: source secret handoff must remain AwaitingEvidence")
    for field_name, expected_value in (
        ("solver_outcome", "AwaitingEvidence"),
        ("source_secret_handoff_status", "AwaitingEvidence"),
        ("operator_approval_status", "Satisfied"),
        ("target_witness_kind", "rollback_proof"),
        ("rollback_proof_status", "AwaitingEvidence"),
        ("preflight_decision", "blocked_until_rollback_proof_exists"),
        ("planning_only", True),
        ("read_only", True),
    ...
    _validate_required_rollback_proof(preflight, errors, label)
...
def _validate_required_rollback_proof(
    preflight: Mapping[str, Any],
    errors: list[str],
    label: str,
) -> None:
    proof = _mapping(preflight.get("required_rollback_proof"))
    if not proof:
        errors.append(f"{label}: required_rollback_proof must be an object")
        return
    for field_name in (
        "proof_ref",
        "required_rollback_plan_ref",
        "required_state_snapshot_ref",
        "required_inverse_delta_ref",
        "required_replay_receipt_ref",
        "required_recovery_verification_ref",
        "required_secret_handoff_ref",
    ):
        if not proof.get(field_name):
            errors.append(f"{label}: required_rollback_proof.{field_name} required")
    if proof.get("status") != "AwaitingEvidence":
        errors.append(f"{label}: required rollback proof status must be AwaitingEvidence")
    if proof.get("blocks_live_producer") is not True:
        errors.append(f"{label}: required rollback proof must block live producer")
    for flag_name in ("rollback_executed", "recovery_verified", "authority_granted"):
        if proof.get(flag_name) is not False:
            errors.append(f"{label}: required rollback proof {flag_name} must be false")
...
def _validate_no_live_claim(payload: Any, errors: list[str], label: str) -> None:
    for path, value in _walk_strings(payload):
        if FORBIDDEN_LIVE_CLAIM.search(value):
            errors.append(f"{label}: live rollback proof claim at {path}")
...
def main(argv: list[str] | None = None) -> int:
    """Run the live producer rollback proof preflight validator."""
    ...
    if args.json:
        ...
        print("AGENTIC SERVICE HARNESS LIVE PRODUCER ROLLBACK PROOF PREFLIGHT VALID")
    else:
        print(
            "AGENTIC SERVICE HARNESS LIVE PRODUCER ROLLBACK PROOF PREFLIGHT INVALID "
