#!/usr/bin/env python3
"""Validate operator value-binding verifier execution decision-value absence."""
...
DEFAULT_SCHEMA = REPO_ROOT / "schemas" / "personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_absence.schema.json"
DEFAULT_RECEIPT_SCHEMA = REPO_ROOT / "schemas" / "personal_assistant_receipt.schema.json"
RUNTIME_GENERATED_AT = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_ABSENCE_GENERATED_AT
...
class PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierExecutionDecisionValueAbsenceValidation:
    """Validation result for verifier execution decision-value absence."""

    valid: bool
    runtime_validated: bool
    decision_value_absence_count: int
    operator_decision_required_count: int
    operator_decision_value_absent_count: int
...
def validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_absence(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    receipt_schema_path: Path = DEFAULT_RECEIPT_SCHEMA,
) -> PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierExecutionDecisionValueAbsenceValidation:
    """Validate runtime verifier execution decision-value absence."""

    errors: list[str] = []
    schema = _load_json_object(schema_path, "decision-value absence schema", errors)
    ...
    errors.extend(_validate_decision_value_absence_semantics(envelope, receipt_schema))
    _scan_secret_values(envelope, errors, path="$runtime")
    return PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierExecutionDecisionValueAbsenceValidation(
        valid=not errors,
        runtime_validated=not errors,
        decision_value_absence_count=int(envelope.get("decision_value_absence_count", 0)),
        operator_decision_required_count=int(summary.get("operator_decision_required_count", 0)),
        operator_decision_value_absent_count=int(summary.get("operator_decision_value_absent_count", 0)),
...
def _validate_decision_value_absence_semantics(envelope: Mapping[str, Any], receipt_schema: Mapping[str, Any]) -> tuple[str, ...]:
    errors: list[str] = []
    expected_top = {
    ...
    _require_false_fields(
        effect_boundary,
        FALSE_FIELDS | FALSE_AUTHORITY_FIELDS | {"raw_operator_value_present", "deployment_mutation_allowed", "nested_mind_live_activation_allowed", "public_readiness_claim_allowed"},
        "effect_boundary",
        errors,
    )

    records = envelope.get("decision_value_absences")
    if not isinstance(records, list):
        errors.append("decision_value_absences must be a list")
        return tuple(errors)
    if envelope.get("decision_value_absence_count") != len(records):
        errors.append("decision_value_absence_count must equal decision_value_absences length")
    item_ids: list[str] = []
    receipt_ids: list[str] = []
    coverage: dict[str, set[str]] = {}
    for index, record in enumerate(records):
        if not isinstance(record, dict):
            errors.append(f"decision_value_absences[{index}] must be an object")
            continue
        item_ids.append(str(record.get("decision_value_absence_item_id", "")))
        ...
        _require_source_ref(index, _mapping(record.get("decision_intake_preflight_ref")), errors)
        _require_absence(index, _mapping(record.get("decision_value_absence")), errors)
        _require_false_fields(_mapping(record.get("authority_status")), FALSE_AUTHORITY_FIELDS, f"decision_value_absences[{index}].authority_status", errors)
        receipt = _mapping(record.get("receipt"))
        if receipt_schema:
            errors.extend(f"decision_value_absences[{index}].receipt {message}" for message in _validate_schema_instance(dict(receipt_schema), receipt))
        errors.extend(f"decision_value_absences[{index}].receipt {message}" for message in validate_personal_assistant_receipt_payload(receipt))
        metadata = _mapping(receipt.get("metadata"))
        if metadata.get("operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_absence_is_execution") is not False:
            errors.append(f"decision_value_absences[{index}].receipt.metadata execution flag must be false")
        if metadata.get("operator_decision_value_absent") is not True:
            errors.append(f"decision_value_absences[{index}].receipt.metadata operator_decision_value_absent must be true")
        _require_false_fields(metadata, FALSE_FIELDS | FALSE_AUTHORITY_FIELDS | {"external_write_allowed"}, f"decision_value_absences[{index}].receipt.metadata", errors)
        receipt_ids.append(str(receipt.get("receipt_id", "")))
    if set(coverage) != EXPECTED_EVIDENCE_KINDS:
        errors.append("decision_value_absences must cover all governed evidence kinds")
    for evidence_kind, requirement_kinds in coverage.items():
        if requirement_kinds != EXPECTED_REQUIREMENT_KINDS:
            errors.append(f"decision_value_absences for {evidence_kind} must cover all verifier requirement kinds")
    if envelope.get("decision_value_absence_item_ids") != item_ids:
        errors.append("decision_value_absence_item_ids must match item order")
...
def _require_source_ref(index: int, payload: Mapping[str, Any], errors: list[str]) -> None:
    ...
    for field_name, expected_value in expected.items():
        if payload.get(field_name) != expected_value:
            errors.append(f"decision_value_absences[{index}].decision_intake_preflight_ref.{field_name} must be {expected_value}")


def _require_absence(index: int, payload: Mapping[str, Any], errors: list[str]) -> None:
    for field_name in ("operator_decision_required", "operator_decision_value_required", "operator_decision_value_absent"):
        if payload.get(field_name) is not True:
            errors.append(f"decision_value_absences[{index}].decision_value_absence.{field_name} must be true")
    _require_false_fields(payload, FALSE_FIELDS, f"decision_value_absences[{index}].decision_value_absence", errors)
    if payload.get("blocking_reason") != "operator_decision_value_absent":
        errors.append(f"decision_value_absences[{index}].decision_value_absence blocking_reason must remain operator_decision_value_absent")
...
def _require_summary(envelope: Mapping[str, Any], records: list[Any], errors: list[str]) -> None:
    ...
    expected = {
        ...
        "operator_decision_value_absent_count": len(records),
...
def main() -> int:
    ...
    result = validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_absence(
    ...
    if args.json:
        ...
        print("PERSONAL ASSISTANT OPERATOR REAPPROVAL DECISION RECEIPT VALUE BINDING RECORD VERIFIER EXECUTION DECISION VALUE ABSENCE VALID")
    else:
