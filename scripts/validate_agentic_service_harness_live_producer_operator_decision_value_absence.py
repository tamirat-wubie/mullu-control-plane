#!/usr/bin/env python3
"""Validate Agentic Service Harness operator decision value absence.

Purpose: prove no explicit operator approval or rejection value has been
provided and no live producer authority is granted.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: gateway.agentic_service_harness_live_producer_operator_decision_value_absence,
schemas/agentic_service_harness_live_producer_operator_decision_value_absence.schema.json,
examples/agentic_service_harness_live_producer_operator_decision_value_absence.local.json,
and scripts.validate_agentic_service_harness_live_producer_operator_decision_record.
Invariants:
  - Decision value absence remains `AwaitingEvidence`.
...
DEFAULT_SCHEMA = (
    REPO_ROOT / "schemas" / "agentic_service_harness_live_producer_operator_decision_value_absence.schema.json"
)
DEFAULT_FIXTURE = (
    REPO_ROOT / "examples" / "agentic_service_harness_live_producer_operator_decision_value_absence.local.json"
)
...
class LiveProducerOperatorDecisionValueAbsenceValidation:
    """Validation result for operator decision value absence."""

    ok: bool
    errors: tuple[str, ...]
    schema_path: str
    fixture_path: str
    absence_boundary_id: str
    absence_status: str
...
def validate_live_producer_operator_decision_value_absence(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    fixture_path: Path = DEFAULT_FIXTURE,
) -> tuple[LiveProducerOperatorDecisionValueAbsenceValidation, dict[str, Any]]:
    """Validate the checked-in decision value absence witness and produced packet."""
    errors: list[str] = []
    schema = _load_json_object(schema_path, "operator decision value absence schema", errors)
    fixture = _load_json_object(fixture_path, "operator decision value absence fixture", errors)
    record_validation, decision_record = validate_live_producer_operator_decision_record()
    errors.extend(f"source operator decision record: {error}" for error in record_validation.errors)

    produced_absence: dict[str, Any] = {}
    if decision_record:
    ...
    if schema and fixture:
        ...
        _validate_absence_semantics(fixture, errors, _path_label(fixture_path))
    if schema and produced_absence:
        errors.extend(
            f"produced operator decision value absence: {error}"
            for error in _validate_schema_instance(schema, produced_absence)
        )
        _validate_absence_semantics(produced_absence, errors, "produced operator decision value absence")
    if fixture and produced_absence:
        _validate_fixture_matches_produced(fixture, produced_absence, errors)

    observed = produced_absence or fixture
    missing_values = observed.get("missing_value_requirements")
    validation = LiveProducerOperatorDecisionValueAbsenceValidation(
        ok=not errors,
        errors=tuple(errors),
        schema_path=_path_label(schema_path),
        fixture_path=_path_label(fixture_path),
        absence_boundary_id=str(observed.get("absence_boundary_id", "")),
        absence_status=str(observed.get("absence_status", "")),
        missing_value_count=len(missing_values) if isinstance(missing_values, list) else 0,
        authority_denial_count=len(FALSE_AUTHORITY_FLAGS) + 1,
    )
    return validation, produced_absence


def _validate_absence_semantics(absence: Mapping[str, Any], errors: list[str], label: str) -> None:
    if absence.get("absence_boundary_id") != OPERATOR_DECISION_VALUE_ABSENCE_ID:
        errors.append(f"{label}: absence_boundary_id mismatch")
    for field_name, expected_value in (
        ...
        if absence.get(field_name) != expected_value:
            errors.append(f"{label}: {field_name} must be {expected_value!r}")
    _validate_scope(absence, errors, label)
    _validate_missing_value_requirements(absence, errors, label)
    _validate_denials(absence, errors, label)
    _validate_secret_surface(absence, errors, label)
    _validate_no_mutation_routes(absence, errors, label)
    _validate_no_implementation_claim(absence, errors, label)


def _validate_scope(absence: Mapping[str, Any], errors: list[str], label: str) -> None:
...
def _validate_missing_value_requirements(absence: Mapping[str, Any], errors: list[str], label: str) -> None:
    missing_values = absence.get("missing_value_requirements")
    if not isinstance(missing_values, list):
        errors.append(f"{label}: missing_value_requirements must be a list")
        return
    observed_kinds = [entry.get("decision_kind") for entry in missing_values if isinstance(entry, Mapping)]
    if tuple(observed_kinds) != ACCEPTED_RECORD_KINDS:
        errors.append(f"{label}: missing value kinds must match required order")
    for entry in missing_values:
        if not isinstance(entry, Mapping):
            errors.append(f"{label}: missing value entries must be objects")
            continue
        ...
        if entry.get("authority_effect") != "missing_value_no_live_authority":
            errors.append(f"{label}: {decision_kind} authority_effect mismatch")


def _validate_denials(absence: Mapping[str, Any], errors: list[str], label: str) -> None:
    authority_denials = _mapping(absence.get("authority_denials"))
    effect_boundary = _mapping(absence.get("effect_boundary"))
...
def _validate_fixture_matches_produced(
    ...
    produced_absence: Mapping[str, Any],
    errors: list[str],
) -> None:
    comparable_fields = (
        "absence_boundary_id",
    ...
    for field_name in comparable_fields:
        if fixture.get(field_name) != produced_absence.get(field_name):
            errors.append(f"fixture does not match produced operator decision value absence field: {field_name}")
...
def main(argv: list[str] | None = None) -> int:
    """Run the live producer operator decision value absence validator."""
    ...
    if args.json:
        ...
        payload["produced_absence"] = produced_absence
        print(json.dumps(payload, indent=2, sort_keys=True))
    elif validation.ok:
        print("AGENTIC SERVICE HARNESS LIVE PRODUCER OPERATOR DECISION VALUE ABSENCE VALID")
    else:
        print(
            "AGENTIC SERVICE HARNESS LIVE PRODUCER OPERATOR DECISION VALUE ABSENCE INVALID "
