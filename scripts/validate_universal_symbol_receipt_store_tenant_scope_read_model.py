#!/usr/bin/env python3
"""Validate the tenant scope witness operator read model.

Purpose: prove the tenant scope operator projection is read-only and bounded
while tenant binding, receipt-store append, raw payload exposure, mutation, and
terminal closure remain denied.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: tenant scope read-model schema/example and tenant scope witness
validator constants.
Invariants:
  - The read model is a projection, not tenant authority.
  - Every tenant requirement appears exactly once.
...
from scripts.validate_universal_symbol_receipt_store_tenant_scope_witness import (  # noqa: E402
...
DEFAULT_SCHEMA_PATH = REPO_ROOT / "schemas" / "universal_symbol_receipt_store_tenant_scope_read_model.schema.json"
DEFAULT_READ_MODEL_PATH = (
    REPO_ROOT / "examples" / "universal_symbol_receipt_store_tenant_scope_read_model.foundation.json"
)

EXPECTED_WITNESS_REF = "examples/universal_symbol_receipt_store_tenant_scope_witness.foundation.json"
EXPECTED_TENANT_SCOPE_DECISION = "blocked_pending_tenant_actor_binding_partition_and_isolation_evidence"
...
EXPECTED_OPERATOR_STATUS_SUMMARY: dict[str, str] = {
    "primary_status": "Tenant scope blocked",
    "binding_status": "Awaiting evidence",
    "partition_status": "Awaiting evidence",
    "raw_detail_visibility": "hidden_by_default",
    "operator_action": "Collect tenant binding and isolation evidence",
}


class UniversalSymbolReceiptStoreTenantScopeReadModelError(ValueError):
    """Raised when the tenant scope read model violates Foundation Mode."""


def validate_universal_symbol_receipt_store_tenant_scope_read_model(
    read_model_path: Path = DEFAULT_READ_MODEL_PATH,
    schema_path: Path = DEFAULT_SCHEMA_PATH,
) -> dict[str, Any]:
    """Validate schema shape, bounded rows, and denied tenant authority."""
    ...
    if errors:
        raise UniversalSymbolReceiptStoreTenantScopeReadModelError("; ".join(errors))
    return report
...
def load_json_object(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise UniversalSymbolReceiptStoreTenantScopeReadModelError(f"missing file: {path}") from exc
    except json.JSONDecodeError as exc:
        raise UniversalSymbolReceiptStoreTenantScopeReadModelError(f"invalid json: {path}: {exc}") from exc
    if not isinstance(value, dict):
        raise UniversalSymbolReceiptStoreTenantScopeReadModelError(f"expected object: {path}")
    return value


def _validate_schema_boundary(schema: Mapping[str, Any], errors: list[str]) -> None:
    expected_id = "urn:mullusi:schema:universal-symbol-receipt-store-tenant-scope-read-model:1"
...
def _validate_read_model_boundary(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if read_model.get("read_model_is_not_tenant_authority") is not True:
        errors.append("read model must not be tenant authority")
...
def _validate_witness_projection(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if projection.get("tenant_scope_decision") != EXPECTED_TENANT_SCOPE_DECISION:
        errors.append("witness_projection.tenant_scope_decision must remain blocked")
    if projection.get("authority_granted") is not False:
        errors.append("witness_projection.authority_granted must remain false")
    if projection.get("tenant_requirement_count") != len(REQUIRED_REQUIREMENT_IDS):
        errors.append("witness_projection.tenant_requirement_count drift")
...
def _validate_source_witness_alignment(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if projection.get("witness_ref") != EXPECTED_WITNESS_REF:
        return
    try:
        witness = load_json_object(DEFAULT_TENANT_SCOPE_WITNESS_PATH)
    except UniversalSymbolReceiptStoreTenantScopeReadModelError as exc:
        errors.append(f"source tenant scope witness unavailable: {exc}")
        return

    witness_summary = _mapping(witness.get("contract_summary"))
    expected_counts = {
        "tenant_requirement_count": witness_summary.get("tenant_requirement_count"),
        "blocked_reason_count": witness_summary.get("blocked_reason_count"),
    }
    for field_name, expected_count in expected_counts.items():
        if projection.get(field_name) != expected_count:
            errors.append(f"witness_projection.{field_name} must match source tenant scope witness")
    if projection.get("tenant_scope_decision") != witness.get("tenant_scope_decision"):
        errors.append("witness_projection.tenant_scope_decision must match source tenant scope witness")
...
def _validate_requirement_rows(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    for row in rows:
        ...
        if row.get("proof_state") not in {"Unknown", "BudgetUnknown", "Fail"}:
            errors.append(f"{row.get('requirement_id')}: proof_state must block tenant scope")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print(f"[FAIL] universal_symbol_receipt_store_tenant_scope_read_model: {exc}")
            print("STATUS: failed")
        return 1
    ...
    if args.json:
        ...
        print("[PASS] universal_symbol_receipt_store_tenant_scope_read_model")
        print("STATUS: passed")
    return 0
