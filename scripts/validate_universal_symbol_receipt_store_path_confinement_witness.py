"""Validate the Universal Symbol receipt-store path confinement witness.

Purpose: prove UniversalSymbol receipt-store path confinement remains blocked
until canonical root, allowed namespace, traversal denial, symlink resolution,
reserved path denial, tenant partition, append-only custody, and audit evidence
exist.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: path confinement schema/example, path custody witness, write-path
witness, writer duty scope witness, tenant scope witness, operator approval
witness, append audit witness, receipt-store authority witness, docs, and tests.
Invariants:
  - The witness is not path authority.
  - Confinement binding, path custody registration, write-path registration,
    receipt append, filesystem escape, runtime dispatch, connector calls,
    mutation, and terminal closure remain denied.
  - Unknown hard preconditions block path confinement and log Delta_reject refs.
"""
...
DEFAULT_SCHEMA_PATH = REPO_ROOT / "schemas" / "universal_symbol_receipt_store_path_confinement_witness.schema.json"
DEFAULT_WITNESS_PATH = (
    REPO_ROOT / "examples" / "universal_symbol_receipt_store_path_confinement_witness.foundation.json"
)

AUTHORITY_DENIAL_FIELDS: tuple[str, ...] = (
    "receipt_store_path_confinement_bound",
    ...
    "filesystem_escape_allowed",
    "state_mutation_performed",
    "terminal_closure_allowed",
)

REQUIRED_REQUIREMENT_IDS: tuple[str, ...] = (
    "requirement://canonical-root",
    "requirement://allowed-namespace",
    "requirement://path-traversal-denial",
    "requirement://symlink-resolution",
    "requirement://reserved-path-denial",
    "requirement://tenant-partition",
    "requirement://append-only-custody",
    "requirement://audit-receipt",
)

PATH_CONFINEMENT_CONSTRAINT_TRUE_FIELDS: tuple[str, ...] = (
    "canonical_root_required",
    "allowed_namespace_required",
    "path_traversal_denial_required",
    "symlink_resolution_required",
    "reserved_path_denial_required",
    "tenant_partition_required",
    "append_only_custody_required",
    "audit_receipt_required",
)

REJECTION_POLICY_TRUE_FIELDS: tuple[str, ...] = (
    "missing_canonical_root_blocks_confinement",
    "missing_allowed_namespace_blocks_confinement",
    "missing_traversal_denial_blocks_confinement",
    "missing_partition_blocks_confinement",
    "unknown_hard_constraint_blocks_confinement",
    "failed_precondition_logs_delta_reject",
)

REQUIRED_BLOCKED_REASONS: tuple[str, ...] = (
    "canonical_root_missing",
    "allowed_namespace_missing",
    "path_traversal_denial_missing",
    "symlink_resolution_missing",
    "reserved_path_denial_missing",
    "tenant_partition_missing",
    "append_only_custody_missing",
    "audit_receipt_missing",
    "receipt_store_path_confinement_forbidden",
...
class UniversalSymbolReceiptStorePathConfinementWitnessError(ValueError):
    """Raised when the path confinement witness violates Foundation Mode."""
...
def load_json_object(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise UniversalSymbolReceiptStorePathConfinementWitnessError(f"missing file: {path}") from exc
    except json.JSONDecodeError as exc:
        raise UniversalSymbolReceiptStorePathConfinementWitnessError(f"invalid json: {path}: {exc}") from exc
    if not isinstance(value, dict):
        raise UniversalSymbolReceiptStorePathConfinementWitnessError(f"expected object: {path}")
    return value


def validate_universal_symbol_receipt_store_path_confinement_witness(
    witness_path: Path = DEFAULT_WITNESS_PATH,
    schema_path: Path = DEFAULT_SCHEMA_PATH,
) -> dict[str, Any]:
    """Validate schema shape, denied path authority, and evidence refs."""
    ...
    _validate_confinement_requirements(witness, errors)
    _validate_authority_denials(witness, errors)
    _validate_path_confinement_constraints(witness, errors)
    ...
    report = {
        ...
        "path_confinement_decision": witness.get("path_confinement_decision", ""),
        "authority_denial_count": len(AUTHORITY_DENIAL_FIELDS),
        "confinement_requirement_count": len(witness.get("confinement_requirements", []))
        if isinstance(witness.get("confinement_requirements"), list)
        else 0,
    ...
    if errors:
        raise UniversalSymbolReceiptStorePathConfinementWitnessError("; ".join(errors))
    return report


def _validate_schema_boundary(schema: Mapping[str, Any], errors: list[str]) -> None:
    if schema.get("$id") != "urn:mullusi:schema:universal-symbol-receipt-store-path-confinement-witness:1":
...
def _validate_witness_boundary(witness: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if (
        witness.get("path_confinement_decision")
        != "blocked_pending_root_namespace_traversal_symlink_partition_and_audit_evidence"
    ):
        errors.append("path_confinement_decision must remain blocked")
    if witness.get("path_confinement_witness_is_not_path_authority") is not True:
        errors.append("path confinement witness must not grant path authority")


def _validate_confinement_requirements(witness: Mapping[str, Any], errors: list[str]) -> None:
    requirements = witness.get("confinement_requirements")
    if not isinstance(requirements, list) or not requirements:
        errors.append("confinement_requirements must be non-empty")
        return
    requirement_ids: list[str] = []
    for requirement in requirements:
        if not isinstance(requirement, dict):
            errors.append("confinement requirement entries must be objects")
            continue
        ...
        if requirement.get("proof_state") not in {"Unknown", "BudgetUnknown", "Fail"}:
            errors.append(f"{requirement.get('requirement_id')}: proof_state must block path confinement")
        if requirement.get("current_decision") != "path_confinement_blocked":
            errors.append(f"{requirement.get('requirement_id')}: current_decision must be path_confinement_blocked")
    ...
    _require_members("confinement_requirements", requirement_ids, REQUIRED_REQUIREMENT_IDS, errors)
...
def _validate_path_confinement_constraints(witness: Mapping[str, Any], errors: list[str]) -> None:
    constraints = _mapping(witness.get("path_confinement_constraints"))
    for field_name in PATH_CONFINEMENT_CONSTRAINT_TRUE_FIELDS:
        if constraints.get(field_name) is not True:
            errors.append(f"path_confinement_constraints.{field_name} must remain true")
...
def _validate_contract_summary(witness: Mapping[str, Any], errors: list[str]) -> None:
    ...
    expected_values = {
        "confinement_requirement_count": _list_len(witness.get("confinement_requirements")),
        "authority_denial_count": len(AUTHORITY_DENIAL_FIELDS),
        "path_confinement_constraint_count": len(PATH_CONFINEMENT_CONSTRAINT_TRUE_FIELDS),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate the UniversalSymbol receipt-store path confinement witness."
    )
    ...
    try:
        report = validate_universal_symbol_receipt_store_path_confinement_witness(args.witness, args.schema)
    except UniversalSymbolReceiptStorePathConfinementWitnessError as exc:
        if args.json:
            print(json.dumps({"valid": False, "errors": [str(exc)]}, indent=2, sort_keys=True))
        else:
            print(f"[FAIL] universal_symbol_receipt_store_path_confinement_witness: {exc}")
            print("STATUS: failed")
        return 1
    if args.json:
        print(json.dumps(report, indent=2, sort_keys=True))
    else:
        print("[PASS] universal_symbol_receipt_store_path_confinement_witness")
        print("STATUS: passed")
    return 0
