#!/usr/bin/env python3
"""Validate the ReadOnlyWorkerRuntimeDispatchEndpointRegistrationWitness contract.

Purpose: verify Foundation Mode witness evidence for a future live
read_only_repo_inspection runtime dispatch endpoint registration boundary
without registering that endpoint, binding a route, invoking a worker, or
admitting dispatch.
...
Invariants:
  ...
  - Dispatch endpoint registration, endpoint registry writes, endpoint route
    binding, runtime dispatch, worker invocation, and runtime receipt emission
    remain unperformed.
  - Operator approval, live runner registration, endpoint identity digest,
    route boundary, UAO, Phi_gov, temporal lease, WorkerFailureReceipt, and
...
DEFAULT_SCHEMA_PATH = WORKSPACE_ROOT / "schemas" / "read_only_worker_runtime_dispatch_endpoint_registration_witness.schema.json"
DEFAULT_RECEIPT_PATH = (
    WORKSPACE_ROOT / "examples" / "read_only_worker_runtime_dispatch_endpoint_registration_witness.foundation.json"
)
EXPECTED_SCHEMA_ID = "urn:mullusi:schema:read-only-worker-runtime-dispatch-endpoint-registration-witness:1"
EXPECTED_SCHEMA_TITLE = "Read-Only Worker Runtime Dispatch Endpoint Registration Witness"
EXPECTED_RECEIPT_VERSION = "read_only_worker_runtime_dispatch_endpoint_registration_witness.v1"
...
EXPECTED_WITNESS_MODE = "LIVE_DISPATCH_ENDPOINT_REGISTRATION_WITNESS_ONLY"
EXPECTED_RUNTIME_RECEIPT_KIND = "future_worker_runtime_receipt"
EXPECTED_TARGET_DISPATCH_ENDPOINT_REF = "candidate://runtime-dispatch-endpoint/live-registration/read-only-repo-inspection"
EXPECTED_ENDPOINT_PROFILE = "LOCAL_READ_ONLY_REPO_INSPECTION_DISPATCH_ENDPOINT"
EXPECTED_ADMISSION_DECISION = (
    "LIVE_DISPATCH_ENDPOINT_REGISTRATION_WITNESS_BLOCKED_AWAITING_OPERATOR_APPROVAL_AND_LIVE_EVIDENCE"
)
...
REQUIRED_REGISTRATION_INPUT_REFS = (
    "evidence://operator-approval/live-dispatch-endpoint-registration",
    "evidence://runtime-runner/live-registration",
    "evidence://dispatch-endpoint-identity-digest",
    "evidence://dispatch-endpoint-route-boundary/read-only-repo-inspection",
...
REQUIRED_BLOCKED_REASON_REFS = (
    "blocked://operator-approval/live-dispatch-endpoint-registration-missing",
    "blocked://runtime-runner/not-registered",
    "blocked://dispatch-endpoint/not-registered",
    "blocked://dispatch-endpoint/identity-digest-missing",
    "blocked://dispatch-endpoint/route-not-bound",
...
REGISTRATION_EVALUATION_TRUE_FIELDS = (
    ...
    "endpoint_identity_digest_required",
    "endpoint_route_boundary_required",
    "tenant_actor_boundary_required",
    "runner_registration_required_before_endpoint_activation",
...
REGISTRATION_EVALUATION_DENIED_FIELDS = (
    ...
    "endpoint_registry_write_performed",
...
class ReadOnlyWorkerRuntimeDispatchEndpointRegistrationWitnessError(ValueError):
    """Raised when a dispatch endpoint registration witness artifact cannot load."""
...
def load_json_object(json_path: Path, label: str) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ReadOnlyWorkerRuntimeDispatchEndpointRegistrationWitnessError(f"{label} must be a JSON object")
    return payload
...
def validate_schema_artifact(schema: dict[str, Any]) -> list[str]:
    ...
    if isinstance(required_fields, list) and isinstance(properties, dict):
        for field_name in (
            ...
            "receipt_version",
            "authority_scope",
            "dispatch_endpoint_registration_witness_contract",
...
def validate_dispatch_endpoint_registration_witness_record(
    record: Any,
    schema: dict[str, Any] | None = None,
    runner_registration_witness: dict[str, Any] | None = None,
) -> list[str]:
    """Return deterministic validation errors for one dispatch endpoint witness."""

    schema_payload = schema or _load_schema(DEFAULT_SCHEMA_PATH)
    errors = _validate_schema_instance(schema_payload, record)
    if not isinstance(record, dict):
        errors.append("read-only worker runtime dispatch endpoint registration witness must be a JSON object")
        return errors
    ...
    _validate_top_level_refs(record, runner_payload, errors)
    _validate_authority_scope(record.get("authority_scope"), errors)
    _validate_registration_contract(record.get("dispatch_endpoint_registration_witness_contract"), runner_payload, errors)
...
def validate_dispatch_endpoint_registration_witness(
    ...
    receipt = load_json_object(receipt_path, "ReadOnlyWorkerRuntimeDispatchEndpointRegistrationWitness")
    ...
    errors.extend(validate_dispatch_endpoint_registration_witness_record(receipt, schema, runner_registration_witness))
    return errors


def build_mutated_dispatch_endpoint_registration_witness(**updates: Any) -> dict[str, Any]:
    """Build a deterministic mutated copy of the default witness receipt."""

    receipt = load_json_object(DEFAULT_RECEIPT_PATH, "ReadOnlyWorkerRuntimeDispatchEndpointRegistrationWitness")
...
def _validate_top_level_refs(
    ...
    if record.get("receipt_version") != EXPECTED_RECEIPT_VERSION:
        errors.append("receipt_version must match read_only_worker_runtime_dispatch_endpoint_registration_witness.v1")
    expected_refs = {
    ...
    if record.get("selected_worker_path") != runner_registration_witness.get("selected_worker_path"):
        errors.append("dispatch endpoint registration witness selected_worker_path must match runner registration witness")
...
def _validate_authority_scope(scope: Any, errors: list[str]) -> None:
    ...
    if scope.get("live_dispatch_endpoint_registration_witness_defined") is not True:
        errors.append("authority_scope.live_dispatch_endpoint_registration_witness_defined must be true")
...
def _validate_registration_contract(
    ...
    if not isinstance(contract, dict):
        errors.append("dispatch_endpoint_registration_witness_contract must be an object")
        return
    ...
    if contract.get("worker_id") != EXPECTED_WORKER_ID:
        errors.append("dispatch_endpoint_registration_witness_contract.worker_id must select worker_local_read_only_repo_inspection")
    if contract.get("worker_id") != runner_contract.get("worker_id"):
        errors.append("dispatch_endpoint_registration_witness_contract.worker_id must match runner registration witness worker_id")
    if contract.get("capability") != EXPECTED_WORKER_PATH:
        errors.append("dispatch_endpoint_registration_witness_contract.capability must be read_only_repo_inspection")
    if contract.get("operation_family") != EXPECTED_OPERATION_FAMILY:
        errors.append("dispatch_endpoint_registration_witness_contract.operation_family must be local_repo_inspection")
    ...
    if contract.get("runtime_receipt_kind") != EXPECTED_RUNTIME_RECEIPT_KIND:
        errors.append("dispatch_endpoint_registration_witness_contract.runtime_receipt_kind must be future_worker_runtime_receipt")
    if contract.get("source_runner_registration_witness_ref") != EXPECTED_RUNNER_REGISTRATION_WITNESS_REF:
        errors.append("dispatch_endpoint_registration_witness_contract.source_runner_registration_witness_ref is invalid")
    if contract.get("target_dispatch_endpoint_ref") != EXPECTED_TARGET_DISPATCH_ENDPOINT_REF:
        errors.append("dispatch_endpoint_registration_witness_contract.target_dispatch_endpoint_ref is invalid")
    if contract.get("endpoint_profile") != EXPECTED_ENDPOINT_PROFILE:
        errors.append("dispatch_endpoint_registration_witness_contract.endpoint_profile is invalid")
    ...
    if not isinstance(obligations, list) or len(obligations) < 8:
        errors.append("registration_obligations_checked must list future dispatch endpoint registration obligations")
        ...
        errors.append("registration_obligations_checked must require endpoint identity digest binding")
...
def _validate_admission_decision(decision: Any, errors: list[str]) -> None:
    ...
    if decision.get("dispatch_endpoint_registration_witness_defined") is not True:
        errors.append("admission_decision.dispatch_endpoint_registration_witness_defined must be true")
    for field_name in (
        "runtime_runner_registered",
        "dispatch_endpoint_registered",
        "endpoint_route_bound",
...
def _validate_contract_summary(record: dict[str, Any], errors: list[str]) -> None:
    contract = record.get("dispatch_endpoint_registration_witness_contract")
    ...
    if not all(isinstance(value, dict) for value in (contract, evaluation, decision, refs, summary)):
        errors.append("dispatch endpoint contract, evaluation, decision, refs, and summary must be objects")
        return
    expected_counts = {
...
def main(argv: list[str] | None = None) -> int:
    """Validate ReadOnlyWorkerRuntimeDispatchEndpointRegistrationWitness artifacts from the CLI."""

    parser = argparse.ArgumentParser(
        description="Validate ReadOnlyWorkerRuntimeDispatchEndpointRegistrationWitness contract."
    )
    ...
    if args.json:
        print(
            json.dumps(
                {
                    "receipt_id": "read_only_worker_runtime_dispatch_endpoint_registration_witness_validation",
        ...
        print("[PASS] read_only_worker_runtime_dispatch_endpoint_registration_witness")
