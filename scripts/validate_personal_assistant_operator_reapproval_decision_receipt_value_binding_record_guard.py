#!/usr/bin/env python3
"""Validate personal-assistant operator decision value-binding record guards.

Purpose: prove value-binding record guards record candidate requirements
...
boundaries.
Dependencies: personal-assistant value-binding record guard runtime helper,
schema validators, and receipt validator.
Invariants:
  - Record guards are not binding records.
...
from mcoi_runtime.personal_assistant import (  # noqa: E402
    DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_GUARD_GENERATED_AT,
...
DEFAULT_SCHEMA = (
    REPO_ROOT / "schemas" / "personal_assistant_operator_reapproval_decision_receipt_value_binding_record_guard.schema.json"
)
DEFAULT_RECEIPT_SCHEMA = REPO_ROOT / "schemas" / "personal_assistant_receipt.schema.json"
RUNTIME_GENERATED_AT = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_GUARD_GENERATED_AT
...
class PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordGuardValidation:
    """Validation result for no-effect operator decision value-binding record guards."""

    valid: bool
    runtime_validated: bool
    record_guard_count: int
    receipt_count: int
...
def validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_guard(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    receipt_schema_path: Path = DEFAULT_RECEIPT_SCHEMA,
) -> PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordGuardValidation:
    """Validate the runtime value-binding record guard envelope."""

    errors: list[str] = []
    schema = _load_json_object(schema_path, "operator reapproval decision value-binding record guard schema", errors)
    ...
    errors.extend(_validate_value_binding_record_guard_semantics(envelope, receipt_schema))
    _scan_private_or_secret_payload(envelope, errors, path="$runtime")
    return PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordGuardValidation(
        valid=not errors,
        runtime_validated=not errors,
        record_guard_count=int(envelope.get("record_guard_count", 0)),
...
def _validate_value_binding_record_guard_semantics(
    ...
    guards = envelope.get("record_guards")
    if not isinstance(guards, list):
        errors.append("record_guards must be a list")
        return tuple(errors)
    if envelope.get("record_guard_count") != len(guards):
        errors.append("record_guard_count must equal record_guards length")
    guard_ids: list[str] = []
    source_contract_ids: list[str] = []
    receipt_ids: list[str] = []
    for index, guard in enumerate(guards):
        if not isinstance(guard, dict):
            errors.append(f"record_guards[{index}] must be an object")
            continue
        guard_ids.append(str(guard.get("record_guard_id", "")))
        source_contract_ids.append(str(guard.get("source_binding_contract_id", "")))
        _require_binding_contract_ref(index, _mapping(guard.get("binding_contract_ref")), errors)
        _require_record_candidate_requirements(index, _mapping(guard.get("record_candidate_requirements")), errors)
        _require_execution_block(index, _mapping(guard.get("execution_admission_block")), errors)
        receipt = _mapping(guard.get("receipt"))
        if receipt_schema:
            errors.extend(
                f"record_guards[{index}].receipt {message}"
                for message in _validate_schema_instance(dict(receipt_schema), receipt)
            )
        errors.extend(f"record_guards[{index}].receipt {message}" for message in validate_personal_assistant_receipt_payload(receipt))
        if receipt.get("decision") != "deferred":
            errors.append(f"record_guards[{index}].receipt.decision must be deferred")
        if receipt.get("approval_ref") != guard.get("approval_id"):
            errors.append(f"record_guards[{index}].receipt.approval_ref must match approval_id")
        metadata = _mapping(receipt.get("metadata"))
        if metadata.get("operator_reapproval_decision_receipt_value_binding_record_guard_is_execution") is not False:
            errors.append(
                "record_guards["
                f"{index}].receipt.metadata.operator_reapproval_decision_receipt_value_binding_record_guard_is_execution must be false"
            )
        if metadata.get("binding_contract_ref_bound") is not True:
            errors.append(f"record_guards[{index}].receipt.metadata.binding_contract_ref_bound must be true")
        _require_false_fields(
            ...
            f"record_guards[{index}].receipt.metadata",
            errors,
        )
        receipt_ids.append(str(receipt.get("receipt_id", "")))
    if envelope.get("record_guard_ids") != guard_ids:
        errors.append("record_guard_ids must match guard order")
    if envelope.get("source_binding_contract_ids") != source_contract_ids:
        errors.append("source_binding_contract_ids must match guard order")
    if envelope.get("receipt_ids") != receipt_ids:
        errors.append("receipt_ids must match guard receipts")
    assurance = _mapping(envelope.get("assurance"))
...
def _require_binding_contract_ref(index: int, payload: Mapping[str, Any], errors: list[str]) -> None:
    if payload.get("contract_outcome") != "AwaitingEvidence":
        errors.append(f"record_guards[{index}].binding_contract_ref.contract_outcome must be AwaitingEvidence")
    for field_name in ("operator_value_bound", "binding_record_created", "execution_worker_admission_allowed"):
        if payload.get(field_name) is not False:
            errors.append(f"record_guards[{index}].binding_contract_ref.{field_name} must be false")


def _require_record_candidate_requirements(index: int, payload: Mapping[str, Any], errors: list[str]) -> None:
    for field_name in (
        ...
        if payload.get(field_name) is not True:
            errors.append(f"record_guards[{index}].record_candidate_requirements.{field_name} must be true")
    if tuple(payload.get("allowed_decision_values", ())) != EXPECTED_DECISION_VALUES:
        errors.append(f"record_guards[{index}].record_candidate_requirements.allowed_decision_values must match policy")
    for field_name in (
        "accepted_value_present",
        "operator_value_bound",
        ...
        if payload.get(field_name) is not False:
            errors.append(f"record_guards[{index}].record_candidate_requirements.{field_name} must be false")


def _require_execution_block(index: int, payload: Mapping[str, Any], errors: list[str]) -> None:
    if payload.get("execution_worker_admission_state") != "blocked_pending_value_binding_record_admission_preflight":
        errors.append(f"record_guards[{index}].execution_admission_block.execution_worker_admission_state must remain blocked")
    _require_false_fields(
        payload,
        frozenset(
            {
        ...
        f"record_guards[{index}].execution_admission_block",
        errors,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    validation = validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_guard(
    ...
    if args.json:
        ...
        print("PERSONAL ASSISTANT OPERATOR REAPPROVAL DECISION RECEIPT VALUE BINDING RECORD GUARD VALID")
    else:
