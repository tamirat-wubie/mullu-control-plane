#!/usr/bin/env python3
"""Validate operator value-binding verifier execution decision-value record value template."""
...
RUNTIME_GENERATED_AT = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_VALUE_TEMPLATE_GENERATED_AT
...
class PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierExecutionDecisionValueRecordValueTemplateValidation:
    """Validation result for verifier execution decision-value record value template."""

    valid: bool
    runtime_validated: bool
    record_value_template_count: int
...
def validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_template(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    receipt_schema_path: Path = DEFAULT_RECEIPT_SCHEMA,
) -> PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierExecutionDecisionValueRecordValueTemplateValidation:
    """Validate runtime verifier execution decision-value record value template."""

    errors: list[str] = []
    schema = _load_json_object(schema_path, "decision-value record value template schema", errors)
    ...
    errors.extend(_validate_decision_value_record_value_template_semantics(envelope, receipt_schema))
    _scan_secret_values(envelope, errors, path="$runtime")
    return PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierExecutionDecisionValueRecordValueTemplateValidation(
        valid=not errors,
        runtime_validated=not errors,
        record_value_template_count=int(envelope.get("record_value_template_count", 0)),
...
def _validate_decision_value_record_value_template_semantics(
    ...
    records = envelope.get("record_value_templates")
    if not isinstance(records, list):
        errors.append("record_value_templates must be a list")
        return tuple(errors)
    if envelope.get("record_value_template_count") != len(records):
        errors.append("record_value_template_count must equal record_value_templates length")
    item_ids: list[str] = []
    receipt_ids: list[str] = []
    coverage: dict[str, set[str]] = {}
    for index, record in enumerate(records):
        if not isinstance(record, dict):
            errors.append(f"record_value_templates[{index}] must be an object")
            continue
        item_ids.append(str(record.get("record_value_template_item_id", "")))
        ...
        _require_source_ref(index, _mapping(record.get("record_value_request_ref")), errors)
        _require_record_value_template(index, _mapping(record.get("record_value_template")), errors)
        _require_false_fields(
            _mapping(record.get("authority_status")),
            FALSE_AUTHORITY_FIELDS,
            f"record_value_templates[{index}].authority_status",
            errors,
        )
        receipt = _mapping(record.get("receipt"))
        if receipt_schema:
            errors.extend(f"record_value_templates[{index}].receipt {message}" for message in _validate_schema_instance(dict(receipt_schema), receipt))
        errors.extend(f"record_value_templates[{index}].receipt {message}" for message in validate_personal_assistant_receipt_payload(receipt))
        metadata = _mapping(receipt.get("metadata"))
        if metadata.get("operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_template_is_execution") is not False:
            errors.append(f"record_value_templates[{index}].receipt.metadata execution flag must be false")
        if metadata.get("record_value_template_created") is not True:
            errors.append(f"record_value_templates[{index}].receipt.metadata record_value_template_created must be true")
        _require_false_fields(metadata, FALSE_FIELDS | {"external_write_allowed"}, f"record_value_templates[{index}].receipt.metadata", errors)
        receipt_ids.append(str(receipt.get("receipt_id", "")))
    if set(coverage) != EXPECTED_EVIDENCE_KINDS:
        errors.append("record_value_templates must cover all governed evidence kinds")
    for evidence_kind, requirement_kinds in coverage.items():
        if requirement_kinds != EXPECTED_REQUIREMENT_KINDS:
            errors.append(f"record_value_templates for {evidence_kind} must cover all verifier requirement kinds")
    if envelope.get("record_value_template_item_ids") != item_ids:
        errors.append("record_value_template_item_ids must match item order")
...
def _require_source_ref(index: int, payload: Mapping[str, Any], errors: list[str]) -> None:
    ...
    for field_name, expected_value in expected.items():
        if payload.get(field_name) != expected_value:
            errors.append(f"record_value_templates[{index}].record_value_request_ref.{field_name} must be {expected_value}")


def _require_record_value_template(index: int, payload: Mapping[str, Any], errors: list[str]) -> None:
    for field_name in (
        "record_contract_ready",
        ...
        if payload.get(field_name) is not True:
            errors.append(f"record_value_templates[{index}].record_value_template.{field_name} must be true")
    if tuple(payload.get("accepted_record_kinds", ())) != EXPECTED_RECORD_KINDS:
        errors.append(f"record_value_templates[{index}].record_value_template.accepted_record_kinds must match canonical values")
    if tuple(payload.get("rejected_input_kinds", ())) != EXPECTED_REJECTED_INPUT_KINDS:
        errors.append(f"record_value_templates[{index}].record_value_template.rejected_input_kinds must reject generic continuations and templates")
    if tuple(payload.get("required_fields", ())) != EXPECTED_REQUIRED_FIELDS:
        errors.append(f"record_value_templates[{index}].record_value_template.required_fields must match required operator refs")
    _require_false_fields(
        payload,
        FALSE_FIELDS | {"accepts_generic_continuation", "accepts_template_packet"},
        f"record_value_templates[{index}].record_value_template",
        errors,
    )
    if payload.get("blocking_reason") != "actual_operator_decision_value_absent_record_value_request_unadmitted":
        errors.append(f"record_value_templates[{index}].record_value_template blocking_reason must remain governed")
...
def _require_summary(envelope: Mapping[str, Any], records: list[Any], errors: list[str]) -> None:
    ...
    expected = {
        ...
        "record_value_template_creation_count": len(records),
...
def main() -> int:
    ...
    result = validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_template(
