#!/usr/bin/env python3
"""Validate Agentic Service Harness live producer secret handoff preflight.

Purpose: make the secret handoff requirement explicit without reading,
printing, serializing, storing, or authorizing credential values.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: schemas/agentic_service_harness_live_producer_secret_handoff_preflight.schema.json,
examples/agentic_service_harness_live_producer_secret_handoff_preflight.local.json,
and scripts.validate_agentic_service_harness_live_producer_external_adapter_evidence_preflight.
Invariants:
  - Secret handoff remains AwaitingEvidence.
  - Credential possession, reading, printing, serialization, and storage remain denied.
...
DEFAULT_SCHEMA = (
    REPO_ROOT
    / "schemas"
    / "agentic_service_harness_live_producer_secret_handoff_preflight.schema.json"
)
DEFAULT_FIXTURE = (
    REPO_ROOT
    / "examples"
    / "agentic_service_harness_live_producer_secret_handoff_preflight.local.json"
)
PREFLIGHT_ID = "agentic-service-harness-live-producer-secret-handoff-preflight"
...
FORBIDDEN_LIVE_CLAIM = re.compile(
    r"\b(?:secret_values_present|secret_values_read|secret_values_serialized|secret_values_printed|"
    r"external_adapter_integrated|adapter_credentials_present|adapter_credentials_serialized|"
...
class LiveProducerSecretHandoffPreflightValidation:
    """Validation result for the live producer secret handoff preflight."""
...
def validate_live_producer_secret_handoff_preflight(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    fixture_path: Path = DEFAULT_FIXTURE,
) -> LiveProducerSecretHandoffPreflightValidation:
    """Validate the checked-in live producer secret handoff preflight."""
    errors: list[str] = []
    schema = _load_json_object(schema_path, "secret handoff preflight schema", errors)
    fixture = _load_json_object(fixture_path, "secret handoff preflight fixture", errors)
    ...
    return LiveProducerSecretHandoffPreflightValidation(
        ok=not errors,
        errors=tuple(errors),
        ...
        secret_handoff_status=str(fixture.get("secret_handoff_status", "")),
...
def _validate_preflight_semantics(
    ...
    if source_validation.ok is not True:
        errors.append(f"{label}: source external adapter evidence preflight must validate")
    if source_validation.external_adapter_evidence_status != "AwaitingEvidence":
        errors.append(f"{label}: source external adapter evidence must remain AwaitingEvidence")
    for field_name, expected_value in (
        ("solver_outcome", "AwaitingEvidence"),
        ("source_external_adapter_evidence_status", "AwaitingEvidence"),
        ("operator_approval_status", "Satisfied"),
        ("target_witness_kind", "secret_handoff"),
        ("secret_handoff_status", "AwaitingEvidence"),
        ("preflight_decision", "blocked_until_secret_handoff_exists"),
        ("planning_only", True),
        ("read_only", True),
    ...
    _validate_required_secret_handoff(preflight, errors, label)
...
def _validate_required_secret_handoff(
    preflight: Mapping[str, Any],
    errors: list[str],
    label: str,
) -> None:
    handoff = _mapping(preflight.get("required_secret_handoff"))
    if not handoff:
        errors.append(f"{label}: required_secret_handoff must be an object")
        return
    for field_name in (
        "handoff_ref",
        "required_authority_scope_ref",
        "required_storage_boundary_ref",
        "required_redaction_ref",
        "required_rotation_policy_ref",
        "required_audit_log_ref",
        "required_external_adapter_evidence_ref",
    ):
        if not handoff.get(field_name):
            errors.append(f"{label}: required_secret_handoff.{field_name} required")
    if handoff.get("status") != "AwaitingEvidence":
        errors.append(f"{label}: required secret handoff status must be AwaitingEvidence")
    if handoff.get("blocks_live_producer") is not True:
        errors.append(f"{label}: required secret handoff must block live producer")
    for flag_name in (
        "secret_values_present",
        "secret_values_read",
        "secret_values_serialized",
        ...
        if handoff.get(flag_name) is not False:
            errors.append(f"{label}: required secret handoff {flag_name} must be false")
...
def _validate_no_live_claim(payload: Any, errors: list[str], label: str) -> None:
    for path, value in _walk_strings(payload):
        if FORBIDDEN_LIVE_CLAIM.search(value):
            errors.append(f"{label}: live secret handoff claim at {path}")
...
def main(argv: list[str] | None = None) -> int:
    """Run the live producer secret handoff preflight validator."""
    ...
    if args.json:
        ...
        print("AGENTIC SERVICE HARNESS LIVE PRODUCER SECRET HANDOFF PREFLIGHT VALID")
    else:
        print(
            "AGENTIC SERVICE HARNESS LIVE PRODUCER SECRET HANDOFF PREFLIGHT INVALID "
