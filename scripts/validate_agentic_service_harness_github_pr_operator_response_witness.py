...
from scripts.validate_agentic_service_harness_github_pr_operator_approval_request import (  # noqa: E402
    DEFAULT_EXAMPLES as DEFAULT_SOURCE_APPROVAL_REQUEST_EXAMPLES,
    DEFAULT_SCHEMA as DEFAULT_SOURCE_APPROVAL_REQUEST_SCHEMA,
    validate_agentic_service_harness_github_pr_operator_approval_request,
)
from scripts.validate_schemas import _validate_schema_instance  # noqa: E402


DEFAULT_SCHEMA = REPO_ROOT / "schemas" / "agentic_service_harness_github_pr_operator_response_witness.schema.json"
DEFAULT_EXAMPLES = (
    REPO_ROOT / "examples" / "agentic_service_harness_github_pr_operator_response_witness.foundation.json",
)
DEFAULT_OUTPUT = (
...
class GitHubPrOperatorResponseWitnessValidation:
    """Schema and semantic validation report for GitHub PR response witness."""
...
def validate_agentic_service_harness_github_pr_operator_response_witness(
    *,
    ...
    errors: list[str] = []
    schema = _load_json_object(schema_path, "GitHub PR operator response witness schema", errors)
    source_validation = validate_agentic_service_harness_github_pr_operator_approval_request(
        schema_path=source_approval_request_schema_path,
        example_paths=source_approval_request_example_paths,
    )
...
def write_github_pr_operator_response_witness_validation(
    validation: GitHubPrOperatorResponseWitnessValidation,
    output_path: Path,
) -> Path:
    """Write one deterministic GitHub PR operator response witness report."""
...
def _validate_response_witness_semantics(
    ...
    _require_equal(payload, ("response_status",), "AwaitingEvidence", errors, label)
    _require_equal(payload, ("response_kind",), "operator_pr_approval_response_missing", errors, label)
    _require_equal(payload, ("operator_response", "approval_request_id"), EXPECTED_APPROVAL_REQUEST_ID, errors, label)
    _require_equal(payload, ("operator_response", "approver_role"), "operator", errors, label)
    _require_equal(
        payload,
        ("operator_response", "default_response_kind"),
        "record_operator_pr_rejection_witness",
        errors,
        label,
    )
    ...
    _require_equal(
        payload,
        ("operator_response", "approval_effect"),
...
def build_mutated_response_witness(**updates: Any) -> dict[str, Any]:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    validation = validate_agentic_service_harness_github_pr_operator_response_witness(
        schema_path=args.schema,
        example_paths=tuple(args.examples) if args.examples else DEFAULT_EXAMPLES,
        source_approval_request_schema_path=args.source_approval_request_schema,
        source_approval_request_example_paths=(
    ...
    if args.json:
        ...
        print("AGENTIC SERVICE HARNESS GITHUB PR OPERATOR RESPONSE WITNESS VALID")
    else:
