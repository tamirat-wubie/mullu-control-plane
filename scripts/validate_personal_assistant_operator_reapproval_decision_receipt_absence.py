#!/usr/bin/env python3
"""Validate personal-assistant operator reapproval decision receipt absence.

Purpose: prove a required operator decision receipt is absent and therefore
execution-worker admission remains blocked.
Governance scope: receipt-contract refs, absence proof, receipt conformance,
...
Invariants:
  - Absence witnesses must not claim an operator decision receipt.
...
from mcoi_runtime.personal_assistant import (  # noqa: E402
    DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_ABSENCE_GENERATED_AT,
...
DEFAULT_SCHEMA = REPO_ROOT / "schemas" / "personal_assistant_operator_reapproval_decision_receipt_absence.schema.json"
DEFAULT_RECEIPT_SCHEMA = REPO_ROOT / "schemas" / "personal_assistant_receipt.schema.json"
RUNTIME_GENERATED_AT = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_ABSENCE_GENERATED_AT
...
class PersonalAssistantOperatorReapprovalDecisionReceiptAbsenceValidation:
    """Validation result for no-effect operator reapproval decision receipt absence."""
...
def validate_personal_assistant_operator_reapproval_decision_receipt_absence(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    receipt_schema_path: Path = DEFAULT_RECEIPT_SCHEMA,
) -> PersonalAssistantOperatorReapprovalDecisionReceiptAbsenceValidation:
    """Validate the runtime operator reapproval decision receipt absence envelope."""
    errors: list[str] = []
    schema = _load_json_object(schema_path, "operator reapproval decision receipt absence schema", errors)
    ...
    errors.extend(_validate_operator_reapproval_decision_receipt_absence_semantics(envelope, receipt_schema))
    _scan_private_or_secret_payload(envelope, errors, path="$runtime")
    return PersonalAssistantOperatorReapprovalDecisionReceiptAbsenceValidation(
        valid=not errors,
...
def _validate_operator_reapproval_decision_receipt_absence_semantics(
    ...
    policy = _mapping(envelope.get("private_payload_policy"))
    if policy.get("raw_private_payload_serialized") is not False:
        errors.append("private_payload_policy.raw_private_payload_serialized must be false")
    if policy.get("secret_values_serialized") is not False:
        errors.append("private_payload_policy.secret_values_serialized must be false")
    if policy.get("receipt_contract_projection") != "ref_only":
        errors.append("private_payload_policy.receipt_contract_projection must be ref_only")
    if policy.get("decision_receipt_projection") != "absent":
    ...
    for index, absence in enumerate(absences):
        ...
        source_contract_ids.append(str(absence.get("source_contract_id", "")))
        approval_id = str(absence.get("approval_id", ""))
        _require_receipt_contract_ref(index, approval_id, _mapping(absence.get("receipt_contract_ref")), errors)
        ...
        if metadata.get("receipt_contract_ref_bound") is not True:
            errors.append(f"absences[{index}].receipt.metadata.receipt_contract_ref_bound must be true")
        for field_name in (
            "decision_receipt_present",
            "fresh_operator_decision_present",
            "operator_identity_ref_present",
            ...
            if metadata.get(field_name) is not False:
                errors.append(f"absences[{index}].receipt.metadata.{field_name} must be false")
        receipt_id = receipt.get("receipt_id")
        if isinstance(receipt_id, str):
            receipt_ids.append(receipt_id)
    if envelope.get("absence_ids") != absence_ids:
        errors.append("absence_ids must match absences order")
    if envelope.get("source_contract_ids") != source_contract_ids:
        errors.append("source_contract_ids must match absences order")
    if envelope.get("receipt_ids") != receipt_ids:
        errors.append("receipt_ids must match embedded receipts")
    return tuple(errors)


def _require_receipt_contract_ref(index: int, approval_id: str, payload: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if payload.get("required_receipt_ref") != expected_ref:
        errors.append(f"absences[{index}].receipt_contract_ref.required_receipt_ref must match approval_id")
    if payload.get("decision_receipt_required") is not True:
        errors.append(f"absences[{index}].receipt_contract_ref.decision_receipt_required must be true")
    for field_name in ("decision_receipt_present", "raw_operator_decision_serialized", "secret_values_serialized"):
        if payload.get(field_name) is not False:
            errors.append(f"absences[{index}].receipt_contract_ref.{field_name} must be false")
...
def _require_absence_witness(index: int, payload: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if payload.get("required_next_evidence") != "governed_operator_reapproval_decision_receipt":
...
def _require_execution_block(index: int, payload: Mapping[str, Any], errors: list[str]) -> None:
    if payload.get("execution_worker_admission_state") != "blocked_missing_operator_reapproval_decision_receipt":
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Parse operator reapproval decision receipt absence validation arguments."""
    parser = argparse.ArgumentParser(
        description="Validate personal-assistant operator reapproval decision receipt absence evidence.",
    )
...
def main(argv: list[str] | None = None) -> int:
    """CLI entry point for operator reapproval decision receipt absence validation."""
    args = parse_args(argv)
    result = validate_personal_assistant_operator_reapproval_decision_receipt_absence(
    ...
    if args.json:
        ...
        print(
            "personal-assistant operator reapproval decision receipt absence ok "
