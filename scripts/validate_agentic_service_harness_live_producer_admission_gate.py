#!/usr/bin/env python3
"""Validate Agentic Service Harness live producer admission gate.

Purpose: prove the future live producer implementation remains blocked behind
an explicit read-only admission gate after local producer rehearsal succeeds.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: gateway.agentic_service_harness_live_producer_admission,
schemas/agentic_service_harness_live_producer_admission_gate.schema.json,
examples/agentic_service_harness_live_producer_admission_gate.local.json, and
scripts.validate_agentic_service_harness_live_task_run_producer_rehearsal.
Invariants:
  - Gate output is blocked, read-only, local-only, and non-terminal.
  - Live execution and all high-risk authorities remain explicitly denied.
...
DEFAULT_SCHEMA = REPO_ROOT / "schemas" / "agentic_service_harness_live_producer_admission_gate.schema.json"
DEFAULT_FIXTURE = REPO_ROOT / "examples" / "agentic_service_harness_live_producer_admission_gate.local.json"
ALLOWED_SECRET_KEYS = {
    "secret_handoff_ref",
...
class LiveProducerAdmissionGateValidation:
    """Validation result for the live producer admission gate."""
...
def validate_live_producer_admission_gate(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    fixture_path: Path = DEFAULT_FIXTURE,
) -> tuple[LiveProducerAdmissionGateValidation, dict[str, Any]]:
    """Validate the checked-in admission fixture and produced gate."""
    errors: list[str] = []
    schema = _load_json_object(schema_path, "admission gate schema", errors)
    fixture = _load_json_object(fixture_path, "admission gate fixture", errors)
    rehearsal_validation, rehearsal_report = validate_live_task_run_producer_rehearsal()
    errors.extend(f"source rehearsal: {error}" for error in rehearsal_validation.errors)
    ...
    if schema and produced_gate:
        errors.extend(
            f"produced admission gate: {error}" for error in _validate_schema_instance(schema, produced_gate)
        )
        _validate_gate_semantics(produced_gate, errors, "produced admission gate")
    if fixture and produced_gate:
        _validate_fixture_matches_produced_gate(fixture, produced_gate, errors)

    observed_gate = produced_gate or fixture
    validation = LiveProducerAdmissionGateValidation(
        ok=not errors,
        errors=tuple(errors),
        schema_path=_path_label(schema_path),
        fixture_path=_path_label(fixture_path),
        gate_id=str(observed_gate.get("gate_id", "")),
        gate_state=str(observed_gate.get("gate_state", "")),
        blocked_reason_count=len(observed_gate.get("blocked_reasons", ()))
        if isinstance(observed_gate.get("blocked_reasons"), list)
        else 0,
...
def _validate_gate_semantics(
    ...
    if gate.get("gate_id") != ADMISSION_GATE_ID:
        errors.append(f"{label}: gate_id mismatch")
    if gate.get("admission_decision") != "blocked":
    ...
    for field_name, expected_value in (
        ...
        if gate.get(field_name) is not expected_value:
    ...
    if not isinstance(blocked_reasons, list):
        ...
        for reason in DEFAULT_BLOCKED_REASONS:
            if reason not in blocked_reasons:
                errors.append(f"{label}: missing blocked reason {reason}")
...
def _validate_required_evidence(gate: Mapping[str, Any], errors: list[str], label: str) -> None:
    ...
    if not required_evidence:
        ...
        return
    for field_name in (
        "local_rehearsal_report_ref",
        "live_task_run_producer_evidence_ref",
        "read_only_status_route_ref",
        "approval_gate_ref",
        "effect_receipt_ref",
        ...
        if not required_evidence.get(field_name):
            errors.append(f"{label}: required_evidence.{field_name} required")
...
def _validate_fixture_matches_produced_gate(
    ...
    for field_name in comparable_fields:
        if fixture.get(field_name) != produced_gate.get(field_name):
            errors.append(f"fixture does not match produced admission gate field: {field_name}")
...
def main(argv: list[str] | None = None) -> int:
    """Run the live producer admission gate validator."""
    ...
    if args.json:
        ...
        print("AGENTIC SERVICE HARNESS LIVE PRODUCER ADMISSION GATE VALID")
    else:
        print(
            "AGENTIC SERVICE HARNESS LIVE PRODUCER ADMISSION GATE INVALID "
