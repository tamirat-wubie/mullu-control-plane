#!/usr/bin/env python3
"""Build a Local Developer Workflow v1 PR admission packet.

Purpose: convert local command review evidence into a projection-only
branch-write and pull-request creation admission packet.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: Local Developer Workflow v1 artifacts, closure packet, and
command preview packet.
Invariants: the builder writes only JSON proof artifacts and never pushes
branches, opens pull requests, merges, deploys, or calls connectors.
"""
...
DEFAULT_OUTPUT = DEFAULT_ARTIFACT_DIR / PR_ADMISSION_PACKET_FILENAME


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    """Parse PR admission packet builder arguments."""

    parser = argparse.ArgumentParser(description="Build Local Developer Workflow v1 PR admission packet.")
...
def main(argv: Sequence[str] | None = None) -> int:
    """CLI entry point for Local Developer Workflow PR admission packet."""
    ...
    output_path = write_local_developer_workflow_pr_admission_packet(packet, Path(args.output))
    ...
    if args.json:
        ...
        print(f"LOCAL WORKFLOW PR ADMISSION PACKET BUILT path={output_path}")
    else:
        print(f"LOCAL WORKFLOW PR ADMISSION PACKET INVALID errors={list(validation.errors)}")
