#!/usr/bin/env python3
"""Validate personal-assistant operator reapproval decision receipt intake.

Purpose: prove the future operator reapproval decision receipt intake shape is
recorded without collecting a decision or admitting execution.
Governance scope: receipt-absence refs, intake requirements, receipt
...
Invariants:
  - Intake preflight records required future receipt fields only.
...
DEFAULT_SCHEMA = REPO_ROOT / "schemas" / "personal_assistant_operator_reapproval_decision_receipt_intake.schema.json"
DEFAULT_RECEIPT_SCHEMA = REPO_ROOT / "schemas" / "personal_assistant_receipt.schema.json"
RUNTIME_GENERATED_AT = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_INTAKE_GENERATED_AT
...
class PersonalAssistantOperatorReapprovalDecisionReceiptIntakeValidation:
    """Validation result for no-effect operator reapproval decision receipt intake."""
...
def validate_personal_assistant_operator_reapproval_decision_receipt_intake(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    receipt_schema_path: Path = DEFAULT_RECEIPT_SCHEMA,
) -> PersonalAssistantOperatorReapprovalDecisionReceiptIntakeValidation:
    """Validate the runtime operator reapproval decision receipt intake envelope."""

    errors: list[str] = []
    schema = _load_json_object(schema_path, "operator reapproval decision receipt intake schema", errors)
    ...
    errors.extend(_validate_operator_reapproval_decision_receipt_intake_semantics(envelope, receipt_schema))
    _scan_private_or_secret_payload(envelope, errors, path="$runtime")
    return PersonalAssistantOperatorReapprovalDecisionReceiptIntakeValidation(
        valid=not errors,
...
def _validate_operator_reapproval_decision_receipt_intake_semantics(
    ...
    for index, intake in enumerate(intakes):
        ...
        source_absence_ids.append(str(intake.get("source_absence_id", "")))
        approval_id = str(intake.get("approval_id", ""))
        _require_absence_witness_ref(index, approval_id, _mapping(intake.get("absence_witness_ref")), errors)
        _require_receipt_intake_request(index, approval_id, _mapping(intake.get("receipt_intake_request")), errors)
        _require_execution_block(index, _mapping(intake.get("execution_admission_block")), errors)
        ...
        errors.extend(f"intakes[{index}].receipt {message}" for message in validate_personal_assistant_receipt_payload(receipt))
        ...
        if metadata.get("operator_reapproval_decision_receipt_intake_is_execution") is not False:
            errors.append(
                f"intakes[{index}].receipt.metadata.operator_reapproval_decision_receipt_intake_is_execution must be false"
            )
        if metadata.get("receipt_absence_ref_bound") is not True:
            errors.append(f"intakes[{index}].receipt.metadata.receipt_absence_ref_bound must be true")
        _require_false_fields(
            ...
            f"intakes[{index}].receipt.metadata",
            errors,
        )
    ...
    if envelope.get("source_absence_ids") != source_absence_ids:
        errors.append("source_absence_ids must match intakes order")
...
def _require_private_payload_policy(payload: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if payload.get("absence_payload_projection") != "ref_only":
        errors.append("private_payload_policy.absence_payload_projection must be ref_only")
    if payload.get("decision_receipt_payload_projection") != "absent_until_operator_submits_receipt":
        errors.append("private_payload_policy.decision_receipt_payload_projection must be absent_until_operator_submits_receipt")


def _require_absence_witness_ref(
    index: int,
    approval_id: str,
    payload: Mapping[str, Any],
    errors: list[str],
) -> None:
    expected_ref = f"receipt://personal-assistant/operator-reapproval-decision/{approval_id}"
    if payload.get("required_receipt_ref") != expected_ref:
        errors.append(f"intakes[{index}].absence_witness_ref.required_receipt_ref must match approval_id")
    if payload.get("decision_receipt_required") is not True:
        errors.append(f"intakes[{index}].absence_witness_ref.decision_receipt_required must be true")
    if payload.get("decision_receipt_present") is not False:
        errors.append(f"intakes[{index}].absence_witness_ref.decision_receipt_present must be false")
    if payload.get("absence_reason") != "operator_reapproval_decision_receipt_absent":
        errors.append(f"intakes[{index}].absence_witness_ref.absence_reason must be canonical")
    if payload.get("execution_worker_admission_allowed") is not False:
        errors.append(f"intakes[{index}].absence_witness_ref.execution_worker_admission_allowed must be false")


def _require_receipt_intake_request(
    index: int,
    approval_id: str,
    payload: Mapping[str, Any],
    errors: list[str],
    ...
    if payload.get("receipt_intake_ref") != expected_ref:
        errors.append(f"intakes[{index}].receipt_intake_request.receipt_intake_ref must match approval_id")
    if tuple(payload.get("accepted_decision_values", ())) != EXPECTED_DECISION_VALUES:
        errors.append(f"intakes[{index}].receipt_intake_request.accepted_decision_values must match canonical values")
    for field_name in (
        ...
        if payload.get(field_name) is not True:
            errors.append(f"intakes[{index}].receipt_intake_request.{field_name} must be true")
    for field_name in (
        "operator_decision_value_present",
        "operator_identity_ref_present",
        "operator_signature_ref_present",
        ...
        if payload.get(field_name) is not False:
            errors.append(f"intakes[{index}].receipt_intake_request.{field_name} must be false")
    if payload.get("submission_payload_projection") != "absent_until_operator_submits_receipt":
        errors.append(f"intakes[{index}].receipt_intake_request.submission_payload_projection must be absent")


def _require_execution_block(index: int, payload: Mapping[str, Any], errors: list[str]) -> None:
    if payload.get("execution_worker_admission_state") != "blocked_pending_operator_reapproval_decision_receipt_intake":
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Parse operator reapproval decision receipt intake validation arguments."""

    parser = argparse.ArgumentParser(
        description="Validate personal-assistant operator reapproval decision receipt intake evidence.",
    )
...
def main(argv: list[str] | None = None) -> int:
    """CLI entry point for operator reapproval decision receipt intake validation."""

    args = parse_args(argv)
    result = validate_personal_assistant_operator_reapproval_decision_receipt_intake(
    ...
    if args.json:
        ...
        print(
            "personal-assistant operator reapproval decision receipt intake ok "
