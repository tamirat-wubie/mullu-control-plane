#!/usr/bin/env python3
"""Validate operator value-binding verifier execution decision-intake preflight."""
...
DEFAULT_SCHEMA = REPO_ROOT / "schemas" / "personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_intake_preflight.schema.json"
DEFAULT_RECEIPT_SCHEMA = REPO_ROOT / "schemas" / "personal_assistant_receipt.schema.json"
RUNTIME_GENERATED_AT = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_INTAKE_PREFLIGHT_GENERATED_AT
...
class PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierExecutionDecisionIntakePreflightValidation:
    """Validation result for verifier execution decision-intake preflight."""

    valid: bool
    runtime_validated: bool
    decision_intake_preflight_count: int
...
def validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_intake_preflight(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    receipt_schema_path: Path = DEFAULT_RECEIPT_SCHEMA,
) -> PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierExecutionDecisionIntakePreflightValidation:
    """Validate runtime verifier execution decision-intake preflight."""

    errors: list[str] = []
    schema = _load_json_object(schema_path, "decision-intake preflight schema", errors)
    ...
    errors.extend(_validate_decision_intake_preflight_semantics(envelope, receipt_schema))
    _scan_secret_values(envelope, errors, path="$runtime")
    return PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierExecutionDecisionIntakePreflightValidation(
        valid=not errors,
        runtime_validated=not errors,
        decision_intake_preflight_count=int(envelope.get("decision_intake_preflight_count", 0)),
...
def _validate_decision_intake_preflight_semantics(envelope: Mapping[str, Any], receipt_schema: Mapping[str, Any]) -> tuple[str, ...]:
    errors: list[str] = []
    expected_top = {
    ...
    records = envelope.get("decision_intake_preflights")
    if not isinstance(records, list):
        errors.append("decision_intake_preflights must be a list")
        return tuple(errors)
    if envelope.get("decision_intake_preflight_count") != len(records):
        errors.append("decision_intake_preflight_count must equal decision_intake_preflights length")
    item_ids: list[str] = []
    receipt_ids: list[str] = []
    coverage: dict[str, set[str]] = {}
    for index, record in enumerate(records):
        if not isinstance(record, dict):
            errors.append(f"decision_intake_preflights[{index}] must be an object")
            continue
        item_ids.append(str(record.get("decision_intake_preflight_item_id", "")))
        ...
        _require_source_ref(index, _mapping(record.get("verifier_execution_approval_request_ref")), errors)
        _require_preflight(index, _mapping(record.get("decision_intake_preflight")), errors)
        _require_false_fields(_mapping(record.get("authority_status")), FALSE_AUTHORITY_FIELDS, f"decision_intake_preflights[{index}].authority_status", errors)
        receipt = _mapping(record.get("receipt"))
        if receipt_schema:
            errors.extend(f"decision_intake_preflights[{index}].receipt {message}" for message in _validate_schema_instance(dict(receipt_schema), receipt))
        errors.extend(f"decision_intake_preflights[{index}].receipt {message}" for message in validate_personal_assistant_receipt_payload(receipt))
        metadata = _mapping(receipt.get("metadata"))
        if metadata.get("operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_intake_preflight_is_execution") is not False:
            errors.append(f"decision_intake_preflights[{index}].receipt.metadata execution flag must be false")
        _require_false_fields(metadata, FALSE_FIELDS | FALSE_AUTHORITY_FIELDS | {"external_write_allowed"}, f"decision_intake_preflights[{index}].receipt.metadata", errors)
        receipt_ids.append(str(receipt.get("receipt_id", "")))
    if set(coverage) != EXPECTED_EVIDENCE_KINDS:
        errors.append("decision_intake_preflights must cover all governed evidence kinds")
    for evidence_kind, requirement_kinds in coverage.items():
        if requirement_kinds != EXPECTED_REQUIREMENT_KINDS:
            errors.append(f"decision_intake_preflights for {evidence_kind} must cover all verifier requirement kinds")
    if envelope.get("decision_intake_preflight_item_ids") != item_ids:
        errors.append("decision_intake_preflight_item_ids must match item order")
...
def _require_source_ref(index: int, payload: Mapping[str, Any], errors: list[str]) -> None:
    ...
    for field_name, expected_value in expected.items():
        if payload.get(field_name) != expected_value:
            errors.append(f"decision_intake_preflights[{index}].verifier_execution_approval_request_ref.{field_name} must be {expected_value}")
...
def _require_preflight(index: int, payload: Mapping[str, Any], errors: list[str]) -> None:
    for field_name in ("operator_decision_required", "operator_decision_value_required"):
        if payload.get(field_name) is not True:
            errors.append(f"decision_intake_preflights[{index}].decision_intake_preflight.{field_name} must be true")
    _require_false_fields(payload, FALSE_FIELDS, f"decision_intake_preflights[{index}].decision_intake_preflight", errors)
    if payload.get("blocking_reason") != "operator_decision_value_not_present":
        errors.append(f"decision_intake_preflights[{index}].decision_intake_preflight blocking_reason must remain operator_decision_value_not_present")
...
def main() -> int:
    ...
    result = validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_intake_preflight(
    ...
    if args.json:
        ...
        print("PERSONAL ASSISTANT OPERATOR REAPPROVAL DECISION RECEIPT VALUE BINDING RECORD VERIFIER EXECUTION DECISION INTAKE PREFLIGHT VALID")
    else:
