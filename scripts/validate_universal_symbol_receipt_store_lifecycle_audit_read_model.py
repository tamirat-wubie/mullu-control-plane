#!/usr/bin/env python3
"""Validate the lifecycle audit receipt operator read model.

Purpose: prove the lifecycle audit operator projection is read-only and
bounded while lifecycle audit recording, receipt-store append, raw payload
exposure, mutation, and terminal closure remain denied.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: lifecycle audit read-model schema/example and lifecycle audit
receipt validator constants.
Invariants:
  - The read model is a projection, not lifecycle authority.
  - Every audit requirement appears exactly once.
...
from scripts.validate_universal_symbol_receipt_store_lifecycle_audit_receipt import (  # noqa: E402
    AUTHORITY_DENIAL_FIELDS,
    DEFAULT_RECEIPT_PATH as DEFAULT_AUDIT_RECEIPT_PATH,
...
DEFAULT_SCHEMA_PATH = (
    REPO_ROOT / "schemas" / "universal_symbol_receipt_store_lifecycle_audit_read_model.schema.json"
)
DEFAULT_READ_MODEL_PATH = (
    REPO_ROOT / "examples" / "universal_symbol_receipt_store_lifecycle_audit_read_model.foundation.json"
)

EXPECTED_RECEIPT_REF = "examples/universal_symbol_receipt_store_lifecycle_audit_receipt.foundation.json"
...
EXPECTED_OPERATOR_STATUS_SUMMARY: dict[str, str] = {
    "primary_status": "Lifecycle audit blocked",
    "audit_status": "Awaiting evidence",
    "recording_status": "Blocked",
    "raw_detail_visibility": "hidden_by_default",
    "operator_action": "Collect lifecycle witness evidence",
}


class UniversalSymbolReceiptStoreLifecycleAuditReadModelError(ValueError):
    """Raised when the lifecycle audit read model violates Foundation Mode."""


def validate_universal_symbol_receipt_store_lifecycle_audit_read_model(
    read_model_path: Path = DEFAULT_READ_MODEL_PATH,
    schema_path: Path = DEFAULT_SCHEMA_PATH,
) -> dict[str, Any]:
    """Validate schema shape, bounded rows, and denied lifecycle authority."""
    ...
    if errors:
        raise UniversalSymbolReceiptStoreLifecycleAuditReadModelError("; ".join(errors))
    return report
...
def load_json_object(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise UniversalSymbolReceiptStoreLifecycleAuditReadModelError(f"missing file: {path}") from exc
    except json.JSONDecodeError as exc:
        raise UniversalSymbolReceiptStoreLifecycleAuditReadModelError(f"invalid json: {path}: {exc}") from exc
    if not isinstance(value, dict):
        raise UniversalSymbolReceiptStoreLifecycleAuditReadModelError(f"expected object: {path}")
    return value


def _validate_schema_boundary(schema: Mapping[str, Any], errors: list[str]) -> None:
    expected_id = "urn:mullusi:schema:universal-symbol-receipt-store-lifecycle-audit-read-model:1"
...
def _validate_receipt_projection(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if projection.get("audit_decision") != EXPECTED_AUDIT_DECISION:
        errors.append("receipt_projection.audit_decision must remain blocked")
    if projection.get("authority_granted") is not False:
        errors.append("receipt_projection.authority_granted must remain false")
    if projection.get("audit_requirement_count") != len(REQUIRED_REQUIREMENT_IDS):
        errors.append("receipt_projection.audit_requirement_count drift")
...
def _validate_source_receipt_alignment(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if projection.get("receipt_ref") != EXPECTED_RECEIPT_REF:
        return
    try:
        receipt = load_json_object(DEFAULT_AUDIT_RECEIPT_PATH)
    except UniversalSymbolReceiptStoreLifecycleAuditReadModelError as exc:
        errors.append(f"source audit receipt unavailable: {exc}")
        return

    receipt_summary = _mapping(receipt.get("contract_summary"))
    expected_counts = {
        "audit_requirement_count": receipt_summary.get("audit_requirement_count"),
        "blocked_reason_count": receipt_summary.get("blocked_reason_count"),
    }
    for field_name, expected_count in expected_counts.items():
        if projection.get(field_name) != expected_count:
            errors.append(f"receipt_projection.{field_name} must match source audit receipt")
    if projection.get("audit_decision") != receipt.get("audit_decision"):
        errors.append("receipt_projection.audit_decision must match source audit receipt")
...
def _validate_requirement_rows(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    for row in rows:
        ...
        if row.get("proof_state") not in {"Unknown", "BudgetUnknown", "Fail"}:
            errors.append(f"{row.get('requirement_id')}: proof_state must block audit")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print(f"[FAIL] universal_symbol_receipt_store_lifecycle_audit_read_model: {exc}")
            print("STATUS: failed")
        return 1
    ...
    if args.json:
        ...
        print("[PASS] universal_symbol_receipt_store_lifecycle_audit_read_model")
        print("STATUS: passed")
    return 0
