#!/usr/bin/env python3
"""Validate operator value-binding verifier ref intake."""
...
DEFAULT_SCHEMA = REPO_ROOT / "schemas" / "personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_ref_intake.schema.json"
DEFAULT_RECEIPT_SCHEMA = REPO_ROOT / "schemas" / "personal_assistant_receipt.schema.json"
RUNTIME_GENERATED_AT = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_REF_INTAKE_GENERATED_AT
...
class PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierRefIntakeValidation:
    """Validation result for verifier ref intake."""

    valid: bool
    runtime_validated: bool
    verifier_ref_record_count: int
...
def validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_ref_intake(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    receipt_schema_path: Path = DEFAULT_RECEIPT_SCHEMA,
) -> PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierRefIntakeValidation:
    """Validate runtime verifier ref intake."""

    errors: list[str] = []
    schema = _load_json_object(schema_path, "operator verifier ref intake schema", errors)
    ...
    errors.extend(_validate_verifier_ref_intake_semantics(envelope, receipt_schema))
    _scan_private_or_secret_payload(envelope, errors, path="$runtime")
    return PersonalAssistantOperatorReapprovalDecisionReceiptValueBindingRecordVerifierRefIntakeValidation(
        valid=not errors,
        runtime_validated=not errors,
        verifier_ref_record_count=int(envelope.get("verifier_ref_record_count", 0)),
...
def _validate_verifier_ref_intake_semantics(
    ...
    _require_false_fields(effect_boundary, FALSE_AUTHORITY_FIELDS | {"raw_verifier_payload_present", "raw_evidence_payload_present", "raw_operator_value_present"}, "effect_boundary", errors)
    _require_private_payload_policy(_mapping(envelope.get("private_payload_policy")), errors)

    records = envelope.get("verifier_ref_records")
    if not isinstance(records, list):
        errors.append("verifier_ref_records must be a list")
        return tuple(errors)
    if envelope.get("verifier_ref_record_count") != len(records):
        errors.append("verifier_ref_record_count must equal verifier_ref_records length")
    record_ids: list[str] = []
    ...
    for index, record in enumerate(records):
        if not isinstance(record, dict):
            errors.append(f"verifier_ref_records[{index}] must be an object")
            continue
        record_ids.append(str(record.get("verifier_ref_record_id", "")))
        submitted_verifier_refs.append(str(record.get("submitted_verifier_ref", "")))
        ...
        _require_source_ref(index, _mapping(record.get("evidence_verification_preflight_ref")), requirement_kind, errors)
        _require_verifier_ref(index, _mapping(record.get("verifier_ref")), record, errors)
        _require_false_fields(_mapping(record.get("authority_status")), FALSE_AUTHORITY_FIELDS - {"verifier_ref_validated", "verification_requirement_satisfied", "evidence_verified", "evidence_accepted", "evidence_rejected"}, f"verifier_ref_records[{index}].authority_status", errors)
        receipt = _mapping(record.get("receipt"))
        if receipt_schema:
            errors.extend(
                f"verifier_ref_records[{index}].receipt {message}"
                for message in _validate_schema_instance(dict(receipt_schema), receipt)
            )
        errors.extend(f"verifier_ref_records[{index}].receipt {message}" for message in validate_personal_assistant_receipt_payload(receipt))
        if receipt.get("decision") != "blocked" or receipt.get("outcome") != "AwaitingEvidence":
            errors.append(f"verifier_ref_records[{index}].receipt must remain blocked AwaitingEvidence")
        metadata = _mapping(receipt.get("metadata"))
        if metadata.get("operator_reapproval_decision_receipt_value_binding_record_verifier_ref_intake_is_execution") is not False:
            errors.append(f"verifier_ref_records[{index}].receipt.metadata execution flag must be false")
        _require_false_fields(
            metadata,
            FALSE_AUTHORITY_FIELDS | {"raw_verifier_payload_present", "raw_evidence_payload_present", "raw_operator_value_present"},
            f"verifier_ref_records[{index}].receipt.metadata",
            errors,
        )
        receipt_ids.append(str(receipt.get("receipt_id", "")))
    if set(coverage) != EXPECTED_EVIDENCE_KINDS:
        errors.append("verifier_ref_records must cover all governed evidence kinds")
    for evidence_kind, requirement_kinds in coverage.items():
        if requirement_kinds != EXPECTED_REQUIREMENT_KINDS:
            errors.append(f"verifier_ref_records for {evidence_kind} must cover all verifier requirement kinds")
    if envelope.get("verifier_ref_record_ids") != record_ids:
        errors.append("verifier_ref_record_ids must match record order")
    if envelope.get("submitted_verifier_refs") != submitted_verifier_refs:
        errors.append("submitted_verifier_refs must match record order")
    ...
    for field_name in (
        "ready_for_verifier_ref_validation",
...
def _require_source_ref(index: int, payload: Mapping[str, Any], requirement_kind: str, errors: list[str]) -> None:
    expected = {
    ...
    for field_name, expected_value in expected.items():
        if payload.get(field_name) != expected_value:
            errors.append(f"verifier_ref_records[{index}].evidence_verification_preflight_ref.{field_name} must be {expected_value}")


def _require_verifier_ref(index: int, payload: Mapping[str, Any], record: Mapping[str, Any], errors: list[str]) -> None:
    expected_true = {"verifier_ref_only", "verifier_ref_present", "verifier_ref_submitted"}
    expected_false = {
    ...
    if payload.get("submitted_verifier_ref") != record.get("submitted_verifier_ref"):
        errors.append(f"verifier_ref_records[{index}].verifier_ref submitted ref must match record")
    if payload.get("requirement_kind") != record.get("requirement_kind"):
        errors.append(f"verifier_ref_records[{index}].verifier_ref requirement kind must match record")
    for field_name in expected_true:
        if payload.get(field_name) is not True:
            errors.append(f"verifier_ref_records[{index}].verifier_ref.{field_name} must be true")
    for field_name in expected_false:
        if payload.get(field_name) is not False:
            errors.append(f"verifier_ref_records[{index}].verifier_ref.{field_name} must be false")
    if payload.get("blocking_reason") != "submitted_verifier_ref_not_validated":
        errors.append(f"verifier_ref_records[{index}].verifier_ref blocking_reason must remain not validated")
...
def main() -> int:
    ...
    result = validate_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_ref_intake(
    ...
    if args.json:
        ...
        print("PERSONAL ASSISTANT OPERATOR REAPPROVAL DECISION RECEIPT VALUE BINDING RECORD VERIFIER REF INTAKE VALID")
    else:
