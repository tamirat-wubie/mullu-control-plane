...
DEFAULT_SCHEMA = REPO_ROOT / "schemas" / "agentic_service_harness_github_pr_effect_reconciliation_witness.schema.json"
DEFAULT_EXAMPLES = (
    REPO_ROOT / "examples" / "agentic_service_harness_github_pr_effect_reconciliation_witness.foundation.json",
)
DEFAULT_OUTPUT = (
    REPO_ROOT / ".change_assurance" / "agentic_service_harness_github_pr_effect_reconciliation_witness_validation.json"
)
...
EXPECTED_BINDING_ID = "agentic_service_harness_github_pr_effect_reconciliation_witness"
EXPECTED_EFFECT_RECONCILIATION_REQUEST_ID = "effect-reconciliation.github-pr-before-terminal-closure"
EXPECTED_SOURCE_CI_GATE_REQUEST_ID = "ci-gate.github-pr-before-ready-for-review"
EXPECTED_REQUESTED_EVIDENCE_REF = "evidence://effect-reconciliation-before-terminal-closure"
EXPECTED_SOURCE_CI_GATE_EVIDENCE_REF = "evidence://ci-gate-before-ready-for-review"
...
REQUIRED_FALSE_FLAGS = (
    ...
    "ci_gate_before_ready_for_review_satisfied",
    "terminal_closure_authorized_after_reconciliation",
...
REQUIRED_TRUE_FLAGS = (
    ...
    "reconciles_pull_request_state",
    "reconciles_check_state",
    "reconciles_merge_state",
    "effect_reconciliation_consumes_command_preview_ci_gate_evidence",
    "effect_reconciliation_consumes_actual_diff_ci_gate_evidence",
    "effect_reconciliation_remains_uncollected",
    "effect_reconciliation_remains_non_authorizing",
...
class GitHubPrEffectReconciliationWitnessValidation:
    """Schema and semantic validation report for effect reconciliation witness."""
...
def validate_agentic_service_harness_github_pr_effect_reconciliation_witness(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    example_paths: Sequence[Path] = DEFAULT_EXAMPLES,
    source_ci_gate_witness_schema_path: Path = DEFAULT_SOURCE_CI_GATE_WITNESS_SCHEMA,
    source_ci_gate_witness_example_paths: Sequence[Path] = DEFAULT_SOURCE_CI_GATE_WITNESS_EXAMPLES,
) -> GitHubPrEffectReconciliationWitnessValidation:
    """Validate GitHub PR effect reconciliation witness examples."""
    errors: list[str] = []
    schema = _load_json_object(schema_path, "GitHub PR effect reconciliation witness schema", errors)
    source_validation = validate_agentic_service_harness_github_pr_ci_gate_before_ready_for_review_witness(
        schema_path=source_ci_gate_witness_schema_path,
        example_paths=source_ci_gate_witness_example_paths,
    )
    if not source_validation.ok:
        errors.extend(f"source PR CI gate before ready-for-review witness: {error}" for error in source_validation.errors)
    source_ci_gate_witness = _load_json_object(
        source_ci_gate_witness_example_paths[0],
        "GitHub PR CI gate before ready-for-review witness source",
        errors,
    )
    ...
    for example_path in example_paths:
        example = _load_json_object(
            example_path,
            f"GitHub PR effect reconciliation witness {_path_label(example_path)}",
            errors,
        )
        if not example:
            continue
        ...
        _validate_effect_reconciliation_witness_semantics(
            example,
            source_ci_gate_witness,
            errors,
            _path_label(example_path),
        )
    return GitHubPrEffectReconciliationWitnessValidation(
        ok=not errors,
        errors=tuple(errors),
...
def write_github_pr_effect_reconciliation_witness_validation(
    validation: GitHubPrEffectReconciliationWitnessValidation,
    output_path: Path,
) -> Path:
    """Write one deterministic effect reconciliation witness validation report."""
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(validation.as_dict(), indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return output_path


def _validate_effect_reconciliation_witness_semantics(
    payload: Mapping[str, Any],
    source_ci_gate_witness: Mapping[str, Any],
    errors: list[str],
    label: str,
) -> None:
    ...
    _require_equal(
        payload,
        ("source_ci_gate_before_ready_for_review_witness_ref",),
        EXPECTED_SOURCE_CI_GATE_WITNESS_REF,
        errors,
        label,
    )
    _require_equal(payload, ("solver_outcome",), "AwaitingEvidence", errors, label)
    _require_equal(payload, ("witness_kind",), "effect_reconciliation_witness", errors, label)
    _require_equal(payload, ("requested_evidence_ref",), EXPECTED_REQUESTED_EVIDENCE_REF, errors, label)
    _require_equal(payload, ("effect_reconciliation_status",), "AwaitingEvidence", errors, label)
    _require_equal(
        payload,
        ("effect_reconciliation", "effect_reconciliation_request_id"),
        EXPECTED_EFFECT_RECONCILIATION_REQUEST_ID,
        errors,
        label,
    )
    _require_equal(
        payload,
        ("effect_reconciliation", "source_ci_gate_request_id"),
        EXPECTED_SOURCE_CI_GATE_REQUEST_ID,
        errors,
        label,
    )
    _require_equal(
        payload,
        ("effect_reconciliation", "source_ci_gate_evidence_ref"),
        EXPECTED_SOURCE_CI_GATE_EVIDENCE_REF,
        errors,
        label,
    )
    _require_equal(
    ...
    _require_equal(
        payload,
        ("effect_reconciliation", "required_evidence_ref"),
    ...
    _require_equal(
        payload,
        ("effect_reconciliation", "required_reconciliation_result"),
        "branch_pull_request_checks_and_merge_effects_reconciled_before_terminal_closure",
        errors,
        label,
    )
    ...
    _require_equal(
        payload,
        ("command_preview_ci_gate_evidence", "source_ci_gate_before_ready_for_review_witness_ref"),
        EXPECTED_SOURCE_CI_GATE_WITNESS_REF,
        errors,
        label,
    )
    _require_equal(
        payload,
        ("command_preview_ci_gate_evidence", "source_binding_id"),
    ...
    _require_equal(
        payload,
        ("command_preview_ci_gate_evidence", "source_requested_evidence_ref"),
        EXPECTED_SOURCE_CI_GATE_EVIDENCE_REF,
        errors,
        label,
    )
    _require_equal(
        payload,
        ("command_preview_ci_gate_evidence", "source_ci_gate_request_id"),
        EXPECTED_SOURCE_CI_GATE_REQUEST_ID,
        errors,
        label,
    )
    _require_equal(
        payload,
        ("command_preview_ci_gate_evidence", "source_command_preview_repository_effect_rollback_evidence_ref"),
        "command_preview_repository_effect_rollback_evidence",
        errors,
        label,
    )
    _require_equal(
        payload,
        ("command_preview_ci_gate_evidence", "source_repository_effect_rollback_plan_witness_id"),
    ...
    _require_equal(
        payload,
        ("command_preview_ci_gate_evidence", "source_repository_effect_rollback_plan_witness_ref"),
        EXPECTED_SOURCE_ROLLBACK_PLAN_WITNESS_REF,
        errors,
        label,
    )
    _require_equal(
        payload,
        ("command_preview_ci_gate_evidence", "source_uao_admission_witness_id"),
    ...
    _require_equal(
        payload,
        ("command_preview_ci_gate_evidence", "source_uao_admission_witness_ref"),
        EXPECTED_SOURCE_UAO_ADMISSION_WITNESS_REF,
        errors,
        label,
    )
    _require_equal(
        payload,
        ("command_preview_ci_gate_evidence", "source_branch_write_binding_id"),
    ...
    _require_equal(
        payload,
        ("command_preview_ci_gate_evidence", "source_branch_write_binding_ref"),
        EXPECTED_SOURCE_BRANCH_WRITE_BINDING_REF,
        errors,
        label,
    )
    ...
    if source_ci_gate_witness:
        _require_equal(
            ...
            _get_nested(source_ci_gate_witness, ("scope", "repository_slug")),
            errors,
            label,
        )
        _require_equal(
        ...
        source_ci_gate_evidence = _mapping(
            _get_nested(source_ci_gate_witness, ("command_preview_repository_effect_rollback_evidence",))
        )
        _require_equal(
            payload,
            ("effect_reconciliation", "command_preview_repository_effect_rollback_plan_witness_ref"),
        ...
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_binding_id"),
            source_ci_gate_witness.get("binding_id"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_requested_evidence_ref"),
            source_ci_gate_witness.get("requested_evidence_ref"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_ci_gate_request_id"),
            source_ci_gate.get("ci_gate_request_id"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_repository_effect_rollback_plan_witness_id"),
            source_ci_gate_evidence.get("source_binding_id"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_repository_effect_rollback_plan_witness_ref"),
            source_ci_gate_evidence.get("source_repository_effect_rollback_plan_witness_ref"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_uao_admission_witness_id"),
            source_ci_gate_evidence.get("source_uao_admission_witness_id"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_uao_admission_witness_ref"),
            source_ci_gate_evidence.get("source_uao_admission_witness_ref"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_branch_write_binding_id"),
            source_ci_gate_evidence.get("source_branch_write_binding_id"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_branch_write_binding_ref"),
            source_ci_gate_evidence.get("source_branch_write_binding_ref"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_command_preview_ref"),
            source_ci_gate.get("command_preview_ref"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_redacted_command_preview"),
            source_ci_gate.get("redacted_command_preview"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_argument_vector_template"),
            source_ci_gate.get("argument_vector_template"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_placeholder_refs"),
            source_ci_gate.get("placeholder_refs"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_actual_non_empty_diff_receipt_ref"),
            source_ci_gate.get("actual_non_empty_diff_receipt_ref"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_changed_file_refs"),
            source_ci_gate.get("changed_file_refs"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_diff_refs"),
            source_ci_gate.get("diff_refs"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_redacted_diff_bundle_ref"),
            source_ci_gate.get("redacted_diff_bundle_ref"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_redacted_output_ref"),
            source_ci_gate.get("redacted_output_ref"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_required_check_result"),
            source_ci_gate.get("required_check_result"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_ci_gate_before_ready_for_review_collected"),
            source_ci_gate.get("ci_gate_before_ready_for_review_collected"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_ready_for_review_authorized_after_ci_gate"),
            source_ci_gate.get("ready_for_review_authorized_after_ci_gate"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_pull_request_creation_authorized_after_ci_gate"),
            source_ci_gate.get("pull_request_creation_authorized_after_ci_gate"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_authority_granted"),
            source_ci_gate_witness.get("authority_granted"),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_ready_for_review_enabled"),
            _get_nested(source_ci_gate_witness, ("authority_denials", "ready_for_review_enabled")),
            errors,
            label,
        )
        _require_equal(
            payload,
            ("command_preview_ci_gate_evidence", "source_pull_request_creation_enabled"),
            _get_nested(source_ci_gate_witness, ("authority_denials", "pull_request_creation_enabled")),
            errors,
            label,
        )
    ...
    if not isinstance(observed_witnesses, list):
        ...
        errors.append(f"{label}: remaining_witnesses must be empty after effect reconciliation request")
...
def build_mutated_effect_reconciliation_witness(**updates: Any) -> dict[str, Any]:
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--source-ci-gate-witness-schema", type=Path, default=DEFAULT_SOURCE_CI_GATE_WITNESS_SCHEMA)
    parser.add_argument(
        "--source-ci-gate-witness-example",
        type=Path,
        action="append",
        dest="source_ci_gate_witness_examples",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    validation = validate_agentic_service_harness_github_pr_effect_reconciliation_witness(
        schema_path=args.schema,
        example_paths=tuple(args.examples) if args.examples else DEFAULT_EXAMPLES,
        source_ci_gate_witness_schema_path=args.source_ci_gate_witness_schema,
        source_ci_gate_witness_example_paths=(
            tuple(args.source_ci_gate_witness_examples)
            if args.source_ci_gate_witness_examples
            else DEFAULT_SOURCE_CI_GATE_WITNESS_EXAMPLES
        ),
    )
    write_github_pr_effect_reconciliation_witness_validation(validation, args.output)
    if args.json:
        print(json.dumps(validation.as_dict(), indent=2, sort_keys=True))
    elif validation.ok:
        print("AGENTIC SERVICE HARNESS GITHUB PR EFFECT RECONCILIATION WITNESS VALID")
    else:
