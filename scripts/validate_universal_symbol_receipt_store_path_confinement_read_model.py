#!/usr/bin/env python3
"""Validate the path confinement witness operator read model.

Purpose: prove the path confinement operator projection is read-only and
bounded while path confinement binding, filesystem escape, receipt-store append,
raw payload exposure, mutation, and terminal closure remain denied.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: path confinement read-model schema/example and path confinement
witness validator constants.
Invariants:
  - The read model is a projection, not path authority.
  - Every confinement requirement appears exactly once.
  - Raw detail visibility, filesystem escape, and authority remain denied.
...
from scripts.validate_universal_symbol_receipt_store_path_confinement_witness import (  # noqa: E402
...
DEFAULT_SCHEMA_PATH = (
    REPO_ROOT / "schemas" / "universal_symbol_receipt_store_path_confinement_read_model.schema.json"
)
DEFAULT_READ_MODEL_PATH = (
    REPO_ROOT / "examples" / "universal_symbol_receipt_store_path_confinement_read_model.foundation.json"
)

EXPECTED_WITNESS_REF = "examples/universal_symbol_receipt_store_path_confinement_witness.foundation.json"
EXPECTED_PATH_CONFINEMENT_DECISION = (
...
EXPECTED_OPERATOR_STATUS_SUMMARY: dict[str, str] = {
    "primary_status": "Path confinement blocked",
    "root_status": "Awaiting evidence",
    "escape_status": "Denied",
    "raw_detail_visibility": "hidden_by_default",
    "operator_action": "Collect root, namespace, traversal, and partition evidence",
}


class UniversalSymbolReceiptStorePathConfinementReadModelError(ValueError):
    """Raised when the path confinement read model violates Foundation Mode."""


def validate_universal_symbol_receipt_store_path_confinement_read_model(
    read_model_path: Path = DEFAULT_READ_MODEL_PATH,
    schema_path: Path = DEFAULT_SCHEMA_PATH,
) -> dict[str, Any]:
    """Validate schema shape, bounded rows, and denied path authority."""
    ...
    if errors:
        raise UniversalSymbolReceiptStorePathConfinementReadModelError("; ".join(errors))
    return report
...
def load_json_object(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise UniversalSymbolReceiptStorePathConfinementReadModelError(f"missing file: {path}") from exc
    except json.JSONDecodeError as exc:
        raise UniversalSymbolReceiptStorePathConfinementReadModelError(f"invalid json: {path}: {exc}") from exc
    if not isinstance(value, dict):
        raise UniversalSymbolReceiptStorePathConfinementReadModelError(f"expected object: {path}")
    return value


def _validate_schema_boundary(schema: Mapping[str, Any], errors: list[str]) -> None:
    expected_id = "urn:mullusi:schema:universal-symbol-receipt-store-path-confinement-read-model:1"
...
def _validate_read_model_boundary(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if read_model.get("read_model_is_not_path_authority") is not True:
        errors.append("read model must not be path authority")
...
def _validate_witness_projection(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if projection.get("path_confinement_decision") != EXPECTED_PATH_CONFINEMENT_DECISION:
        errors.append("witness_projection.path_confinement_decision must remain blocked")
    if projection.get("authority_granted") is not False:
        errors.append("witness_projection.authority_granted must remain false")
    if projection.get("confinement_requirement_count") != len(REQUIRED_REQUIREMENT_IDS):
        errors.append("witness_projection.confinement_requirement_count drift")
...
def _validate_source_witness_alignment(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if projection.get("witness_ref") != EXPECTED_WITNESS_REF:
        return
    try:
        witness = load_json_object(DEFAULT_PATH_CONFINEMENT_WITNESS_PATH)
    except UniversalSymbolReceiptStorePathConfinementReadModelError as exc:
        errors.append(f"source path confinement witness unavailable: {exc}")
        return

    witness_summary = _mapping(witness.get("contract_summary"))
    expected_counts = {
        "confinement_requirement_count": witness_summary.get("confinement_requirement_count"),
        "blocked_reason_count": witness_summary.get("blocked_reason_count"),
    }
    for field_name, expected_count in expected_counts.items():
        if projection.get(field_name) != expected_count:
            errors.append(f"witness_projection.{field_name} must match source path confinement witness")
    if projection.get("path_confinement_decision") != witness.get("path_confinement_decision"):
        errors.append("witness_projection.path_confinement_decision must match source path confinement witness")
...
def _validate_requirement_rows(read_model: Mapping[str, Any], errors: list[str]) -> None:
    ...
    for row in rows:
        ...
        if row.get("proof_state") not in {"Unknown", "BudgetUnknown", "Fail"}:
            errors.append(f"{row.get('requirement_id')}: proof_state must block path confinement")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print(f"[FAIL] universal_symbol_receipt_store_path_confinement_read_model: {exc}")
            print("STATUS: failed")
        return 1
    ...
    if args.json:
        ...
        print("[PASS] universal_symbol_receipt_store_path_confinement_read_model")
        print("STATUS: passed")
    return 0
