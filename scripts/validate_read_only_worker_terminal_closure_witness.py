#!/usr/bin/env python3
"""Validate the ReadOnlyWorkerTerminalClosureWitness contract.

Purpose: verify Foundation Mode witness evidence for future terminal closure
...
ReadOnlyWorkerReceiptAppendWitness validation.
Invariants:
  - Validation is read-only and deterministic.
  - The witness applies only to read_only_repo_inspection.
  - UAO dispatch authorization, terminal closure, runtime dispatch
  ...
  - Future terminal closure still requires receipt append, final judgment,
...
from scripts.validate_read_only_worker_receipt_append_witness import (  # noqa: E402
    DEFAULT_RECEIPT_PATH as DEFAULT_RECEIPT_APPEND_WITNESS_PATH,
    DEFAULT_SCHEMA_PATH as DEFAULT_RECEIPT_APPEND_WITNESS_SCHEMA_PATH,
    load_json_object as load_receipt_append_json_object,
...
DEFAULT_SCHEMA_PATH = (
    WORKSPACE_ROOT
    / "schemas"
    / "read_only_worker_terminal_closure_witness.schema.json"
)
DEFAULT_RECEIPT_PATH = (
    WORKSPACE_ROOT
    / "examples"
    / "read_only_worker_terminal_closure_witness.foundation.json"
)
EXPECTED_SCHEMA_ID = "urn:mullusi:schema:read-only-worker-terminal-closure-witness:1"
EXPECTED_SCHEMA_TITLE = "Read-Only Worker Terminal Closure Witness"
EXPECTED_RECEIPT_VERSION = "read_only_worker_terminal_closure_witness.v1"
EXPECTED_RECEIPT_APPEND_WITNESS_REF = (
...
EXPECTED_WITNESS_MODE = "TERMINAL_CLOSURE_WITNESS_ONLY"
EXPECTED_TARGET_TERMINAL_CLOSURE_REF = (
    "candidate://terminal-closure/read-only-repo-inspection"
)
EXPECTED_CLOSURE_PROFILE = (
    "LOCAL_READ_ONLY_REPO_INSPECTION_TERMINAL_CLOSURE"
)
EXPECTED_ADMISSION_DECISION = (
    "TERMINAL_CLOSURE_WITNESS_BLOCKED_AWAITING_CLOSURE_EVIDENCE"
)
...
REQUIRED_AUTHORIZATION_INPUT_REFS = (
    ...
    "evidence://terminal-closure/final-judgment",
    "evidence://terminal-closure/no-unexpected-effects",
    "evidence://terminal-closure/evidence-bundle",
    "evidence://terminal-closure/replay-verification",
...
REQUIRED_BLOCKED_REASON_REFS = (
    "blocked://receipt-append/not-proven",
    "blocked://terminal-closure/final-judgment-missing",
    "blocked://terminal-closure/unexpected-effects-not-ruled-out",
    "blocked://terminal-closure/evidence-bundle-missing",
    "blocked://terminal-closure/replay-verification-missing",
...
REQUIRED_RECEIPT_REFS = {
    ...
    "terminal_closure_certificate_schema": (
...
TRUE_ACTIVATION_FIELDS = (
    "upstream_receipt_append_witness_validated",
    "upstream_effect_reconciliation_witness_bound",
    "append_only_store_required",
    "receipt_digest_required",
    "idempotency_key_required",
    "final_judgment_required",
    "no_unexpected_effects_required",
...
class ReadOnlyWorkerTerminalClosureWitnessError(ValueError):
...
def load_json_object(json_path: Path, label: str) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ReadOnlyWorkerTerminalClosureWitnessError(
...
def _validate_default_upstream_witnesses() -> tuple[str, ...]:
    ...
    receipt_append_payload = load_receipt_append_json_object(
        DEFAULT_RECEIPT_APPEND_WITNESS_PATH,
        "ReadOnlyWorkerReceiptAppendWitness",
    )
    receipt_append_schema = _load_schema(
        DEFAULT_RECEIPT_APPEND_WITNESS_SCHEMA_PATH
    )
    upstream_errors.extend(
        f"receipt_append_witness: {error}"
        for error in validate_receipt_append_witness_record(
            receipt_append_payload,
            receipt_append_schema,
        )
    )
    return tuple(upstream_errors)


def validate_terminal_closure_witness_record(
    record: Any,
    schema: dict[str, Any] | None = None,
    receipt_append_witness: dict[str, Any] | None = None,
) -> list[str]:
    ...
    if not isinstance(record, dict):
        errors.append("ReadOnlyWorkerTerminalClosureWitness must be object")
        return errors

    errors.extend(_validate_schema_instance(schema_payload, record))

    if receipt_append_witness is None:
        errors.extend(_validate_default_upstream_witnesses())
    else:
        receipt_append_schema = _load_schema(DEFAULT_RECEIPT_APPEND_WITNESS_SCHEMA_PATH)
        errors.extend(
            f"receipt_append_witness: {error}"
            for error in validate_receipt_append_witness_record(
                receipt_append_witness,
                receipt_append_schema,
            )
        )
    ...
    if record.get("receipt_version") != EXPECTED_RECEIPT_VERSION:
        errors.append(
            "receipt_version must match read_only_worker_terminal_closure_witness.v1"
        )
    if record.get("receipt_append_witness_ref") != (
        EXPECTED_RECEIPT_APPEND_WITNESS_REF
    ):
        errors.append("receipt_append_witness_ref is invalid")
    ...
    if not isinstance(authority, dict):
        ...
        if authority.get("terminal_closure_witness_defined") is not True:
            errors.append(
                "authority_scope.terminal_closure_witness_defined must be true"
            )
    ...
    contract = record.get("terminal_closure_contract")
    if not isinstance(contract, dict):
        errors.append("terminal_closure_contract must be an object")
    else:
        if contract.get("worker_id") != EXPECTED_WORKER_ID:
            errors.append("terminal_closure_contract.worker_id is invalid")
        if contract.get("capability") != EXPECTED_WORKER_PATH:
            errors.append("terminal_closure_contract.capability is invalid")
        if contract.get("operation_family") != EXPECTED_OPERATION_FAMILY:
            errors.append("terminal_closure_contract.operation_family is invalid")
        ...
        if contract.get("source_receipt_append_witness_ref") != (
            EXPECTED_RECEIPT_APPEND_WITNESS_REF
        ):
            errors.append(
                "terminal_closure_contract."
                "source_receipt_append_witness_ref is invalid"
            )
        ...
        if contract.get("target_terminal_closure_ref") != (
            EXPECTED_TARGET_TERMINAL_CLOSURE_REF
        ):
            errors.append(
                "terminal_closure_contract."
                "target_terminal_closure_ref is invalid"
            )
        if contract.get("closure_profile") != EXPECTED_CLOSURE_PROFILE:
            errors.append(
                "terminal_closure_contract.closure_profile is invalid"
            )
        _require_refs(
            contract.get("required_source_receipt_refs"),
            REQUIRED_SOURCE_RECEIPT_REFS,
            "terminal_closure_contract.required_source_receipt_refs",
            errors,
        )
        _require_refs(
            contract.get("required_authorization_input_refs"),
            REQUIRED_AUTHORIZATION_INPUT_REFS,
            "terminal_closure_contract.required_authorization_input_refs",
            errors,
        )
        _require_refs(
            contract.get("validation_refs"),
            REQUIRED_VALIDATION_REFS,
            "terminal_closure_contract.validation_refs",
            errors,
        )
    ...
    if not isinstance(decision, dict):
        ...
        if decision.get("terminal_closure_witness_defined") is not True:
            errors.append(
                "admission_decision.terminal_closure_witness_defined must be true"
            )
        _require_refs(
...
def validate_terminal_closure_witness(
    ...
    receipt = load_json_object(receipt_path, "ReadOnlyWorkerTerminalClosureWitness")
    return validate_terminal_closure_witness_record(receipt, schema)


def build_mutated_terminal_closure_witness(**updates: Any) -> dict[str, Any]:
...
def main(argv: list[str] | None = None) -> int:
    """Validate ReadOnlyWorkerTerminalClosureWitness artifacts."""

    parser = argparse.ArgumentParser(
        description="Validate ReadOnlyWorkerTerminalClosureWitness contract."
    )
    ...
    errors = validate_terminal_closure_witness(args.schema, args.receipt)
    if args.json:
        payload = {
            ...
            "errors": errors,
            "receipt": {
                "receipt_id": "read_only_worker_terminal_closure_witness_validation",
        ...
        print("[FAIL] read_only_worker_terminal_closure_witness", file=sys.stderr)
        for error in errors:
            print(f"  - {error}", file=sys.stderr)
    else:
        print("[PASS] read_only_worker_terminal_closure_witness")
