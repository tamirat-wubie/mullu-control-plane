#!/usr/bin/env python3
"""Validate Agentic Service Harness live producer execution authority evidence packet.

Purpose: bind execution authority evidence to the admission gate while keeping
live producer execution authority AwaitingEvidence and denied.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: execution authority evidence packet schema, fixture,
scripts.validate_agentic_service_harness_live_producer_execution_authority_admission,
and scripts.validate_schemas.
Invariants: no live execution, connector call, receipt append, runtime write,
secret access, provider call, mutation route, or terminal closure is authorized.
"""
...
DEFAULT_SCHEMA = (
    REPO_ROOT
    / "schemas"
    / "agentic_service_harness_live_producer_execution_authority_evidence_packet.schema.json"
)
DEFAULT_FIXTURE = (
    REPO_ROOT
    / "examples"
...
PACKET_ID = "agentic-service-harness-live-producer-execution-authority-evidence-packet"
...
ALLOWED_SECRET_KEYS = {"secret_handoff_packet_ref", "secret_mutation_enabled", "secret_values_allowed"}
...
FORBIDDEN_LIVE_CLAIM = re.compile(
    r"\b(?:live_execution_authorized|authority_granted|live_producer_implemented|"
...
class LiveProducerExecutionAuthorityEvidencePacketValidation:
    """Validation result for the live producer execution authority evidence packet."""
...
def validate_live_producer_execution_authority_evidence_packet(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    fixture_path: Path = DEFAULT_FIXTURE,
) -> tuple[LiveProducerExecutionAuthorityEvidencePacketValidation, dict[str, Any]]:
    """Validate the checked-in live producer execution authority evidence packet."""
    errors: list[str] = []
    schema = _load_json_object(schema_path, "execution authority evidence packet schema", errors)
    fixture = _load_json_object(fixture_path, "execution authority evidence packet fixture", errors)
    admission_validation, _admission = validate_live_producer_execution_authority_admission()
    errors.extend(f"source execution authority admission: {error}" for error in admission_validation.errors)
    if admission_validation.admission_status != ADMISSION_STATUS:
        errors.append("source execution authority admission must remain blocked AwaitingEvidence")
    ...
    validation = LiveProducerExecutionAuthorityEvidencePacketValidation(
        ok=not errors,
        errors=tuple(errors),
...
def _validate_packet_semantics(packet: Mapping[str, Any], errors: list[str], label: str) -> None:
    ...
    if packet.get("source_admission_ref") != (
        "examples/agentic_service_harness_live_producer_execution_authority_admission.awaiting_evidence.json"
    ):
        errors.append(f"{label}: source_admission_ref mismatch")
    _validate_source_binding(packet, errors, label)
    _validate_evidence_collection(packet.get("required_evidence"), "evidence_id", True, errors, label)
    _validate_evidence_collection(packet.get("missing_evidence"), "evidence_id", False, errors, label)
    _validate_authority_denials(_mapping(packet.get("authority_denials")), errors, label)
    _validate_effect_boundary(_mapping(packet.get("effect_boundary")), errors, label)
...
def _validate_source_binding(packet: Mapping[str, Any], errors: list[str], label: str) -> None:
    binding = _mapping(packet.get("source_admission_binding"))
    if not binding:
        errors.append(f"{label}: source_admission_binding must be an object")
        return
    for field_name, expected in (
        ("source_admission_id", "agentic-service-harness-live-producer-execution-authority-admission"),
        (
        ...
        (
            ...
            "python scripts/validate_agentic_service_harness_live_producer_execution_authority_admission.py",
        ),
        ("source_status", ADMISSION_STATUS),
        ("source_blocks_live_execution", True),
        ("source_authority_granted", False),
    ):
        if binding.get(field_name) != expected:
            errors.append(f"{label}: source_admission_binding.{field_name} must be {expected!r}")


def _validate_evidence_collection(
    value: Any,
    id_key: str,
    require_future_ref: bool,
    errors: list[str],
    label: str,
) -> None:
    ...
    if observed_ids != REQUIRED_EVIDENCE_IDS:
        errors.append(f"{label}: {id_key} order mismatch")
        missing_ids = [evidence_id for evidence_id in REQUIRED_EVIDENCE_IDS if evidence_id not in observed_ids]
        if missing_ids:
            errors.append(f"{label}: missing {id_key} values: {', '.join(missing_ids)}")
    for item in value:
        if not isinstance(item, Mapping):
            errors.append(f"{label}: {id_key} entries must be objects")
            continue
        evidence_id = str(item.get(id_key, ""))
        ...
        if require_future_ref and not str(item.get("required_ref", "")).startswith("future://"):
            errors.append(f"{label}: {evidence_id} required_ref must stay future://")


def _validate_authority_denials(value: Mapping[str, Any], errors: list[str], label: str) -> None:
    if not value:
        errors.append(f"{label}: authority_denials must be an object")
        return
    for flag in FALSE_AUTHORITY_FLAGS:
        if value.get(flag) is not False:
            errors.append(f"{label}: authority_denials.{flag} must be false")
...
def main(argv: list[str] | None = None) -> int:
    """Run the live producer execution authority evidence packet validator."""
    ...
    validation, _fixture = validate_live_producer_execution_authority_evidence_packet(
    ...
    if args.json:
        ...
        print("AGENTIC SERVICE HARNESS LIVE PRODUCER EXECUTION AUTHORITY EVIDENCE PACKET VALID")
    else:
