#!/usr/bin/env python3
"""Validate Agentic Service Harness live producer rollback proof packet.

Purpose: define the rollback proof witness packet while keeping rollback proof,
rollback plan, state snapshot, inverse delta, replay receipt, recovery verification,
and secret handoff linkage AwaitingEvidence.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: schemas/agentic_service_harness_live_producer_rollback_proof_packet.schema.json,
examples/agentic_service_harness_live_producer_rollback_proof_packet.awaiting_evidence.json,
scripts.validate_agentic_service_harness_live_producer_rollback_proof_preflight,
and scripts.validate_schemas.
Invariants:
  - Packet remains AwaitingEvidence and non-terminal.
  - Required rollback component refs remain future refs.
  - Rollback execution, recovery verification, destructive operations, mutation routes,
    credential access, and live execution fail closed.
"""
...
DEFAULT_SCHEMA = REPO_ROOT / "schemas" / "agentic_service_harness_live_producer_rollback_proof_packet.schema.json"
DEFAULT_FIXTURE = (
    REPO_ROOT / "examples" / "agentic_service_harness_live_producer_rollback_proof_packet.awaiting_evidence.json"
)
PACKET_ID = "agentic-service-harness-live-producer-rollback-proof-packet"
...
FORBIDDEN_LIVE_CLAIM = re.compile(
    r"\b(?:rollback_executed|recovery_verified|destructive_operation_enabled|"
    r"secret_values_present|secret_values_read|secret_values_serialized|secret_values_printed|"
    r"external_adapter_integrated|adapter_credentials_present|adapter_credentials_serialized|"
    r"live_producer_implemented|live_effect_observed|effect_receipt_collected|"
    r"live_execution_authorized)=true\b",
    re.IGNORECASE,
)


@dataclass(frozen=True, slots=True)
class LiveProducerRollbackProofPacketValidation:
    """Validation result for the live producer rollback proof packet."""
...
def validate_live_producer_rollback_proof_packet(
    *,
    schema_path: Path = DEFAULT_SCHEMA,
    fixture_path: Path = DEFAULT_FIXTURE,
) -> tuple[LiveProducerRollbackProofPacketValidation, dict[str, Any]]:
    """Validate the checked-in live producer rollback proof packet."""
    errors: list[str] = []
    schema = _load_json_object(schema_path, "rollback proof packet schema", errors)
    fixture = _load_json_object(fixture_path, "rollback proof packet fixture", errors)
    source_validation = validate_live_producer_rollback_proof_preflight()
    errors.extend(f"source rollback proof preflight: {error}" for error in source_validation.errors)
    if getattr(source_validation, "rollback_proof_status", "") != "AwaitingEvidence":
        errors.append("source rollback proof preflight must remain AwaitingEvidence")
    ...
    validation = LiveProducerRollbackProofPacketValidation(
        ok=not errors,
        errors=tuple(errors),
...
def _validate_packet_semantics(packet: Mapping[str, Any], errors: list[str], label: str) -> None:
    for field_name, expected_value in (
        ...
        ("packet_status", "blocked_awaiting_rollback_proof_components"),
    ...
    if packet.get("source_preflight_ref") != "examples/agentic_service_harness_live_producer_rollback_proof_preflight.local.json":
...
def _validate_validator_ref(packet: Mapping[str, Any], errors: list[str], label: str) -> None:
    ...
    if validator.get("command") != "python scripts/validate_agentic_service_harness_live_producer_rollback_proof_packet.py":
...
def main(argv: list[str] | None = None) -> int:
    """Run the live producer rollback proof packet validator."""
    args = build_arg_parser().parse_args(argv)
    validation, fixture = validate_live_producer_rollback_proof_packet(
    ...
    if args.json:
        ...
        print("AGENTIC SERVICE HARNESS LIVE PRODUCER ROLLBACK PROOF PACKET VALID")
    else:
        print(
            "AGENTIC SERVICE HARNESS LIVE PRODUCER ROLLBACK PROOF PACKET INVALID "
