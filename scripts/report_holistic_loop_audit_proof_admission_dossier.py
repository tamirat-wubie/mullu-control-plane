#!/usr/bin/env python3
"""Report the audit/proof holistic loop admission dossier.

Purpose: expose a read-only admission dossier for the Audit Proof Verification
loop candidate and its registry admission state.
...
repository-local audit, proof, trust-ledger, and anchor verification surfaces.
Invariants:
  - The dossier is read-only and deterministic.
  - The dossier does not mutate the audit proof verification loop registration.
  - The dossier does not execute or rewrite audit/proof verification behavior.
...
DOSSIER_ID = "holistic_loop_audit_proof_admission_dossier"
DOSSIER_VERSION = "holistic_loop_audit_proof_admission_dossier.v1"
CANDIDATE_ID = "audit_proof_verification_loop"
ADMISSION_STATUS_READY = "ready_for_operator_decision"


def _audit_proof_candidate() -> Any:
...
def _proposed_manifest(*, registered: bool) -> dict[str, Any]:
    candidate = _audit_proof_candidate()
...
def build_dossier() -> dict[str, Any]:
    """Build the read-only audit/proof admission dossier."""
    ...
    return {
        ...
        "operator_decision_report": {
            ...
            "decision_ref": (
                "default_registry:audit_proof_verification_loop"
                if registered
                else ""
            ),
        ...
        "learning_policy_readiness": {
            ...
            "learning_candidates": [
                "audit_verification_gaps",
                "proof_verification_gaps",
                "anchor_verification_report_failures",
            ],
        },
    }
...
def validate_dossier(dossier: dict[str, Any] | None = None) -> list[str]:
    """Return validation errors for the audit/proof admission dossier."""
    ...
    if report.get("candidate_id") != CANDIDATE_ID:
        errors.append("candidate_id must be audit_proof_verification_loop")
    registered_loop_ids = set(build_default_loop_registry().manifests)
...
def main(argv: list[str] | None = None) -> int:
    """Emit the audit/proof admission dossier."""

    parser = argparse.ArgumentParser(description="Report the audit/proof holistic loop admission dossier.")
