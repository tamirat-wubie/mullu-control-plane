"""Purpose: redact sensitive field values from governed browser observations.
Governance scope: masking of secret-bearing element values and page text only.
...
Invariants:
  ...
  - Selector identity, status, tags, url, and title are preserved unchanged.
...
# Keyword fragments that mark a selector (or its description) as secret-bearing.
# Matched case-insensitively as substrings so that `#password`, `login_pwd`,
# `name=otp-code`, and `Credit Card Number` all classify as sensitive.
...
def redact_page(
    ...
    *,
    ...
    """Return a page with secret-bearing element values and text masked."""
