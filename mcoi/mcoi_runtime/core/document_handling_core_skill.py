...
DOCUMENT_SYMBOLIC_STATE = "𝕊_doc := ⟨Ι_doc, Λ_doc, Σ_doc, Γ_doc, H_doc⟩"
DOCUMENT_STATE_JUDGMENT = "Σ_doc ⊨ Λ_doc over Ι_doc"
    ...
    return SkillDescriptor(
        ...
        preconditions=(
            SkillPrecondition(
                ...
                description="document workflow policy permits governed handling-core selection",
            ),
            SkillPrecondition(
                condition_id="document.core.capability_available",
                condition_type=PreconditionType.CAPABILITY_AVAILABLE,
                description="document worker capability family is admitted in the governed registry",
            ),
            SkillPrecondition(
                condition_id="document.core.source_immutable",
                condition_type=PreconditionType.STATE_CHECK,
                description="source bytes and source hash are present before extraction or mutation",
            ),
            SkillPrecondition(
                condition_id="document.core.write_approval_expected",
                condition_type=PreconditionType.POLICY_ALLOWS,
                description="write-capable edit, redaction, conversion, or publication requires explicit approval",
            ),
        ),
        postconditions=(
            SkillPostcondition(
                ...
                description="terminal receipt carries the original source hash and derived artifact hash",
            ),
            SkillPostcondition(
                condition_id="document.handling_core.v1.validation_passed",
                condition_type=PostconditionType.VERIFICATION_PASSED,
                description="all required extraction, patch, layout, privacy, and audit gates have explicit results",
            ),
            SkillPostcondition(
                condition_id="document.handling_core.v1.audit_history_recorded",
                condition_type=PostconditionType.FILE_EXISTS,
                description="causal history records ingest, inference, patch, validation, and exposure decisions",
            ),
        ),
        steps=(
            SkillStep(
            ...
            SkillStep(
                ...
                output_keys=("document_graph_ref", "extraction_coverage", "extractor_warnings"),
                provider_class_required=DOCUMENT_HANDLING_CORE_PROVIDER,
            ),
            SkillStep(
            ...
            SkillStep(
                ...
                output_keys=("patch_plan_ref", "target_resolution_report", "rollback_plan_ref"),
                provider_class_required=DOCUMENT_HANDLING_CORE_PROVIDER,
            ),
            SkillStep(
            ...
            SkillStep(
                ...
                output_keys=("validation_report_ref", "privacy_report_ref", "render_report_ref"),
                provider_class_required=DOCUMENT_HANDLING_CORE_PROVIDER,
            ),
            SkillStep(
        ...
        description=(
            ...
            "approved transformations, and closes with privacy, render, and audit evidence."
        ),
        confidence=0.25,
        metadata={
...
def validate_document_handling_core_descriptor(descriptor: SkillDescriptor | None = None) -> None:
    ...
    for step in candidate.steps:
        if step.action_type in PROHIBITED_ACTION_TYPES:
            msg = f"document handling core contains prohibited action type: {step.action_type}"
            raise ValueError(msg)
    ...
    missing_gates = set(DOCUMENT_VALIDATION_GATES) - set(candidate.metadata["validation_gates"])
    if missing_gates:
