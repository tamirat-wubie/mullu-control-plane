...
Governance scope: evidence typing only.
Invariants:
...
class NestedMindReceiptBridgeReport(ContractRecord):
    ...
    def __post_init__(self) -> None:
        for field_name in ("report_id", "proposal_evidence_id", "commit_witness_id"):
            object.__setattr__(self, field_name, require_non_empty_text(getattr(self, field_name), field_name))
        object.__setattr__(self, "status", _enum(NestedMindBridgeStatus, self.status, "status"))
        ...
        if self.status is NestedMindBridgeStatus.BRIDGED and blockers:
            raise ValueError("bridged report must not contain blockers")
        if self.status is NestedMindBridgeStatus.BLOCKED and not blockers:
        ...
        if self.bridge_hash != bridge_report_hash(self):
