...
def bootstrap_operational_services(
    *,
    ...
    prom_exporter.register_gauge("health_score", "Platform health score as a unit ratio")
    prom_exporter.register_gauge("uptime_seconds", "Platform uptime in seconds")
    prom_exporter.register_counter("llm_requests_total", "Total LLM requests")
    prom_exporter.register_gauge("llm_latency_p99_seconds", "Observed LLM p99 latency in seconds")
    prom_exporter.register_counter("llm_tokens_total", "Total LLM input and output tokens")
    prom_exporter.register_gauge("llm_budget_utilization_ratio", "LLM budget utilization as a unit ratio")
    prom_exporter.register_counter("policy_violations_total", "Total policy violations")
    prom_exporter.register_counter("audit_events_total", "Total audit events")
    prom_exporter.register_gauge("circuit_breaker_open", "Circuit breaker state (1=open, 0=closed)")
    prom_exporter.register_gauge("active_agents", "Active agent count")
    prom_exporter.register_counter("tasks_completed_total", "Total completed tasks")
    prom_exporter.register_gauge("chain_success_rate", "Governance chain success rate as a unit ratio")
    prom_exporter.register_counter("memory_ops_total", "Total memory operations")
