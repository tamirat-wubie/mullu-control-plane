"""Phase 199 — Governed HTTP Server (FastAPI).

Purpose: HTTP boundary for the governed platform. All requests enter governed execution.
    Phase 199 adds: LLM completion endpoint (governed, budgeted), certification endpoint,
    persistence-backed ledger, and budget reporting.
Dependencies: fastapi, production_surface, llm_integration, live_path_certification.
...
llm_bridge = LLMIntegrationBridge(
    clock=_clock,
...
# Default budget for development
llm_bridge.register_budget(LLMBudget(budget_id="default", tenant_id="system", max_cost=100.0))
