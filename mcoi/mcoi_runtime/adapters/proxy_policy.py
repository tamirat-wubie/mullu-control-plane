"""Runtime proxy usage policy.

Purpose: bound process-level HTTP proxy environment variables before MCOI
    runtime network transports can use them implicitly.
Governance scope: provider adapters, governed HTTP connector egress, proxy
    credential redaction, and production fail-closed behavior.
...
def assert_proxy_environment_allowed(
    *,
    ...
    """Fail closed when production-like runtimes expose proxy env variables."""
    ...
    if effective_policy.fail_closed and unapproved:
        ...
        raise RuntimeError(f"proxy environment blocked:{joined_names}")
