"""Purpose: verifier execution operator decision-value record preflight.
Governance scope: no-effect preflight after decision-value record-path
projection, without collecting, storing, admitting, or executing an operator
value.
Dependencies: personal-assistant verifier execution decision-value record-path
runtime and contracts.
Invariants:
  - The value-record creation preflight is projected but not admitted.
  - The source record path remains unadmitted.
...
from .operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_path import (
...
DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_PREFLIGHT_ID = (
    "pa_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight_foundation_001"
)
DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_PREFLIGHT_GENERATED_AT = (
    "2026-06-14T01:35:00+00:00"
)

_RECORD_PREFLIGHT_ID_PATTERN = re.compile(
    r"^pa_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight_[a-z0-9][a-z0-9_:-]*$"
)
_ITEM_ID_PATTERN = re.compile(
    r"^pa_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight_item_[a-z0-9][a-z0-9_:-]*$"
)
...
_FALSE_RECORD_FLAGS = {
    "record_preflight_admitted": False,
...
_SOURCE_FALSE_RECORD_FLAGS = {
    field_name: False for field_name in _FALSE_RECORD_FLAGS if field_name != "record_preflight_admitted"
}
_EFFECT_BOUNDARY = {
    "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight_allowed": True,
    "decision_value_record_path_ref_binding_allowed": True,
    "operator_decision_value_record_preflight_projection_allowed": True,
    "record_paths_present": True,
...
_PRIVATE_PAYLOAD_POLICY = {
    ...
    "record_path_projection": "ref_only",
...
def build_default_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight(
    *,
    generated_at: str = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_PREFLIGHT_GENERATED_AT,
    record_preflight_id: str = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_PREFLIGHT_ID,
) -> dict[str, Any]:
    """Build deterministic no-effect verifier execution decision-value record preflight."""

    return build_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight_envelope(
        generated_at=generated_at,
        record_preflight_id=record_preflight_id,
        operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_path=(
            build_default_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_path()
        ),
    )


def build_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight_envelope(
    *,
    generated_at: str,
    operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_path: Mapping[str, Any],
    record_preflight_id: str = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_PREFLIGHT_ID,
) -> dict[str, Any]:
    """Build blocked value-record preflight packet from record-path evidence."""

    timestamp = _require_non_empty_text(generated_at, "generated_at")
    envelope_id = _require_pattern(record_preflight_id, "record_preflight_id", _RECORD_PREFLIGHT_ID_PATTERN)
    source_envelope = _require_mapping(
        operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_path,
        "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_path",
    )
    _scan_private_or_secret_payload(source_envelope, path="operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_path")
    _assert_record_path_boundary(source_envelope)
    source_record_path_id = _require_non_empty_text(source_envelope.get("record_path_id"), "record_path_id")
    source_records = _require_sequence(source_envelope.get("record_paths"), "record_paths")
    ...
    for source_record in source_records:
        if not isinstance(source_record, Mapping):
            raise PersonalAssistantInvariantError("decision-value record preflight source record must be a mapping")
        _assert_record_path_item_boundary(source_record)
        record = _record_preflight_item(source_record_path_id, source_record, timestamp=timestamp)
        if record["record_preflight_item_id"] in item_ids:
            raise PersonalAssistantInvariantError(f"duplicate record_preflight_item_id {record['record_preflight_item_id']}")
        item_ids.append(record["record_preflight_item_id"])
        receipt_ids.append(record["receipt"]["receipt_id"])
        records.append(record)
    if not records:
        raise PersonalAssistantInvariantError("decision-value record preflight requires at least one record path")

    envelope = {
        "record_preflight_id": envelope_id,
        "generated_at": timestamp,
        "governed": True,
        "source_projection": "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_path",
        "source_record_path_id": source_record_path_id,
        "record_preflight_state": "operator_decision_value_record_preflight_blocked_awaiting_actual_operator_value",
        "decision": "blocked",
        "outcome": "AwaitingEvidence",
        "record_preflight_count": len(records),
        "record_preflight_item_ids": item_ids,
        "receipt_ids": receipt_ids,
        "record_preflights": records,
        "effect_boundary": dict(_EFFECT_BOUNDARY),
        "private_payload_policy": dict(_PRIVATE_PAYLOAD_POLICY),
        "summary": _summary(records),
        "assurance": {
            "assurance_id": "personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight_no_effect_assurance",
            "outcome": "AwaitingEvidence",
            "foundation_only": True,
            "record_preflight_only": True,
            "record_contract_ready": True,
            "operator_decision_required": True,
            "operator_decision_value_required": True,
            "actual_operator_decision_value_required": True,
            ...
            "next_action": "collect a governed explicit operator decision value before admitting the value-record preflight",
        },
        "metadata": {
            "foundation_only": True,
            "projection_contract": "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight",
            "runtime_boundary": "operator_decision_value_record_preflight_blocked_awaiting_actual_operator_value",
            "record_preflight_only": True,
...
def _record_preflight_item(source_record_path_id: str, source_record: Mapping[str, Any], *, timestamp: str) -> dict[str, Any]:
    source_item_id = _require_non_empty_text(source_record.get("record_path_item_id"), "record_path_item_id")
    ...
    item_id = _require_pattern(
        f"pa_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight_item_{evidence_kind}_{requirement_kind}_{suffix}",
        "record_preflight_item_id",
        _ITEM_ID_PATTERN,
    )
    return {
        "record_preflight_item_id": item_id,
        "source_record_path_item_id": source_item_id,
        ...
        "record_path_ref": {
            "source_record_path_id": source_record_path_id,
            "source_record_path_item_id": source_item_id,
            "source_record_path_state": "operator_decision_value_record_path_ready_blocked_awaiting_explicit_operator_value",
            "source_outcome": "AwaitingEvidence",
            "source_record_contract_ready": True,
            "source_record_path_created": True,
        ...
        "record_preflight": {
            ...
            **dict(_FALSE_RECORD_FLAGS),
            "blocking_reason": "actual_operator_decision_value_absent_record_path_unadmitted",
        },
        "authority_status": _authority_status(),
        "receipt": _record_preflight_receipt(
            receipt_id=f"pa_receipt_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight_{evidence_kind}_{requirement_kind}_{suffix}",
            request_id=request_id,
            skill_id=skill_id,
            risk_level=risk_level,
            approval_id=approval_id,
...
def _record_preflight_receipt(
    *,
    ...
    return {
        ...
        "inputs_used": [
            "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_path",
            "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight_policy",
        ],
        ...
        "actions_taken": [
            f"{evidence_kind}_{requirement_kind}_decision_value_record_preflight_projected",
            "operator_decision_value_record_creation_preflight_checked",
            "decision_value_record_preflight_receipt_created",
        ],
        "actions_not_taken": [
            "record_preflight_not_admitted",
            "record_path_not_admitted",
            "collection_gate_not_satisfied",
        ...
        "timestamp": timestamp,
        "evidence_refs": [
            f"proof://personal-assistant/operator-reapproval-decision-receipt-value-binding-record-verifier-execution-decision-value-record-preflight/{approval_id}/{evidence_kind}/{requirement_kind}"
        ],
        "memory_observation_refs": [],
        "replay_refs": [
            f"replay://personal-assistant/operator-reapproval-decision-receipt-value-binding-record-verifier-execution-decision-value-record-preflight/{approval_id}/{evidence_kind}/{requirement_kind}"
        ],
        "outcome": "AwaitingEvidence",
        "metadata": {
            "foundation_only": True,
            "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_preflight_is_execution": False,
            "evidence_kind": evidence_kind,
            "requirement_kind": requirement_kind,
            "record_preflight_only": True,
            "record_contract_ready": True,
...
def _summary(records: Sequence[Mapping[str, Any]]) -> dict[str, int]:
    preflights = [_require_mapping(record.get("record_preflight"), "record_preflight") for record in records]
    authorities = [_require_mapping(record.get("authority_status"), "authority_status") for record in records]
    return {
        "record_preflight_count": len(records),
        "record_contract_ready_count": sum(1 for record in preflights if record.get("record_contract_ready") is True),
        "operator_decision_required_count": sum(1 for record in preflights if record.get("operator_decision_required") is True),
        "operator_decision_value_required_count": sum(1 for record in preflights if record.get("operator_decision_value_required") is True),
        "actual_operator_decision_value_required_count": sum(1 for record in preflights if record.get("actual_operator_decision_value_required") is True),
        "record_preflight_creation_count": sum(1 for record in preflights if record.get("record_preflight_created") is True),
        "record_preflight_admission_count": sum(1 for record in preflights if record.get("record_preflight_admitted") is True),
        "record_path_admission_count": sum(1 for record in preflights if record.get("record_path_admitted") is True),
        "collection_gate_satisfied_count": sum(1 for record in preflights if record.get("collection_gate_satisfied") is True),
        "operator_value_record_creation_count": sum(1 for record in preflights if record.get("operator_value_record_created") is True),
        "operator_value_record_admission_count": sum(1 for record in preflights if record.get("operator_value_record_admitted") is True),
        "operator_decision_value_storage_count": sum(1 for record in preflights if record.get("operator_decision_value_stored") is True),
        ...
        "operator_decision_value_admitted_count": sum(1 for record in preflights if record.get("operator_decision_value_admitted") is True),
...
def _assert_record_path_boundary(source_envelope: Mapping[str, Any]) -> None:
    ...
    for field_name in (
        ...
        if effect_boundary.get(field_name) is not True:
            raise PersonalAssistantInvariantError(f"decision-value record path effect_boundary.{field_name} must be true")
    for field_name, expected_value in _SOURCE_FALSE_RECORD_FLAGS.items():
        if field_name in effect_boundary and effect_boundary.get(field_name) is not expected_value:
            raise PersonalAssistantInvariantError(f"decision-value record path effect_boundary.{field_name} must be false")
    if source_envelope.get("record_path_state") != "operator_decision_value_record_path_ready_blocked_awaiting_explicit_operator_value":
        raise PersonalAssistantInvariantError("decision-value record path must remain blocked awaiting explicit value")
    if source_envelope.get("decision") != "blocked" or source_envelope.get("outcome") != "AwaitingEvidence":
        raise PersonalAssistantInvariantError("decision-value record path must remain blocked AwaitingEvidence")


def _assert_record_path_item_boundary(source_record: Mapping[str, Any]) -> None:
    path = _require_mapping(source_record.get("record_path"), "record_path")
    for field_name in (
        "record_contract_ready",
        "record_path_created",
        "operator_decision_required",
        ...
        if path.get(field_name) is not True:
            raise PersonalAssistantInvariantError(f"decision-value record path record_path.{field_name} must be true")
    if tuple(path.get("accepted_record_kinds", ())) != _ACCEPTED_RECORD_KINDS:
        raise PersonalAssistantInvariantError("decision-value record path accepted_record_kinds drifted")
    if tuple(path.get("rejected_input_kinds", ())) != _REJECTED_INPUT_KINDS:
        raise PersonalAssistantInvariantError("decision-value record path rejected_input_kinds drifted")
    if tuple(path.get("required_fields", ())) != _REQUIRED_FIELDS:
        raise PersonalAssistantInvariantError("decision-value record path required_fields drifted")
    for field_name, expected_value in _SOURCE_FALSE_RECORD_FLAGS.items():
        if field_name in path and path.get(field_name) is not expected_value:
            raise PersonalAssistantInvariantError(f"decision-value record path record_path.{field_name} must be false")
    ...
    for field_name in (
        ...
        if authority_status.get(field_name) is not False:
            raise PersonalAssistantInvariantError(f"decision-value record path authority_status.{field_name} must be false")
