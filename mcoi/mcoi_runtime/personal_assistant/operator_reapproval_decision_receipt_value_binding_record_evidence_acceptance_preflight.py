"""Purpose: operator value-binding submitted evidence acceptance preflight.
...
binding record submitted evidence ref intake runtime and contracts.
Invariants:
  - Submitted evidence refs may be checked for governed presence only.
  ...
  - Evidence is not accepted, rejected, bound, admitted, dispatched, or used to
    grant connector, memory, deployment, or public-readiness authority.
"""
...
DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_EVIDENCE_ACCEPTANCE_PREFLIGHT_ID = (
    "pa_operator_reapproval_decision_receipt_value_binding_record_evidence_acceptance_preflight_foundation_001"
)
DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_EVIDENCE_ACCEPTANCE_PREFLIGHT_GENERATED_AT = (
    "2026-06-14T00:25:00+00:00"
)

_PREFLIGHT_ID_PATTERN = re.compile(
    r"^pa_operator_reapproval_decision_receipt_value_binding_record_evidence_acceptance_preflight_[a-z0-9][a-z0-9_:-]*$"
)
_EVALUATION_ID_PATTERN = re.compile(
    r"^pa_operator_reapproval_decision_receipt_value_binding_record_evidence_acceptance_preflight_item_[a-z0-9][a-z0-9_:-]*$"
)
_EVIDENCE_KINDS = (
...
_TRUE_EFFECT_BOUNDARY = {
    "operator_reapproval_decision_receipt_value_binding_record_evidence_acceptance_preflight_allowed": True,
    "submitted_evidence_ref_intake_ref_binding_allowed": True,
    "submitted_evidence_ref_presence_check_allowed": True,
    "submitted_evidence_ref_only_validation_allowed": True,
    "evidence_acceptance_preflight_decision_allowed": True,
    "submitted_evidence_refs_present": True,
    "evidence_submitted": True,
    "evidence_ref_only": True,
    "submitted_evidence_ref_intake_is_source": True,
}
...
def build_default_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_evidence_acceptance_preflight(
    *,
    generated_at: str = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_EVIDENCE_ACCEPTANCE_PREFLIGHT_GENERATED_AT,
    evidence_acceptance_preflight_id: str = (
        DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_EVIDENCE_ACCEPTANCE_PREFLIGHT_ID
    ),
) -> dict[str, Any]:
    """Build deterministic no-effect submitted evidence acceptance preflight."""

    return build_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_evidence_acceptance_preflight_envelope(
        generated_at=generated_at,
        evidence_acceptance_preflight_id=evidence_acceptance_preflight_id,
        operator_reapproval_decision_receipt_value_binding_record_submitted_evidence_ref_intake=(
            build_default_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_submitted_evidence_ref_intake()
        ),
    )


def build_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_evidence_acceptance_preflight_envelope(
    *,
    generated_at: str,
    operator_reapproval_decision_receipt_value_binding_record_submitted_evidence_ref_intake: Mapping[str, Any],
    evidence_acceptance_preflight_id: str = (
        DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_EVIDENCE_ACCEPTANCE_PREFLIGHT_ID
    ),
) -> dict[str, Any]:
    """Build blocked acceptance preflights from submitted evidence refs."""

    timestamp = _require_non_empty_text(generated_at, "generated_at")
    envelope_id = _require_pattern(evidence_acceptance_preflight_id, "evidence_acceptance_preflight_id", _PREFLIGHT_ID_PATTERN)
    source_envelope = _require_mapping(
        operator_reapproval_decision_receipt_value_binding_record_submitted_evidence_ref_intake,
        "operator_reapproval_decision_receipt_value_binding_record_submitted_evidence_ref_intake",
    )
    _scan_private_or_secret_payload(source_envelope, path="operator_reapproval_decision_receipt_value_binding_record_submitted_evidence_ref_intake")
    _assert_submitted_evidence_ref_intake_boundary(source_envelope)
    source_intake_id = _require_non_empty_text(source_envelope.get("submitted_evidence_ref_intake_id"), "submitted_evidence_ref_intake_id")

    evaluations: list[dict[str, Any]] = []
    evaluation_ids: list[str] = []
    source_submission_record_ids: list[str] = []
    submitted_evidence_refs: list[str] = []
    receipt_ids: list[str] = []
    source_records = source_envelope.get("submission_records")
    if isinstance(source_records, (str, bytes)) or not isinstance(source_records, Sequence):
        raise PersonalAssistantInvariantError("evidence acceptance preflight requires submission_records sequence")
    for source_record in source_records:
        if not isinstance(source_record, Mapping):
            raise PersonalAssistantInvariantError("evidence acceptance preflight source record must be a mapping")
        _assert_submission_record_boundary(source_record)
        evaluation = _acceptance_evaluation(source_intake_id, source_record, timestamp=timestamp)
        if evaluation["acceptance_preflight_item_id"] in evaluation_ids:
            raise PersonalAssistantInvariantError(f"duplicate acceptance_preflight_item_id {evaluation['acceptance_preflight_item_id']}")
        evaluation_ids.append(evaluation["acceptance_preflight_item_id"])
        source_submission_record_ids.append(evaluation["source_submission_record_id"])
        submitted_evidence_refs.append(evaluation["submitted_evidence_ref"])
        receipt_ids.append(evaluation["receipt"]["receipt_id"])
        evaluations.append(evaluation)
    if not evaluations:
        raise PersonalAssistantInvariantError("evidence acceptance preflight requires at least one submitted evidence ref")

    envelope = {
        "evidence_acceptance_preflight_id": envelope_id,
        "generated_at": timestamp,
        "governed": True,
        "source_projection": "operator_reapproval_decision_receipt_value_binding_record_submitted_evidence_ref_intake",
        "source_operator_reapproval_decision_receipt_value_binding_record_submitted_evidence_ref_intake_id": source_intake_id,
        "acceptance_state": "submitted_refs_checked_not_verified_not_accepted",
        "decision": "blocked",
        "outcome": "AwaitingEvidence",
        "acceptance_preflight_count": len(evaluations),
        "acceptance_preflight_item_ids": evaluation_ids,
        "source_submission_record_ids": source_submission_record_ids,
        "submitted_evidence_refs": submitted_evidence_refs,
        "receipt_ids": receipt_ids,
        "acceptance_preflights": evaluations,
        "effect_boundary": dict(_EFFECT_BOUNDARY),
        "private_payload_policy": dict(_PRIVATE_PAYLOAD_POLICY),
        "summary": _summary(evaluations),
        "assurance": {
            "assurance_id": "personal_assistant_operator_reapproval_decision_receipt_value_binding_record_evidence_acceptance_preflight_no_effect_assurance",
            "outcome": "AwaitingEvidence",
            ...
            "checked_controls": [
                "submitted_evidence_ref_intake_required",
            ...
            "blocking_reasons": [
                "submitted_evidence_refs_require_external_verification",
                "evidence_acceptance_requires_separate_governed_verifier",
                "value_binding_record_not_created",
                "execution_authority_not_granted",
            ],
            "next_action": "verify submitted evidence refs in a separate governed verifier before any evidence acceptance or value binding",
        },
        "metadata": {
            "foundation_only": True,
            "projection_contract": "operator_reapproval_decision_receipt_value_binding_record_evidence_acceptance_preflight",
            "runtime_boundary": "submitted_refs_checked_not_verified_not_accepted",
            "ref_only_submission": True,
...
def _acceptance_evaluation(source_intake_id: str, source_record: Mapping[str, Any], *, timestamp: str) -> dict[str, Any]:
    source_submission_record_id = _require_non_empty_text(source_record.get("submission_record_id"), "source_submission_record_id")
    ...
    item_id = _require_pattern(
        f"pa_operator_reapproval_decision_receipt_value_binding_record_evidence_acceptance_preflight_item_{evidence_kind}_{suffix}",
        "acceptance_preflight_item_id",
        _EVALUATION_ID_PATTERN,
    )
    return {
        "acceptance_preflight_item_id": item_id,
        "source_submission_record_id": source_submission_record_id,
        ...
        "submitted_evidence_ref_intake_ref": {
            "source_submitted_evidence_ref_intake_id": source_intake_id,
            "source_submission_record_id": source_submission_record_id,
        ...
        "receipt": _acceptance_receipt(
            receipt_id=f"pa_receipt_operator_reapproval_decision_receipt_value_binding_record_evidence_acceptance_preflight_{evidence_kind}_{suffix}",
            request_id=request_id,
            skill_id=skill_id,
            risk_level=risk_level,
            approval_id=approval_id,
...
def _acceptance_receipt(
    *,
    ...
    return {
        ...
        "inputs_used": [
            "operator_reapproval_decision_receipt_value_binding_record_submitted_evidence_ref_intake",
            "operator_reapproval_decision_receipt_value_binding_record_evidence_acceptance_preflight_policy",
        ],
        ...
        "actions_taken": [
            f"{evidence_kind}_submitted_evidence_ref_presence_checked",
            "evidence_acceptance_preflight_receipt_created",
        ],
        "actions_not_taken": [
        ...
        "timestamp": timestamp,
        "evidence_refs": [
            f"proof://personal-assistant/operator-reapproval-decision-receipt-value-binding-record-evidence-acceptance-preflight/{approval_id}/{evidence_kind}"
        ],
        "memory_observation_refs": [],
        "replay_refs": [
            f"replay://personal-assistant/operator-reapproval-decision-receipt-value-binding-record-evidence-acceptance-preflight/{approval_id}/{evidence_kind}"
        ],
        "outcome": "AwaitingEvidence",
        "metadata": {
            "foundation_only": True,
            "operator_reapproval_decision_receipt_value_binding_record_evidence_acceptance_preflight_is_execution": False,
            "evidence_kind": evidence_kind,
            "ref_only_submission": True,
            "raw_evidence_payload_present": False,
            "raw_operator_value_present": False,
...
def _summary(evaluations: Sequence[Mapping[str, Any]]) -> dict[str, int]:
    preflights = [_require_mapping(record.get("acceptance_preflight"), "acceptance_preflight") for record in evaluations]
    authorities = [_require_mapping(record.get("authority_status"), "authority_status") for record in evaluations]
    return {
        "acceptance_preflight_count": len(evaluations),
...
def _assert_submitted_evidence_ref_intake_boundary(source_envelope: Mapping[str, Any]) -> None:
    ...
    for field_name in (
        ...
        if effect_boundary.get(field_name) is not True:
            raise PersonalAssistantInvariantError(f"submitted evidence ref intake effect_boundary.{field_name} must be true")
    for field_name in (
        ...
        if effect_boundary.get(field_name) is not False:
            raise PersonalAssistantInvariantError(f"submitted evidence ref intake effect_boundary.{field_name} must be false")
    if source_envelope.get("intake_state") != "submitted_refs_recorded_not_accepted" or source_envelope.get("outcome") != "AwaitingEvidence":
        raise PersonalAssistantInvariantError("submitted evidence ref intake must remain submitted_refs_recorded_not_accepted AwaitingEvidence")


def _assert_submission_record_boundary(source_record: Mapping[str, Any]) -> None:
    evidence_kind = _require_non_empty_text(source_record.get("evidence_kind"), "evidence_kind")
    if evidence_kind not in _EVIDENCE_KINDS:
        raise PersonalAssistantInvariantError("submitted evidence ref intake record kind must remain governed")
    submitted_evidence = _require_mapping(source_record.get("submitted_evidence"), "submitted_evidence")
    for field_name in ("submitted_evidence_ref_only", "evidence_submitted"):
        if submitted_evidence.get(field_name) is not True:
            raise PersonalAssistantInvariantError(f"submitted_evidence.{field_name} must be true")
    for field_name in ("raw_evidence_payload_present", "raw_operator_value_present", "evidence_accepted", "evidence_rejected", "requirement_satisfied"):
        if submitted_evidence.get(field_name) is not False:
            raise PersonalAssistantInvariantError(f"submitted_evidence.{field_name} must be false")
    for field_name in ("accepted_evidence_refs", "rejected_evidence_refs"):
        if submitted_evidence.get(field_name) != []:
            raise PersonalAssistantInvariantError(f"submitted_evidence.{field_name} must remain empty")
    ...
    for field_name in (
        ...
        if authority_status.get(field_name) is not False:
            raise PersonalAssistantInvariantError(f"submitted evidence ref intake authority_status.{field_name} must be false")
