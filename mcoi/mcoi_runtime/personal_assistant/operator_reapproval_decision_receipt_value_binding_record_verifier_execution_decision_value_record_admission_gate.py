"""Purpose: verifier execution operator decision-value record admission gate.
Governance scope: no-effect admission gate after decision-value record-path
...
Invariants:
  - The value-record admission gate is projected but not admitted.
...
DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_ADMISSION_GATE_ID = (
    "pa_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_admission_gate_foundation_001"
)
DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_ADMISSION_GATE_GENERATED_AT = (
    "2026-06-14T01:35:00+00:00"
)

_RECORD_ADMISSION_GATE_ID_PATTERN = re.compile(
    r"^pa_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_admission_gate_[a-z0-9][a-z0-9_:-]*$"
)
_ITEM_ID_PATTERN = re.compile(
    r"^pa_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_admission_gate_item_[a-z0-9][a-z0-9_:-]*$"
)
...
_SOURCE_FALSE_RECORD_FLAGS = {
    field_name: False for field_name in _FALSE_RECORD_FLAGS if field_name != "record_admission_gate_admitted"
}
_EFFECT_BOUNDARY = {
    "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_admission_gate_allowed": True,
    "decision_value_record_path_ref_binding_allowed": True,
    "operator_decision_value_record_admission_gate_projection_allowed": True,
...
def build_default_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_admission_gate(
    *,
    generated_at: str = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_ADMISSION_GATE_GENERATED_AT,
    record_admission_gate_id: str = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_ADMISSION_GATE_ID,
) -> dict[str, Any]:
    """Build deterministic no-effect verifier execution decision-value record admission gate."""

    return build_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_admission_gate_envelope(
        generated_at=generated_at,
        record_admission_gate_id=record_admission_gate_id,
...
def build_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_admission_gate_envelope(
    *,
    generated_at: str,
    operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_path: Mapping[str, Any],
    record_admission_gate_id: str = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_ADMISSION_GATE_ID,
) -> dict[str, Any]:
    """Build blocked value-record admission gate packet from record-path evidence."""

    timestamp = _require_non_empty_text(generated_at, "generated_at")
    envelope_id = _require_pattern(record_admission_gate_id, "record_admission_gate_id", _RECORD_ADMISSION_GATE_ID_PATTERN)
    ...
    for source_record in source_records:
        if not isinstance(source_record, Mapping):
            raise PersonalAssistantInvariantError("decision-value record admission gate source record must be a mapping")
        _assert_record_path_item_boundary(source_record)
        record = _record_admission_gate_item(source_record_path_id, source_record, timestamp=timestamp)
        if record["record_admission_gate_item_id"] in item_ids:
            raise PersonalAssistantInvariantError(f"duplicate record_admission_gate_item_id {record['record_admission_gate_item_id']}")
        item_ids.append(record["record_admission_gate_item_id"])
        receipt_ids.append(record["receipt"]["receipt_id"])
        records.append(record)
    if not records:
        raise PersonalAssistantInvariantError("decision-value record admission gate requires at least one record path")

    envelope = {
        "record_admission_gate_id": envelope_id,
        ...
        "record_admission_gate_state": "operator_decision_value_record_admission_gate_blocked_awaiting_actual_operator_value",
        "decision": "blocked",
        "outcome": "AwaitingEvidence",
        "record_admission_gate_count": len(records),
        "record_admission_gate_item_ids": item_ids,
        "receipt_ids": receipt_ids,
        "record_admission_gates": records,
        "effect_boundary": dict(_EFFECT_BOUNDARY),
        "private_payload_policy": dict(_PRIVATE_PAYLOAD_POLICY),
        "summary": _summary(records),
        "assurance": {
            "assurance_id": "personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_admission_gate_no_effect_assurance",
            "outcome": "AwaitingEvidence",
            "foundation_only": True,
            "record_admission_gate_only": True,
            "record_contract_ready": True,
            "operator_decision_required": True,
            "operator_decision_value_required": True,
            "actual_operator_decision_value_required": True,
            ...
            "next_action": "collect a governed explicit operator decision value before admitting the value-record admission gate",
        },
        "metadata": {
            "foundation_only": True,
            "projection_contract": "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_admission_gate",
            "runtime_boundary": "operator_decision_value_record_admission_gate_blocked_awaiting_actual_operator_value",
            "record_admission_gate_only": True,
...
def _record_admission_gate_item(source_record_path_id: str, source_record: Mapping[str, Any], *, timestamp: str) -> dict[str, Any]:
    ...
    item_id = _require_pattern(
        f"pa_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_admission_gate_item_{evidence_kind}_{requirement_kind}_{suffix}",
        "record_admission_gate_item_id",
        _ITEM_ID_PATTERN,
    )
    return {
        "record_admission_gate_item_id": item_id,
        ...
        "record_admission_gate": {
            "record_contract_ready": True,
            "record_admission_gate_created": True,
            "operator_decision_required": True,
            "operator_decision_value_required": True,
            "actual_operator_decision_value_required": True,
        ...
        "receipt": _record_admission_gate_receipt(
            receipt_id=f"pa_receipt_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_admission_gate_{evidence_kind}_{requirement_kind}_{suffix}",
            request_id=request_id,
            skill_id=skill_id,
            risk_level=risk_level,
            approval_id=approval_id,
...
def _record_admission_gate_receipt(
    *,
    receipt_id: str,
    request_id: str,
    skill_id: str,
    ...
    return {
        ...
        "inputs_used": [
            "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_path",
            "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_admission_gate_policy",
        ],
        ...
        "actions_taken": [
            f"{evidence_kind}_{requirement_kind}_decision_value_record_admission_gate_projected",
            "operator_decision_value_record_admission_gate_checked",
            "decision_value_record_admission_gate_receipt_created",
        ],
        "actions_not_taken": [
        ...
        "timestamp": timestamp,
        "evidence_refs": [
            f"proof://personal-assistant/operator-reapproval-decision-receipt-value-binding-record-verifier-execution-decision-value-record-admission-gate/{approval_id}/{evidence_kind}/{requirement_kind}"
        ],
        "memory_observation_refs": [],
        "replay_refs": [
            f"replay://personal-assistant/operator-reapproval-decision-receipt-value-binding-record-verifier-execution-decision-value-record-admission-gate/{approval_id}/{evidence_kind}/{requirement_kind}"
        ],
        "outcome": "AwaitingEvidence",
        "metadata": {
            "foundation_only": True,
            "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_admission_gate_is_execution": False,
            "evidence_kind": evidence_kind,
            "requirement_kind": requirement_kind,
            "record_admission_gate_only": True,
            "record_contract_ready": True,
...
def _summary(records: Sequence[Mapping[str, Any]]) -> dict[str, int]:
    admission_gates = [_require_mapping(record.get("record_admission_gate"), "record_admission_gate") for record in records]
    ...
    return {
        ...
        "record_admission_gate_creation_count": sum(1 for record in admission_gates if record.get("record_admission_gate_created") is True),
