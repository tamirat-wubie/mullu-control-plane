"""Purpose: verifier execution operator decision-value record value template.
Governance scope: no-effect value template after decision-value record value
request projection, without collecting, storing, admitting, or executing an
operator value.
Dependencies: personal-assistant verifier execution decision-value
record-value-request runtime and contracts.
Invariants:
  - The value-record template is projected but not admitted.
  - The source record value request remains unadmitted.
...
from .operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_request import (
...
DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_VALUE_TEMPLATE_ID = (
    "pa_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_template_foundation_001"
)
DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_VALUE_TEMPLATE_GENERATED_AT = (
    "2026-06-14T01:35:00+00:00"
)

_RECORD_VALUE_TEMPLATE_ID_PATTERN = re.compile(
    r"^pa_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_template_[a-z0-9][a-z0-9_:-]*$"
)
_ITEM_ID_PATTERN = re.compile(
    r"^pa_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_template_item_[a-z0-9][a-z0-9_:-]*$"
)
...
_SOURCE_FALSE_RECORD_FLAGS = {
    field_name: False for field_name in _FALSE_RECORD_FLAGS if field_name != "record_value_template_admitted"
}
_EFFECT_BOUNDARY = {
    "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_template_allowed": True,
    "decision_value_record_value_request_ref_binding_allowed": True,
    "operator_decision_value_record_value_template_projection_allowed": True,
    "record_value_requests_present": True,
...
_PRIVATE_PAYLOAD_POLICY = {
    ...
    "record_value_request_projection": "ref_only",
...
def build_default_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_template(
    *,
    generated_at: str = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_VALUE_TEMPLATE_GENERATED_AT,
    record_value_template_id: str = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_VALUE_TEMPLATE_ID,
) -> dict[str, Any]:
    """Build deterministic no-effect verifier execution decision-value record value template."""

    return build_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_template_envelope(
        generated_at=generated_at,
        record_value_template_id=record_value_template_id,
        operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_request=(
            build_default_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_request()
        ),
    )


def build_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_template_envelope(
    *,
    generated_at: str,
    operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_request: Mapping[str, Any],
    record_value_template_id: str = DEFAULT_OPERATOR_REAPPROVAL_DECISION_RECEIPT_VALUE_BINDING_RECORD_VERIFIER_EXECUTION_DECISION_VALUE_RECORD_VALUE_TEMPLATE_ID,
) -> dict[str, Any]:
    """Build blocked value-record template packet from value-request evidence."""

    timestamp = _require_non_empty_text(generated_at, "generated_at")
    envelope_id = _require_pattern(record_value_template_id, "record_value_template_id", _RECORD_VALUE_TEMPLATE_ID_PATTERN)
    source_envelope = _require_mapping(
        operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_request,
        "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_request",
    )
    _scan_private_or_secret_payload(source_envelope, path="operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_request")
    _assert_record_value_request_boundary(source_envelope)
    source_record_value_request_id = _require_non_empty_text(source_envelope.get("record_value_request_id"), "record_value_request_id")
    source_records = _require_sequence(source_envelope.get("record_value_requests"), "record_value_requests")
    ...
    for source_record in source_records:
        if not isinstance(source_record, Mapping):
            raise PersonalAssistantInvariantError("decision-value record value template source record must be a mapping")
        _assert_record_value_request_item_boundary(source_record)
        record = _record_value_template_item(source_record_value_request_id, source_record, timestamp=timestamp)
        if record["record_value_template_item_id"] in item_ids:
            raise PersonalAssistantInvariantError(f"duplicate record_value_template_item_id {record['record_value_template_item_id']}")
        item_ids.append(record["record_value_template_item_id"])
        receipt_ids.append(record["receipt"]["receipt_id"])
        records.append(record)
    if not records:
        raise PersonalAssistantInvariantError("decision-value record value template requires at least one record value request")

    envelope = {
        "record_value_template_id": envelope_id,
        "generated_at": timestamp,
        "governed": True,
        "source_projection": "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_request",
        "source_record_value_request_id": source_record_value_request_id,
        "record_value_template_state": "operator_decision_value_record_value_template_blocked_awaiting_actual_operator_value",
        "decision": "blocked",
        "outcome": "AwaitingEvidence",
        "record_value_template_count": len(records),
        "record_value_template_item_ids": item_ids,
        "receipt_ids": receipt_ids,
        "record_value_templates": records,
        "effect_boundary": dict(_EFFECT_BOUNDARY),
        "private_payload_policy": dict(_PRIVATE_PAYLOAD_POLICY),
        "summary": _summary(records),
        "assurance": {
            "assurance_id": "personal_assistant_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_template_no_effect_assurance",
            "outcome": "AwaitingEvidence",
            "foundation_only": True,
            "record_value_template_only": True,
            "record_contract_ready": True,
            "operator_decision_required": True,
            "operator_decision_value_required": True,
            "actual_operator_decision_value_required": True,
            ...
            "next_action": "collect a governed explicit operator decision value before admitting the value-record template",
        },
        "metadata": {
            "foundation_only": True,
            "projection_contract": "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_template",
            "runtime_boundary": "operator_decision_value_record_value_template_blocked_awaiting_actual_operator_value",
            "record_value_template_only": True,
...
def _record_value_template_item(source_record_value_request_id: str, source_record: Mapping[str, Any], *, timestamp: str) -> dict[str, Any]:
    source_item_id = _require_non_empty_text(source_record.get("record_value_request_item_id"), "record_value_request_item_id")
    ...
    item_id = _require_pattern(
        f"pa_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_template_item_{evidence_kind}_{requirement_kind}_{suffix}",
        "record_value_template_item_id",
        _ITEM_ID_PATTERN,
    )
    return {
        "record_value_template_item_id": item_id,
        "source_record_value_request_item_id": source_item_id,
        ...
        "record_value_request_ref": {
            "source_record_value_request_id": source_record_value_request_id,
            "source_record_value_request_item_id": source_item_id,
            "source_record_value_request_state": "operator_decision_value_record_value_request_blocked_awaiting_actual_operator_value",
            "source_outcome": "AwaitingEvidence",
            "source_record_contract_ready": True,
            "source_record_value_request_created": True,
        ...
        "record_value_template": {
            "record_contract_ready": True,
            "record_value_template_created": True,
            "operator_decision_required": True,
            "operator_decision_value_required": True,
            "actual_operator_decision_value_required": True,
            ...
            **dict(_FALSE_RECORD_FLAGS),
            "blocking_reason": "actual_operator_decision_value_absent_record_value_request_unadmitted",
        },
        "authority_status": _authority_status(),
        "receipt": _record_value_template_receipt(
            receipt_id=f"pa_receipt_operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_template_{evidence_kind}_{requirement_kind}_{suffix}",
            request_id=request_id,
            skill_id=skill_id,
            risk_level=risk_level,
            approval_id=approval_id,
...
def _record_value_template_receipt(
    *,
    receipt_id: str,
    request_id: str,
    skill_id: str,
    ...
    return {
        ...
        "inputs_used": [
            "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_request",
            "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_template_policy",
        ],
        ...
        "actions_taken": [
            f"{evidence_kind}_{requirement_kind}_decision_value_record_value_template_projected",
            "operator_decision_value_record_value_template_checked",
            "decision_value_record_value_template_receipt_created",
        ],
        "actions_not_taken": [
        ...
        "timestamp": timestamp,
        "evidence_refs": [
            f"proof://personal-assistant/operator-reapproval-decision-receipt-value-binding-record-verifier-execution-decision-value-record-value-template/{approval_id}/{evidence_kind}/{requirement_kind}"
        ],
        "memory_observation_refs": [],
        "replay_refs": [
            f"replay://personal-assistant/operator-reapproval-decision-receipt-value-binding-record-verifier-execution-decision-value-record-value-template/{approval_id}/{evidence_kind}/{requirement_kind}"
        ],
        "outcome": "AwaitingEvidence",
        "metadata": {
            "foundation_only": True,
            "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_template_is_execution": False,
            "evidence_kind": evidence_kind,
            "requirement_kind": requirement_kind,
            "record_value_template_only": True,
            "record_contract_ready": True,
...
def _summary(records: Sequence[Mapping[str, Any]]) -> dict[str, int]:
    admission_gates = [_require_mapping(record.get("record_value_template"), "record_value_template") for record in records]
    ...
    return {
        ...
        "record_value_template_creation_count": sum(1 for record in admission_gates if record.get("record_value_template_created") is True),
...
def _assert_record_value_request_boundary(source_envelope: Mapping[str, Any]) -> None:
    effect_boundary = _require_mapping(source_envelope.get("effect_boundary"), "effect_boundary")
    for field_name in (
        "operator_reapproval_decision_receipt_value_binding_record_verifier_execution_decision_value_record_value_request_allowed",
        "decision_value_record_admission_gate_ref_binding_allowed",
        "operator_decision_value_record_value_request_projection_allowed",
        "record_admission_gates_present",
        ...
        if effect_boundary.get(field_name) is not True:
            raise PersonalAssistantInvariantError(f"decision-value record value request effect_boundary.{field_name} must be true")
    for field_name, expected_value in _SOURCE_FALSE_RECORD_FLAGS.items():
        if field_name in effect_boundary and effect_boundary.get(field_name) is not expected_value:
            raise PersonalAssistantInvariantError(f"decision-value record value request effect_boundary.{field_name} must be false")
    if source_envelope.get("record_value_request_state") != "operator_decision_value_record_value_request_blocked_awaiting_actual_operator_value":
        raise PersonalAssistantInvariantError("decision-value record value request must remain blocked awaiting explicit value")
    if source_envelope.get("decision") != "blocked" or source_envelope.get("outcome") != "AwaitingEvidence":
        raise PersonalAssistantInvariantError("decision-value record value request must remain blocked AwaitingEvidence")


def _assert_record_value_request_item_boundary(source_record: Mapping[str, Any]) -> None:
    path = _require_mapping(source_record.get("record_value_request"), "record_value_request")
    for field_name in (
        "record_contract_ready",
        "record_value_request_created",
        "operator_decision_required",
        "operator_decision_value_required",
        "actual_operator_decision_value_required",
        ...
        if path.get(field_name) is not True:
            raise PersonalAssistantInvariantError(f"decision-value record value request record_value_request.{field_name} must be true")
    if tuple(path.get("accepted_record_kinds", ())) != _ACCEPTED_RECORD_KINDS:
        raise PersonalAssistantInvariantError("decision-value record value request accepted_record_kinds drifted")
    if tuple(path.get("rejected_input_kinds", ())) != _REJECTED_INPUT_KINDS:
        raise PersonalAssistantInvariantError("decision-value record value request rejected_input_kinds drifted")
    if tuple(path.get("required_fields", ())) != _REQUIRED_FIELDS:
        raise PersonalAssistantInvariantError("decision-value record value request required_fields drifted")
    for field_name, expected_value in _SOURCE_FALSE_RECORD_FLAGS.items():
        if field_name in path and path.get(field_name) is not expected_value:
            raise PersonalAssistantInvariantError(f"decision-value record value request record_value_request.{field_name} must be false")
