...
def test_default_registry_exposes_governed_loop_manifests() -> None:
    ...
    assert registry.get_manifest("workflow_execution_loop").owner == "workflow_governance"
    assert "workflow_operator_ref" in registry.get_manifest(
        "workflow_execution_loop"
    ).required_authority
    assert "workflow_descriptor_valid" in registry.get_manifest(
        "workflow_execution_loop"
    ).required_evidence
...
def test_workflow_execution_loop_is_registered_read_only_and_blocked() -> None:
    registry = build_default_loop_registry()
    manifest = registry.get_manifest("workflow_execution_loop")
    summary = registry.summarize("workflow_execution_loop")

    assert manifest.owner == "workflow_governance"
    assert manifest.metadata["behavior_rewrite"] is False
    assert "workflow_operator_ref" in manifest.required_authority
    assert "workflow_descriptor_valid" in manifest.required_evidence
    assert summary.status is LoopStatus.BLOCKED
    assert "missing_authority:workflow_operator_ref" in summary.open_blockers
