...
def test_db_backend_rejects_memory_in_production() -> None:
    with pytest.raises(
        ...
        server._validate_db_backend_for_env("memory", "production")
...
def test_db_backend_warns_for_unknown_non_dev_env() -> None:
    warning = server._validate_db_backend_for_env("memory", "staging")
...
def test_resolve_cors_origins_uses_dev_defaults() -> None:
    origins = server._resolve_cors_origins(None, "local_dev")
...
def test_validate_cors_origins_rejects_empty_in_production() -> None:
    with pytest.raises(
        ...
        server._validate_cors_origins_for_env([], "production")
...
def test_validate_cors_origins_warns_for_unknown_non_dev_env() -> None:
    warning = server._validate_cors_origins_for_env([], "staging")
    assert warning is not None
