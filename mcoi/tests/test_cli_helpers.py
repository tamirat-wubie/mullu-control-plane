...
def _assert_stage_dependency_bindings(body: dict) -> None:
    assert body["stage_dependency_bindings"] == [
        {
            "stage_id": stage_id,
            "receipt_ref": receipt_ref,
            "predecessor_stage_ids": list(predecessor_stage_ids),
            "dependency_status": "satisfied",
        }
        for stage_id, receipt_ref, predecessor_stage_ids in zip(
            body["execution_stage_ids"],
            body["step_receipt_refs"],
            (
                (),
...
def _assert_stage_repair_bindings(body: dict) -> None:
    assert body["stage_repair_bindings"] == [
        {
            "stage_id": stage_id,
            "receipt_ref": receipt_ref,
            "attempt_count": 1,
...
def _assert_stage_error_bindings(body: dict) -> None:
    assert body["stage_error_bindings"] == [
        {
            "stage_id": stage_id,
            "receipt_ref": receipt_ref,
...
def _assert_stage_outcome_bindings(body: dict) -> None:
    assert body["stage_outcome_bindings"] == [
        {
            "stage_id": stage_id,
            "receipt_ref": receipt_ref,
            "dispatched": True,
...
def _assert_stage_next_action_bindings(body: dict) -> None:
    assert body["stage_next_action_bindings"] == [
        {
            "stage_id": stage_id,
            "receipt_ref": receipt_ref,
            "outcome_status": "settled",
...
def test_autonomous_demo_renders_json_continuation_summary(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    _assert_stage_dependency_bindings(body)
    _assert_stage_repair_bindings(body)
...
def test_autonomous_demo_receipt_dir_writes_latest_receipt(
    ...
    _assert_stage_dependency_bindings(receipt_body)
    _assert_stage_repair_bindings(receipt_body)
    ...
    assert latest_body["stage_dependency_bindings"] == receipt_body["stage_dependency_bindings"]
    assert latest_body["stage_repair_bindings"] == receipt_body["stage_repair_bindings"]
