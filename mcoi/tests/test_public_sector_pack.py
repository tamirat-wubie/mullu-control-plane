...
class TestDemoGenerator:
    ...
    def test_remediation_titles_are_bounded(self, monkeypatch):
        ...
        result = gen.generate("ps-bounded")

        assert result["remediations"]["imported"] == 5
        assert {item["title"] for item in captured["remediations"]} == {"Public sector remediation"}
        assert "Permit backlog - commercial zoning review" not in captured["remediations"][0]["title"]
