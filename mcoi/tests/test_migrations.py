...
class TestMigrationEngine:
    ...
    def test_failed_migration_message_is_bounded(self, engine):
        ...
        with pytest.raises(RuntimeError, match=r"Migration 1 \(bad\) failed \(RuntimeError\)") as exc_info:
    ...
    def test_current_version_raises_bounded_error_on_lookup_failure(self, engine):
        ...
        with pytest.raises(RuntimeError, match=r"migration state lookup failed \(RuntimeError\)") as exc_info:
            engine.current_version(BrokenVersionConn())

        assert "RuntimeError" in str(exc_info.value)
        assert "secret version lookup failure" not in str(exc_info.value)
        assert "migration state lookup failed" in str(exc_info.value)
    ...
    def test_history_raises_bounded_error_on_lookup_failure(self, engine):
        ...
        with pytest.raises(RuntimeError, match=r"migration history lookup failed \(RuntimeError\)") as exc_info:
