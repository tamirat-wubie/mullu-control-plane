...
class TestStatePersistenceRoundTrip:
    ...
    def test_atomic_write(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            sp.save("test", {"key": "val2"})  # overwrite
            loaded = sp.load("test")
...
class TestServerPersistenceWiring:
    ...
    def test_shutdown_saves_budgets(self):
        os.environ["MULLU_ENV"] = "local_dev"
        os.environ["MULLU_DB_BACKEND"] = "memory"
    ...
    def test_db_backend_warning_in_non_dev(self):
        """Memory backend in production should warn."""
        # This is tested by checking the server module loaded with warning
        os.environ["MULLU_ENV"] = "local_dev"
        os.environ["MULLU_DB_BACKEND"] = "memory"
        from mcoi_runtime.app.server import store
        assert store is not None  # should work in local_dev without warning
