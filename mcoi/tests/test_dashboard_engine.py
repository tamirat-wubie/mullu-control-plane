...
class TestBuildNoteMemorySummary:
    ...
    def test_builds_note_memory_summary_from_legacy_snapshot_without_total(self):
        engine = _make_engine()
        snapshot = _note_memory_snapshot(retrieval_influence_count=2)
        summary = snapshot["summary"]
        if isinstance(summary, dict):
            summary.pop("retrieval_influence_total_count")
    ...
    def test_rejects_non_boolean_retrieval_filter_active(self):
        engine = _make_engine()
        try:
            engine.build_note_memory_summary(_note_memory_snapshot(retrieval_filter_active="true"))
        except ValueError as exc:
            assert "retrieval_filter_active" in str(exc)
