...
def test_loop_read_model_exposes_registered_blocked_loops() -> None:
    ...
    assert payload["blocked_count"] == len(EXPECTED_LOOP_IDS)
    assert payload["returned_count"] == len(EXPECTED_LOOP_IDS)
    assert {loop["loop_id"] for loop in payload["loops"]} == EXPECTED_LOOP_IDS
...
def test_loop_read_model_limit_is_bounded_and_truncated() -> None:
    ...
    assert payload["total_count"] == len(EXPECTED_LOOP_IDS)
