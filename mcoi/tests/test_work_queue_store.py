...
def test_work_queue_store_missing_file_error_is_bounded(tmp_path: Path) -> None:
    store = WorkQueueStore(tmp_path / "work-queue")

    with pytest.raises(CorruptedDataError, match=r"^work queue file not found$") as excinfo:
        store.load_state()

    assert str(tmp_path) not in str(excinfo.value)


def test_work_queue_store_read_error_is_bounded(tmp_path: Path, monkeypatch) -> None:
    store = WorkQueueStore(tmp_path / "work-queue")
    payload_path = tmp_path / "work-queue" / "work_queue.json"
    payload_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    with pytest.raises(CorruptedDataError, match=r"^malformed work queue file \(OSError\)$") as excinfo:
        store.load_state()
