...
| Deployment runtime input witness | `DEPLOYMENT_STATUS.md` records the published deployment witness state, `MULLU_GATEWAY_URL=https://api.mullusi.com`, `MULLU_AUTHORITY_OPERATOR_SECRET` presence, `https://api.mullusi.com/health` as the public production health endpoint, runtime and authority responsibility-debt closure, production evidence closure, and the operator-approved public-health declaration receipt | Reflected |
