...
| Operational witness | Pilot health evidence exists, but runtime deployment publication and public production health remain undeclared; the repository landing page exposes that boundary and routes closure evidence to `DEPLOYMENT_STATUS.md` | Partially reflected |
...
|---|---|---|
| Deployment status not published | `DEPLOYMENT_STATUS.md` declares no public production endpoint evidence yet; `.change_assurance/deployment_witness.json` still records `deployment_claim=not-published` because runtime and conformance signatures are not verified and the public-health declaration receipt is absent | Collect signed `/health`, `/gateway/witness`, `/runtime/conformance`, upstream-readiness, and production-evidence receipts, then apply a public-health declaration with operator approval |
