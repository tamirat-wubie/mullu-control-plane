"""Tests for the Foundation Mode patent/disclosure deferral validator.

Purpose: prove patent/disclosure deferral stays local and does not become
patent filing, patent protection, invention finality, disclosure approval,
publication, secrecy protection, legal clearance, company formation, payment,
customer-access, or deployment evidence.
Governance scope: Foundation Mode, local patent/disclosure deferral,
private-value exclusion, legal/company blocking, payment blocking,
customer-access blocking, publication blocking, and deployment blocking.
Dependencies: scripts.validate_foundation_patent_disclosure_deferral_boundary.
Invariants: only public-safe deferral labels are allowed; all
patent/disclosure surfaces remain AwaitingEvidence; no filing values,
invention records, inventor identities, ownership records, prior-art
conclusions, novelty or patentability opinions, publication receipts, secrecy
claims, readiness claims, or deployment claims are accepted.
"""
...
from scripts.validate_foundation_patent_disclosure_deferral_boundary import (
    BLOCKED_CLAIMS,
    DEFAULT_DOC_PATH,
    DEFAULT_PACKET_PATH,
    DEFERRAL_LABELS,
...
def test_witness_contract_matches_expected_labels_and_claims() -> None:
    payload = load_json_object(DEFAULT_PACKET_PATH, "patent/disclosure deferral witness")
...
def test_false_flag_drift_is_reported(tmp_path: Path) -> None:
    ...
    candidate["disclosure_approval_claimed"] = True
    packet_path = _write_packet(tmp_path, candidate)

    findings = validate_artifacts(packet_path=packet_path)

    assert any("patent_filing_allowed must remain false" in finding.message for finding in findings)
    assert any("patent_protection_claimed must remain false" in finding.message for finding in findings)
...
def test_surface_state_and_note_drift_are_reported(tmp_path: Path) -> None:
    ...
    candidate["surfaces"][2]["public_safe_note"] = "Invention boundary is final."
    packet_path = _write_packet(tmp_path, candidate)
...
def test_forbidden_live_values_are_reported(tmp_path: Path) -> None:
    payload = load_json_object(DEFAULT_PACKET_PATH, "patent/disclosure deferral witness")
    candidate = copy.deepcopy(payload)
    candidate["surfaces"][0]["public_safe_note"] = (
        "Patent filing at https://example.test with patent_application=private-id and hash "
...
def test_promotion_phrase_is_reported(tmp_path: Path) -> None:
    payload = load_json_object(DEFAULT_PACKET_PATH, "patent/disclosure deferral witness")
    candidate = copy.deepcopy(payload)
    candidate["surfaces"][8]["public_safe_note"] = "Disclosure approved."
    packet_path = _write_packet(tmp_path, candidate)

    findings = validate_artifacts(packet_path=packet_path)

    assert any("forbidden promotion pattern: disclosure_approved" in finding.message for finding in findings)
    assert any("surface note drifted" in finding.message for finding in findings)
    assert len(findings) >= 2


def test_doc_required_phrase_drift_is_reported(tmp_path: Path) -> None:
    doc_text = load_text(DEFAULT_DOC_PATH, "patent/disclosure deferral doc")
    candidate_text = doc_text.replace(REQUIRED_DOC_PHRASES[0], "Foundation Patent Disclosure Draft")
    ...
    assert "Foundation Patent Disclosure Deferral Boundary" in doc_text
