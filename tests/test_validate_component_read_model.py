...
def test_component_read_model_schema_valid(tmp_path: Path) -> None:
    ...
    assert validation.bound_route_count == 31
    assert validation.route_family_classification_count == 78
    assert validation.classified_declared_route_count == 444
...
def test_component_read_model_example_matches_runtime_projection() -> None:
    ...
    assert example["summary"]["route_family_classification_count"] == 78
    assert example["summary"]["classified_declared_route_count"] == 444
