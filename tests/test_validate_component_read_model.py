...
def test_component_read_model_schema_valid(tmp_path: Path) -> None:
    ...
    assert validation.route_family_classification_count == 76
    assert validation.classified_declared_route_count == 424
...
def test_component_read_model_example_matches_runtime_projection() -> None:
    ...
    assert example["summary"]["route_family_classification_count"] == 76
    assert example["summary"]["classified_declared_route_count"] == 424
