"""Tests for tenant scope read-model validation.

Purpose: prove the tenant scope operator projection exposes bounded status
without tenant authority, raw detail visibility, or tenant binding.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: tenant scope read-model validator and schema.
Invariants:
...
def test_tenant_scope_read_model_rejects_authority_drift(tmp_path: Path) -> None:
    read_model = json.loads(DEFAULT_READ_MODEL_PATH.read_text(encoding="utf-8"))
    changed = copy.deepcopy(read_model)
    changed["effective_denials"]["receipt_store_tenant_scope_bound"] = True

    with pytest.raises(
        UniversalSymbolReceiptStoreTenantScopeReadModelError,
        match="effective_denials.receipt_store_tenant_scope_bound",
    ):
        validate_universal_symbol_receipt_store_tenant_scope_read_model(
    ...
    assert changed["read_model_is_not_tenant_authority"] is True
    assert changed["effective_denials"]["receipt_store_tenant_scope_bound"] is True
    assert len(changed["requirement_rows"]) == 8


def test_tenant_scope_read_model_rejects_raw_detail_visibility(tmp_path: Path) -> None:
    read_model = json.loads(DEFAULT_READ_MODEL_PATH.read_text(encoding="utf-8"))
    changed = copy.deepcopy(read_model)
    ...
    assert changed["requirement_rows"][0]["requirement_id"] == "requirement://tenant-identity"
    assert changed["requirement_rows"][0]["authority_granted"] is False
    assert changed["effective_denials"]["raw_payload_stored"] is False


def test_tenant_scope_read_model_rejects_missing_requirement(tmp_path: Path) -> None:
    read_model = json.loads(DEFAULT_READ_MODEL_PATH.read_text(encoding="utf-8"))
    changed = copy.deepcopy(read_model)
    changed["requirement_rows"] = [
        row for row in changed["requirement_rows"] if row["requirement_id"] != "requirement://tenant-identity"
    ]
    changed["contract_summary"]["requirement_row_count"] = len(changed["requirement_rows"])

    with pytest.raises(
        UniversalSymbolReceiptStoreTenantScopeReadModelError,
        match="requirement://tenant-identity",
    ):
        validate_universal_symbol_receipt_store_tenant_scope_read_model(
            _write_case(tmp_path, changed),
            DEFAULT_SCHEMA_PATH,
...
def test_tenant_scope_read_model_rejects_duplicate_requirement_row(tmp_path: Path) -> None:
    read_model = json.loads(DEFAULT_READ_MODEL_PATH.read_text(encoding="utf-8"))
    changed = copy.deepcopy(read_model)
...
def test_tenant_scope_read_model_rejects_missing_delta_reject_log(tmp_path: Path) -> None:
    read_model = json.loads(DEFAULT_READ_MODEL_PATH.read_text(encoding="utf-8"))
    changed = copy.deepcopy(read_model)
...
def test_tenant_scope_read_model_rejects_witness_projection_drift(tmp_path: Path) -> None:
    read_model = json.loads(DEFAULT_READ_MODEL_PATH.read_text(encoding="utf-8"))
    changed = copy.deepcopy(read_model)
    ...
    assert changed["witness_projection"]["tenant_requirement_count"] == 8
    assert len(changed["requirement_rows"]) == 8


def test_tenant_scope_read_model_rejects_evidence_ref_count_drift(tmp_path: Path) -> None:
    read_model = json.loads(DEFAULT_READ_MODEL_PATH.read_text(encoding="utf-8"))
    changed = copy.deepcopy(read_model)
