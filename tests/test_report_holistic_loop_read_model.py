...
def test_default_report_exposes_blocked_loop_summaries() -> None:
    ...
    assert all(loop["transition_bindings"] for loop in loops)
    ...
    assert all(loop["receipt_lineage_bindings"] for loop in loops)
    assert all(
    ...
    assert all(
        ...
        and loop["learning_policy"] in loop["audit_evolution_view"]["learning_candidate_refs"]
...
def test_report_accepts_complete_observed_authority_and_evidence_refs() -> None:
    ...
    assert all(
        loop["proof_obligation_view"]["obligation_state"]
        == "proof_obligations_satisfied_terminal_review_required"
        and loop["proof_obligation_view"]["missing_evidence_refs"] == []
        and loop["proof_obligation_view"]["missing_authority_refs"] == []
        and loop["proof_obligation_view"]["blocker_refs"] == []
        and loop["proof_obligation_view"]["read_only"] is True
        and loop["proof_obligation_view"]["executes_validator"] is False
        and loop["proof_obligation_view"]["terminal_closure"] is False
        for loop in report["loops"]
    ...
    assert all(
        loop["audit_evolution_view"]["audit_state"]
        == "audit_ready_for_terminal_review"
        and loop["audit_evolution_view"]["audit_blocker_refs"] == []
        and loop["audit_evolution_view"]["learning_candidate_refs"] == []
        and loop["audit_evolution_view"]["read_only"] is True
        and loop["audit_evolution_view"]["emits_receipt"] is False
        and loop["audit_evolution_view"]["admits_learning"] is False
        and loop["audit_evolution_view"]["terminal_closure"] is False
    ...
    assert all(
        set(binding["observed_evidence_refs"]) == set(loop["evidence_refs"])
        for loop in report["loops"]
        for binding in loop["receipt_lineage_bindings"]
    )
