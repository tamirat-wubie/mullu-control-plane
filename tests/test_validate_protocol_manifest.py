...
def test_protocol_manifest_indexes_agentic_service_harness_live_producer_execution_authority_evidence_packet() -> None:
    manifest = load_manifest()
    entries = {entry["schema_id"]: entry for entry in manifest["schemas"]}
    packet_entry = entries["agentic-service-harness-live-producer-execution-authority-evidence-packet"]

    assert validate_protocol_manifest(manifest) == []
    assert packet_entry["path"] == (
        "schemas/agentic_service_harness_live_producer_execution_authority_evidence_packet.schema.json"
    )
    assert packet_entry["urn"] == (
        "urn:mullusi:schema:agentic-service-harness-live-producer-execution-authority-evidence-packet:1"
    )
    assert packet_entry["surface"] == "runtime"


def test_protocol_manifest_indexes_agentic_service_harness_live_producer_execution_authority_review_input_contract() -> None:
    manifest = load_manifest()
    entries = {entry["schema_id"]: entry for entry in manifest["schemas"]}
    contract_entry = entries["agentic-service-harness-live-producer-execution-authority-review-input-contract"]

    assert validate_protocol_manifest(manifest) == []
    assert contract_entry["path"] == (
        "schemas/agentic_service_harness_live_producer_execution_authority_review_input_contract.schema.json"
    )
    assert contract_entry["urn"] == (
        "urn:mullusi:schema:agentic-service-harness-live-producer-execution-authority-review-input-contract:1"
    )
    assert contract_entry["surface"] == "runtime"


def test_protocol_manifest_indexes_agentic_service_harness_live_producer_execution_authority_review_intake_preflight() -> None:
    manifest = load_manifest()
    entries = {entry["schema_id"]: entry for entry in manifest["schemas"]}
    preflight_entry = entries["agentic-service-harness-live-producer-execution-authority-review-intake-preflight"]

    assert validate_protocol_manifest(manifest) == []
    assert preflight_entry["path"] == (
        "schemas/agentic_service_harness_live_producer_execution_authority_review_intake_preflight.schema.json"
    )
    assert preflight_entry["urn"] == (
        "urn:mullusi:schema:agentic-service-harness-live-producer-execution-authority-review-intake-preflight:1"
    )
    assert preflight_entry["surface"] == "runtime"


def test_protocol_manifest_indexes_agentic_service_harness_live_producer_execution_authority_review_intake_digest_packet() -> None:
    manifest = load_manifest()
    entries = {entry["schema_id"]: entry for entry in manifest["schemas"]}
    packet_entry = entries[
        "agentic-service-harness-live-producer-execution-authority-review-intake-digest-packet"
    ]

    assert validate_protocol_manifest(manifest) == []
    assert packet_entry["path"] == (
        "schemas/agentic_service_harness_live_producer_execution_authority_review_intake_digest_packet.schema.json"
    )
    assert packet_entry["urn"] == (
        "urn:mullusi:schema:agentic-service-harness-live-producer-execution-authority-review-intake-digest-packet:1"
    )
    assert packet_entry["surface"] == "runtime"


def test_protocol_manifest_indexes_agentic_service_harness_live_producer_execution_authority_review_submission_admission_preflight() -> None:
    manifest = load_manifest()
    entries = {entry["schema_id"]: entry for entry in manifest["schemas"]}
    preflight_entry = entries[
        "agentic-service-harness-live-producer-execution-authority-review-submission-admission-preflight"
    ]

    assert validate_protocol_manifest(manifest) == []
    assert preflight_entry["path"] == (
        "schemas/agentic_service_harness_live_producer_execution_authority_review_submission_admission_preflight.schema.json"
    )
    assert preflight_entry["urn"] == (
        "urn:mullusi:schema:agentic-service-harness-live-producer-execution-authority-review-submission-admission-preflight:1"
    )
    assert preflight_entry["surface"] == "runtime"
...
def test_protocol_manifest_indexes_agentic_service_harness_live_producer_execution_authority_review_submission_receipt_emission_preflight() -> None:
    manifest = load_manifest()
    entries = {entry["schema_id"]: entry for entry in manifest["schemas"]}
    preflight_entry = entries[
        "agentic-service-harness-live-producer-execution-authority-review-submission-receipt-emission-preflight"
    ]

    assert validate_protocol_manifest(manifest) == []
    assert preflight_entry["path"] == (
        "schemas/agentic_service_harness_live_producer_execution_authority_review_submission_receipt_emission_preflight.schema.json"
    )
    assert preflight_entry["urn"] == (
        "urn:mullusi:schema:agentic-service-harness-live-producer-execution-authority-review-submission-receipt-emission-preflight:1"
    )
    assert preflight_entry["surface"] == "runtime"


def test_protocol_manifest_indexes_agentic_service_harness_live_producer_execution_authority_review_submission_receipt_append_admission_preflight() -> None:
    manifest = load_manifest()
    entries = {entry["schema_id"]: entry for entry in manifest["schemas"]}
    preflight_entry = entries[
        "agentic-service-harness-live-producer-execution-authority-review-submission-receipt-append-admission-preflight"
    ]

    assert validate_protocol_manifest(manifest) == []
    assert preflight_entry["path"] == (
        "schemas/agentic_service_harness_live_producer_execution_authority_review_submission_receipt_append_admission_preflight.schema.json"
    )
    assert preflight_entry["urn"] == (
        "urn:mullusi:schema:agentic-service-harness-live-producer-execution-authority-review-submission-receipt-append-admission-preflight:1"
    )
    assert preflight_entry["surface"] == "runtime"


def test_protocol_manifest_indexes_personal_assistant_ref_absence_closure_review() -> None:
    manifest = load_manifest()
    entries = {entry["schema_id"]: entry for entry in manifest["schemas"]}
    review_entry = entries["personal-assistant-ref-absence-closure-review"]

    assert validate_protocol_manifest(manifest) == []
    assert review_entry["path"] == "schemas/pa_ref_absence_closure_review.schema.json"
    assert review_entry["urn"] == "urn:mullusi:schema:personal-assistant-ref-absence-closure-review:1"
    assert review_entry["surface"] == "personal-assistant"
...
def test_protocol_manifest_indexes_collaboration_case() -> None:
    ...
    assert (
        operator_sandbox_patch_readiness_entry["path"]
        == "schemas/operator_sandbox_patch_readiness_compact_read_model.schema.json"
    )
    assert operator_sandbox_patch_readiness_entry["urn"] == (
...
def test_protocol_manifest_indexes_personal_assistant_operator_reapproval_decision_receipt_value_binding_guard() -> None:
    manifest = load_manifest()
    entries = {entry["schema_id"]: entry for entry in manifest["schemas"]}
    value_binding_guard_entry = entries[
        "personal-assistant-operator-reapproval-decision-receipt-value-binding-guard"
    ]

    assert validate_protocol_manifest(manifest) == []
    assert value_binding_guard_entry["path"] == (
        "schemas/personal_assistant_operator_reapproval_decision_receipt_value_binding_guard.schema.json"
    )
    assert value_binding_guard_entry["urn"] == (
        "urn:mullusi:schema:personal-assistant-operator-reapproval-decision-receipt-value-binding-guard:1"
    )
    assert value_binding_guard_entry["surface"] == "approval"


def test_protocol_manifest_indexes_personal_assistant_operator_reapproval_decision_receipt_value_binding_absence() -> None:
    manifest = load_manifest()
    entries = {entry["schema_id"]: entry for entry in manifest["schemas"]}
    value_binding_absence_entry = entries[
        "personal-assistant-operator-reapproval-decision-receipt-value-binding-absence"
    ]

    assert validate_protocol_manifest(manifest) == []
    assert value_binding_absence_entry["path"] == (
        "schemas/personal_assistant_operator_reapproval_decision_receipt_value_binding_absence.schema.json"
    )
    assert value_binding_absence_entry["urn"] == (
        "urn:mullusi:schema:personal-assistant-operator-reapproval-decision-receipt-value-binding-absence:1"
    )
    assert value_binding_absence_entry["surface"] == "approval"


def test_protocol_manifest_indexes_personal_assistant_operator_reapproval_decision_receipt_value_binding_admission_preflight() -> None:
    manifest = load_manifest()
    entries = {entry["schema_id"]: entry for entry in manifest["schemas"]}
    value_binding_admission_preflight_entry = entries[
        "personal-assistant-operator-reapproval-decision-receipt-value-binding-admission-preflight"
    ]

    assert validate_protocol_manifest(manifest) == []
    assert value_binding_admission_preflight_entry["path"] == (
        "schemas/personal_assistant_operator_reapproval_decision_receipt_value_binding_admission_preflight.schema.json"
    )
    assert value_binding_admission_preflight_entry["urn"] == (
        "urn:mullusi:schema:personal-assistant-operator-reapproval-decision-receipt-value-binding-admission-preflight:1"
    )
    assert value_binding_admission_preflight_entry["surface"] == "approval"


def test_protocol_manifest_indexes_personal_assistant_operator_reapproval_decision_receipt_value_binding_contract() -> None:
    manifest = load_manifest()
    entries = {entry["schema_id"]: entry for entry in manifest["schemas"]}
    value_binding_contract_entry = entries[
        "personal-assistant-operator-reapproval-decision-receipt-value-binding-contract"
    ]

    assert validate_protocol_manifest(manifest) == []
    assert value_binding_contract_entry["path"] == (
        "schemas/personal_assistant_operator_reapproval_decision_receipt_value_binding_contract.schema.json"
    )
    assert value_binding_contract_entry["urn"] == (
        "urn:mullusi:schema:personal-assistant-operator-reapproval-decision-receipt-value-binding-contract:1"
    )
    assert value_binding_contract_entry["surface"] == "approval"


def test_protocol_manifest_indexes_personal_assistant_operator_reapproval_decision_receipt_value_binding_record_guard() -> None:
    manifest = load_manifest()
    entries = {entry["schema_id"]: entry for entry in manifest["schemas"]}
    value_binding_record_guard_entry = entries[
        "personal-assistant-operator-reapproval-decision-receipt-value-binding-record-guard"
    ]

    assert validate_protocol_manifest(manifest) == []
    assert value_binding_record_guard_entry["path"] == (
        "schemas/personal_assistant_operator_reapproval_decision_receipt_value_binding_record_guard.schema.json"
    )
    assert value_binding_record_guard_entry["urn"] == (
        "urn:mullusi:schema:personal-assistant-operator-reapproval-decision-receipt-value-binding-record-guard:1"
    )
...
def test_protocol_manifest_indexes_patch_proposal_contract() -> None:
    ...
    assert entries["local-developer-workflow-pr-admission-packet"]["urn"] == (
        "urn:mullusi:schema:local-developer-workflow-pr-admission-packet:1"
    )
    assert entries["local-developer-workflow-pr-admission-packet"]["surface"] == "software_dev"
