"""Purpose: verify the workflow holistic loop admission dossier.
Governance scope: workflow loop admission readiness, registry-admission boundary,
read-only projection, and terminal-closure separation.
Dependencies: scripts.report_holistic_loop_workflow_admission_dossier.
Invariants:
  - Workflow admission dossier reports default registry admission when present.
  - Workflow admission dossier does not mutate loop registration.
  - Workflow admission dossier remains read-only and non-terminal.
...
from scripts import report_holistic_loop_workflow_admission_dossier as reporter


def test_workflow_admission_dossier_builds_proposed_manifest() -> None:
    dossier = reporter.build_dossier()
    errors = reporter.validate_dossier(dossier)
    manifest = dossier["proposed_manifest"]

    assert errors == []
    assert dossier["candidate_id"] == "workflow_execution_loop"
...
def test_workflow_admission_dossier_reports_registry_admission() -> None:
    dossier = reporter.build_dossier()
    ...
    assert operator_decision["decision_ref"] == "default_registry:workflow_execution_loop"
    assert operator_decision["blocks_registration"] is False


def test_workflow_admission_dossier_does_not_register_or_mutate_runtime() -> None:
    dossier = reporter.build_dossier()
...
def test_workflow_admission_dossier_reports_complete_readiness_sections() -> None:
    dossier = reporter.build_dossier()
...
def test_workflow_admission_dossier_rejects_registration_or_terminal_claim() -> None:
    dossier = reporter.build_dossier()
...
def test_workflow_admission_dossier_rejects_missing_source_or_gap_regression() -> None:
    dossier = reporter.build_dossier()
    invalid_dossier = copy.deepcopy(dossier)
    invalid_dossier["existing_surface_refs"].append("missing/workflow_surface.json")
    invalid_dossier["evidence_gap_report"]["missing_evidence"] = ["workflow_descriptor_valid"]
    invalid_dossier["rollback_readiness"]["rollback_available"] = False

    errors = reporter.validate_dossier(invalid_dossier)

    assert "missing surface: missing/workflow_surface.json" in errors
