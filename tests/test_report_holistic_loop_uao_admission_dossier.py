...
Governance scope: UAO loop admission readiness, non-registration boundary,
read-only projection, and terminal-closure separation.
Dependencies: scripts.report_holistic_loop_uao_admission_dossier.
Invariants:
  - UAO admission dossier can become ready for operator decision only.
  - UAO admission dossier does not register loops.
...
def test_uao_admission_dossier_is_ready_only_for_operator_decision() -> None:
    dossier = reporter.build_dossier()
