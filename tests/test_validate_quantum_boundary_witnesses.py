...
from scripts.validate_quantum_boundary_witnesses import (
    ...
    FIXTURE_CATALOG_ID,
    FIXTURE_SERIALIZER_AUTHORITY_REQUIREMENTS_EXAMPLE,
...
def test_aggregate_rejects_fixture_serializer_implementation_authority(tmp_path: pathlib.Path) -> None:
    payload = _payload(FIXTURE_SERIALIZER_AUTHORITY_REQUIREMENTS_EXAMPLE)
    payload["denied_authorities"]["serializer_implementation_enabled"] = True
    altered_path = _write_payload(tmp_path / "fixture-serializer-authority-invalid.json", payload)

    result = validate_witnesses({FIXTURE_SERIALIZER_AUTHORITY_REQUIREMENTS_ID: altered_path})

    assert result["passed"] is False
    assert result["witness_count"] == 6
    assert any(
        (
            f"{FIXTURE_SERIALIZER_AUTHORITY_REQUIREMENTS_ID}: "
            "denied_authorities.serializer_implementation_enabled must be false"
        )
        == error
        for error in result["errors"]
    )
    assert next(
        ...
        if witness["binding_id"] == FIXTURE_SERIALIZER_AUTHORITY_REQUIREMENTS_ID
    )["passed"] is False
