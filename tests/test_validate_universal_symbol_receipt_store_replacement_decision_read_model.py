"""Tests for replacement decision read-model validation.

Purpose: prove the replacement decision operator projection exposes bounded
status without decision authority, raw detail visibility, or replacement
recording.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: replacement decision read-model validator and schema.
Invariants:
...
from scripts.validate_universal_symbol_receipt_store_replacement_decision_read_model import (
    ...
    DEFAULT_SCHEMA_PATH,
    UniversalSymbolReceiptStoreReplacementDecisionReadModelError,
...
def _write_case(tmp_path: Path, value: dict[str, object]) -> Path:
    case_path = tmp_path / "replacement-decision-read-model.json"
    case_path.write_text(json.dumps(value), encoding="utf-8")
    return case_path


def test_foundation_replacement_decision_read_model_validates() -> None:
...
def test_replacement_decision_read_model_rejects_authority_drift(tmp_path: Path) -> None:
    read_model = json.loads(DEFAULT_READ_MODEL_PATH.read_text(encoding="utf-8"))
    changed = copy.deepcopy(read_model)
    changed["effective_denials"]["replacement_decision_recorded"] = True

    with pytest.raises(
        UniversalSymbolReceiptStoreReplacementDecisionReadModelError,
        match="effective_denials.replacement_decision_recorded",
    ):
        validate_universal_symbol_receipt_store_replacement_decision_read_model(
    ...
    assert changed["read_model_is_not_decision_authority"] is True
    assert changed["effective_denials"]["replacement_decision_recorded"] is True
    assert len(changed["requirement_rows"]) == 8


def test_replacement_decision_read_model_rejects_raw_detail_visibility(tmp_path: Path) -> None:
    read_model = json.loads(DEFAULT_READ_MODEL_PATH.read_text(encoding="utf-8"))
    changed = copy.deepcopy(read_model)
    ...
    assert changed["requirement_rows"][0]["requirement_id"] == "requirement://superseded-approval-decision"
    assert changed["requirement_rows"][0]["authority_granted"] is False
    assert changed["effective_denials"]["raw_payload_stored"] is False


def test_replacement_decision_read_model_rejects_missing_requirement(tmp_path: Path) -> None:
    read_model = json.loads(DEFAULT_READ_MODEL_PATH.read_text(encoding="utf-8"))
    changed = copy.deepcopy(read_model)
    changed["requirement_rows"] = [
        row
        for row in changed["requirement_rows"]
        if row["requirement_id"] != "requirement://replacement-reason"
    ]
    changed["contract_summary"]["requirement_row_count"] = len(changed["requirement_rows"])

    with pytest.raises(
        UniversalSymbolReceiptStoreReplacementDecisionReadModelError,
        match="requirement://replacement-reason",
    ):
        validate_universal_symbol_receipt_store_replacement_decision_read_model(
            _write_case(tmp_path, changed),
            DEFAULT_SCHEMA_PATH,
...
def test_replacement_decision_read_model_rejects_duplicate_requirement_row(tmp_path: Path) -> None:
    read_model = json.loads(DEFAULT_READ_MODEL_PATH.read_text(encoding="utf-8"))
    changed = copy.deepcopy(read_model)
...
def test_replacement_decision_read_model_rejects_missing_delta_reject_log(tmp_path: Path) -> None:
    read_model = json.loads(DEFAULT_READ_MODEL_PATH.read_text(encoding="utf-8"))
    changed = copy.deepcopy(read_model)
...
def test_replacement_decision_read_model_rejects_receipt_projection_drift(tmp_path: Path) -> None:
    read_model = json.loads(DEFAULT_READ_MODEL_PATH.read_text(encoding="utf-8"))
    changed = copy.deepcopy(read_model)
    ...
    assert changed["receipt_projection"]["replacement_requirement_count"] == 8
    assert len(changed["requirement_rows"]) == 8


def test_replacement_decision_read_model_rejects_evidence_ref_count_drift(tmp_path: Path) -> None:
    read_model = json.loads(DEFAULT_READ_MODEL_PATH.read_text(encoding="utf-8"))
    changed = copy.deepcopy(read_model)
