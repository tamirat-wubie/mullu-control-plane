"""Tests for the Foundation Mode runtime witness deferral validator.

Purpose: prove runtime witness deferral stays local and does not become
signature verification, endpoint evidence, conformance, readiness, publication,
or deployment evidence.
Governance scope: Foundation Mode, issue #330 runtime witness deferral,
payload blocking, signature blocking, endpoint-probe blocking, readiness
blocking, and deployment restraint.
Dependencies: scripts.validate_foundation_runtime_witness_deferral_boundary.
Invariants: only public-safe deferral labels are allowed; all runtime witness
surfaces remain AwaitingEvidence; no payloads, signatures, endpoints, secrets,
...
from scripts.validate_foundation_runtime_witness_deferral_boundary import (
    BLOCKED_CLAIMS,
    DEFAULT_DOC_PATH,
    DEFAULT_PACKET_PATH,
    DEFERRAL_LABELS,
...
def test_witness_contract_matches_expected_labels_and_claims() -> None:
    payload = load_json_object(DEFAULT_PACKET_PATH, "runtime witness deferral witness")
...
def test_false_flag_drift_is_reported(tmp_path: Path) -> None:
    ...
    candidate["runtime_witness_signature_verified"] = True
    packet_path = _write_packet(tmp_path, candidate)

    findings = validate_artifacts(packet_path=packet_path)

    assert any("runtime_witness_signature_verified must remain false" in finding.message for finding in findings)
...
def test_surface_state_and_note_drift_are_reported(tmp_path: Path) -> None:
    payload = load_json_object(DEFAULT_PACKET_PATH, "runtime witness deferral witness")
    candidate = copy.deepcopy(payload)
    candidate["surfaces"][4]["state"] = "SolvedVerified"
    candidate["surfaces"][4]["public_safe_note"] = "Signature is verified."
    packet_path = _write_packet(tmp_path, candidate)
...
def test_forbidden_live_values_are_reported(tmp_path: Path) -> None:
    payload = load_json_object(DEFAULT_PACKET_PATH, "runtime witness deferral witness")
    candidate = copy.deepcopy(payload)
    candidate["surfaces"][3]["public_safe_note"] = (
        "Witness payload recorded at https://api.example.test with token sk-runtime-secret and hash "
...
def test_promotion_phrase_is_reported(tmp_path: Path) -> None:
    payload = load_json_object(DEFAULT_PACKET_PATH, "runtime witness deferral witness")
    candidate = copy.deepcopy(payload)
    candidate["surfaces"][6]["public_safe_note"] = "Runtime conformance has passed."
    packet_path = _write_packet(tmp_path, candidate)

    findings = validate_artifacts(packet_path=packet_path)

    assert any("forbidden promotion pattern: conformance_passed" in finding.message for finding in findings)
    assert any("surface note drifted" in finding.message for finding in findings)
    assert len(findings) >= 2


def test_doc_required_phrase_drift_is_reported(tmp_path: Path) -> None:
    doc_text = load_text(DEFAULT_DOC_PATH, "runtime witness deferral doc")
    candidate_text = doc_text.replace(REQUIRED_DOC_PHRASES[0], "Foundation Runtime Witness Draft")
    ...
    assert "Foundation Runtime Witness Deferral Boundary" in doc_text
