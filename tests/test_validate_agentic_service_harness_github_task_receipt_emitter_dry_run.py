"""Test GitHub task receipt-emitter dry-run validation.

Purpose: verify the harness GitHub task receipt-emitter dry-run remains
contract-only and append-denied.
Governance scope: [OCE, RAG, CDCV, CQTE, UWMA, SRCA, PRS]
Dependencies: scripts.validate_agentic_service_harness_github_task_receipt_emitter_dry_run.
Invariants:
  - Dry-run receipt emission never executes adapters or writes repositories.
...
from scripts import validate_agentic_service_harness_github_task_receipt_emitter_dry_run as validator


def test_github_task_receipt_emitter_dry_run_passes() -> None:
    validation = validator.validate_agentic_service_harness_github_task_receipt_emitter_dry_run()
...
def test_github_task_receipt_emitter_rejects_authority_drift() -> None:
    payload = validator.build_mutated_dry_run(
        ...
        simulated_receipt_emission__repository_written=True,
    ...
    assert "simulated_receipt_emission.repository_written must be false" in serialized_errors
...
def test_github_task_receipt_emitter_rejects_append_gate_drift() -> None:
    payload = validator.build_mutated_dry_run(
...
def test_github_task_receipt_emitter_rejects_missing_required_refs() -> None:
    payload = validator.build_mutated_dry_run(
...
def test_github_task_receipt_emitter_rejects_mutation_route_and_secret_like_payload() -> None:
    payload = validator.build_mutated_dry_run(
        next_action="POST /api/github/tasks/append should never be admitted",
    )
...
def test_github_task_receipt_emitter_cli_writes_report(tmp_path: Path, capsys) -> None:
    output_path = tmp_path / "github-task-receipt-emitter-dry-run-validation.json"
