...
def test_capability_friction_control_validates_and_writes(tmp_path: Path) -> None:
    ...
    assert validation.capability_count == 12
...
def test_capability_friction_control_runtime_projection_is_bounded() -> None:
    ...
    assert read_model["summary"]["capability_count"] == 12
    ...
    assert validators["operator_safe_local_action_read_model_builder"]["command"] == (
        "python scripts/build_operator_safe_local_action_read_model.py --json"
    )
    assert validators["operator_local_developer_workflow_receipt_read_model_builder"]["command"] == (
    ...
    assert validators["operator_safe_local_action_read_model_tests"]["command"] == (
        "python -m pytest tests/test_build_operator_safe_local_action_read_model.py -q"
    )
    assert validators["operator_local_developer_workflow_receipt_read_model_tests"]["command"] == (
