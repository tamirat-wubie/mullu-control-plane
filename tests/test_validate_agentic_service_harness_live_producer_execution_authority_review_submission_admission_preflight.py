"""Tests for live producer execution authority review submission admission preflight validation."""
...
def _write_fixture(tmp_path: Path, admission: dict[str, object]) -> Path:
    path = tmp_path / "execution_authority_review_submission_admission.json"
    path.write_text(json.dumps(admission, indent=2), encoding="utf-8")
    return path


def test_accepts_default_fixture() -> None:
    validation, fixture = validate_live_producer_execution_authority_review_submission_admission_preflight()
...
def test_rejects_submission_admission(tmp_path: Path) -> None:
    changed = copy.deepcopy(_load_fixture())
    changed["submission_admission_requirements"][0]["required_digest_ref"] = "receipt://already-digested"
    changed["submission_admission_requirements"][0]["status"] = "SolvedVerified"
    changed["submission_admission_requirements"][0]["review_submission_admissible"] = True
    changed["missing_submission_admission_requirements"][0]["status"] = "SolvedVerified"

    validation, _ = validate_live_producer_execution_authority_review_submission_admission_preflight(
...
def test_rejects_missing_input_id(tmp_path: Path) -> None:
    changed = copy.deepcopy(_load_fixture())
    changed["missing_submission_admission_requirements"] = changed["missing_submission_admission_requirements"][:-1]
