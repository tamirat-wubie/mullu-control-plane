...
def test_representative_routes_are_not_unclassified() -> None:
    ...
    assert classified_routes["/api/v1/multi-agent/handoff"]["surface_id"] == "multi_agent_coordination_runtime"
    assert classified_routes["/api/v1/multi-agent/conflicts/unresolved"]["surface_id"] == (
...
def test_multi_agent_coordination_runtime_surface_is_witnessed() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    route_records = {
    ...
    assert "multi_agent_delegation_request_typed" in witnesses
    assert "multi_agent_delegation_resolution_guarded" in witnesses
    assert "multi_agent_handoff_preserves_context_ids" in witnesses
    assert "multi_agent_merge_requires_valid_outcome" in witnesses
    assert "multi_agent_conflict_requires_valid_strategy" in witnesses
    assert "multi_agent_unresolved_conflicts_read_model_bounded" in witnesses
    assert "multi_agent_summary_is_governed_read_model" in witnesses
    assert "multi_agent_errors_are_sanitized" in witnesses
    ...
    assert route_records["/api/v1/multi-agent/merge"]["coverage_state"] == "witnessed"
    assert route_records["/api/v1/multi-agent/merge"]["surface_id"] == "multi_agent_coordination_runtime"
