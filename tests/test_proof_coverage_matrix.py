...
def test_witness_integrity_report_tracks_exact_test_anchors() -> None:
    ...
    assert surfaces["governed_operational_intelligence"]["exact_test_anchor_count"] == 16
...
def test_governed_operational_intelligence_surface_is_witnessed() -> None:
    ...
    assert "bind_repository_world_state_projection_to_problem_star_evidence" in operational_surface["representative_paths"]
    ...
    assert "repository_world_state_projection_binds_problem_star_evidence" in witnesses
    assert "repository_world_state_projection_blocks_problem_star_evidence" in witnesses
...
def test_universal_symbol_runtime_admission_policy_surface_blocks_runtime_authority() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_runtime_admission_policy"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_runtime_admission_policy.schema.json" in surface["representative_paths"]
    assert "examples/universal_symbol_runtime_admission_policy.foundation.json" in surface["representative_paths"]
    assert "scripts/validate_universal_symbol_runtime_admission_policy.py" in surface["evidence_files"]
    assert "foundation_universal_symbol_runtime_admission_policy_validates" in witnesses
    assert "runtime_admission_policy_rejects_live_dispatch_drift" in witnesses
    assert "runtime_admission_policy_rejects_receipt_append_drift" in witnesses
    assert witness_surfaces["universal_symbol_runtime_admission_policy"]["exact_test_anchor_count"] == 5
    assert witness_surfaces["universal_symbol_runtime_admission_policy"]["unanchored_witness_count"] == 0
...
def test_universal_symbol_runtime_live_witness_input_receipt_surface_blocks_live_witness() -> None:
    ...
    assert "scripts/produce_universal_symbol_runtime_live_witness_input_receipt.py" in surface[
        "evidence_files"
    ]
    assert "tests/test_produce_universal_symbol_runtime_live_witness_input_receipt.py" in surface[
        "evidence_files"
    ]
    ...
    assert "producer_builds_foundation_receipt_without_authority" in witnesses
    assert "producer_accepts_reference_overrides_without_accepting_live_witness" in witnesses
    assert "cli_stdout_receipt_validates_without_writing" in witnesses
    assert witness_surfaces["universal_symbol_runtime_live_witness_input_receipt"]["exact_test_anchor_count"] == 11
...
def test_repository_observation_evidence_packet_surface_blocks_hard_planning() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["repository_observation_evidence_packet"]
    witnesses = set(surface["runtime_witnesses"])
...
def test_universal_symbol_lane_runtime_authority_evidence_receipt_surface_blocks_lane_authority() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_lane_runtime_authority_evidence_receipt"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_lane_runtime_authority_evidence_receipt.schema.json" in surface[
        "representative_paths"
    ]
    assert "examples/universal_symbol_lane_runtime_authority_evidence_receipt.foundation.json" in surface[
        "representative_paths"
    ]
    assert "scripts/validate_universal_symbol_lane_runtime_authority_evidence_receipt.py" in surface[
        "evidence_files"
    ]
    assert "foundation_universal_symbol_lane_runtime_authority_evidence_receipt_validates" in witnesses
    assert "lane_runtime_authority_evidence_receipt_rejects_authority_drift" in witnesses
    assert "lane_runtime_authority_evidence_receipt_rejects_observed_evidence_drift" in witnesses
    assert witness_surfaces["universal_symbol_lane_runtime_authority_evidence_receipt"]["exact_test_anchor_count"] == 6
    assert witness_surfaces["universal_symbol_lane_runtime_authority_evidence_receipt"]["unanchored_witness_count"] == 0
...
def test_universal_symbol_lane_runtime_authority_evidence_value_receipt_surface_blocks_lane_authority() -> None:
    ...
    assert "scripts/verify_universal_symbol_lane_runtime_authority_evidence_value_refs.py" in surface[
        "evidence_files"
    ]
...
def test_universal_symbol_runtime_authority_witness_surface_blocks_runtime_authority() -> None:
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_runtime_authority_witness"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_runtime_authority_witness.schema.json" in surface["representative_paths"]
    assert "examples/universal_symbol_runtime_authority_witness.foundation.json" in surface["representative_paths"]
    assert "scripts/validate_universal_symbol_runtime_authority_witness.py" in surface["evidence_files"]
    assert "foundation_universal_symbol_runtime_authority_witness_validates" in witnesses
    assert "runtime_authority_witness_rejects_runtime_authority_drift" in witnesses
    assert "runtime_authority_witness_rejects_missing_delta_reject" in witnesses
...
def test_universal_symbol_adapter_receipt_persistence_policy_surface_blocks_append_authority() -> None:
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_adapter_receipt_persistence_policy"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_adapter_receipt_persistence_policy.schema.json" in surface["representative_paths"]
    assert "examples/universal_symbol_adapter_receipt_persistence_policy.foundation.json" in surface["representative_paths"]
    assert "scripts/validate_universal_symbol_adapter_receipt_persistence_policy.py" in surface["evidence_files"]
    assert "foundation_universal_symbol_adapter_receipt_persistence_policy_validates" in witnesses
    assert "adapter_receipt_persistence_policy_rejects_append_drift" in witnesses
    assert "adapter_receipt_persistence_policy_rejects_raw_payload_drift" in witnesses
    assert witness_surfaces["universal_symbol_adapter_receipt_persistence_policy"]["exact_test_anchor_count"] == 5
    assert witness_surfaces["universal_symbol_adapter_receipt_persistence_policy"]["unanchored_witness_count"] == 0
    assert closure_actions["publish_universal_symbol_adapter_receipt_persistence_policy"]["status"] == "closed"


def test_universal_symbol_append_audit_witness_surface_blocks_append_authority() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_append_audit_witness"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_append_audit_witness.schema.json" in surface["representative_paths"]
    assert "examples/universal_symbol_append_audit_witness.foundation.json" in surface["representative_paths"]
    assert "scripts/validate_universal_symbol_append_audit_witness.py" in surface["evidence_files"]
    assert "foundation_universal_symbol_append_audit_witness_validates" in witnesses
    assert "append_audit_witness_rejects_append_authority_drift" in witnesses
    assert "append_audit_witness_rejects_missing_delta_reject" in witnesses
    assert witness_surfaces["universal_symbol_append_audit_witness"]["exact_test_anchor_count"] == 5
    assert witness_surfaces["universal_symbol_append_audit_witness"]["unanchored_witness_count"] == 0
    assert closure_actions["publish_universal_symbol_append_audit_witness"]["status"] == "closed"


def test_universal_symbol_receipt_store_authority_witness_surface_blocks_authority() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_authority_witness"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_receipt_store_authority_witness.schema.json" in surface["representative_paths"]
    assert "examples/universal_symbol_receipt_store_authority_witness.foundation.json" in surface["representative_paths"]
    assert "scripts/validate_universal_symbol_receipt_store_authority_witness.py" in surface["evidence_files"]
    assert "foundation_universal_symbol_receipt_store_authority_witness_validates" in witnesses
    assert "receipt_store_authority_witness_rejects_authority_grant_drift" in witnesses
    assert "receipt_store_authority_witness_rejects_append_precondition_drift" in witnesses
    assert "receipt_store_authority_witness_rejects_missing_lifecycle_requirement" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_authority_witness"]["exact_test_anchor_count"] == 6
    assert witness_surfaces["universal_symbol_receipt_store_authority_witness"]["unanchored_witness_count"] == 0
    assert closure_actions["publish_universal_symbol_receipt_store_authority_witness"]["status"] == "closed"


def test_universal_symbol_receipt_store_operator_approval_witness_surface_blocks_approval_authority() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_operator_approval_witness"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_receipt_store_operator_approval_witness.schema.json" in surface["representative_paths"]
    assert "examples/universal_symbol_receipt_store_operator_approval_witness.foundation.json" in surface["representative_paths"]
    assert "scripts/validate_universal_symbol_receipt_store_operator_approval_witness.py" in surface["evidence_files"]
    assert "foundation_universal_symbol_receipt_store_operator_approval_witness_validates" in witnesses
    assert "operator_approval_witness_rejects_approval_authority_drift" in witnesses
    assert "operator_approval_witness_rejects_missing_delta_reject" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_operator_approval_witness"]["exact_test_anchor_count"] == 7
    assert witness_surfaces["universal_symbol_receipt_store_operator_approval_witness"]["unanchored_witness_count"] == 0
    assert closure_actions["publish_universal_symbol_receipt_store_operator_approval_witness"]["status"] == "closed"


def test_universal_symbol_receipt_store_operator_identity_witness_surface_blocks_identity_authority() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_operator_identity_witness"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_receipt_store_operator_identity_witness.schema.json" in surface["representative_paths"]
    assert "examples/universal_symbol_receipt_store_operator_identity_witness.foundation.json" in surface["representative_paths"]
    assert "scripts/validate_universal_symbol_receipt_store_operator_identity_witness.py" in surface["evidence_files"]
    assert "foundation_universal_symbol_receipt_store_operator_identity_witness_validates" in witnesses
    assert "operator_identity_witness_rejects_identity_authority_drift" in witnesses
    assert "operator_identity_witness_rejects_missing_delta_reject" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_operator_identity_witness"]["exact_test_anchor_count"] == 6
    assert witness_surfaces["universal_symbol_receipt_store_operator_identity_witness"]["unanchored_witness_count"] == 0
    assert closure_actions["publish_universal_symbol_receipt_store_operator_identity_witness"]["status"] == "closed"


def test_universal_symbol_receipt_store_operator_approval_decision_witness_surface_blocks_decision_authority() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_operator_approval_decision_witness"]
    witnesses = set(surface["runtime_witnesses"])
    ...
    assert (
        ...
        in surface["representative_paths"]
    )
    assert (
        "examples/universal_symbol_receipt_store_operator_approval_decision_witness.foundation.json"
        in surface["representative_paths"]
    )
    assert "scripts/validate_universal_symbol_receipt_store_operator_approval_decision_witness.py" in surface[
        "evidence_files"
    ]
    assert "foundation_universal_symbol_receipt_store_operator_approval_decision_witness_validates" in witnesses
    assert "operator_approval_decision_witness_rejects_decision_authority_drift" in witnesses
    assert "operator_approval_decision_witness_rejects_missing_delta_reject" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_operator_approval_decision_witness"][
        "exact_test_anchor_count"
    ] == 6
    assert witness_surfaces["universal_symbol_receipt_store_operator_approval_decision_witness"][
        "unanchored_witness_count"
    ...
    assert (
        closure_actions["publish_universal_symbol_receipt_store_operator_approval_decision_witness"]["status"]
        == "closed"
    )


def test_universal_symbol_receipt_store_operator_reapproval_expiry_witness_surface_blocks_temporal_authority() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_operator_reapproval_expiry_witness"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_receipt_store_operator_reapproval_expiry_witness.schema.json" in surface[
        "representative_paths"
    ]
    assert "examples/universal_symbol_receipt_store_operator_reapproval_expiry_witness.foundation.json" in surface[
        "representative_paths"
    ]
    assert "scripts/validate_universal_symbol_receipt_store_operator_reapproval_expiry_witness.py" in surface[
        "evidence_files"
    ]
    assert "foundation_universal_symbol_receipt_store_operator_reapproval_expiry_witness_validates" in witnesses
    assert "operator_reapproval_expiry_witness_rejects_temporal_authority_drift" in witnesses
    assert "operator_reapproval_expiry_witness_rejects_missing_delta_reject" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_operator_reapproval_expiry_witness"][
        "exact_test_anchor_count"
    ] == 6
    assert witness_surfaces["universal_symbol_receipt_store_operator_reapproval_expiry_witness"][
        "unanchored_witness_count"
    ...
    assert (
        closure_actions["publish_universal_symbol_receipt_store_operator_reapproval_expiry_witness"]["status"]
        == "closed"
    )


def test_universal_symbol_receipt_store_operator_revocation_witness_surface_blocks_revocation_authority() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_operator_revocation_witness"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_receipt_store_operator_revocation_witness.schema.json" in surface[
        "representative_paths"
    ]
    assert "examples/universal_symbol_receipt_store_operator_revocation_witness.foundation.json" in surface[
        "representative_paths"
    ]
    assert "scripts/validate_universal_symbol_receipt_store_operator_revocation_witness.py" in surface[
        "evidence_files"
    ]
    assert "foundation_universal_symbol_receipt_store_operator_revocation_witness_validates" in witnesses
    assert "operator_revocation_witness_rejects_revocation_authority_drift" in witnesses
    assert "operator_revocation_witness_rejects_missing_delta_reject" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_operator_revocation_witness"][
        "exact_test_anchor_count"
    ] == 6
    assert witness_surfaces["universal_symbol_receipt_store_operator_revocation_witness"][
        "unanchored_witness_count"
    ] == 0
    assert closure_actions["publish_universal_symbol_receipt_store_operator_revocation_witness"]["status"] == "closed"


def test_universal_symbol_receipt_store_reapproval_revocation_witness_surface_blocks_lifecycle_authority() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_reapproval_revocation_witness"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_receipt_store_reapproval_revocation_witness.schema.json" in surface[
        "representative_paths"
    ]
    assert "examples/universal_symbol_receipt_store_reapproval_revocation_witness.foundation.json" in surface[
        "representative_paths"
    ]
    assert "scripts/validate_universal_symbol_receipt_store_reapproval_revocation_witness.py" in surface[
        "evidence_files"
    ]
    assert "foundation_universal_symbol_receipt_store_reapproval_revocation_witness_validates" in witnesses
    assert "reapproval_revocation_witness_rejects_lifecycle_authority_drift" in witnesses
    assert "reapproval_revocation_witness_rejects_missing_delta_reject" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_reapproval_revocation_witness"][
        "exact_test_anchor_count"
    ] == 6
    assert witness_surfaces["universal_symbol_receipt_store_reapproval_revocation_witness"][
        "unanchored_witness_count"
    ...
    assert (
        closure_actions["publish_universal_symbol_receipt_store_reapproval_revocation_witness"]["status"]
        == "closed"
    )


def test_universal_symbol_receipt_store_lifecycle_evidence_receipt_surface_blocks_lifecycle_recording() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_lifecycle_evidence_receipt"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_receipt_store_lifecycle_evidence_receipt.schema.json" in surface[
        "representative_paths"
    ]
    assert "examples/universal_symbol_receipt_store_lifecycle_evidence_receipt.foundation.json" in surface[
        "representative_paths"
    ]
    assert "schemas/universal_symbol_receipt_store_operator_revocation_witness.schema.json" in surface[
        "evidence_files"
    ]
    assert "schemas/universal_symbol_receipt_store_replacement_decision_receipt.schema.json" in surface[
        "evidence_files"
    ]
    ...
    assert "scripts/verify_universal_symbol_receipt_store_lifecycle_evidence_refs.py" in surface[
        "evidence_files"
    ]
...
def test_universal_symbol_receipt_store_lifecycle_audit_receipt_surface_blocks_audit_authority() -> None:
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_lifecycle_audit_receipt"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_receipt_store_lifecycle_audit_receipt.schema.json" in surface[
        "representative_paths"
    ]
    assert "examples/universal_symbol_receipt_store_lifecycle_audit_receipt.foundation.json" in surface[
        "representative_paths"
    ]
    assert "scripts/validate_universal_symbol_receipt_store_lifecycle_audit_receipt.py" in surface[
        "evidence_files"
    ]
    assert "foundation_universal_symbol_receipt_store_lifecycle_audit_receipt_validates" in witnesses
    assert "lifecycle_audit_receipt_rejects_lifecycle_authority_drift" in witnesses
    assert "lifecycle_audit_receipt_rejects_missing_delta_reject" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_lifecycle_audit_receipt"][
        "exact_test_anchor_count"
    ] == 6
    assert witness_surfaces["universal_symbol_receipt_store_lifecycle_audit_receipt"][
        "unanchored_witness_count"
    ] == 0
    assert closure_actions["publish_universal_symbol_receipt_store_lifecycle_audit_receipt"]["status"] == "closed"


def test_universal_symbol_receipt_store_lifecycle_audit_read_model_surface_is_read_only() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_lifecycle_audit_read_model"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "read_model"
    assert surface["action_proof"] == "read_model"
    assert "schemas/universal_symbol_receipt_store_lifecycle_audit_read_model.schema.json" in surface[
        "representative_paths"
    ]
    assert "examples/universal_symbol_receipt_store_lifecycle_audit_read_model.foundation.json" in surface[
        "representative_paths"
    ]
    assert "scripts/validate_universal_symbol_receipt_store_lifecycle_audit_read_model.py" in surface[
        "evidence_files"
    ]
    assert "tests/test_validate_universal_symbol_receipt_store_lifecycle_audit_read_model.py" in surface[
        "evidence_files"
    ]
    assert "foundation_lifecycle_audit_read_model_validates" in witnesses
    assert "lifecycle_audit_read_model_rejects_authority_drift" in witnesses
    assert "lifecycle_audit_read_model_rejects_raw_detail_visibility" in witnesses
    assert "lifecycle_audit_read_model_rejects_missing_requirement" in witnesses
    assert "lifecycle_audit_read_model_rejects_duplicate_requirement_row" in witnesses
    assert "lifecycle_audit_read_model_rejects_missing_delta_reject_log" in witnesses
    assert "lifecycle_audit_read_model_rejects_receipt_projection_drift" in witnesses
    assert "lifecycle_audit_read_model_rejects_evidence_ref_count_drift" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_lifecycle_audit_read_model"][
        "exact_test_anchor_count"
    ] == 8
    assert witness_surfaces["universal_symbol_receipt_store_lifecycle_audit_read_model"][
        "unanchored_witness_count"
    ] == 0
    assert closure_actions["publish_universal_symbol_receipt_store_lifecycle_audit_read_model"]["status"] == "closed"


def test_universal_symbol_receipt_store_replacement_decision_receipt_surface_blocks_decision_authority() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_replacement_decision_receipt"]
    witnesses = set(surface["runtime_witnesses"])
    ...
    assert "examples/universal_symbol_receipt_store_replacement_decision_receipt.foundation.json" in surface[
        "representative_paths"
    ]
    assert "scripts/validate_universal_symbol_receipt_store_replacement_decision_receipt.py" in surface[
        "evidence_files"
    ]
    assert "foundation_universal_symbol_receipt_store_replacement_decision_receipt_validates" in witnesses
    assert "replacement_decision_receipt_rejects_decision_authority_drift" in witnesses
    assert "replacement_decision_receipt_rejects_missing_delta_reject" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_replacement_decision_receipt"][
        "exact_test_anchor_count"
    ] == 6
    assert witness_surfaces["universal_symbol_receipt_store_replacement_decision_receipt"][
        "unanchored_witness_count"
    ] == 0
    assert closure_actions["publish_universal_symbol_receipt_store_replacement_decision_receipt"]["status"] == "closed"


def test_universal_symbol_receipt_store_replacement_decision_read_model_surface_is_read_only() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_replacement_decision_read_model"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "read_model"
    assert surface["action_proof"] == "read_model"
    assert "schemas/universal_symbol_receipt_store_replacement_decision_read_model.schema.json" in surface[
        "representative_paths"
    ]
    assert "examples/universal_symbol_receipt_store_replacement_decision_read_model.foundation.json" in surface[
        "representative_paths"
    ]
    assert "scripts/validate_universal_symbol_receipt_store_replacement_decision_read_model.py" in surface[
        "evidence_files"
    ]
    assert "tests/test_validate_universal_symbol_receipt_store_replacement_decision_read_model.py" in surface[
        "evidence_files"
    ]
    assert "foundation_replacement_decision_read_model_validates" in witnesses
    assert "replacement_decision_read_model_rejects_authority_drift" in witnesses
    assert "replacement_decision_read_model_rejects_raw_detail_visibility" in witnesses
    assert "replacement_decision_read_model_rejects_missing_requirement" in witnesses
    assert "replacement_decision_read_model_rejects_duplicate_requirement_row" in witnesses
    assert "replacement_decision_read_model_rejects_missing_delta_reject_log" in witnesses
    assert "replacement_decision_read_model_rejects_receipt_projection_drift" in witnesses
    assert "replacement_decision_read_model_rejects_evidence_ref_count_drift" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_replacement_decision_read_model"][
        "exact_test_anchor_count"
    ] == 8
    assert witness_surfaces["universal_symbol_receipt_store_replacement_decision_read_model"][
        "unanchored_witness_count"
    ...
    assert (
        closure_actions["publish_universal_symbol_receipt_store_replacement_decision_read_model"]["status"]
        == "closed"
    )


def test_universal_symbol_receipt_store_replacement_decision_replay_idempotency_witness_surface_blocks_replay_authority() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_replacement_decision_replay_idempotency_witness"]
    witnesses = set(surface["runtime_witnesses"])
    ...
    assert (
        ...
        in surface["evidence_files"]
    )
    assert (
        "foundation_universal_symbol_receipt_store_replacement_decision_replay_idempotency_witness_validates"
        in witnesses
    )
    assert "replacement_replay_idempotency_witness_rejects_replay_authority_drift" in witnesses
    assert "replacement_replay_idempotency_witness_rejects_missing_delta_reject" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_replacement_decision_replay_idempotency_witness"][
        "exact_test_anchor_count"
    ] == 6
    assert witness_surfaces["universal_symbol_receipt_store_replacement_decision_replay_idempotency_witness"][
        "unanchored_witness_count"
    ...
    assert (
        closure_actions["publish_universal_symbol_receipt_store_replacement_decision_replay_idempotency_witness"][
            "status"
        ]
        == "closed"
    )
...
def test_universal_symbol_receipt_store_replacement_replay_idempotency_read_model_surface_is_read_only() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_replacement_decision_replay_idempotency_read_model"]
    witnesses = set(surface["runtime_witnesses"])
    ...
    assert "foundation_replacement_replay_read_model_validates" in witnesses
    assert "replacement_replay_read_model_rejects_authority_drift" in witnesses
    assert "replacement_replay_read_model_rejects_raw_detail_visibility" in witnesses
    assert "replacement_replay_read_model_rejects_missing_requirement" in witnesses
    assert "replacement_replay_read_model_rejects_duplicate_requirement_row" in witnesses
    assert "replacement_replay_read_model_rejects_missing_delta_reject_log" in witnesses
    assert "replacement_replay_read_model_rejects_witness_projection_drift" in witnesses
    assert "replacement_replay_read_model_rejects_evidence_ref_count_drift" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_replacement_decision_replay_idempotency_read_model"][
        "exact_test_anchor_count"
    ] == 8
    assert witness_surfaces["universal_symbol_receipt_store_replacement_decision_replay_idempotency_read_model"][
        "unanchored_witness_count"
    ...
    assert (
        closure_actions["publish_universal_symbol_receipt_store_replacement_decision_replay_idempotency_read_model"][
            "status"
        ]
        == "closed"
    )
...
def test_universal_symbol_receipt_store_tenant_scope_witness_surface_blocks_tenant_authority() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_tenant_scope_witness"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_receipt_store_tenant_scope_witness.schema.json" in surface["representative_paths"]
    assert "examples/universal_symbol_receipt_store_tenant_scope_witness.foundation.json" in surface["representative_paths"]
    assert "scripts/validate_universal_symbol_receipt_store_tenant_scope_witness.py" in surface["evidence_files"]
    assert "foundation_universal_symbol_receipt_store_tenant_scope_witness_validates" in witnesses
    assert "tenant_scope_witness_rejects_tenant_authority_drift" in witnesses
    assert "tenant_scope_witness_rejects_missing_delta_reject" in witnesses
...
def test_universal_symbol_receipt_store_tenant_scope_read_model_surface_is_read_only() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_tenant_scope_read_model"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "read_model"
    assert surface["action_proof"] == "read_model"
    assert "schemas/universal_symbol_receipt_store_tenant_scope_read_model.schema.json" in surface["representative_paths"]
    assert (
        "examples/universal_symbol_receipt_store_tenant_scope_read_model.foundation.json"
        in surface["representative_paths"]
    )
    assert "scripts/validate_universal_symbol_receipt_store_tenant_scope_read_model.py" in surface["evidence_files"]
    assert "tests/test_validate_universal_symbol_receipt_store_tenant_scope_read_model.py" in surface["evidence_files"]
    assert "foundation_tenant_scope_read_model_validates" in witnesses
    assert "tenant_scope_read_model_rejects_authority_drift" in witnesses
    assert "tenant_scope_read_model_rejects_raw_detail_visibility" in witnesses
    assert "tenant_scope_read_model_rejects_missing_requirement" in witnesses
    assert "tenant_scope_read_model_rejects_duplicate_requirement_row" in witnesses
    assert "tenant_scope_read_model_rejects_missing_delta_reject_log" in witnesses
    assert "tenant_scope_read_model_rejects_witness_projection_drift" in witnesses
    assert "tenant_scope_read_model_rejects_evidence_ref_count_drift" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_tenant_scope_read_model"][
        "exact_test_anchor_count"
    ] == 8
    assert witness_surfaces["universal_symbol_receipt_store_tenant_scope_read_model"][
        "unanchored_witness_count"
    ] == 0
    assert closure_actions["publish_universal_symbol_receipt_store_tenant_scope_read_model"]["status"] == "closed"
...
def test_universal_symbol_receipt_store_writer_duty_scope_read_model_surface_is_read_only() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_writer_duty_scope_read_model"]
    witnesses = set(surface["runtime_witnesses"])
    ...
    assert "tests/test_validate_universal_symbol_receipt_store_writer_duty_scope_read_model.py" in surface[
        "evidence_files"
    ]
    assert "foundation_writer_duty_scope_read_model_validates" in witnesses
    assert "writer_duty_scope_read_model_rejects_authority_drift" in witnesses
    assert "writer_duty_scope_read_model_rejects_raw_detail_visibility" in witnesses
    assert "writer_duty_scope_read_model_rejects_missing_requirement" in witnesses
    assert "writer_duty_scope_read_model_rejects_duplicate_requirement_row" in witnesses
    assert "writer_duty_scope_read_model_rejects_missing_delta_reject_log" in witnesses
    assert "writer_duty_scope_read_model_rejects_witness_projection_drift" in witnesses
    assert "writer_duty_scope_read_model_rejects_evidence_ref_count_drift" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_writer_duty_scope_read_model"][
        "exact_test_anchor_count"
    ] == 8
    assert witness_surfaces["universal_symbol_receipt_store_writer_duty_scope_read_model"][
        "unanchored_witness_count"
    ] == 0
    assert closure_actions["publish_universal_symbol_receipt_store_writer_duty_scope_read_model"]["status"] == "closed"
...
def test_universal_symbol_receipt_store_path_confinement_read_model_surface_is_read_only() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_path_confinement_read_model"]
    witnesses = set(surface["runtime_witnesses"])
    ...
    assert "tests/test_validate_universal_symbol_receipt_store_path_confinement_read_model.py" in surface[
        "evidence_files"
    ]
    assert "foundation_path_confinement_read_model_validates" in witnesses
    assert "path_confinement_read_model_rejects_authority_drift" in witnesses
    assert "path_confinement_read_model_rejects_filesystem_escape_drift" in witnesses
    assert "path_confinement_read_model_rejects_raw_detail_visibility" in witnesses
    assert "path_confinement_read_model_rejects_missing_requirement" in witnesses
    assert "path_confinement_read_model_rejects_duplicate_requirement_row" in witnesses
    assert "path_confinement_read_model_rejects_missing_delta_reject_log" in witnesses
    assert "path_confinement_read_model_rejects_witness_projection_drift" in witnesses
    assert "path_confinement_read_model_rejects_evidence_ref_count_drift" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_path_confinement_read_model"][
        "exact_test_anchor_count"
    ] == 9
    assert witness_surfaces["universal_symbol_receipt_store_path_confinement_read_model"][
        "unanchored_witness_count"
    ] == 0
    assert closure_actions["publish_universal_symbol_receipt_store_path_confinement_read_model"]["status"] == "closed"
...
def test_universal_symbol_receipt_store_write_path_idempotency_read_model_surface_is_read_only() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_write_path_idempotency_read_model"]
    witnesses = set(surface["runtime_witnesses"])
    ...
    assert "tests/test_validate_universal_symbol_receipt_store_write_path_idempotency_read_model.py" in surface[
        "evidence_files"
    ]
    assert "foundation_write_path_idempotency_read_model_validates" in witnesses
    assert "write_path_idempotency_read_model_rejects_authority_drift" in witnesses
    assert "write_path_idempotency_read_model_rejects_duplicate_append_drift" in witnesses
    assert "write_path_idempotency_read_model_rejects_raw_detail_visibility" in witnesses
    assert "write_path_idempotency_read_model_rejects_missing_requirement" in witnesses
    assert "write_path_idempotency_read_model_rejects_duplicate_requirement_row" in witnesses
    assert "write_path_idempotency_read_model_rejects_missing_delta_reject_log" in witnesses
    assert "write_path_idempotency_read_model_rejects_witness_projection_drift" in witnesses
    assert "write_path_idempotency_read_model_rejects_evidence_ref_count_drift" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_write_path_idempotency_read_model"][
        "exact_test_anchor_count"
    ] == 9
    assert witness_surfaces["universal_symbol_receipt_store_write_path_idempotency_read_model"][
        "unanchored_witness_count"
    ] == 0
    assert closure_actions["publish_universal_symbol_receipt_store_write_path_idempotency_read_model"]["status"] == "closed"
...
def test_universal_symbol_receipt_store_durability_replay_read_model_surface_is_read_only() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_durability_replay_read_model"]
    witnesses = set(surface["runtime_witnesses"])
    ...
    assert "scripts/validate_universal_symbol_receipt_store_durability_replay_read_model.py" in surface[
        "evidence_files"
    ]
    assert "tests/test_validate_universal_symbol_receipt_store_durability_replay_read_model.py" in surface[
        "evidence_files"
    ]
    assert "foundation_durability_replay_read_model_validates" in witnesses
    assert "durability_replay_read_model_rejects_authority_drift" in witnesses
    assert "durability_replay_read_model_rejects_replay_execution_drift" in witnesses
    assert "durability_replay_read_model_rejects_raw_detail_visibility" in witnesses
    assert "durability_replay_read_model_rejects_missing_requirement" in witnesses
    assert "durability_replay_read_model_rejects_duplicate_requirement_row" in witnesses
    assert "durability_replay_read_model_rejects_missing_delta_reject_log" in witnesses
    assert "durability_replay_read_model_rejects_witness_projection_drift" in witnesses
    assert "durability_replay_read_model_rejects_evidence_ref_count_drift" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_durability_replay_read_model"][
        "exact_test_anchor_count"
    ] == 9
    assert witness_surfaces["universal_symbol_receipt_store_durability_replay_read_model"][
        "unanchored_witness_count"
    ] == 0
    assert closure_actions["publish_universal_symbol_receipt_store_durability_replay_read_model"]["status"] == "closed"


def test_universal_symbol_receipt_store_recovery_witness_surface_blocks_execution_authority() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_recovery_witness"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_receipt_store_recovery_witness.schema.json" in surface["representative_paths"]
    assert "examples/universal_symbol_receipt_store_recovery_witness.foundation.json" in surface["representative_paths"]
    assert "scripts/validate_universal_symbol_receipt_store_recovery_witness.py" in surface["evidence_files"]
    assert "foundation_universal_symbol_receipt_store_recovery_witness_validates" in witnesses
    assert "recovery_witness_rejects_execution_authority_drift" in witnesses
    assert "recovery_witness_rejects_missing_delta_reject" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_recovery_witness"]["exact_test_anchor_count"] == 6
    assert witness_surfaces["universal_symbol_receipt_store_recovery_witness"]["unanchored_witness_count"] == 0
    assert closure_actions["publish_universal_symbol_receipt_store_recovery_witness"]["status"] == "closed"


def test_universal_symbol_receipt_store_recovery_read_model_surface_is_read_only() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_recovery_read_model"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "read_model"
    assert surface["action_proof"] == "read_model"
    assert "schemas/universal_symbol_receipt_store_recovery_read_model.schema.json" in surface[
        "representative_paths"
    ]
    assert "examples/universal_symbol_receipt_store_recovery_read_model.foundation.json" in surface[
        "representative_paths"
    ]
    assert "scripts/validate_universal_symbol_receipt_store_recovery_read_model.py" in surface["evidence_files"]
    assert "tests/test_validate_universal_symbol_receipt_store_recovery_read_model.py" in surface["evidence_files"]
    assert "foundation_recovery_read_model_validates" in witnesses
    assert "recovery_read_model_rejects_authority_drift" in witnesses
    assert "recovery_read_model_rejects_recovery_execution_drift" in witnesses
    assert "recovery_read_model_rejects_rollback_compensation_and_replay_commit_drift" in witnesses
    assert "recovery_read_model_rejects_raw_detail_visibility" in witnesses
    assert "recovery_read_model_rejects_missing_requirement" in witnesses
    assert "recovery_read_model_rejects_duplicate_requirement_row" in witnesses
    assert "recovery_read_model_rejects_missing_delta_reject_log" in witnesses
    assert "recovery_read_model_rejects_witness_projection_drift" in witnesses
    assert "recovery_read_model_rejects_evidence_ref_count_drift" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_recovery_read_model"]["exact_test_anchor_count"] == 10
    assert witness_surfaces["universal_symbol_receipt_store_recovery_read_model"]["unanchored_witness_count"] == 0
    assert closure_actions["publish_universal_symbol_receipt_store_recovery_read_model"]["status"] == "closed"
...
def test_universal_symbol_receipt_store_write_path_witness_surface_blocks_write_authority() -> None:
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_write_path_witness"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_receipt_store_write_path_witness.schema.json" in surface["representative_paths"]
    assert "examples/universal_symbol_receipt_store_write_path_witness.foundation.json" in surface["representative_paths"]
    assert "scripts/validate_universal_symbol_receipt_store_write_path_witness.py" in surface["evidence_files"]
    assert "foundation_universal_symbol_receipt_store_write_path_witness_validates" in witnesses
    assert "write_path_witness_rejects_append_authority_drift" in witnesses
    assert "write_path_witness_rejects_missing_delta_reject" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_write_path_witness"]["exact_test_anchor_count"] == 7
    assert witness_surfaces["universal_symbol_receipt_store_write_path_witness"]["unanchored_witness_count"] == 0
    assert closure_actions["publish_universal_symbol_receipt_store_write_path_witness"]["status"] == "closed"


def test_universal_symbol_receipt_store_path_custody_witness_surface_blocks_path_authority() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_path_custody_witness"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_receipt_store_path_custody_witness.schema.json" in surface["representative_paths"]
    assert "examples/universal_symbol_receipt_store_path_custody_witness.foundation.json" in surface["representative_paths"]
    assert "scripts/validate_universal_symbol_receipt_store_path_custody_witness.py" in surface["evidence_files"]
    assert "foundation_universal_symbol_receipt_store_path_custody_witness_validates" in witnesses
    assert "path_custody_witness_rejects_write_path_authority_drift" in witnesses
    assert "path_custody_witness_rejects_missing_delta_reject" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_path_custody_witness"]["exact_test_anchor_count"] == 6
    assert witness_surfaces["universal_symbol_receipt_store_path_custody_witness"]["unanchored_witness_count"] == 0
    assert closure_actions["publish_universal_symbol_receipt_store_path_custody_witness"]["status"] == "closed"


def test_universal_symbol_receipt_store_writer_identity_witness_surface_blocks_identity_authority() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_writer_identity_witness"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_receipt_store_writer_identity_witness.schema.json" in surface["representative_paths"]
    assert "examples/universal_symbol_receipt_store_writer_identity_witness.foundation.json" in surface["representative_paths"]
    assert "scripts/validate_universal_symbol_receipt_store_writer_identity_witness.py" in surface["evidence_files"]
    assert "foundation_universal_symbol_receipt_store_writer_identity_witness_validates" in witnesses
    assert "writer_identity_witness_rejects_registration_authority_drift" in witnesses
    assert "writer_identity_witness_rejects_missing_delta_reject" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_writer_identity_witness"]["exact_test_anchor_count"] == 6
    assert witness_surfaces["universal_symbol_receipt_store_writer_identity_witness"]["unanchored_witness_count"] == 0
    assert closure_actions["publish_universal_symbol_receipt_store_writer_identity_witness"]["status"] == "closed"


def test_universal_symbol_receipt_store_writer_registration_witness_surface_blocks_writer_authority() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["universal_symbol_receipt_store_writer_registration_witness"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "witnessed"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "schemas/universal_symbol_receipt_store_writer_registration_witness.schema.json" in surface["representative_paths"]
    assert "examples/universal_symbol_receipt_store_writer_registration_witness.foundation.json" in surface["representative_paths"]
    assert "scripts/validate_universal_symbol_receipt_store_writer_registration_witness.py" in surface["evidence_files"]
    assert "foundation_universal_symbol_receipt_store_writer_registration_witness_validates" in witnesses
    assert "writer_registration_witness_rejects_append_authority_drift" in witnesses
    assert "writer_registration_witness_rejects_missing_delta_reject" in witnesses
    assert witness_surfaces["universal_symbol_receipt_store_writer_registration_witness"]["exact_test_anchor_count"] == 8
    assert witness_surfaces["universal_symbol_receipt_store_writer_registration_witness"]["unanchored_witness_count"] == 0
    assert closure_actions["publish_universal_symbol_receipt_store_writer_registration_witness"]["status"] == "closed"


def test_cross_channel_conversation_binding_policy_surface_blocks_ambiguous_handoffs() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["cross_channel_conversation_binding_policy"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "proven"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "gateway/cross_channel_conversation_binding.py" in surface["evidence_files"]
    assert "tests/test_gateway/test_cross_channel_conversation_binding.py" in surface["evidence_files"]
    assert "cross_channel_reply_without_witness_is_blocked" in witnesses
    assert "cross_channel_bound_request_reply_is_allowed" in witnesses
    assert "binding_receipt_hides_raw_message_and_is_stable" in witnesses
    assert witness_surfaces["cross_channel_conversation_binding_policy"]["exact_test_anchor_count"] == 8
    assert witness_surfaces["cross_channel_conversation_binding_policy"]["unanchored_witness_count"] == 0
    assert closure_actions["publish_cross_channel_conversation_binding_policy"]["status"] == "closed"


def test_policy_denial_response_composer_surface_redacts_user_facing_denials() -> None:
    matrix = _load_fixture()
    surfaces = {surface["surface_id"]: surface for surface in matrix["surfaces"]}
    witness_surfaces = {
        surface["surface_id"]: surface
    ...
    closure_actions = {action["action_id"]: action for action in matrix["closure_actions"]}
    surface = surfaces["policy_denial_response_composer"]
    witnesses = set(surface["runtime_witnesses"])

    assert surface["coverage_state"] == "proven"
    assert surface["request_proof"] == "request_proof"
    assert surface["action_proof"] == "action_proof"
    assert "gateway/denial_response.py" in surface["evidence_files"]
    assert "gateway/router.py" in surface["evidence_files"]
    assert "tests/test_gateway/test_denial_response.py" in surface["evidence_files"]
    assert "tenant_denial_template_is_user_facing_and_redacted" in witnesses
    assert "approval_strength_denial_preserves_controls_without_raw_detail" in witnesses
    assert "channel_approval_callback_denies_mismatched_identity" in witnesses
    assert witness_surfaces["policy_denial_response_composer"]["exact_test_anchor_count"] == 7
    assert witness_surfaces["policy_denial_response_composer"]["unanchored_witness_count"] == 0
    assert closure_actions["publish_policy_denial_response_composer"]["status"] == "closed"
