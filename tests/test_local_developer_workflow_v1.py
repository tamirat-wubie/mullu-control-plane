...
from scripts.build_local_developer_workflow_approval_evidence_closure_packet import (  # noqa: E402
    main as build_approval_evidence_packet_main,
)
...
from scripts.validate_local_developer_workflow_approval_evidence_closure_packet import (  # noqa: E402
    main as validate_approval_evidence_packet_main,
)
...
def test_local_developer_workflow_v1_cli_writes_json(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert payload["approval_evidence_closure_status"] == "blocked_waiting_evidence"
    assert payload["approval_evidence_missing_count"] == 6
    assert Path(payload["approval_evidence_closure_packet_path"]).exists()
...
def test_local_developer_workflow_v1_validator_cli(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    exit_code = validate_main([
        ...
        str(tmp_path / APPROVAL_EVIDENCE_CLOSURE_PACKET_FILENAME),
        "--output",
        str(output_path),
        "--strict",
        "--json",
    ...
    assert saved["approval_evidence_closure_status"] == "not_present"
...
def test_local_developer_workflow_approval_evidence_closure_packet_lists_missing_refs() -> None:
    artifacts = build_local_developer_workflow_v1_artifacts(
        repo_root=_ROOT,
        repo_status=FIXTURE_REPO_STATUS,
    )
    ...
    admission_packet = build_local_developer_workflow_pr_admission_packet(
        artifacts=artifacts,
        command_preview_packet=command_packet,
        closure_packet=closure_packet,
    )
    packet = build_local_developer_workflow_approval_evidence_closure_packet(
        artifacts=artifacts,
        command_preview_packet=command_packet,
        pr_admission_packet=admission_packet,
        closure_packet=closure_packet,
    )
    ...
    assert validation.ok is True
    assert packet["packet_id"] == APPROVAL_EVIDENCE_CLOSURE_PACKET_ID
    assert packet["projection_only"] is True
    assert packet["execution_performed"] is False
    assert packet["approval_performed"] is False
    assert packet["branch_write_allowed"] is False
    assert packet["pr_creation_allowed"] is False
    assert tuple(packet["missing_evidence_refs"]) == MISSING_EVIDENCE_REF_IDS
    assert packet["closure_receipt"]["closure_status"] == "blocked_waiting_evidence"
    assert packet["closure_receipt"]["missing_evidence_count"] == 6
    assert packet["source_admission"]["admission_remains_blocked"] is True


def test_local_developer_workflow_approval_evidence_closure_packet_rejects_authority_overclaim() -> None:
    artifacts = build_local_developer_workflow_v1_artifacts(
        repo_root=_ROOT,
        repo_status=FIXTURE_REPO_STATUS,
    )
    ...
    packet["approval_performed"] = True
    packet["branch_write_allowed"] = True
    packet["required_evidence_refs"][0]["present"] = True
    packet["closure_receipt"]["execution_ready"] = True
    ...
    assert "approval_performed_must_be_false" in serialized_errors
    assert "branch_write_allowed_must_be_false" in serialized_errors
    assert "required_evidence_refs[0]_present_must_be_false" in serialized_errors
    assert "execution_ready_must_be_false" in serialized_errors
    assert "packet_hash_mismatch" in serialized_errors


def test_local_developer_workflow_approval_evidence_closure_packet_cli_builds_and_validates(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    artifacts = build_local_developer_workflow_v1_artifacts(
    ...
    admission_packet = build_local_developer_workflow_pr_admission_packet(
        ...
        closure_packet=closure_packet,
        artifact_paths=paths,
        command_preview_packet_path=command_path,
        closure_packet_path=closure_path,
    )
    admission_path = write_local_developer_workflow_pr_admission_packet(
        admission_packet,
        tmp_path / PR_ADMISSION_PACKET_FILENAME,
    )
    output_path = tmp_path / APPROVAL_EVIDENCE_CLOSURE_PACKET_FILENAME

    build_exit = build_approval_evidence_packet_main([
        "--artifact-dir",
        str(tmp_path),
        "--command-preview-packet",
        str(command_path),
    ...
    build_payload = json.loads(capsys.readouterr().out)
    validate_exit = validate_approval_evidence_packet_main([
        "--artifact-dir",
        str(tmp_path),
        "--packet",
        str(output_path),
        ...
        str(closure_path),
        "--output",
        str(tmp_path / "approval-evidence-closure-validation.json"),
        "--strict",
        "--json",
    ])
    validate_payload = json.loads(capsys.readouterr().out)
    ...
    assert validate_payload["ok"] is True
    assert packet["packet_id"] == APPROVAL_EVIDENCE_CLOSURE_PACKET_ID
    assert packet["source_refs"]["pr_admission_packet_id"] == PR_ADMISSION_PACKET_ID
