"""Purpose: verify the authority holistic loop admission dossier.
Governance scope: authority loop admission readiness, non-registration boundary,
read-only projection, and terminal-closure separation.
Dependencies: scripts.report_holistic_loop_authority_admission_dossier.
Invariants:
  - Authority admission dossier reports default registry admission.
  - Authority admission dossier does not mutate loop registration.
  - Authority admission dossier remains read-only and non-terminal.
...
from scripts import report_holistic_loop_authority_admission_dossier as reporter


def test_authority_admission_dossier_builds_proposed_manifest() -> None:
    dossier = reporter.build_dossier()
    errors = reporter.validate_dossier(dossier)
    manifest = dossier["proposed_manifest"]

    assert errors == []
    assert dossier["candidate_id"] == "authority_obligation_loop"
...
def test_authority_admission_dossier_reports_registry_admission() -> None:
    dossier = reporter.build_dossier()
    ...
    assert operator_decision["decision_ref"] == "default_registry:authority_obligation_loop"
    assert operator_decision["blocks_registration"] is False


def test_authority_admission_dossier_does_not_register_or_mutate_runtime() -> None:
    dossier = reporter.build_dossier()
...
def test_authority_admission_dossier_reports_complete_readiness_sections() -> None:
    dossier = reporter.build_dossier()
...
def test_authority_admission_dossier_rejects_registration_or_terminal_claim() -> None:
    dossier = reporter.build_dossier()
...
def test_authority_admission_dossier_rejects_missing_source_or_gap_regression() -> None:
    dossier = reporter.build_dossier()
    invalid_dossier = copy.deepcopy(dossier)
    invalid_dossier["existing_surface_refs"].append("missing/authority_surface.json")
    invalid_dossier["evidence_gap_report"]["missing_evidence"] = [
    ...
    assert "missing surface: missing/authority_surface.json" in errors
