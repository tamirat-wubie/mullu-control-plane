"""Tests for live producer review submission receipt emission preflight validation."""
...
def _write_fixture(tmp_path: Path, preflight: dict[str, object]) -> Path:
    path = tmp_path / "execution_authority_review_submission_receipt_emission_preflight.json"
    path.write_text(json.dumps(preflight, indent=2), encoding="utf-8")
    return path


def test_accepts_default_fixture() -> None:
    validation, fixture = validate_live_producer_execution_authority_review_submission_receipt_emission_preflight()

    assert validation.ok is True
    assert validation.errors == ()
    assert validation.preflight_id == PREFLIGHT_ID
    assert validation.preflight_status == PREFLIGHT_STATUS
    assert validation.emission_requirement_count == len(REVIEW_INPUT_IDS)
    assert validation.missing_emission_requirement_count == len(REVIEW_INPUT_IDS)
    assert fixture["review_submission_receipt_emission_admitted"] is False
...
def test_rejects_receipt_emission(tmp_path: Path) -> None:
    changed = copy.deepcopy(_load_fixture())
    changed["receipt_emission_requirements"][0]["required_receipt_ref"] = "receipt://already-emitted"
    changed["receipt_emission_requirements"][0]["status"] = "SolvedVerified"
    changed["receipt_emission_requirements"][0]["receipt_emission_admissible"] = True
    changed["missing_receipt_emission_requirements"][0]["status"] = "SolvedVerified"

    validation, _ = validate_live_producer_execution_authority_review_submission_receipt_emission_preflight(
    ...
    assert "must not be admissible for receipt emission" in serialized_errors


def test_rejects_authority_drift(tmp_path: Path) -> None:
    changed = copy.deepcopy(_load_fixture())
    changed["review_submission_receipt_emission_admitted"] = True
    ...
    assert validation.ok is False
    assert "review_submission_receipt_emission_admitted" in serialized_errors
...
def test_rejects_missing_input_id(tmp_path: Path) -> None:
    changed = copy.deepcopy(_load_fixture())
    changed["missing_receipt_emission_requirements"] = changed["missing_receipt_emission_requirements"][:-1]
...
def test_cli_json_reports_valid(capsys) -> None:
    ...
    assert payload["emission_requirement_count"] == len(REVIEW_INPUT_IDS)
