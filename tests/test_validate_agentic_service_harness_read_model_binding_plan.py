...
def test_readiness_map_rejects_missing_current_next_pr_marker(
    ...
    map_path.write_text(
        map_text.replace(
            "1. `harness(live-producer): add review submission receipt append admission preflight`",
    ...
    assert validation.ok is False
    assert (
        "missing next PR marker: harness(live-producer): add review submission receipt append admission preflight"
        in serialized_errors
    )
