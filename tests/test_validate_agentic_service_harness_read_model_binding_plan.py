...
def test_readiness_map_rejects_missing_concrete_filesystem_write_next_pr(
    ...
    map_path.write_text(
        map_text.replace(
            "1. `harness(pr): bind PR creation dry-run receipt to command-preview terminal certificate admission evidence`",
    ...
    assert validation.ok is False
    assert (
        "missing next PR marker: harness(pr): bind PR creation dry-run receipt to command-preview terminal certificate admission evidence"
        in serialized_errors
    )
