...
def test_readiness_map_rejects_missing_concrete_filesystem_write_next_pr(
    ...
    map_path.write_text(
        map_text.replace(
            "1. `harness(pr): bind CI gate before ready-for-review to command-preview rollback evidence`",
    ...
    assert validation.ok is False
    assert (
        "missing next PR marker: harness(pr): bind CI gate before ready-for-review to command-preview rollback evidence"
        in serialized_errors
    )
