...
def test_readiness_map_rejects_missing_receipt_evidence_first_pr(tmp_path: Path) -> None:
    map_text = Path("MULLUSI_AGENTIC_SERVICE_HARNESS_READINESS_MAP.md").read_text(
        encoding="utf-8"
    )
    map_path = tmp_path / "readiness-map.md"
    ...
    assert validation.ok is False
    assert "missing first next PR: Receipt and EvidenceBundle read models" in serialized_errors
