...
def test_readiness_map_rejects_missing_github_pr_terminal_closure_ready_row(
    ...
    map_path.write_text(
        map_text.replace(
            "| GitHub PR terminal closure certificate actual-diff effect reconciliation binding PR | READY |",
            "| GitHub PR terminal closure certificate actual-diff effect reconciliation binding PR | PARTIAL |",
        ),
        encoding="utf-8",
    )
...
def test_readiness_map_rejects_missing_concrete_filesystem_write_next_pr(
    ...
    map_path.write_text(
        map_text.replace(
            "1. `harness(pr): bind terminal closure generic continuation rejection to actual-diff decision contract`",
    ...
    assert validation.ok is False
    assert (
        "missing next PR marker: harness(pr): bind terminal closure generic continuation rejection to actual-diff decision contract"
        in serialized_errors
    )
