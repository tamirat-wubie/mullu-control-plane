...
def test_readiness_map_rejects_missing_non_empty_diff_file_summary_next_pr(
    ...
    map_path.write_text(
        map_text.replace(
            "1. `harness(write): add concrete filesystem write receipt with redacted diff evidence`",
    ...
    assert validation.ok is False
    assert (
        "missing next PR marker: harness(write): add concrete filesystem write receipt with redacted diff evidence"
        in serialized_errors
    )
