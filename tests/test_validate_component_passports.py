...
from mcoi_runtime.app.component_passports import ComponentPassportError, build_component_passports
from scripts.validate_component_passports import (
    DEFAULT_AUTHORITY_WITNESSES,
    DEFAULT_OUTPUT,
    DEFAULT_PASSPORTS,
    DEFAULT_REGISTRY,
...
def test_component_passports_reject_non_boolean_authority_values(tmp_path: Path) -> None:
    registry = json.loads(DEFAULT_REGISTRY.read_text(encoding="utf-8"))
    witnesses = json.loads(DEFAULT_AUTHORITY_WITNESSES.read_text(encoding="utf-8"))
    component = registry["components"][0]
    witness = witnesses["authority_witnesses"][0]
    assert component["id"] == witness["component_id"]
    assert isinstance(component["authority"], dict)
    assert isinstance(witness["authority"], dict)
    component["authority"]["can_emit_receipt"] = "true"
    witness["authority"]["can_emit_receipt"] = "true"
    registry_path = tmp_path / "component_registry.json"
    witnesses_path = tmp_path / "component_authority_witnesses.json"
    registry_path.write_text(json.dumps(registry, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    witnesses_path.write_text(json.dumps(witnesses, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    with pytest.raises(ComponentPassportError, match=r"authority\.can_emit_receipt must be boolean"):
        build_component_passports(
            registry_path=registry_path,
            authority_witnesses_path=witnesses_path,
        )
