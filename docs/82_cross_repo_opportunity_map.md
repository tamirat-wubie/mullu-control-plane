...
| Direct Rust runtime binding from `maf/rust` | The current repository states receipt-shape parity, ABI/CLI contract evidence, subprocess boundary evidence, and static deterministic fixture parity, but not Python runtime binding. | Failure receipt path witness. |
...
2. MAF failure receipt path witness
   - Follows `MafDeterministicFixtureParityWitness` and proves runtime failure materialization paths before executable MAF binding can be reconsidered.
   - Must remain `AwaitingEvidence` until failure receipt path handling is independently validated without command execution authority.
