...
| GAP-WORKER-001 | Worker contract matrix incomplete | Worker | high | implemented / partial | Each worker needs allowed inputs, secrets, network scope, tenant scope, timeout, retry, and receipts. | `ReadOnlyWorkerBinding` defines the first worker contract matrix for local repo inspection, including denied network, secrets, writes, connector authority, terminal closure, and raw output retention. | Add local rehearsal-only worker lease preflight. |
| GAP-WORKER-002 | Read-only first worker path not selected | Worker / Product | medium | implemented / partial | The first pilot path should prove the spine without mutation risk. | `read_only_repo_inspection` is selected as the first worker path. | Add local rehearsal-only worker lease preflight. |
| GAP-WORKER-003 | Partial execution recovery rules incomplete | Worker / Evidence | high | implemented / partial | Worker failures can leave unclear state. | `WorkerFailureReceipt` records failed steps, partial effects, unknown effects, rollback refs, recovery refs, and no-success/no-terminal-closure guards; `ReadOnlyWorkerBinding` makes the failure receipt mandatory for the selected first worker path. | Add local rehearsal-only worker lease preflight. |
...
5. Select one read-only first worker path and bind it to SearchReceipt emission.
