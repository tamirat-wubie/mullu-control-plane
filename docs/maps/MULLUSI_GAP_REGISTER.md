...
2. Add deeper Plan Review receipt exports.
3. Select one read-only first worker path.
4. Add search freshness and budget controls.
5. Add one external channel only after web identity and approval work.
```
...
```text
The register confirms that search freshness and cost controls, deeper plan receipt exports, worker selection, and channel hardening are still not closed.
```
