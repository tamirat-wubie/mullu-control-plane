...
| GAP-SEARCH-001 | Search need classifier missing | Search | medium | implemented / partial | Every chat message should not trigger search. | `gateway/search_governance.py` classifies no-search, cache, local search, light web search, and deep search; `schemas/search_decision_receipt.schema.json` and `tests/test_gateway/test_search_governance.py` validate the decision receipt; `gateway/causal_closure_kernel.py` attaches and durably records search decision receipts for live `enterprise.knowledge_search` execution before proof validation; `gateway/operator_receipt_viewer.py` exposes `search_decision_receipt` as a dedicated receipt type with query-hash, freshness, budget, and retrieval-authority drilldowns while tests verify raw query text is not exposed. | Repeat the read-only worker contract pattern for search worker execution. |
...
3. Repeat the read-only worker contract pattern for search worker execution.
4. Add worker-failure receipt ids to future worker UI drilldowns.
5. Draft channel trust and approval-strength policy.
```
...
```text
The register confirms that channel hardening, search worker contract repetition, worker-failure UI drilldowns, and channel approval-strength policy are still not closed.
```
