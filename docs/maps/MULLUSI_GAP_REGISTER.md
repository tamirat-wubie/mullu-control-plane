...
3. Add worker-failure receipt ids to future worker UI drilldowns.
4. Draft channel trust and approval-strength policy.
5. Bind source-level freshness evidence to future search result receipts.
```
...
```text
The register confirms that channel hardening, worker-failure UI drilldowns, source-level search freshness evidence, and channel approval-strength policy are still not closed.
```
