...
| GAP-SEARCH-002 | Freshness gate missing | Search | medium | implemented / partial | Current-info questions need freshness; stable facts may not. | `SearchDecision.freshness` records freshness_required, current_info_claim_allowed, proof_state, and max age before current claims. | Add post-retrieval SearchReceipt freshness evidence. |
