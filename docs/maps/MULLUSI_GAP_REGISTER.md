...
3. Add dedicated search decision receipt drilldowns to the receipt viewer.
4. Repeat the read-only worker contract pattern for search worker execution.
...
```text
The register confirms that search receipt drilldowns, channel hardening, search worker contract repetition, and worker-failure UI drilldowns are still not closed.
```
