...
7. Define the UAO dispatch authorization witness for the selected read-only first worker path.
...
```text
The register confirms that channel hardening, source-level search freshness evidence, channel-native approval-strength enforcement, UAO dispatch authorization, live receipt-store write path, durable interpretation readback, Plan Review receipt export hardening, and admin surfaces are still not fully closed.
```
