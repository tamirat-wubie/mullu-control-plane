...
5. Define the live runtime receipt-store activation witness for the selected read-only first worker path.
...
```text
The register confirms that product UI hardening, durable interpretation readback, search cost/freshness controls, Plan Review receipt export hardening, live runtime receipt-store activation, live receipt-store write path, and channel hardening are still not fully closed.
```
