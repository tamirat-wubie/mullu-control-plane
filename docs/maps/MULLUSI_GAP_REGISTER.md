...
| GAP-INTERP-001 | Durable InterpretedRequest object missing | Interpretation | high | missing / partial | User text cannot safely become plans without an auditable interpretation object. | Dataclass/schema records intent, slots, confidence, risk, action/search need, and raw message hash. | Add `InterpretedRequest` design doc or implementation. |
| GAP-INTERP-002 | Interpretation receipt missing | Interpretation / Evidence | high | missing | The system needs proof of what it believed the user meant. | `InterpretationReceipt` stored with request trace. | Add interpretation receipt contract. |
| GAP-INTERP-003 | Clarification engine incomplete | Interpretation | high | missing / partial | Vague goals must not trigger unsafe or wrong execution. | Missing-slot detection with focused user questions. | Define slot requirements for first safe use case. |
| GAP-INTERP-004 | LLM-assisted interpretation not bounded | Interpretation / LLM | medium | missing / partial | LLMs can misclassify user intent or overstate available authority. | Deterministic rules run first; LLM output validated as proposal only. | Add interpretation authority boundary. |
...
| GAP-EVIDENCE-001 | Receipt taxonomy not implemented end-to-end | Evidence | high | partial | Every stage needs evidence or a blocker. | Message, identity, interpretation, plan, approval, worker, closure, and final receipts linked by trace. | Add receipt taxonomy to implementation plan. |
