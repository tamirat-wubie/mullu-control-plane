...
| GAP-GOV-003 | Policy denial explanation needs user-friendly composer | Governance / Response | medium | implemented / partial | Denials must be understandable without leaking internals. | `gateway/denial_response.py` provides redacted user-facing templates for tenant binding, approval authority, approval-strength, capability-admission, and generic policy denials; `gateway/router.py` uses those templates on representative policy-block paths while preserving existing audit metadata; tests verify redaction flags, template IDs, bounded controls, and fallback behavior. | Expand templates to remaining rare denial paths and expose denial template metadata in the receipt viewer. |
