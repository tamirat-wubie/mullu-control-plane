...
5. Define the live dispatch endpoint registration witness for the selected read-only first worker path.
...
```text
The register confirms that product UI, durable interpretation, search cost controls, live dispatch endpoint registration, live runtime receipt schema binding, live receipt-store write path, and channel hardening are still not closed.
```
