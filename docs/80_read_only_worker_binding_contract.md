...
`ReadOnlyWorkerRuntimeAuthorityChainWitness` binds the lease admission, UAO dispatch authorization, `Phi_gov` dispatch authorization, runtime dispatch admission, receipt emission, receipt-store write path, WorkerFailureReceipt, recovery, replay, and effect-reconciliation obligations into one denied-until-complete authority chain. It prevents partial promotion by keeping active lease admission, dispatch authorization, runtime dispatch, worker invocation, runtime receipt emission, receipt append, rollback completion, replay completion, terminal closure, and success claims false until every live evidence ref is present.
