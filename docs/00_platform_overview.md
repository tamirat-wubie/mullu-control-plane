...
- `ReadOnlyWorkerEffectReconciliationWitness Contract` records the missing effect reconciliation evidence request before worker terminal closure while keeping reconciliation uncollected, dispatch, worker invocation, receipt emission, receipt append, filesystem writes, connector calls, success claims, and terminal closure unperformed.
