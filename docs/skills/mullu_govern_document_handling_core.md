...
```text
S_doc := <I_doc, L_doc, Sigma_doc, Gamma_doc, H_doc>
Sigma_doc satisfies L_doc over I_doc
```

The skill preserves immutable source facts in `I_doc`, applies parsing/editing/privacy rules in `L_doc`, maintains extracted graphs and patch queues in `Sigma_doc`, exposes only governed summaries/artifacts through `Gamma_doc`, and records every read, inference, edit, validation, render, rejection, and export in `H_doc`.
...
- Adds document-layer constants for byte, format, structure, layout, semantics, governance, and causality.
