...
12. Run `scripts\produce_durable_gmail_account_binding_receipt.py` and validate `scripts\validate_durable_gmail_account_binding_receipt.py` against the fresh source live receipt before claiming the token belongs to the intended tenant mailbox.
13. Run `scripts\produce_durable_gmail_write_authority_rehearsal_receipt.py` and validate `scripts\validate_durable_gmail_write_authority_rehearsal_receipt.py` before considering any Gmail draft or send authority.
14. Run `scripts\emit_durable_gmail_live_write_operator_input_request.py` and validate `scripts\validate_durable_gmail_live_write_operator_input_request.py` before any live write evidence workflow is admitted.
15. Run live approval-gated send/draft evidence only if write operations are in scope and an approval receipt exists.
16. Update release readiness only after all receipts pass.
