...
| Coverage invariant not CI-enforced | Medium | Add `scripts/validate_receipt_coverage.py` enumerating routes and asserting each has a known emission path or is in the "Acknowledged exclusions" list. | **Closed** — `scripts/validate_receipt_coverage.py` enumerates every state-mutating route and classifies it as MIDDLEWARE_API / MIDDLEWARE_GATEWAY / EXCLUDED / UNCOVERED. The ratchet test `mcoi/tests/test_receipt_coverage_invariant.py` pins the UNCOVERED count to a baseline (currently 31); any drift (regression *or* progress) fails the test, so coverage changes are reviewer-visible. The script supports `--strict` for CI gating. |
| MUSIA + gateway-internal routes not on the receipt path | High | 32 state-mutating routes (`/cognition`, `/constructs`, `/domains`, `/musia/*`, `/ucja`, `/capability/execute`) sit outside the `/api/` prefix that GovernanceMiddleware filters on. Resolution: route under `/api/`, OR add a MUSIA-side receipt middleware mirroring GovernanceMiddleware, OR add per-route EXCLUSIONS entries in `scripts/validate_receipt_coverage.py` with justifications. The ratchet test (`mcoi/tests/test_receipt_coverage_invariant.py`) pins the count at 32 so any new uncovered route — and any progress reducing the count — is reviewer-visible. | Open — surfaced 2026-04-28 by the new validator |
