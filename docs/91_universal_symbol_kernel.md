...
## Lane runtime authority evidence value receipt

The lane-level runtime authority evidence value receipt is:
...
It records operator-supplied reference values for each lane and each required
evidence kind without verifying those values or storing raw evidence payloads.
The value receipt remains Foundation Mode evidence input only: it is not lane
authority, not runtime admission, not dispatch, not receipt-store append, and
not terminal closure.

`scripts/produce_universal_symbol_lane_runtime_authority_evidence_value_receipt.py`
materializes the same blocked receipt from override refs without probing
endpoints, calling connectors, appending receipts, mutating state, or granting
authority.

`scripts/verify_universal_symbol_lane_runtime_authority_evidence_value_refs.py`
checks the supplied refs against evidence-kind schemes, repository-local JSON
shape, authority-denial flags, and secret-like markers. Verification remains
non-authorizing: placeholder refs keep status
`blocked_placeholder_or_missing_lane_evidence_values`, and even structurally
verified refs keep proof state `Unknown`.
