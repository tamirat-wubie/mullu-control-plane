...
| `gateway_runtime_witness` | `/gateway/witness`, `/runtime/witness`, `/anchors/latest` | read-model | read-model | deployment witness | hash-chain | witnessed | Runtime witness surfaces publish bounded operational and responsibility debt state; orchestration receipts bind ingress render, preflight, and dispatch evidence before deployment witness readiness. |
