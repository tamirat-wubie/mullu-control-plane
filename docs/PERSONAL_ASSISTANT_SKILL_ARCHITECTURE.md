...
the canonical no-effect authority set, the no-effect boundary from the canonical
no-effect flag projection, and lineage deltas from recomputed closure state. The
packet ID is recomputed from the current packet body, so a changed packet cannot
reuse an old valid-looking identity. The closure summary is recomputed from the
source receipt records and no-effect boundary before the closure gate is
accepted. Digest mismatch, schema mismatch, source-kind binding drift, source
closure field drift, source serialized-length drift, source replay-projection
drift, authority-denial projection drift, no-effect projection drift, lineage
