...
Status: **DRAFT for review — no code in this PR.** Author: Claude (Opus 4.7, 1M context).
Date: 2026-06-03.
...
but was written before Stages A/B/C/D/E landed. This doc updates the context and pins the
code-level decisions that still need to be made before any ledger code ships.

## 0. Campaign state (2026-06-03, code-grounded)
...
## 1. Goal — what "ledger" means concretely
...
2. **Multi-worker-coherent**: every worker reads the same ordered event stream, so the gate's
   ...
   running rate are consistent across workers (eventually, depending on backing — see §3).
...
chain_hash: str          # SHA-256 over (sequence, content_hash, previous_chain_hash)
previous_chain_hash: str # links to the prior event; genesis = 64 zeros
```
...
## 3. Backing options (the architecture decision)

### Option A — file-backed `HashChainStore` (recommended for v1)
...
`{sequence, ts, content_hash, chain_hash, previous_chain_hash, body}`.
...
this is sufficient; multi-host needs Option B or C.
...
### Option C — in-process deque (NOT recommended; no durability)
...
### Option D — externalize to UAO command ledger
...
(default-OFF). When the flag is on AND the Stage-C learner is on, the learner writes a chain
entry as its FINAL step inside `_lock`-serialised admission (rollback-safe: a write failure
aborts the LEARN without partial state). When the flag is on AND a worker starts, a rehydrate
helper reads the JSONL forward and replays each event into `meta_reasoning` + `episodic_memory`
BEFORE the server starts serving traffic.

Then **add Option B (Postgres backing) as an additive `MULLU_COGNITIVE_LOOP_LEDGER_BACKEND`
selector**. The same `CognitiveOutcomeLedger` Protocol; the swap is a deps wiring change.

## 4. Integration points (where the code goes)
...
is the only new "must-run-before-serving" startup step; it has its own timeout +
fail-closed behavior (a corrupted chain refuses to serve, mirroring the live cognitive gate's
fail-OPEN posture for run-time errors but fail-CLOSED for startup integrity).

## 5. Determinism / replay semantics
...
4. **Concurrent-write serialisation**: 8 threads each LEARN 100 events; assert sequence is
   strictly monotone and the chain validates end-to-end.
...
## 7. What the user needs to decide before code lands

This doc surfaces **three concrete decisions** the code layer cannot defensibly make alone.

| # | Decision | Default if you don't pick |
|---|---|---|
| L1 | Backing for v1: **file (Option A)** or jump straight to **Postgres (Option B)**? | A — fits pilot, lowest-risk path |
| L2 | Per-tenant or single-stream? Per-tenant scopes the rehydrate and the lock contention; single-stream is simpler ops. | per-tenant — matches existing tenant-scoping invariants |
| L3 | Rehydrate timeout posture: hard cap (e.g. 30s) and abort startup, or unbounded with a startup-blocker probe? | hard cap, fail-CLOSED |

Bonus (not blocking): is `MULLU_COGNITIVE_LOOP_LEDGER` a single flag, or does it split into
`LEDGER_BACKEND` (file/postgres) + `LEDGER_ENABLED` so backends can be swapped without flipping
the enable state? Default: split, mirrors how `LLM_PROVIDER` + `LLM_ENABLED` work elsewhere.

## 8. What I will NOT do without explicit go

- Land any ledger code (Protocol or file impl) before L1/L2/L3 are answered.
- Touch the LEARN path's existing `_lock`-serialised admission semantics.
...
*Resume*: user answers L1–L3; then implement the chosen backing behind
`MULLU_COGNITIVE_LOOP_LEDGER` with rehydrate-on-startup as the first ledger PR.
