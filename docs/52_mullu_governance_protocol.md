...
101. TeamOps shared inbox live-probe authority receipts are public contracts when they bind handoff readiness, separate probe approval evidence, read-only probe bounds, and zero provider-effect flags before any shared-inbox connector call.
102. TeamOps shared inbox live-probe operator input requests are public contracts when they convert blocked or admitted probe authority into missing-input lists, blocked actions, source artifacts, allowed probe summaries, and no-secret/no-effect flags.
