...
- Do not wire SNet into runtime routes without a future design decision, security review, rollback plan, and live evidence receipt.
...
```powershell
python -m pytest mcoi/tests/test_snet_recursive_mesh.py tests/test_validate_snet_mesh_receipt.py tests/test_validate_snet_operator_read_model.py -q
```
...
| --- | --- |
| Future design decision for runtime wiring | `AwaitingEvidence` |
| Security review for route or connector exposure | `AwaitingEvidence` |
| Rollback and recovery plan | `AwaitingEvidence` |
| Live evidence receipt proving no authority expansion | `AwaitingEvidence` |
| Workspace governance preflight receipt | Required before closure claim |

Until those witnesses exist, SNet stays local and read-only.
...
| Runtime route is proposed without the promotion gate | Classify as `GovernanceBlocked`. |
...
STATUS:
  Completeness: 100%
  Invariants verified: local-only SNet boundary, read-only operator projection, non-terminal receipt, no raw-answer exposure, no authority grant, promotion gate remains AwaitingEvidence
  Open issues: runtime integration remains AwaitingEvidence
