...
| `schemas/agentic_service_harness_live_producer_rollback_proof_preflight.schema.json` | Schema for the blocked rollback proof preflight. |
| `examples/agentic_service_harness_live_producer_rollback_proof_preflight.local.json` | Local fixture proving rollback proof remains `AwaitingEvidence` and no rollback execution or recovery verification is claimed. |
...
| 31 | `feat(harness): add rollback proof preflight` | Rollback proof preflight defines rollback-plan, state-snapshot, inverse-delta, replay-receipt, recovery-verification, and secret-handoff linkage requirements while keeping actual rollback proof blocked. No rollback execution, recovery verification, live producer execution path, UI, mutation endpoint, branch write, pull-request creation, or external adapter integration. | Closed by `scripts/validate_agentic_service_harness_live_producer_rollback_proof_preflight.py` |
...
| Rollback proof preflight | `python scripts/validate_agentic_service_harness_live_producer_rollback_proof_preflight.py` passes and keeps actual rollback proof, rollback execution, and recovery verification blocked. |
