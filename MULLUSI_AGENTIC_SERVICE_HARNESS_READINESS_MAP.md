...
Open PRs after readiness-map refresh: the live open PR queue includes ready PRs #2066 and #2068 plus draft PRs #2058 and #2069; the queue remains live, may change after this map-only closure, and remains outside this map-only closure.
...
| Open PR queue | PARTIAL | `gh pr list --state open --limit 30` returned ready PRs #2066 and #2068 plus draft PRs #2058 and #2069 after fetching `origin/main` at `1e94f9b786891f992bf195036fd344f0b26868a5`; the queue is live, may change after this map-only closure, and does not grant harness execution authority. |
...
| WorkspaceSandbox | PARTIAL | Temporary branch workspace preflight schema, fixture, validator, protocol manifest entry, workspace-preflight wiring, and tests bind command allowlist, path allowlist, timeout budget, network denial, cleanup receipt, and branch-create denial. Actual branch workspace creation remains blocked. | Add EvidenceBundle projection next; keep branch workspace creation blocked until approval and cleanup evidence exist. |
...
| Diff collection | PARTIAL | Diff and file-change receipts exist in lower-level surfaces; planned file-change collection preflight and actual file-change summary receipt now feed an actual diff collection admission preflight. The preflight binds branch/workspace authority, cleanup, redaction, UAO admission, receipt-store write path, path policy, and workflow-stage topology while collecting no diffs. Actual diff collection remains unadmitted. | Add actual diff collection receipt schema only after branch-write authority, cleanup receipt, UAO admission, and receipt-store write path evidence are explicit. |
...
| files changed | PARTIAL | AgentRunReceipt dry-run coverage keeps runtime state writes disabled and binds source read-model evidence; planned file-change collection is preflight-bound, actual file-change summary is zero-file gated, and actual diff collection now has an admission preflight that blocks non-empty diffs until authority, cleanup, redaction, UAO, and receipt-store evidence are explicit. No non-empty actual file-change collection is admitted yet. | Add non-empty diff/file summary receipt only after workspace write authority, cleanup receipt emission, UAO admission, and receipt-store write path evidence are verified. |
...
| connect GitHub repo | PARTIAL | RepositoryConnection read model, redacted GitHub installation binding, GitHub repo task intake, dashboard data binding, and contract-only adapter registry are closed for read-only projection; no connect UI or provider mutation route is authorized. | Add task creation admission preflight only after EvidenceBundle, receipt, and approval evidence remain blocked by default. |
...
| loop/readiness dashboard | PARTIAL | Loop read models, readiness docs, read-only dashboard data contract, and contract-only adapter registry exist; no dashboard build, route, mutation control, or adapter execution is authorized. | Add EvidenceBundle and Receipt read models. |
...
2. `harness(receipts): add harness Receipt projection with append disabled`
...
STATUS:
  Completeness: 100%
  Invariants verified: planning-only artifact; no dashboard; no mutation endpoint; no external adapter integration; no high-risk authority; open PR queue recorded without granting execution authority; read-only GitHub repository task intake bound without execution authority; read-only dashboard data contract bound without UI or route authority; contract-only adapter registry bound without subprocess, connector, external model, branch write, PR creation, receipt append, or terminal closure authority
  Open issues: durable Receipt store append, EvidenceBundle, branch workspace creation authority, dashboard UI, and live adapter integration remain partial, missing, externally blocked, or outside this closure
  Next action: start the smallest next PR sequence with EvidenceBundle projection by AgentRun id
