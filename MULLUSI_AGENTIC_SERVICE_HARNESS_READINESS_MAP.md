...
Open PRs after readiness-map refresh: the live open PR queue contains PR #2111 `ci(product): add OpenAPI export witness artifact`; it remains outside this map-only closure.
...
| Task creation admission preflight PR | READY | This proof thread adds `agentic_service_harness_task_creation_admission_preflight` schema, fixture, validator, manifest entry, workspace-preflight wiring, and tests. It records task creation admission as `AwaitingEvidence` while task route creation, task record writes, runtime writes, adapter execution, branch creation, PR creation, receipt append, connector calls, secret serialization, and terminal closure remain blocked. |
| Remote CI | READY | Latest completed main Build Verification for `7334ea159209406151f18343b93b30b5c77717af` passed on 2026-06-21. |
...
| Open PR queue | PARTIAL | PR #2111 is open and outside this harness closure; the queue is live and does not grant harness execution authority. |
...
| create agent task | PARTIAL | AgentTask exists as a contract, read-only repo task intake validates RepositoryConnection and task scope, dashboard data contract exposes it as display-only, and task creation admission preflight is blocked as `AwaitingEvidence`; no user-facing task creation route or task record write is authorized. | Add approved branch workspace creation preflight before any user-facing task creation route. |
...
2. `harness(tests): add dry-run test runner plan receipt`
