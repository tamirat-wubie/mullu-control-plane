...
Current `origin/main`: `2f0f299a4c2728074f2d7a87be1b6749427bd345`

Open PRs after readiness-map refresh: PR #2111 remains open outside this harness dry-run test runner plan closure; the queue is live, may change after this dry-run test runner plan receipt branch, and remains outside this map-only closure.
...
| Dry-run test runner plan receipt PR | READY | This proof thread adds `agentic_service_harness_dry_run_test_runner_plan_receipt` schema, fixture, validator, manifest entry, workspace-preflight wiring, and tests. It records selected harness test commands, allowlist refs, expected evidence refs, timeout budgets, and redaction policy while command execution, test execution, raw output capture, receipt append, adapter execution, filesystem writes, branch writes, secret serialization, and terminal closure remain blocked. |
| Remote CI | READY | `origin/main` is verified at `2f0f299a4c2728074f2d7a87be1b6749427bd345`; this dry-run test runner plan receipt branch requires current PR CI before merge. |
...
| Open PR queue | PARTIAL | PR #2111 remains open outside this harness dry-run test runner plan closure; the queue is live, may change after this dry-run test runner plan receipt branch, and does not grant harness execution authority. |
...
| 4. First MVP adapter path | PARTIAL | GitHub/Codex-style planning receipts, dry-run boundaries, and approved branch workspace preflight exist, but diff, test, and PR creation flow are not closed. |
...
| Test runner | READY | Dry-run test runner plan receipt schema, fixture, validator, protocol manifest entry, workspace-preflight wiring, and tests record selected harness commands, allowlist refs, expected evidence refs, timeout budgets, and redaction rules without command execution, raw output capture, receipt append, adapter execution, filesystem writes, branch writes, or terminal closure authority. | None for plan receipt. Future test execution remains blocked until operator approval, UAO, redaction, receipt-store, and rollback evidence exist. |
...
| connect GitHub repo | PARTIAL | RepositoryConnection read model, redacted GitHub installation binding, GitHub repo task intake, dashboard data contract, contract-only adapter registry, Receipt/EvidenceBundle projections, LoopStatus projection, task creation admission preflight, approved branch workspace creation preflight, and dry-run test runner plan receipt are closed for read-only/admission-only projection; no connect UI or provider mutation route is authorized. | Add task record write UAO admission preflight before any connect UI or provider mutation route. |
...
2. `harness(receipts): add receipt-store append admission preflight`
