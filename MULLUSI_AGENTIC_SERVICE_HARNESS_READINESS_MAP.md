...
Current `origin/main`: `d05eb4f104c21741ba05cc405e31c5fc94d5f755`

Open PRs after readiness-map refresh: the external PR queue remains outside this executed-test receipt admission preflight closure; the queue is live, may change after this branch, and does not grant harness execution authority.
...
| Remote CI | READY | `origin/main` is verified at `d05eb4f104c21741ba05cc405e31c5fc94d5f755`; this executed test receipt admission preflight branch requires current PR CI before merge. |
...
| Open PR queue | PARTIAL | External open PRs remain outside this executed-test receipt admission preflight closure; the queue is live, may change after this branch, and does not grant harness execution authority. |
...
| 4. First MVP adapter path | PARTIAL | GitHub/Codex-style planning receipts, dry-run boundaries, approved branch workspace preflight, dry-run test runner plan receipt, task-record write UAO admission, receipt-store append preflight, and executed-test receipt admission preflight exist, but non-empty diff and PR creation flow are not closed. |
...
| 7. Receipt and evidence model | PARTIAL | Required run receipt fields, read-only Receipt projection, AgentRun-indexed EvidenceBundle projection, receipt-store append preflight, and executed-test receipt admission preflight exist; durable harness receipt emission and store append remain blocked. |
...
2. `harness(pr): add GitHub PR admission preflight`
