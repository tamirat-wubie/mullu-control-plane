...
| `read_only_worker_runtime_foundation_closure_summary.schema.json` | Foundation Mode closure summary for the read-only worker runtime chain with live runtime blocked |
| `read_only_worker_runtime_status_read_model.schema.json` | Read-only Foundation Mode status projection for the worker runtime chain: evidence accepted, admission denied, promotion denied, closure complete |
